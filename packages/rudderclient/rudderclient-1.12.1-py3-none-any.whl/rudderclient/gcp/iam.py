...
def remove_user_from_all_projects(credentials, user):
    ...
    for project in response["projects"]:
        ...
        #     resource=project['projectId'], body={}).execute().get('bindings')
        ...
        if iam_policy_has_been_modified:
            set_policy(credentials, project["projectId"], policy)
            # service.projects().setIamPolicy(resource=project['projectId'], body={"policy": {
            #     "bindings": policy
            # }}).execute()
