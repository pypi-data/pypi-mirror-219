...
if __name__ == '__main__':
    ...
    p = Path(r'Z:\dataset\zenghua\trial_Jan.2022\idt\16mm\2560x1440_200fps_55cm.avi')
    cap = cv2.VideoCapture(str(p))
    frame_nb = cap.get(cv2.CAP_PROP_FRAME_COUNT)
    ids = random.choices(list(range(int(frame_nb))), k=5)
    for i in ids:
        cap.set(cv2.CAP_PROP_POS_FRAMES, i)
        _, f = cap.read()
        sp = p.parent / f'{p.stem}_{i}.tif'
        cv2.imwrite(str(sp), f)
    exit()
    ...
    while True:
        ...
        ret, frame = cap.read()
        cv2.waitKeyEx(20)
        ts.append(time.time() - tt0)
        if not ret: break
        # cv2.imshow('', frame)
        # k = cv2.waitKeyEx(1)
        # if k == 13: break
        if len(ts) == 1000: break
    ...
    td = ts
    td *= 1000
    print(td.mean(), td.min(), td.max())
    print(f'fps: {1000 / td.mean()}')
