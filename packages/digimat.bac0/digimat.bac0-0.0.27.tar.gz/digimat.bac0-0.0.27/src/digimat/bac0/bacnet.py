...
class BACPoint(object):
    ...
    def updateBacnetProperties(self):
    ...
    def value(self):
        try:
            ...
            #FIXME: presentValue property doesn't seems to be refreshed by BAC0. Using ".value" may generate BACnet traffic
...
class BACPoints(object):
    ...
    def dump(self, keys=None):
        ...
        if points:
            t=PrettyTable()
            t.field_names=['#', 'class', 'name', 'description', 'type', 'address', 'value', 'unit', 'COV', 'OoS']
            t.align['name']='l'
            t.align['class']='l'
...
class BACDevice(object):
    ...
    @property
    def segmentationSupported(self):
    ...
    def dump(self):
        ...
        t.add_row(['segmentation', self.segmentationSupported()])
