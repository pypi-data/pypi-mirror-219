...
class BACPoint(object):
    def __init__(self, bac0point, index=None, autoConvertFtoC=True):
        self._bac0point=bac0point
        self._index=index
        self._autoConvertFtoC=autoConvertFtoC
    ...
    def unit(self):
        ...
        if self._autoConvertFtoC and unit=='degreesFahrenheit':
            unit='degreesCelsius'
...
class BACPoints(object):
    ...
    def dump(self, keys=None):
        ...
        if points:
            t=PrettyTable()
            t.field_names=['#', 'name', 'description', 'type', 'address', 'value', 'unit', 'COV', 'OoS']
            t.align['name']='l'
