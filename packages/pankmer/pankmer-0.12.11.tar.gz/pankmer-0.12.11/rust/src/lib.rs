...
use crate::score_list_io::{load_scores, dump_scores, compress_scores,
                           compress_scores_exclusive, subset_scores_to_file,
                           subset_scores_to_list};
...
pub type ScoreList = Vec<Score>; // List of observed scores
pub type ScoreToIDX = HashMap<Score, usize>; // Map from score to its index in the ScoreList
pub type IDXMap = HashMap<u64, usize>; // Map of indices from one ScoreList to a subset of the ScoreList
pub type PKTbl = HashMap<Kmer, usize>; // Map of k-mers to score indices
...
fn index(_py: Python, m: &PyModule) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(subset_scores_to_file, m)?)?;
    m.add_function(wrap_pyfunction!(subset_scores_to_list, m)?)?;
