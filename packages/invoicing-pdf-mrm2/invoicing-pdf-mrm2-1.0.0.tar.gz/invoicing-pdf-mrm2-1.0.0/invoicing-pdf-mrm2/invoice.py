...
def generate(invoices_path, pdfs_path, image_path, product_id, product_name,
    ...
    for filepath in filepaths:
        ...
        # print(filename)
        invoice_nr, date = filename.split("-")  # 0 is file number and 1 is date
        # print(invoice_nr)
        # print(date)

        pdf.set_font(family="Times", size=16, style="B")
        pdfcelltext = f"Invoice nr.{invoice_nr}"
        pdf.cell(w=50, h=8, txt=pdfcelltext, ln=1)

        pdf.set_font(family="Times", size=16, style="B")
        pdfcelltext = f"Date: {date}"
        pdf.cell(w=50, h=8, txt=pdfcelltext, ln=1)
