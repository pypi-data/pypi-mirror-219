...
class TransmissionLoss():
    ...
    def __init__(self,
        ...
        if max_depth > 0.01:
            self.logger.debug(f"Maximum depth in computational domain: {max_depth:.1f}m")
        else:
            raise RuntimeError(f"Maximum depth in computational domain should be stricly positive (i.e. underwater) but was found to be {max_depth:.1f}m")
        ...
        self.logger.debug(f"Initializing computational grid with")
        ...
        self.n2b = index_refr_sq(c0=c0,
                                 c=bottom['sound_speed'],
                                 alpha=bottom['attenuation'])
