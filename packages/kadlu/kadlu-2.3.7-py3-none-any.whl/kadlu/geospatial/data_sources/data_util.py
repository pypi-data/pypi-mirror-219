...
def logmsg(source, var, ntup=(), **kwargs):
    logging.info(f'{source.upper()} {var.lower()}  '
                 f'logged {reduce(np.subtract, ntup[::-1])} values in region\t'
                 f'{json.dumps(kwargs, default=str)}')


def logmsg_nodata(source, var, **kwargs):
    arguments = ("north", "south", "west", "east", "top", "bottom", "start",
                 "end")
    region_string = {k: v for k, v in kwargs.items() if k in arguments}
    logging.warning(
        f'{source.upper()} {var.lower()}: no data found, '
        'returning empty arrays for region\t'
        f'{json.dumps(region_string , default=str, sort_keys=True)}')
