...
def _keys_check(df):
    ...
    for i in col_list1:
        if i not in df.keys():
            counter1 =False
            break
    if ('DCIR(mΩ)' in df.keys()) & (counter1 == True):
        return 0
    elif(counter1 == True):
        return 1
    ...
    for i in col_list2:
        if i not in df.keys():
            counter2 =False
    if (counter2 == True) & ('DCIR(mΩ)' in df.keys()):
        return 0
    elif counter2 == True:
        return 2
...
def get_barcode(nda):
    ...
    Args:
    ----------
    nda -> Path to the nda file.
    
    ----------
    returns barcode for the passed nda file
    '''
    if (nda.split('.')[-1] != 'nda'):
...
def records(nda, rename:bool = False):
    '''
    ----------
    Args:
    ----------
...
def cycle(df,lastcycle_skip:bool = False):  #! Function to group the data cycle-wise
    '''
    ...
    try:
        ...
        raise ValueError('Arguement pushed into the function is neither an appropiate DataFrame nor a path to an nda file')
    ...
    for i in range(min(df['cycle']),max(df['cycle']+lastcycle_counter)):                         
        ...
        dcir_cyl=df2.loc[df2['DCIR(mΩ)']>0,'DCIR(mΩ)']
...
def step(df):  #! Function to group the data step-wise
    ...
    try:
        ...
        raise ValueError('Arguement pushed into the function is neither an appropriate DataFrame nor a path to an nda file')
    # print(df)
    # df['current_mA']=df['current_mA'].div(1000)
    # df['capacity_mAh']=df['capacity_mAh'].div(1000)
    # df['energy_mWh']=df['energy_mWh'].div(1000)
    ...
    'current_mA', 'capacity_mAh','energy_mWh','timestamp','Validated','DCIR(mΩ)']
    if(list(df.keys())!=rec_columns):
        raise ValueError ('DataFrame passed is of wrong format.')
