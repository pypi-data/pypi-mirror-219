...
@contextlib.contextmanager
def ggml_context_set_no_alloc(
    ctx: ggml.ggml_context_p, no_alloc: bool = True, old_no_alloc: bool = False
):
    """Creates a context manager for a ggml context that sets no_alloc to no_alloc and
    restores it after use.

    By default this disables memory allocation (useful for gpu offloading)

    Example:
        ```python
        import ggml
        from ggml.utils import ggml_context_manager, ggml_context_no_alloc

        params = ggml.ggml_init_params(mem_size=16 * 1024 * 1024)
        with ggml_context_manager(params) as ctx:
            with ggml_context_no_alloc(ctx, True) as ctx_no_alloc:
                # do stuff with ctx_no_alloc
        ```

    Parameters:
        ctx: ggml context
        no_alloc: new no_alloc value
        old_no_alloc: old no_alloc value

    Returns:
        (contextlib.AbstractContextManager): ggml_context_p context manager"""
    ggml.ggml_set_no_alloc(ctx, no_alloc)
    try:
        yield ctx
    finally:
        ggml.ggml_set_no_alloc(ctx, old_no_alloc)
