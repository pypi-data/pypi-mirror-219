...
from tuxsuite.cli.tuxtrigger import (
    handlers as tuxtrigger_handlers,
    setup_parser as tuxtrigger_parser,
)
...
def setup_parser():
    ...
    tuxtrigger = sub_parser.add_parser("tuxtrigger", help="Tuxtrigger.").add_subparsers(
        dest="sub_command", help="Commands"
    )
    tuxtrigger.required = True
    tuxtrigger_cmds = tuxtrigger_parser(tuxtrigger)
...
def main():
    ...
    elif options.command == "tuxtrigger":
        return tuxtrigger_handlers[options.sub_command](options, extra_arguments, cfg)
    else:
