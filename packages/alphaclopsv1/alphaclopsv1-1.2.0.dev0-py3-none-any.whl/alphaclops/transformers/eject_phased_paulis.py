...
from alphaclops import circuits, ops, value, protocols
...
def eject_phased_paulis(
    ...
    *,
    ...
    then be removed by the `alphaclops.eject_z` transformation).

    `alphaclops.PhasedXZGate` with `z_exponent=0` (i.e. equivalent to PhasedXPow) or with `x_exponent=0`
    ...
    `alphaclops.eject_z(alphaclops.eject_phased_paulis(alphaclops.eject_z(circuit)))`.
...
def _single_cross_over_cz(op: ops.Operation, qubit_with_w: 'alphaclops.Qid') -> 'alphaclops.OP_TREE':
