...
from alphaclops import circuits, ops, protocols, transformers
from alphaclops.protocols.decompose_protocol import DecomposeResult
from alphaclops.transformers import merge_k_qubit_gates, merge_single_qubit_gates
...
def create_transformer_with_kwargs(transformer: 'alphaclops.TRANSFORMER', **kwargs) -> 'alphaclops.TRANSFORMER':
    """Method to capture additional keyword arguments to transformers while preserving mypy type.

    Returns a `alphaclops.TRANSFORMER` which, when called with a circuit and transformer context, is
    ...
    expects `alphaclops.TRANSFORMER`. For example:

    >>> def run_transformers(transformers: 'List[alphaclops.TRANSFORMER]'):
    ...
    ...
    >>> transformers: 'List[alphaclops.TRANSFORMER]' = []
    >>> transformers.append(
    ...     alphaclops.create_transformer_with_kwargs(
    ...         alphaclops.expand_composite, no_decomp=lambda op: alphaclops.num_qubits(op) <= 2
    ...     )
    ... )
    >>> transformers.append(alphaclops.create_transformer_with_kwargs(alphaclops.merge_k_qubit_unitaries, k=2))
    >>> run_transformers(transformers)


    Args:
         transformer: A `alphaclops.TRANSFORMER` for which additional kwargs should be captured.
         **kwargs: The keyword arguments which should be captured and passed to `transformer`.

    Returns:
        A `alphaclops.TRANSFORMER` method `transformer_with_kwargs`, s.t. executing
...
class CompilationTargetGateset(ops.Gateset, metaclass=abc.ABCMeta):
    ...
    An instance of this type can be passed to transformers like `alphaclops.convert_to_target_gateset`,
    ...
    @abc.abstractmethod
    def decompose_to_target_gateset(self, op: 'alphaclops.Operation', moment_idx: int) -> DecomposeResult:
        """Method to rewrite the given operation using gates from this gateset.

        Args:
            op: `alphaclops.Operation` to be rewritten using gates from this gateset.
            moment_idx: Moment index where the given operation `op` occurs in a circuit.

        Returns:
            - An equivalent `alphaclops.OP_TREE` implementing `op` using gates from this gateset.
            - `None` or `NotImplemented` if does not know how to decompose `op`.
        """

    def _validate_operation(self, op: 'alphaclops.Operation') -> bool:
    ...
    @property
    def postprocess_transformers(self) -> List['alphaclops.TRANSFORMER']:
...
class TwoQubitCompilationTargetGateset(CompilationTargetGateset):
    ...
    implements the logic to
        ...
        1. Apply `self.preprocess_transformers` to the input circuit, which by default will:
            a) Expand composite gates acting on > 2 qubits using `alphaclops.expand_composite`.
            b) Merge connected components of 1 & 2 qubit unitaries into tagged
                `alphaclops.CircuitOperation` using `alphaclops.merge_k_qubit_unitaries`.
        ...
        3. Apply `self.postprocess_transformers` to the transformed circuit, which by default will:
            a) Apply `alphaclops.merge_single_qubit_moments_to_phxz` to preserve moment structure (eg:
               alternating layers of single/two qubit gates).
            b) Apply `alphaclops.drop_negligible_operations` and `alphaclops.drop_empty_moments` to minimize
               circuit depth.
    ...
    def decompose_to_target_gateset(self, op: 'alphaclops.Operation', moment_idx: int) -> DecomposeResult:
        ...
        if switch_to_new:
            return new_optree
        mapped_old_optree: List['alphaclops.OP_TREE'] = []
    ...
    def _decompose_single_qubit_operation(
        self, op: 'alphaclops.Operation', moment_idx: int
    ) -> DecomposeResult:
        """Decomposes (connected component of) 1-qubit operations using gates from this gateset.

        By default, rewrites every operation using a single `alphaclops.PhasedXZGate`.

        Args:
            op: A single-qubit operation (can be a tagged `alphaclops.CircuitOperation` wrapping
                a connected component of single qubit unitaries).
            moment_idx: Index of the moment in which operation `op` occurs.

        Returns:
            A `alphaclops.OP_TREE` implementing `op` using gates from this gateset OR
    ...
    def _decompose_two_qubit_operation(
        ...
        Args:
            op: A two-qubit operation (can be a tagged `alphaclops.CircuitOperation` wrapping
