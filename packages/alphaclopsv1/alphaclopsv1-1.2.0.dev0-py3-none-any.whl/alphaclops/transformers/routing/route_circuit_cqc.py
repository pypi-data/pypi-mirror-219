...
from alphaclops.transformers.routing import mapping_manager, line_initial_mapper
...
def _disjoint_nc2_combinations(
    ...
    For example:
        ...
        >>> q = [*range(5)]
        >>> disjoint_swaps = alphaclops.transformers.routing.route_circuit_cqc._disjoint_nc2_combinations(
...
class RouteCQC:
    ...
    2. Places the logical qubits in the input circuit onto some input device graph by using an
        initial mapper (`alphaclops.LineInitialMapper` by default).
    ...
    For example:
        ...
        >>> import alphaclops_google as cg
        >>> circuit = alphaclops.testing.random_circuit(5, 10, 0.6)
        >>> device = cg.Sycamore
        >>> router = alphaclops.RouteCQC(device.metadata.nx_graph)
        >>> rcirc, initial_map, swap_map = router.route_circuit(circuit)
        >>> fcirc = alphaclops.optimize_for_target_gateset(rcirc, gateset = cg.SycamoreTargetGateset())
    ...
    def __call__(
        ...
        *,
        ...
        initial_mapper: Optional['alphaclops.RandomGridMapper'] = None,
        ...
        Args:
            ...
            tag_inserted_swaps: whether or not a `alphaclops.RoutingSwapTag` should be attached to
            ...
            initial_mapper: an initial mapping strategy (placement) of logical qubits in the
                ...
                instance of `alphaclops.LineInitialMapper`.
    ...
    def route_circuit(
        ...
        *,
        ...
        initial_mapper: Optional['alphaclops.RandomGridMapper'] = None,
        context: Optional['alphaclops.TransformerContext'] = None,
    ) -> Tuple['alphaclops.RandomGrid', Dict['alphaclops.Qid', 'alphaclops.Qid'], Dict[
        ...
        `circuit` contains `alphaclops.CircuitOperation`s and `context.deep` is True then they are first
        unrolled before proceeding. If `context.deep` is False or `context` is None then any
        `alphaclops.CircuitOperation` that acts on more than 2-qubits will also raise an error.
    ...
    def _get_one_and_two_qubit_ops_as_timesteps(
        cls, circuit: 'alphaclops.RandomGrid'
    ) -> Tuple[List[List['alphaclops.Operation']], List[List['alphaclops.Operation']]]:
        ...
        single_qubit_ops: List[List[alphaclops.Operation]] = []
    ...
    def _route(
        ...
        two_qubit_ops: List[List['alphaclops.Operation']],
        single_qubit_ops: List[List['alphaclops.Operation']],
        lookahead_radius: int,
        tag_inserted_swaps: bool = False,
    ) -> List[List['alphaclops.Operation']]:
        ...
        routed_ops: List[List['alphaclops.Operation']] = []

        def process_executable_two_qubit_ops(timestep: int) -> int:
            unexecutable_ops: List['alphaclops.Operation'] = []
    ...
    def __repr__(self) -> str:
        return f'alphaclops.RouteCQC(nx.Graph({dict(self.device_graph.adjacency())}))'
