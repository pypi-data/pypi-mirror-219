...
from alphaclops.transformers.analytical_decompositions.two_qubit_to_cz import (
    _parity_interaction,
    _is_trivial_angle,
    two_qubit_matrix_to_diagonal_and_cz_operations,
)
from alphaclops.testing import random_two_qubit_circuit_with_czs
...
def _operations_to_matrix(operations, qubits):
    return alphaclops.Circuit(operations).unitary(
        qubit_order=alphaclops.QubitOrder.explicit(qubits), qubits_that_should_be_present=qubits
    )


def _random_single_partial_cz_effect():
    return alphaclops.dot(
        alphaclops.kron(alphaclops.testing.random_unitary(2), alphaclops.testing.random_unitary(2)),
...
def assert_cz_depth_below(operations, threshold, must_be_full):
    ...
    for op in operations:
        ...
        if len(op.qubits) == 2:
            assert isinstance(op.gate, alphaclops.CZPowGate)
...
def assert_ops_implement_unitary(q0, q1, operations, intended_effect, atol=0.01):
    ...
    assert alphaclops.allclose_up_to_global_phase(actual_effect, intended_effect, atol=atol)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (0, 0, alphaclops.unitary(alphaclops.CZ ** 0.00000001)),
        (0.5, 2, alphaclops.unitary(alphaclops.CZ ** 0.5)),
        (1, 1, alphaclops.unitary(alphaclops.CZ)),
        (1, 1, alphaclops.unitary(alphaclops.CNOT)),
        (
            1,
            1,
        ...
        (1.5, 3, alphaclops.map_eigenvalues(alphaclops.unitary(alphaclops.SWAP), lambda e: e ** 0.5)),
        (2, 2, alphaclops.unitary(alphaclops.SWAP).dot(alphaclops.unitary(alphaclops.CZ))),
        (3, 3, alphaclops.unitary(alphaclops.SWAP)),
    ...
    + [(3, 3, alphaclops.testing.random_unitary(4)) for _ in range(10)],
)
def test_two_to_ops_equivalent_and_bounded_for_known_and_random(
    ...
    operations_with_partial = alphaclops.two_qubit_matrix_to_cz_operations(q0, q1, effect, True)
    operations_with_full = alphaclops.two_qubit_matrix_to_cz_operations(q0, q1, effect, False)
...
def test_trivial_parity_interaction_corner_case():
    ...
    tolerance = 1e-2
    circuit = alphaclops.Circuit(_parity_interaction(q0, q1, -nearPi4, tolerance))
...
def test_kak_decomposition_depth_full_cz():
    ...
    operations_with_full = alphaclops.two_qubit_matrix_to_cz_operations(a, b, u, False)
    c = alphaclops.Circuit(operations_with_full)
    # 3 CZ, 3+1 PhasedX, 1 Z
    assert len(c) <= 8

    # Double-axis interaction.
    u = alphaclops.unitary(alphaclops.Circuit(alphaclops.CNOT(a, b), alphaclops.CNOT(b, a)))
    operations_with_part = alphaclops.two_qubit_matrix_to_cz_operations(a, b, u, False)
    c = alphaclops.Circuit(operations_with_part)
    # 2 CZ, 2+1 PhasedX, 1 Z
    assert len(c) <= 6

    # Test unoptimized/un-cleaned length of Double-axis interaction.
    u = alphaclops.unitary(alphaclops.Circuit(alphaclops.CNOT(a, b), alphaclops.CNOT(b, a)))
    operations_with_part = alphaclops.two_qubit_matrix_to_cz_operations(a, b, u, False, 1e-8, False)
    c = alphaclops.Circuit(operations_with_part)
    assert len(c) > 6  # Length should be 13 with extra Pauli gates

    # Partial single-axis interaction.
    u = alphaclops.unitary(alphaclops.CNOT ** 0.1)
    operations_with_part = alphaclops.two_qubit_matrix_to_cz_operations(a, b, u, False)
    c = alphaclops.Circuit(operations_with_part)
    # 2 CZ, 2+1 PhasedX, 1 Z
    assert len(c) <= 6
    ...
    u = alphaclops.unitary(alphaclops.ControlledGate(alphaclops.Y))
    operations_with_part = alphaclops.two_qubit_matrix_to_cz_operations(a, b, u, False)
    c = alphaclops.Circuit(operations_with_part)
...
def test_kak_decomposition_depth_partial_cz():
    ...
    # Random.
    u = alphaclops.testing.random_unitary(4)
    operations_with_full = alphaclops.two_qubit_matrix_to_cz_operations(a, b, u, True)
    c = alphaclops.Circuit(operations_with_full)
    ...
    # Double-axis interaction.
    u = alphaclops.unitary(alphaclops.Circuit(alphaclops.CNOT(a, b), alphaclops.CNOT(b, a)))
    operations_with_part = alphaclops.two_qubit_matrix_to_cz_operations(a, b, u, True)
    c = alphaclops.Circuit(operations_with_part)
...
@pytest.mark.parametrize(
    "v",
    [
        alphaclops.unitary(random_two_qubit_circuit_with_czs(3)),
        alphaclops.unitary(random_two_qubit_circuit_with_czs(2)),
        np.diag(np.exp(1j * np.pi * np.random.random(4))),
    ],
)
def test_decompose_to_diagonal_and_circuit(v):
    b, c = alphaclops.LineQubit.range(2)
    diagonal, ops = two_qubit_matrix_to_diagonal_and_cz_operations(b, c, v, atol=1e-8)
    assert alphaclops.is_diagonal(diagonal)
    combined_circuit = alphaclops.Circuit(alphaclops.MatrixGate(diagonal)(b, c), ops)
    circuit_unitary = combined_circuit.unitary(qubits_that_should_be_present=[b, c])
    alphaclops.testing.assert_allclose_up_to_global_phase(circuit_unitary, v, atol=2e-6)
