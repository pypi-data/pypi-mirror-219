...
from alphaclops.linalg import predicates
from alphaclops.linalg.decompositions import num_cnots_required, extract_right_diag

from alphaclops import ops, linalg, protocols, circuits
from alphaclops.transformers.analytical_decompositions import single_qubit_decompositions
from alphaclops.transformers.merge_single_qubit_gates import merge_single_qubit_gates_to_phased_x_and_z
...
def two_qubit_matrix_to_diagonal_and_cz_operations(
    ...
    atol: float = 1e-8,
    clean_operations: bool = True,
) -> Tuple[np.ndarray, List['alphaclops.Operation']]:
...
def _xx_interaction_via_full_czs(q0: 'alphaclops.Qid', q1: 'alphaclops.Qid', x: float):
    a = x * -2 / np.pi
    yield ops.H(q1)
    yield ops.CZ(q0, q1)
    yield ops.X(q0) ** a
...
def _xx_yy_interaction_via_full_czs(q0: 'alphaclops.Qid', q1: 'alphaclops.Qid', x: float, y: float):
    a = x * -2 / np.pi
    b = y * -2 / np.pi
    yield ops.X(q0) ** 0.5
    yield ops.H(q1)
...
def _xx_yy_zz_interaction_via_full_czs(
    q0: 'alphaclops.Qid', q1: 'alphaclops.Qid', x: float, y: float, z: float
):
    a = x * -2 / np.pi + 0.5
    b = y * -2 / np.pi + 0.5
    c = z * -2 / np.pi + 0.5
...
def _parity_interaction(
    q0: 'alphaclops.Qid', q1: 'alphaclops.Qid', rads: float, atol: float, gate: Optional[ops.Gate] = None
):
...
def _do_single_on(u: np.ndarray, q: 'alphaclops.Qid', atol: float = 1e-8):
