...
from alphaclops import circuits, devices, linalg, ops, protocols
...
def decompose_two_qubit_interaction_into_four_fsim_gates(
    interaction: Union['alphaclops.SupportsUnitary', np.ndarray],
    *,
    fsim_gate: Union['alphaclops.FSimGate', 'alphaclops.ISwapPowGate'],
    ...
    Args:
        interaction: The two qubit operation to synthesize. This can either be
            a alphaclops object (such as a gate, operation, or circuit) or a raw numpy
        ...
        qubits: The qubits that the resulting operations should apply the
            ...
            qubits of the given interaction (if it is a `alphaclops.Operation`) or
            else to `alphaclops.LineQubit.range(2)`.
...
def _decompose_two_qubit_interaction_into_two_b_gates(
    interaction: Union['alphaclops.SupportsUnitary', np.ndarray, 'alphaclops.KakDecomposition'],
    *,
    qubits: Sequence['alphaclops.Qid'],
) -> List['alphaclops.Operation']:
    kak = linalg.kak_decomposition(interaction)
...
def _decompose_b_gate_into_two_fsims(
    *, fsim_gate: 'alphaclops.FSimGate', qubits: Sequence['alphaclops.Qid']
) -> List['alphaclops.Operation']:
...
def _decompose_interaction_into_two_b_gates_ignoring_single_qubit_ops(
    qubits: Sequence['alphaclops.Qid'], kak_interaction_coefficients: Iterable[float]
) -> List['alphaclops.Operation']:
...
def _fix_single_qubit_gates_around_kak_interaction(
    *,
    desired: 'alphaclops.KakDecomposition',
    operations: List['alphaclops.Operation'],
    qubits: Sequence['alphaclops.Qid'],
) -> Iterator['alphaclops.Operation']:
