...
def test_decompose_x():
    ...
    for total_qubits_count in range(1, 8):
        qubits = alphaclops.LineQubit.range(total_qubits_count)
        for controls_count in range(total_qubits_count):
            gates = alphaclops.decompose_multi_controlled_x(
                qubits[:controls_count], qubits[controls_count], qubits[controls_count + 1 :]
            )

            circuit1 = alphaclops.Circuit([alphaclops.I.on(q) for q in qubits])
            circuit1.append(gates)
            result_matrix = circuit1.unitary()

            circuit2 = alphaclops.Circuit([alphaclops.I.on(q) for q in qubits])
            circuit2 += alphaclops.ControlledGate(alphaclops.X, num_controls=controls_count).on(
...
def _count_operations(operations):
    ...
    for operation in operations:
        u = alphaclops.unitary(operation)
        if u.shape == (2, 2):
            count_2x2 += 1
        elif u.shape == (4, 4):
            assert np.allclose(u, alphaclops.unitary(alphaclops.CNOT))
            count_cnot += 1
        elif u.shape == (8, 8):
            assert np.allclose(u, alphaclops.unitary(alphaclops.CCNOT))
            count_ccnot += 1
    return count_2x2, count_cnot, count_ccnot


def _test_decompose(matrix, controls_count):
    qubits = alphaclops.LineQubit.range(controls_count + 1)
    operations = alphaclops.decompose_multi_controlled_rotation(matrix, qubits[:-1], qubits[-1])
    _count_operations(operations)
    result_matrix = alphaclops.Circuit(operations).unitary()

    expected_matrix = alphaclops.Circuit(
        [alphaclops.MatrixGate(matrix).on(qubits[-1]).controlled_by(*qubits[:-1])]
    ).unitary()

    alphaclops.testing.assert_allclose_up_to_global_phase(expected_matrix, result_matrix, atol=1e-8)


def test_decompose_specific_matrices():
    for gate in [alphaclops.X, alphaclops.Y, alphaclops.Z, alphaclops.H, alphaclops.I, alphaclops.T, alphaclops.S]:
        for controls_count in range(7):
            _test_decompose(alphaclops.unitary(gate), controls_count)
...
def _decomposition_size(U, controls_count):
    qubits = alphaclops.LineQubit.range(controls_count + 1)
    operations = alphaclops.decompose_multi_controlled_rotation(U, qubits[:controls_count], qubits[-1])
