...
VALID_INITIAL_STATES = [
    ...
    *[np.concatenate([alphaclops.testing.random_superposition(2), np.zeros(2)]) for _ in range(3)],
]

INVALID_INITIAL_STATES = [
    np.array([1 / np.sqrt(2), 0, 0, -1 / np.sqrt(2)]),
    alphaclops.testing.random_superposition(4),
]
...
@pytest.mark.parametrize('unitary_matrix', [alphaclops.testing.random_unitary(4) for _ in range(5)])
...
def test_two_qubit_matrix_to_cz_isometry(
    ...
    decomposed_ops = alphaclops.two_qubit_matrix_to_cz_isometry(
        a, b, unitary_matrix, allow_partial_czs=allow_partial_czs
    )
    circuit = alphaclops.Circuit(decomposed_ops)
    ops_cz = [*circuit.findall_operations(lambda op: isinstance(op.gate, alphaclops.CZPowGate))]
    ops_2q = [*circuit.findall_operations(lambda op: alphaclops.num_qubits(op) > 1)]
    assert ops_cz == ops_2q
    assert len(ops_cz) <= 2

    sim = alphaclops.Simulator()
    base_circuit = alphaclops.Circuit(
        alphaclops.StatePreparationChannel(initial_state).on(a, b), alphaclops.CNOT(b, c)
    )
    original_circuit = base_circuit + alphaclops.Circuit(alphaclops.MatrixGate(unitary_matrix).on(a, b))
    ...
    assert (
            alphaclops.allclose_up_to_global_phase(
