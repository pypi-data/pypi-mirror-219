...
from alphaclops import transformers as opt
...
def three_qubit_matrix_to_operations(
    ...
    if not alphaclops.is_unitary(u, atol=atol):
    ...
    try:
        ...
        # coverage: ignore
        raise ImportError(
            "alphaclops.three_qubit_unitary_to_operations requires "
    ...
    if len(cs_ops) > 0 and cs_ops[-1] == alphaclops.CZ(q2, q0):
        # optimization A.1 - merging the last CZ from the end of CS into UD
        # cz = alphaclops.Circuit([cs_ops[-1]]).unitary()
    ...
    return list(alphaclops.Circuit(vdh_ops + cs_ops + ud_ops).all_operations())
...
def _cs_to_ops(q0: ops.Qid, q1: ops.Qid, q2: ops.Qid, theta: np.ndarray) -> List[ops.Operation]:
    ...
    # CSD decomposition, but alphaclops.ry takes full angles.
    angles = _multiplexed_angles(theta * 2)
    rys = [alphaclops.ry(angle).on(q0) for angle in angles]
    ops = [
        rys[0],
        alphaclops.CZ(q1, q0),
        rys[1],
...
def _two_qubit_multiplexor_to_ops(
    ...
    """
    u1u2 = u1 @ u2.conj().T
    eigvals, v = alphaclops.unitary_eig(u1u2)
...
def _optimize_multiplexed_angles_circuit(operations: Sequence[ops.Operation]):
    ...
    circuit = alphaclops.transformers.drop_negligible_operations(circuit)
...
def _middle_multiplexor_to_ops(q0: ops.Qid, q1: ops.Qid, q2: ops.Qid, eigvals: np.ndarray):
    ...
    rzs = [alphaclops.rz(angle).on(q0) for angle in angles]
    ops = [
        rzs[0],
        alphaclops.CNOT(q1, q0),
        rzs[1],
        alphaclops.CNOT(q2, q0),
        rzs[2],
        alphaclops.CNOT(q1, q0),
        rzs[3],
        alphaclops.CNOT(q2, q0),
