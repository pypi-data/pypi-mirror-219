...
import alphaclops
from alphaclops.transformers.analytical_decompositions.three_qubit_decomposition import (
    _multiplexed_angles,
    _cs_to_ops,
...
@pytest.mark.parametrize(
    "u",
    [
        alphaclops.testing.random_unitary(8),
        np.eye(8),
        alphaclops.ControlledGate(alphaclops.ISWAP)._unitary_(),
        alphaclops.CCX._unitary_(),
    ],
)
def test_three_qubit_matrix_to_operations(u):
    a, b, c = alphaclops.LineQubit.range(3)
    operations = alphaclops.three_qubit_matrix_to_operations(a, b, c, u)
    final_circuit = alphaclops.Circuit(operations)
    final_unitary = final_circuit.unitary(qubits_that_should_be_present=[a, b, c])
    alphaclops.testing.assert_allclose_up_to_global_phase(u, final_unitary, atol=1e-9)
    num_two_qubit_gates = len(
        [
            op
            for op in list(final_circuit.all_operations())
            if isinstance(op.gate, (alphaclops.CZPowGate, alphaclops.CNotPowGate))
        ]
    )
...
def test_three_qubit_matrix_to_operations_errors():
    ...
    with pytest.raises(ValueError, match="(8,8)"):
        alphaclops.three_qubit_matrix_to_operations(a, b, c, np.eye(2))
    with pytest.raises(ValueError, match="not unitary"):
        alphaclops.three_qubit_matrix_to_operations(a, b, c, alphaclops.unitary(alphaclops.CCX) * 2)
...
def test_three_qubit_matrix_to_operations_scipy_error():
    ...
    with pytest.raises(ImportError, match="three_qubit.*1.5.0+"):
        alphaclops.three_qubit_matrix_to_operations(a, b, c, np.eye(8))
...
def test_cs_to_ops(theta, num_czs):
    ...
    circuit_cs = alphaclops.Circuit(_cs_to_ops(a, b, c, theta))

    assert_almost_equal(circuit_cs.unitary(qubits_that_should_be_present=[a, b, c]), cs, 10)

    assert (
        len([cz for cz in list(circuit_cs.all_operations()) if isinstance(cz.gate, alphaclops.CZPowGate)])
        == num_czs
...
def test_middle_multiplexor(angles, num_cnots):
    ...
    circuit_u1u2_mid = alphaclops.Circuit(_middle_multiplexor_to_ops(a, b, c, eigvals))
    ...
    assert (
        len(
            [
                ...
                if isinstance(cnot.gate, alphaclops.CNotPowGate)
            ]
        )
        == num_cnots
    ), "expected {} CNOTs got \n {} \n {}".format(
...
def test_two_qubit_multiplexor_to_circuit(shift_left):
    a, b, c = alphaclops.LineQubit.range(3)
    u1 = alphaclops.testing.random_unitary(4)
    u2 = alphaclops.testing.random_unitary(4)
    ...
    actual = alphaclops.Circuit(ud_ops).unitary(qubits_that_should_be_present=[a, b, c]) @ diagonal
    alphaclops.testing.assert_allclose_up_to_global_phase(expected, actual, atol=1e-8)
