...
from alphaclops.circuits.circuit import CIRCUIT_TYPE
...
def _to_target_circuit_type(
    circuit: circuits.RandomGrid, target_circuit: CIRCUIT_TYPE
) -> CIRCUIT_TYPE:
    return cast(
        CIRCUIT_TYPE,
...
def map_moments(
    ...
    *,
    ...
    Args:
        ...
        map_func: Mapping function from (alphaclops.Moment, moment_index) to a sequence of moments.
...
def map_operations(
    ...
    *,
    ...
    Args:
        circuit: Input circuit to apply the transformations on. The input circuit is not mutated.
        map_func: Mapping function from (alphaclops.Operation, moment_index) to a alphaclops.OP_TREE. If the
            resulting optree spans more than 1 moment, it's inserted in-place in the same moment as
            `alphaclops.CircuitOperation(alphaclops.FrozenCircuit(op_tree)).with_tags(MAPPED_CIRCUIT_OP_TAG)`
            to preserve moment structure. Utility methods like `alphaclops.unroll_circuit_op` can
...
def map_operations_and_unroll(
    ...
    *,
    ...
    """Applies local transformations via `alphaclops.map_operations` & unrolls intermediate circuit ops.

    See `alphaclops.map_operations` and `alphaclops.unroll_circuit_op` for more details.

    Args:
        circuit: Input circuit to apply the transformations on. The input circuit is not mutated.
        map_func: Mapping function from (alphaclops.Operation, moment_index) to a alphaclops.OP_TREE.
        deep: If true, `map_func` will be recursively applied to circuits wrapped inside
            any circuit operations contained within `circuit`.
        raise_if_add_qubits: Set to True by default. If True, raises ValueError if
...
class _MergedCircuit:
    """An optimized internal representation of a circuit, tailored for `alphaclops.merge_operations`
    ...
    qubit_indexes: Dict['alphaclops.Qid', List[int]] = dataclasses.field(
        default_factory=lambda: defaultdict(lambda: [-1])
    )
    mkey_indexes: Dict['alphaclops.MeasurementKey', List[int]] = dataclasses.field(
        default_factory=lambda: defaultdict(lambda: [-1])
    )
    ckey_indexes: Dict['alphaclops.MeasurementKey', List[int]] = dataclasses.field(
        default_factory=lambda: defaultdict(lambda: [-1])
    )
    ops_by_index: List[Dict['alphaclops.Operation', int]] = dataclasses.field(default_factory=list)

    def append_empty_moment(self) -> None:
        self.ops_by_index.append({})

    def add_op_to_moment(self, moment_index: int, op: 'alphaclops.Operation') -> None:
    ...
    def remove_op_from_moment(self, moment_index: int, op: 'alphaclops.Operation') -> None:
    ...
    def get_mergeable_ops(
        self, op: 'alphaclops.Operation', op_qs: Set['alphaclops.Qid']
    ) -> Tuple[int, List['alphaclops.Operation']]:
    ...
    def get_alphaclops_circuit(self) -> 'alphaclops.Circuit':
...
def merge_operations(
    ...
    *,
    ...
    for moment_idx, current_moment in enumerate(cast(List['alphaclops.Moment'], circuit)):
        ...
        for op in sorted(current_moment.operations, key=lambda op: op.qubits):
            ...
            while left_ops and op_qs:
                ...
                left_idx, left_ops = merged_circuit.get_mergeable_ops(op, op_qs)
            merged_circuit.add_op_to_moment(moment_idx, op)
    ret_circuit = merged_circuit.get_alphaclops_circuit()
    if deep:
...
def merge_operations_to_circuit_op(
    circuit: CIRCUIT_TYPE,
    can_merge: Callable[[Sequence['alphaclops.Operation'], Sequence['alphaclops.Operation']], bool],
    *,
    tags_to_ignore: Sequence[Hashable] = (),
    ...
    Uses `alphaclops.merge_operations` to identify connected components of operations. Moment structure
    ...
    using `alphaclops.merge_operations` directly with a custom `merge_func`.
    ...
    def merge_func(op1: 'alphaclops.Operation', op2: 'alphaclops.Operation') -> Optional['alphaclops.Operation']:
        def get_ops(op: 'alphaclops.Operation'):
            op_untagged = op.untagged
            return (
...
def merge_k_qubit_unitaries_to_circuit_op(
    ...
    *,
    ...
    Uses `alphaclops.merge_operations_to_circuit_op` to identify and merge connected components of
    ...
    def can_merge(ops1: Sequence['alphaclops.Operation'], ops2: Sequence['alphaclops.Operation']) -> bool:
        return all(
...
def unroll_circuit_op(
    ...
    *,
    ...
    """Unrolls (tagged) `alphaclops.CircuitOperation`s while preserving the moment structure.
    ...
    def map_func(m: circuits.Moment, _: int):
        to_zip: List['alphaclops.RandomGrid'] = []
        for op in m:
            op_untagged = op.untagged
            if isinstance(op_untagged, circuits.CircuitOperation):
                if deep:
...
def unroll_circuit_op_greedy_earliest(
    ...
    *,
    ...
    """Unrolls (tagged) `alphaclops.CircuitOperation`s by inserting operations using EARLIEST strategy.

    Each matching `alphaclops.CircuitOperation` is replaced by inserting underlying operations using the
    `alphaclops.InsertStrategy.EARLIEST` strategy. The greedy approach attempts to minimize circuit depth
...
def unroll_circuit_op_greedy_frontier(
    ...
    *,
    ...
    """Unrolls (tagged) `alphaclops.CircuitOperation`s by inserting operations inline at qubit frontier.
    ...
    """
    unrolled_circuit = circuit.unfreeze(copy=True)
    frontier: Dict['alphaclops.Qid', int] = defaultdict(lambda: 0)
    idx = 0
...
def toggle_tags(circuit: CIRCUIT_TYPE, tags: Sequence[Hashable], *, deep: bool = False):
    ...
    def map_func(op: 'alphaclops.Operation', _) -> 'alphaclops.Operation':
        return (
            op
