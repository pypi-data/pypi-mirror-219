...
from alphaclops.ops import gate_features, raw_types


def _canonicalize(value: 'alphaclops.TParamVal') -> 'alphaclops.TParamVal':
...
def _zero_mod_pi(param: 'alphaclops.TParamVal') -> bool:
    """Returns True iff param, assumed to be in [-pi, pi), is 0 (mod pi)."""
    return param in (0.0, -np.pi, -sympy.pi)


def _half_pi_mod_pi(param: 'alphaclops.TParamVal') -> bool:
...
class FSimGate(gate_features.InterchangeableQubitsGate, raw_types.Gate):
    ...
    def __init__(self, theta: 'alphaclops.TParamVal', phi: 'alphaclops.TParamVal') -> None:
        """Inits FSimGate.
    ...
    @property
    def theta(self) -> 'alphaclops.TParamVal':
        return self._theta

    @property
    def phi(self) -> 'alphaclops.TParamVal':
        return self._phi
    ...
    def _is_parameterized_(self) -> bool:
        return alphaclops.is_parameterized(self.theta) or alphaclops.is_parameterized(self.phi)

    def _parameter_names_(self) -> AbstractSet[str]:
        return alphaclops.parameter_names(self.theta) | alphaclops.parameter_names(self.phi)
    ...
    def _apply_unitary_(self, args: 'alphaclops.ApplyUnitaryArgs') -> Optional[np.ndarray]:
        if alphaclops.is_parameterized(self):
            return None
        if self.theta != 0:
            inner_matrix = protocols.unitary(alphaclops.rx(2 * self.theta))
            oi = args.subspace_index(0b01)
            io = args.subspace_index(0b10)
            out = alphaclops.apply_matrix_to_slices(
    ...
    def _decompose_(self, qubits) -> 'alphaclops.OP_TREE':
        a, b = qubits
        xx = alphaclops.XXPowGate(exponent=self.theta / np.pi, global_shift=-0.5)
        yy = alphaclops.YYPowGate(exponent=self.theta / np.pi, global_shift=-0.5)
        yield xx(a, b)
        yield yy(a, b)
        yield alphaclops.CZ(a, b) ** (-self.phi / np.pi)
    ...
    def __pow__(self, power) -> 'FSimGate':
        return FSimGate(alphaclops.mul(self.theta, power), alphaclops.mul(self.phi, power))

    def __repr__(self) -> str:
        t = proper_repr(self.theta)
        p = proper_repr(self.phi)
        return f'alphaclops.FSimGate(theta={t}, phi={p})'
...
class PhasedFSimGate(gate_features.InterchangeableQubitsGate, raw_types.Gate):
    ...
    Indeed, for any angle δ
        ...
        alphaclops.PhasedFSimGate.from_fsim_rz(θ, φ, (α0, α1), (β0, β1))

    and

        alphaclops.PhasedFSimGate.from_fsim_rz(θ, φ,
                                         (α0 + δ, α1 + δ),
                                         (β0 - δ, β1 - δ))
    ...
    def __init__(
        self,
        theta: 'alphaclops.TParamVal',
        zeta: 'alphaclops.TParamVal' = 0.0,
        chi: 'alphaclops.TParamVal' = 0.0,
        gamma: 'alphaclops.TParamVal' = 0.0,
        phi: 'alphaclops.TParamVal' = 0.0,
    ) -> None:
    ...
    @property
    def zeta(self) -> 'alphaclops.TParamVal':
        return self._zeta

    @property
    def chi(self) -> 'alphaclops.TParamVal':
        return self._chi

    @property
    def gamma(self) -> 'alphaclops.TParamVal':
        return self._gamma
    ...
    def from_fsim_rz(
        theta: 'alphaclops.TParamVal',
        phi: 'alphaclops.TParamVal',
        rz_angles_before: Tuple['alphaclops.TParamVal', 'alphaclops.TParamVal'],
        rz_angles_after: Tuple['alphaclops.TParamVal', 'alphaclops.TParamVal'],
    ) -> 'PhasedFSimGate':
    ...
    @property
    def rz_angles_before(self) -> Tuple['alphaclops.TParamVal', 'alphaclops.TParamVal']:
    ...
    @property
    def rz_angles_after(self) -> Tuple['alphaclops.TParamVal', 'alphaclops.TParamVal']:
    ...
    def _is_parameterized_(self) -> bool:
        return (
                alphaclops.is_parameterized(self.theta)
                or alphaclops.is_parameterized(self.zeta)
                or alphaclops.is_parameterized(self.chi)
                or alphaclops.is_parameterized(self.gamma)
                or alphaclops.is_parameterized(self.phi)
        )
    ...
    def _apply_unitary_(self, args: 'alphaclops.ApplyUnitaryArgs') -> Optional[np.ndarray]:
        ...
        if self.theta != 0 or self.zeta != 0 or self.chi != 0:
            rx = protocols.unitary(alphaclops.rx(2 * self.theta))
            rz1 = protocols.unitary(alphaclops.rz(-self.zeta + self.chi))
            rz2 = protocols.unitary(alphaclops.rz(-self.zeta - self.chi))
    ...
    def _decompose_(self, qubits) -> 'alphaclops.OP_TREE':
        ...
        def to_exponent(angle_rads: 'alphaclops.TParamVal') -> 'alphaclops.TParamVal':
        ...
        yield alphaclops.Z(q0) ** to_exponent(before[0])
        yield alphaclops.Z(q1) ** to_exponent(before[1])
        yield FSimGate(self.theta, self.phi).on(q0, q1)
        yield alphaclops.Z(q0) ** to_exponent(after[0])
        yield alphaclops.Z(q1) ** to_exponent(after[1])
    ...
    def __repr__(self) -> str:
        ...
        return (
            f'alphaclops.PhasedFSimGate(theta={theta}, zeta={zeta}, chi={chi}, '
