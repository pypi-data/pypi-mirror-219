...
import alphaclops
from alphaclops.ops import qubit_manager as cqi
import pytest
...
def test_clean_qubits():
    ...
    assert str(q) == '_c(1)'
    assert repr(q) == 'alphaclops.ops.CleanQubit(1)'

    q = cqi.CleanQubit(2, dim=3)
    assert q.id == 2
    assert q.dimension == 3
    assert str(q) == '_c(2) (d=3)'
    assert repr(q) == 'alphaclops.ops.CleanQubit(2, dim=3)'
...
def test_borrow_qubits():
    ...
    assert q.dimension == 2
    assert str(q) == '_b(10)'
    assert repr(q) == 'alphaclops.ops.BorrowableQubit(10)'

    q = cqi.BorrowableQubit(20, dim=4)
    assert q.id == 20
    assert q.dimension == 4
    assert str(q) == '_b(20) (d=4)'
    assert repr(q) == 'alphaclops.ops.BorrowableQubit(20, dim=4)'
...
def test_simple_qubit_manager(_):
    qm = alphaclops.ops.SimpleQubitManager()
