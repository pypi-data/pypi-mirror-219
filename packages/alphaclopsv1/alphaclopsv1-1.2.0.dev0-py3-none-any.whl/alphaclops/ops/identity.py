...
class IdentityGate(raw_types.Gate):
    ...
    `alphaclops.I` is the single qubit identity gate.
    """
    ...
    def __repr__(self) -> str:
        ...
        if all(e == 2 for e in self._qid_shape):
            return f'alphaclops.IdentityGate({len(self._qid_shape)})'
        return f'alphaclops.IdentityGate(qid_shape={self._qid_shape!r})'
    ...
    def _mul_with_qubits(self, qubits: Tuple['alphaclops.Qid', ...], other):
        ...
        if isinstance(other, (complex, float, int)):
            from alphaclops.ops.pauli_string import PauliString
...
def identity_each(*qubits: 'alphaclops.Qid') -> 'alphaclops.Operation':
    ...
    for qubit in qubits:
        if not isinstance(qubit, raw_types.Qid):
            raise ValueError(f'Not a alphaclops.Qid: {qubit!r}.')
