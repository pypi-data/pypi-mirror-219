...
from alphaclops import linalg, protocols, value
...
class CCZPowGate(gate_features.InterchangeableQubitsGate, eigen_gate.EigenGate):
    ...
    def __repr__(self) -> str:
        if self._global_shift == 0:
            if self._exponent == 1:
                return 'alphaclops.CCZ'
            return f'(alphaclops.CCZ**{proper_repr(self._exponent)})'
        return 'alphaclops.CCZPowGate(exponent={}, global_shift={!r})'.format(
...
class ThreeQubitDiagonalGate(raw_types.Gate):
    ...
    def __repr__(self) -> str:
        return 'alphaclops.ThreeQubitDiagonalGate([{}])'.format(
...
class CCXPowGate(gate_features.InterchangeableQubitsGate, eigen_gate.EigenGate):
    ...
    def __repr__(self) -> str:
        if self._global_shift == 0:
            if self._exponent == 1:
                return 'alphaclops.TOFFOLI'
            return f'(alphaclops.TOFFOLI**{proper_repr(self._exponent)})'
        return 'alphaclops.CCXPowGate(exponent={}, global_shift={!r})'.format(
            proper_repr(self._exponent), self._global_shift
        )
...
class CSwapGate(gate_features.InterchangeableQubitsGate, raw_types.Gate):
    ...
    def _decompose_inside_control(
        self, target1: 'alphaclops.Qid', control: 'alphaclops.Qid', target2: 'alphaclops.Qid'
    ) -> 'alphaclops.OP_TREE':
    ...
    def _decompose_outside_control(
        self, control: 'alphaclops.Qid', near_target: 'alphaclops.Qid', far_target: 'alphaclops.Qid'
    ) -> 'alphaclops.OP_TREE':
...
document(
    ...
    The `exponent=1` instance of `alphaclops.CCZPowGate`.
...
document(
    ...
    The `exponent=1` instance of `alphaclops.CCXPowGate`.
    ...
    Alternative names: `alphaclops.CCNOT` and `alphaclops.TOFFOLI`.
    """,
)
...
document(
    ...
    An instance of `alphaclops.CSwapGate`.
    ...
    Alternative names: `alphaclops.FREDKIN`.
    """,
)
