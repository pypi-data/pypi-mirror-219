...
def test_projector_matrix():
    ...
    zero_projector = alphaclops.ProjectorString({q0: 0})
    one_projector = alphaclops.ProjectorString({q0: 1})
    coeff_projector = alphaclops.ProjectorString({q0: 0}, 1.23 + 4.56j)
...
def test_projector_repr():
    ...
    assert (
        repr(alphaclops.ProjectorString({q0: 0}))
        == "alphaclops.ProjectorString(projector_dict={alphaclops.NamedQubit('q0'): 0},coefficient=(1+0j))"
    )
...
def test_projector_matrix_missing_qid():
    ...
    proj = alphaclops.ProjectorString({q0: 0})
    proj_with_coefficient = alphaclops.ProjectorString({q0: 0}, 1.23 + 4.56j)
...
def test_equality():
    ...
    obj1a = alphaclops.ProjectorString({q0: 0})
    obj1b = alphaclops.ProjectorString({q0: 0})
    obj2 = alphaclops.ProjectorString({q0: 1})
    obj3 = alphaclops.ProjectorString({q0: 1}, coefficient=0.20160913)
...
def test_get_values():
    q0 = alphaclops.NamedQubit('q0')
    d = alphaclops.ProjectorString({q0: 0}, 1.23 + 4.56j)
...
def test_expectation_from_state_vector_basis_states_single_qubits():
    q0 = alphaclops.NamedQubit('q0')
    d = alphaclops.ProjectorString({q0: 0})
...
def test_expectation_from_state_vector_basis_states_three_qubits():
    ...
    d_1qbit = alphaclops.ProjectorString({q1: 1})
    d_2qbits = alphaclops.ProjectorString({q0: 0, q1: 1})

    state_vector = alphaclops.testing.random_superposition(8)
...
def test_expectation_from_density_matrix_three_qubits():
    ...
    state = alphaclops.testing.random_density_matrix(8)
...
def test_consistency_state_vector_and_density_matrix():
    ...
    for proj_qubit in q0, q1, q2:
        for proj_idx in [0, 1]:
            d = alphaclops.ProjectorString({proj_qubit: proj_idx})
...
def test_expectation_higher_dims():
    qubit = alphaclops.NamedQid('q0', dimension=2)
    qutrit = alphaclops.NamedQid('q1', dimension=3)

    with pytest.raises(ValueError, match="Only qubits are supported"):
        alphaclops.ProjectorString({qutrit: 0})

    d = alphaclops.ProjectorString({qubit: 0})
    with pytest.raises(ValueError, match="Only qubits are supported"):
...
def test_expectation_with_coefficient():
    q0 = alphaclops.NamedQubit('q0')
    d = alphaclops.ProjectorString({q0: 0}, coefficient=(0.6 + 0.4j))
