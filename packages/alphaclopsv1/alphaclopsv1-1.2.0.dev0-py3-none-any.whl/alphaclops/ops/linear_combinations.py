...
from alphaclops import linalg, protocols, qis, value
from alphaclops._doc import document
from alphaclops.linalg import operator_spaces
from alphaclops.ops import identity, raw_types, pauli_gates, pauli_string
from alphaclops.ops.pauli_string import PauliString, _validate_qubit_mapping
from alphaclops.ops.projector import ProjectorString
from alphaclops.value.linear_dict import _format_terms
...
class LinearCombinationOfGates(value.LinearDict[raw_types.Gate]):
    ...
    use overloaded arithmetic operators. For example,
        ...
        alphaclops.LinearCombinationOfGates({alphaclops.X: 2, alphaclops.Z: -2})

    is equivalent to

        2 * alphaclops.X - 2 * alphaclops.Z
    """
    ...
    def _is_compatible(self, gate: 'alphaclops.Gate') -> bool:
...
class LinearCombinationOfOperations(value.LinearDict[raw_types.Operation]):
    ...
    def _is_compatible(self, operation: 'alphaclops.Operation') -> bool:
    ...
    def _pauli_expansion_(self) -> value.LinearDict[str]:
        ...
        def extend_term(
            pauli_names: str, qubits: Tuple['alphaclops.Qid', ...], all_qubits: Tuple['alphaclops.Qid', ...]
        ) -> str:
        ...
        def extend(
            ...
            all_qubits: Tuple['alphaclops.Qid', ...],
...
class PauliSum:
    ...
    Since `alphaclops.PauliString`s store their own coefficients, this class
    does not implement the `alphaclops.LinearDict` interface. Instead, you can
    ...
    >>> a, b = alphaclops.TensorCircuit.rect(1, 2)
    >>> psum = alphaclops.PauliSum.from_pauli_strings([
    ...     alphaclops.PauliString(-1, alphaclops.X(a), alphaclops.Y(b)),
    ...     alphaclops.PauliString(2, alphaclops.Z(a), alphaclops.Z(b)),
    ...     alphaclops.PauliString(0.5, alphaclops.Y(a), alphaclops.Y(b))
    ... ])
    >>> print(psum)
    ...
    >>> a, b = alphaclops.TensorCircuit.rect(1, 2)
    >>> psum = alphaclops.X(a) * alphaclops.X(b) + 3.0 * alphaclops.Y(a)
    >>> print(psum)
    ...
    def __init__(self, linear_dict: Optional[value.LinearDict[UnitPauliStringT]] = None):
        ...
        Args:
            linear_dict: Set of  (`alphaclops.Qid`, `alphaclops.Pauli`) tuples to construct the sum
                from.

        Raises:
            ValueError: If structure of `linear_dict` contains tuples other than the
                form (`alphaclops.Qid`, `alphaclops.Pauli`).
        """
    ...
    def wrap(val: PauliSumLike) -> 'PauliSum':
        """Convert a `alphaclops.PauliSumLike` object to a PauliSum

        Attempts to convert an existing int, float, complex, `alphaclops.PauliString`,
        `alphaclops.PauliSum` or `alphaclops.SingleQubitPauliStringGateOperation` into
        a `alphaclops.PauliSum` object. For example:


        >>> my_psum = alphaclops.PauliSum.wrap(2.345)
        >>> my_psum
        alphaclops.PauliSum(alphaclops.LinearDict({frozenset(): (2.345+0j)}))


        Args:
            `alphaclops.PauliSumLike` to convert to PauliSum.
    ...
    def from_pauli_strings(cls, terms: Union[PauliString, List[PauliString]]) -> 'PauliSum':
        """Returns a PauliSum by combining `alphaclops.PauliString` terms.

        Args:
            terms: `alphaclops.PauliString` or List of `alphaclops.PauliString`s to use inside
                of this PauliSum object.
        Returns:
            PauliSum object representing the addition of all the `alphaclops.PauliString`
                terms in `terms`.
        """
    ...
    def from_boolean_expression(
        cls, boolean_expr: Expr, qubit_map: Dict[str, 'alphaclops.Qid']
    ) -> 'PauliSum':
    ...
    def with_qubits(self, *new_qubits: 'alphaclops.Qid') -> 'PauliSum':
        """Return a new PauliSum on `new_qubits`.

        Args:
            *new_qubits: `alphaclops.Qid` objects to replace existing
    ...
    def expectation_from_state_vector(
        ...
        *,
        ...
        # Github issue: https://github.com/quantumlib/alphaclops/issues/2041
    ...
    def __repr__(self) -> str:
        ...
        return f'alphaclops.{class_name}({self._linear_dict!r})'
