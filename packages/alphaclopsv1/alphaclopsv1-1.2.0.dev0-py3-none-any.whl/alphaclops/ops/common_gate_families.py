...
"""Common Gate Families used in alphaclops-core"""

from typing import Any, cast, Optional, Type, Union

from alphaclops.ops import gateset, raw_types, parallel_gate, eigen_gate
...
class AnyUnitaryGateFamily(gateset.GateFamily):
    ...
    def __repr__(self) -> str:
        return f'alphaclops.AnyUnitaryGateFamily(num_qubits = {self._num_qubits})'
...
class AnyIntegerPowerGateFamily(gateset.GateFamily):
    """GateFamily which accepts instances of a given `alphaclops.EigenGate`, raised to integer power."""

    def __init__(self, gate: Type[eigen_gate.EigenGate]) -> None:
        """Init AnyIntegerPowerGateFamily

        Args:
            gate: A subclass of `alphaclops.EigenGate` s.t. an instance `g` of `gate` will be
                accepted if `g.exponent` is an integer.

        Raises:
            ValueError: If `gate` is not a subclass of `alphaclops.EigenGate`.
        """
        if not (isinstance(gate, type) and issubclass(gate, eigen_gate.EigenGate)):
            raise ValueError(f'{gate} must be a subclass of `alphaclops.EigenGate`.')
        super().__init__(
            gate,
    ...
    def __repr__(self) -> str:
        return f'alphaclops.AnyIntegerPowerGateFamily({self._gate_str()})'
...
class ParallelGateFamily(gateset.GateFamily):
    """GateFamily which accepts instances of `alphaclops.ParallelGate` and its sub_gate.

    ParallelGateFamily is useful for description and validation of scenarios where multiple
    copies of a unitary gate can act in parallel. `alphaclops.ParallelGate` is used to express
    ...
    *    Gate Types whose instances can be applied in parallel.
    ...
    To verify containment of a given `alphaclops.Gate` instance `g`, the gate family verfies that:

    *    `alphaclops.num_qubits(g)` <= `max_parallel_allowed` if `max_parallel_allowed` is not None.
    *    `g` or `g.sub_gate` (if `g` is an instance of `alphaclops.ParallelGate`) is an accepted gate
        ...
        *,
        ...
        Args:
            gate: The gate which can act in parallel. It can be a python `type` inheriting from
                `alphaclops.Gate` or a non-parameterized instance of a `alphaclops.Gate`. If an instance of
                `alphaclops.ParallelGate` is passed, then the corresponding `gate.sub_gate` is used.
    ...
    def _default_description(self) -> str:
        ...
        return (
            f'Accepts\n'
            f'1. `alphaclops.Gate` instances `g` s.t. `{check_type.format(self._gate_str())}` OR\n'
            f'2. `alphaclops.ParallelGate` instance `g` s.t. `g.sub_gate` satisfies 1. and '
            f'`alphaclops.num_qubits(g) <= {self._max_parallel_str()}` OR\n'
            f'3. `alphaclops.Operation` instance `op` s.t. `op.gate` satisfies 1. or 2.'
        )
    ...
    def __repr__(self) -> str:
        ...
        return (
            f'alphaclops.ParallelGateFamily('
