...
from alphaclops.ops import raw_types, pauli_string
...
def measure_single_paulistring(
    ...
    key: Optional[Union[str, 'alphaclops.MeasurementKey']] = None,
) -> raw_types.Operation:
    """Returns a single PauliMeasurementGate which measures the pauli observable

    Args:
        pauli_observable: The `alphaclops.PauliString` observable to measure.
        key: Optional `str` or `alphaclops.MeasurementKey` that gate should use.
    ...
    if not isinstance(pauli_observable, pauli_string.PauliString):
        raise ValueError(
            f'Pauli observable {pauli_observable} should be an instance of alphaclops.PauliString.'
        )
...
def measure_paulistring_terms(
    ...
    Args:
        pauli_basis: The `alphaclops.PauliString` basis in which each qubit should
            be measured.
    ...
    Raises:
        ValueError: if `pauli_basis` is not an instance of `alphaclops.PauliString`.
    """
    if not isinstance(pauli_basis, pauli_string.PauliString):
        raise ValueError(
            f'Pauli observable {pauli_basis} should be an instance of alphaclops.PauliString.'
        )
...
def measure(
    *target,
    ...
    used with the alias `alphaclops.M`.
