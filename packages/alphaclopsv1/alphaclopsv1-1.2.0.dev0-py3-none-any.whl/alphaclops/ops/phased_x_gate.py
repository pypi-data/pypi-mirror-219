...
from alphaclops.ops import common_gates, raw_types
...
class PhasedXPowGate(raw_types.Gate):
    ...
    The unitary matrix of `alphaclops.PhasedXPowGate(exponent=t, phase_exponent=p)` is:
    $$
        \begin{bmatrix}
    ...
    This gate is like an `alphaclops.XPowGate`, but which has been "phased",
    by applying a `alphaclops.ZPowGate` before and after this gate. In the language
    ...
    def _decompose_(self, qubits: Sequence['alphaclops.Qid']) -> 'alphaclops.OP_TREE':
        assert len(qubits) == 1
        q = qubits[0]
        z = alphaclops.Z(q) ** self._phase_exponent
        x = alphaclops.XPowGate(exponent=self._exponent, global_shift=self.global_shift).on(q)
        return z**-1, x, z
    ...
    def _unitary_(self) -> Union[np.ndarray, NotImplementedType]:
        ...
        if self._is_parameterized_():
            return None
        z = protocols.unitary(alphaclops.Z ** self._phase_exponent)
        x = protocols.unitary(alphaclops.X ** self._exponent)
    ...
    def __repr__(self) -> str:
        ...
        args_str = ', '.join(args)
        return f'alphaclops.PhasedXPowGate({args_str})'
