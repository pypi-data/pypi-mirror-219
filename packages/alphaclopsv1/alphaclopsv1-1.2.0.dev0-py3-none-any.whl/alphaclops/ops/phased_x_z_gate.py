...
from alphaclops import value, ops, protocols, linalg
...
class PhasedXZGate(raw_types.Gate):
    ...
    The unitary matrix of `alphaclops.PhasedXZGate(x_exponent=x, z_exponent=z, axis_phase_exponent=a)` is:
    $$
        \begin{bmatrix}
    ...
    This gate can be thought of as a `alphaclops.PhasedXPowGate` followed by a `alphaclops.ZPowGate`.
    ...
    Every single qubit gate can be written as a single `alphaclops.PhasedXZGate`.
    """
    ...
    def _canonical(self) -> 'alphaclops.PhasedXZGate':
        x = self.x_exponent
        z = self.z_exponent
    ...
    @staticmethod
    def from_matrix(mat: np.ndarray) -> 'alphaclops.PhasedXZGate':
    ...
    def with_z_exponent(self, z_exponent: Union[float, sympy.Expr]) -> 'alphaclops.PhasedXZGate':
        return PhasedXZGate(
    ...
    def _qasm_(self, args: 'alphaclops.QasmArgs', qubits: Tuple['alphaclops.Qid', ...]) -> Optional[str]:
        from alphaclops.circuits import qasm_output
    ...
    def _unitary_(self) -> Optional[np.ndarray]:
        """See `alphaclops.SupportsUnitary`."""
    ...
    def _is_parameterized_(self) -> bool:
        """See `alphaclops.SupportsParameterization`."""
        return (
    ...
    def _resolve_parameters_(
        ...
        if isinstance(z_exponent, (complex, numbers.Complex)):
            if isinstance(z_exponent, numbers.Real):
                ...
                raise ValueError(f'Complex exponent {z_exponent} not allowed in alphaclops.PhasedXZGate')
        if isinstance(x_exponent, (complex, numbers.Complex)):
            if isinstance(x_exponent, numbers.Real):
                x_exponent = float(x_exponent)
            else:
                raise ValueError(f'Complex exponent {x_exponent} not allowed in alphaclops.PhasedXZGate')
        if isinstance(axis_phase_exponent, (complex, numbers.Complex)):
            if isinstance(axis_phase_exponent, numbers.Real):
                ...
                raise ValueError(
                    f'Complex exponent {axis_phase_exponent} not allowed in alphaclops.PhasedXZGate'
                )
        return PhasedXZGate(
            z_exponent=z_exponent, x_exponent=x_exponent, axis_phase_exponent=axis_phase_exponent
        )

    def _phase_by_(self, phase_turns, qubit_index) -> 'alphaclops.PhasedXZGate':
    ...
    def _circuit_diagram_info_(self, args: 'alphaclops.CircuitDiagramInfoArgs') -> str:
        """See `alphaclops.SupportsCircuitDiagramInfo`."""
        return (
            f'PhXZ('
