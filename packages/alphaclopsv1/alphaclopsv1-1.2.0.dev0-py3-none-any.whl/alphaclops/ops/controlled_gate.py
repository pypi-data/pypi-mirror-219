...
from alphaclops import protocols, value, _import
...
controlled_gate_decomposition = _import.LazyLoader(
    'controlled_gate_decomposition', globals(), 'alphaclops.transformers.analytical_decompositions'
)
common_gates = _import.LazyLoader('common_gates', globals(), 'alphaclops.ops')
line_qubit = _import.LazyLoader('line_qubit', globals(), 'alphaclops.devices')
...
class ControlledGate(raw_types.Gate):
    ...
    def __init__(
        ...
        if num_controls != protocols.num_qubits(control_values):
            raise ValueError('alphaclops.num_qubits(control_values) != num_controls')
    ...
    @property
    def sub_gate(self) -> 'alphaclops.Gate':
    ...
    ) -> Union[None, NotImplementedType, 'alphaclops.OP_TREE']:
        return self._decompose_with_context_(qubits)

    def _decompose_with_context_(
        self, qubits: Tuple['alphaclops.Qid', ...], context: Optional['alphaclops.DecompositionContext'] = None
    ) -> Union[None, NotImplementedType, 'alphaclops.OP_TREE']:
        if (
            ...
            invert_ops: List['alphaclops.Operation'] = []
        ...
        if isinstance(self.sub_gate, matrix_gates.MatrixGate):
            # Default decompositions of 2/3 qubit `alphaclops.MatrixGate` ignores global phase, which is
    ...
    def on(self, *qubits: 'alphaclops.Qid') -> cop.ControlledOperation:
        if len(qubits) == 0:
    ...
    def __repr__(self) -> str:
        if self.num_controls() == 1 and self.control_values.is_trivial:
            return f'alphaclops.ControlledGate(sub_gate={self.sub_gate!r})'

        if self.control_values.is_trivial and set(self.control_qid_shape) == {2}:
            return (
                f'alphaclops.ControlledGate(sub_gate={self.sub_gate!r}, '
...
def _validate_sub_object(sub_object: Union['alphaclops.Gate', 'alphaclops.Operation']):
