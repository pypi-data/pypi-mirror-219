...
import alphaclops
import alphaclops.ops.boolean_hamiltonian as bh
...
def test_circuit(boolean_str):
    ...
    var_names = alphaclops.parameter_names(boolean_expr)
    qubits = [alphaclops.NamedQubit(name) for name in var_names]
    ...
    circuit.append(alphaclops.H.on_each(*qubits))

    hamiltonian_gate = alphaclops.BooleanHamiltonianGate(
    ...
    phi = alphaclops.Simulator().simulate(circuit, qubit_order=qubits, initial_state=0).state_vector()
...
def test_gate_with_custom_names():
    ...
    gate = alphaclops.BooleanHamiltonianGate(['a', 'b'], ['a'], 0.1)
    assert alphaclops.decompose(gate.on(q0, q1)) == [alphaclops.Rz(rads=-0.05).on(q0)]
    assert alphaclops.decompose_once_with_qubits(gate, (q0, q1)) == [alphaclops.Rz(rads=-0.05).on(q0)]
    assert alphaclops.decompose(gate.on(q2, q3)) == [alphaclops.Rz(rads=-0.05).on(q2)]
    assert alphaclops.decompose_once_with_qubits(gate, (q2, q3)) == [alphaclops.Rz(rads=-0.05).on(q2)]

    with pytest.raises(ValueError, match='Wrong number of qubits'):
        gate.on(q2)
    with pytest.raises(ValueError, match='Wrong shape of qids'):
        gate.on(q0, alphaclops.LineQid(1, 3))


def test_gate_consistent():
    gate = alphaclops.BooleanHamiltonianGate(['a', 'b'], ['a'], 0.1)
    op = gate.on(*alphaclops.LineQubit.range(2))
...
def test_simplify_cnots_triplets(
    ...
    qubits = {qubit_id: alphaclops.NamedQubit(f"{qubit_id}") for qubit_id in qubit_ids}

    target, control = (0, 1) if input_flip_control_and_target else (1, 0)

    circuit_input = alphaclops.Circuit()
    for input_cnot in input_cnots:
        circuit_input.append(alphaclops.CNOT(qubits[input_cnot[target]], qubits[input_cnot[control]]))
    circuit_actual = alphaclops.Circuit()
    for actual_cnot in actual_output_cnots:
        circuit_actual.append(alphaclops.CNOT(qubits[actual_cnot[target]], qubits[actual_cnot[control]]))

    np.testing.assert_allclose(alphaclops.unitary(circuit_input), alphaclops.unitary(circuit_actual), atol=1e-6)
