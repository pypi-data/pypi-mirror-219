...
def test_state_prep_channel_kraus(state):
    ...
    gate = alphaclops.StatePreparationChannel(state)(qubits[0], qubits[1])
    alphaclops.testing.assert_consistent_channel(gate)
    assert not alphaclops.has_mixture(gate)
    state = state / np.linalg.norm(state)
    np.testing.assert_almost_equal(
        alphaclops.kraus(gate),
        (
...
def test_state_prep_channel_kraus_small():
    gate = alphaclops.StatePreparationChannel(np.array([0.0, 1.0]))(alphaclops.LineQubit(0))
    np.testing.assert_almost_equal(
        alphaclops.kraus(gate), (np.array([[0.0, 0.0], [1.0, 0.0]]), np.array([[0.0, 0.0], [0.0, 1.0]]))
    )
    assert alphaclops.has_kraus(gate)
    assert not alphaclops.has_mixture(gate)

    gate = alphaclops.StatePreparationChannel(np.array([1.0, 0.0]))(alphaclops.LineQubit(0))
    np.testing.assert_almost_equal(
        alphaclops.kraus(gate), (np.array([[1.0, 0.0], [0.0, 0.0]]), np.array([[0.0, 1.0], [0.0, 0.0]]))
    )
    assert alphaclops.has_kraus(gate)
    assert not alphaclops.has_mixture(gate)
...
def test_state_prep_gate_printing():
    ...
    qubits = alphaclops.LineQubit.range(2)
    gate = alphaclops.StatePreparationChannel(np.array([1, 0, 0, 1]) / np.sqrt(2))
    circuit.append(alphaclops.H(qubits[0]))
    circuit.append(alphaclops.CNOT(qubits[0], qubits[1]))
...
def test_state_prep_gate_printing_with_name(name):
    circuit = alphaclops.Circuit()
    qubits = alphaclops.LineQubit.range(2)
    gate = alphaclops.StatePreparationChannel(np.array([1, 0, 0, 1]) / np.sqrt(2), name=name)
    circuit.append(alphaclops.H(qubits[0]))
    circuit.append(alphaclops.CNOT(qubits[0], qubits[1]))
    circuit.append(gate(qubits[0], qubits[1]))
    alphaclops.testing.assert_has_diagram(
...
def test_gate_params():
    ...
    gate = alphaclops.StatePreparationChannel(state)
...
def test_gate_error_handling():
    with pytest.raises(ValueError, match='`target_state` must be a 1d numpy array.'):
        alphaclops.StatePreparationChannel(np.eye(2))
    with pytest.raises(ValueError, match='Matrix width \\(5\\) is not a power of 2'):
        alphaclops.StatePreparationChannel(np.ones(shape=5))


def test_equality_of_gates():
    state = np.array([1, 0, 0, 0], dtype=np.complex64)
    gate_1 = alphaclops.StatePreparationChannel(state)
    gate_2 = alphaclops.StatePreparationChannel(state)
    ...
    gate_3 = alphaclops.StatePreparationChannel(state, name='gate_a')
    gate_4 = alphaclops.StatePreparationChannel(state, name='gate_b')
...
def test_approx_equality_of_gates():
    ...
    gate_1 = alphaclops.StatePreparationChannel(state)
    gate_2 = alphaclops.StatePreparationChannel(state)
    assert alphaclops.approx_eq(gate_1, gate_2), "Equal state not leading to same gate"
    assert not alphaclops.approx_eq(gate_1, state), "Different object types cannot be approx equal"
    perturbed_state = np.array([1 - 1e-9, 1e-10, 0, 0], dtype=np.complex64)
    gate_3 = alphaclops.StatePreparationChannel(perturbed_state)
    assert alphaclops.approx_eq(gate_3, gate_1), "Almost equal states should lead to the same gate"
    different_state = np.array([1 - 1e-5, 1e-4, 0, 0], dtype=np.complex64)
    gate_4 = alphaclops.StatePreparationChannel(different_state)
    assert not alphaclops.approx_eq(gate_4, gate_1), "Different states should not lead to the same gate"
