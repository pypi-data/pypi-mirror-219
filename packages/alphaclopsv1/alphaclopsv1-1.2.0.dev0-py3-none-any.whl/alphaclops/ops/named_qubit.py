...
class NamedQid(_BaseNamedQid):
    ...
    containing `alphaclops.NamedQid('qid22', dimension=3)` and
    `alphaclops.NamedQid('qid3', dimension=3)`, the wire for 'qid3' will
    ...
    def __repr__(self) -> str:
        return f'alphaclops.NamedQid({self.name!r}, dimension={self.dimension})'
    ...
    def range(*args, prefix: str, dimension: int) -> List['NamedQid']:
        ...
        each number in the range, e.g.:
            ...
            >>> alphaclops.NamedQid.range(3, prefix='a', dimension=3)
            ... # doctest: +NORMALIZE_WHITESPACE
            [alphaclops.NamedQid('a0', dimension=3), alphaclops.NamedQid('a1', dimension=3),
                alphaclops.NamedQid('a2', dimension=3)]
            >>> alphaclops.NamedQid.range(2, 4, prefix='a', dimension=3)
            [alphaclops.NamedQid('a2', dimension=3), alphaclops.NamedQid('a3', dimension=3)]
...
class NamedQubit(_BaseNamedQid):
    ...
    the name are handled correctly. So, for example, if you print a circuit
    containing `alphaclops.NamedQubit('qubit22')` and `alphaclops.NamedQubit('qubit3')`, the
    ...
    def __repr__(self) -> str:
        return f'alphaclops.NamedQubit({self._name!r})'

    @staticmethod
    def range(*args, prefix: str) -> List['NamedQubit']:
        r"""Returns a range of `alphaclops.NamedQubit`s.

        The range returned starts with the prefix, and followed by a qubit for
        each number in the range, e.g.:

            >>> alphaclops.NamedQubit.range(3, prefix='a')
            ... # doctest: +NORMALIZE_WHITESPACE
            [alphaclops.NamedQubit('a0'), alphaclops.NamedQubit('a1'),
                alphaclops.NamedQubit('a2')]
            >>> alphaclops.NamedQubit.range(2, 4, prefix='a')
            [alphaclops.NamedQubit('a2'), alphaclops.NamedQubit('a3')]
