...
from alphaclops.testing import assert_has_consistent_trace_distance_bound


class CExpZinGate(alphaclops.EigenGate, alphaclops.testing.TwoQubitGate):
...
class ZGateDef(alphaclops.EigenGate, alphaclops.testing.TwoQubitGate):
    @property
    def exponent(self):
        return self._exponent
...
def test_approximate_common_period():
    from alphaclops.ops.eigen_gate import _approximate_common_period as f
...
def test_approx_eq():
    assert alphaclops.approx_eq(CExpZinGate(1.5), CExpZinGate(1.5), atol=0.1)
    assert alphaclops.approx_eq(CExpZinGate(1.5), CExpZinGate(1.7), atol=0.3)
    assert not alphaclops.approx_eq(CExpZinGate(1.5), CExpZinGate(1.7), atol=0.1)

    assert alphaclops.approx_eq(ZGateDef(exponent=1.5), ZGateDef(exponent=1.5), atol=0.1)
    assert not alphaclops.approx_eq(CExpZinGate(1.5), ZGateDef(exponent=1.5), atol=0.1)
    with pytest.raises(
        TypeError,
        match=re.escape("unsupported operand type(s) for -: 'Symbol' and 'PeriodicValue'"),
    ):
        alphaclops.approx_eq(ZGateDef(exponent=1.5), ZGateDef(exponent=sympy.Symbol('a')), atol=0.1)
    assert alphaclops.approx_eq(CExpZinGate(sympy.Symbol('a')), CExpZinGate(sympy.Symbol('a')), atol=0.1)
    with pytest.raises(
        AttributeError,
...
def test_approx_eq_periodic():
    assert alphaclops.approx_eq(CExpZinGate(1.5), CExpZinGate(5.5), atol=1e-9)
    assert alphaclops.approx_eq(CExpZinGate(1.5), CExpZinGate(9.5), atol=1e-9)
    assert alphaclops.approx_eq(CExpZinGate(-2.5), CExpZinGate(1.5), atol=1e-9)
    assert not alphaclops.approx_eq(CExpZinGate(0), CExpZinGate(1.5), atol=1e-9)

    # The tests below do not work with usual canonical exponent comparison.
    assert alphaclops.approx_eq(CExpZinGate(0 - 1e-10), CExpZinGate(0), atol=1e-9)
    assert alphaclops.approx_eq(CExpZinGate(0), CExpZinGate(4 - 1e-10), atol=1e-9)


def test_period():
    class Components(alphaclops.EigenGate, alphaclops.testing.TwoQubitGate):
...
def test_inverse():
    assert alphaclops.inverse(CExpZinGate(0.25)) == CExpZinGate(-0.25)
    assert alphaclops.inverse(CExpZinGate(sympy.Symbol('a'))) == CExpZinGate(-sympy.Symbol('a'))


def test_trace_distance_bound():
    assert alphaclops.trace_distance_bound(CExpZinGate(0.001)) < 0.01
    assert alphaclops.trace_distance_bound(CExpZinGate(sympy.Symbol('a'))) == 1
    assert alphaclops.approx_eq(alphaclops.trace_distance_bound(CExpZinGate(2)), 1)

    class E(alphaclops.EigenGate):
...
def test_extrapolate():
    h = CExpZinGate(2)
    assert alphaclops.pow(h, 1.5) is not None
    assert alphaclops.inverse(h, None) is not None

    p = CExpZinGate(0.1)
    assert alphaclops.pow(p, 1.5) is not None
    assert alphaclops.inverse(p) is not None

    s = CExpZinGate(sympy.Symbol('a'))
    assert alphaclops.pow(s, 1.5) == CExpZinGate(sympy.Symbol('a') * 1.5)
    assert alphaclops.inverse(s) == CExpZinGate(-sympy.Symbol('a'))


def test_matrix():

    for n in [1, 2, 3, 4, 0.0001, 3.9999]:
        assert alphaclops.has_unitary(CExpZinGate(n))

    np.testing.assert_allclose(alphaclops.unitary(CExpZinGate(1)), np.diag([1, 1, 1j, -1j]), atol=1e-8)

    np.testing.assert_allclose(alphaclops.unitary(CExpZinGate(2)), np.diag([1, 1, -1, -1]), atol=1e-8)

    np.testing.assert_allclose(alphaclops.unitary(CExpZinGate(3)), np.diag([1, 1, -1j, 1j]), atol=1e-8)

    np.testing.assert_allclose(alphaclops.unitary(CExpZinGate(4)), np.diag([1, 1, 1, 1]), atol=1e-8)

    np.testing.assert_allclose(
        alphaclops.unitary(CExpZinGate(0.00001)), alphaclops.unitary(CExpZinGate(3.99999)), atol=1e-4
    )

    assert not np.allclose(
        alphaclops.unitary(CExpZinGate(0.00001)), alphaclops.unitary(CExpZinGate(1.99999)), atol=1e-4
    )

    assert not alphaclops.has_unitary(CExpZinGate(sympy.Symbol('a')))
    assert alphaclops.unitary(CExpZinGate(sympy.Symbol('a')), None) is None

    np.testing.assert_allclose(alphaclops.unitary(ZGateDef(exponent=0)), np.eye(2), atol=1e-8)

    np.testing.assert_allclose(alphaclops.unitary(ZGateDef(exponent=1)), np.diag([1, -1]), atol=1e-8)

    np.testing.assert_allclose(alphaclops.unitary(ZGateDef(exponent=0.5)), np.diag([1, 1j]), atol=1e-8)

    np.testing.assert_allclose(
        alphaclops.unitary(ZGateDef(exponent=1, global_shift=0.5)), np.diag([1j, -1j]), atol=1e-8
    )

    np.testing.assert_allclose(
        alphaclops.unitary(ZGateDef(exponent=0.5, global_shift=0.5)),
...
def test_matrix_is_exact_for_quarter_turn():
    np.testing.assert_equal(alphaclops.unitary(CExpZinGate(1)), np.diag([1, 1, 1j, -1j]))


def test_is_parameterized():
    assert not alphaclops.is_parameterized(CExpZinGate(0))
    assert not alphaclops.is_parameterized(CExpZinGate(1))
    assert not alphaclops.is_parameterized(CExpZinGate(3))
    assert alphaclops.is_parameterized(CExpZinGate(sympy.Symbol('a')))
...
def test_resolve_parameters(resolve_fn):
    assert resolve_fn(
        CExpZinGate(sympy.Symbol('a')), alphaclops.ParamResolver({'a': 0.5})
    ) == CExpZinGate(0.5)

    assert resolve_fn(CExpZinGate(0.25), alphaclops.ParamResolver({})) == CExpZinGate(0.25)

    with pytest.raises(ValueError, match='Complex exponent'):
        resolve_fn(CExpZinGate(sympy.Symbol('a')), alphaclops.ParamResolver({'a': 0.5j}))


def test_diagram_period():
    class ShiftyGate(alphaclops.EigenGate, alphaclops.testing.SingleQubitGate):
        def _eigen_components(self) -> List[Tuple[float, np.ndarray]]:
...
class WeightedZPowGate(alphaclops.EigenGate, alphaclops.testing.SingleQubitGate):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (alphaclops.Z, alphaclops.Gate, False),
        ...
        (alphaclops.ZZ ** 1.9, alphaclops.ZZ ** -0.1, True),
        ...
        (alphaclops.rz(np.pi), alphaclops.Z, True),
        (alphaclops.X ** 0.3, alphaclops.Z ** 0.3, False),
        (alphaclops.Z, alphaclops.Z ** (1 - 1e-10), True),
        (alphaclops.Z, alphaclops.Z ** (1 + 1e-10), True),
        (alphaclops.Z, alphaclops.Z ** (3 - 1e-10), True),
        (alphaclops.Z, alphaclops.Z ** (3 + 1e-10), True),
        (alphaclops.Z ** 2, alphaclops.Z ** (4 - 1e-10), True),
        (alphaclops.Z ** 2, alphaclops.Z ** (4 + 1e-10), True),
    ],
)
def test_equal_up_to_global_phase(gate1, gate2, eq_up_to_global_phase):
    assert alphaclops.equal_up_to_global_phase(gate1, gate2) == eq_up_to_global_phase
