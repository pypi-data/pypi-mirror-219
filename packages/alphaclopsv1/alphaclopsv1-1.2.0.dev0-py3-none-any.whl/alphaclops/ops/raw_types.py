...
from alphaclops._compat import __alphaclops_debug__, cached_method
from alphaclops.type_workarounds import NotImplementedType
from alphaclops.ops import control_values as cv

# Lazy imports to break circular dependencies.
ops = LazyLoader("ops", globals(), "alphaclops.ops")
line_qubit = LazyLoader("line_qubit", globals(), "alphaclops.devices.line_qubit")
...
class Qid(metaclass=abc.ABCMeta):
    ...
    example, `alphaclops.LineQubit`'s `_comparison_key` method returns `self.x`. This
...
class Gate(metaclass=value.TwoAltoptives):
    ...
    def validate_args(self, qubits: Sequence['alphaclops.Qid']) -> None:
    ...
    def on(self, *qubits: Qid) -> 'Operation':
        ...
        Returns: a `alphaclops.Operation` which is this gate applied to the given
            qubits.
        """
        return ops.gate_operation.GateOperation(self, list(qubits))

    def on_each(self, *targets: Union[Qid, Iterable[Any]]) -> List['alphaclops.Operation']:
        ...
        """
        operations: List['alphaclops.Operation'] = []
        ...
        if not __alphaclops_debug__.get():
            return [
                op
                for q in targets
                for op in (
                    ...
                    else [self.on(cast('alphaclops.Qid', q))]
                )
            ]
    ...
    def wrap_in_linear_combination(
        ...
        Args:
            coefficient: number coefficient to use in the resulting
                `alphaclops.LinearCombinationOfGates` object.

        Returns:
            `alphaclops.LinearCombinationOfGates` containing self with a
                coefficient of `coefficient`.
        """
        return ops.linear_combinations.LinearCombinationOfGates({self: coefficient})

    def __add__(
        self, other: Union['Gate', 'alphaclops.LinearCombinationOfGates']
    ) -> 'alphaclops.LinearCombinationOfGates':
    ...
    def __neg__(self) -> 'alphaclops.LinearCombinationOfGates':
        return self.wrap_in_linear_combination(coefficient=-1)

    def __mul__(self, other: Union[complex, float, int]) -> 'alphaclops.LinearCombinationOfGates':
        return self.wrap_in_linear_combination(coefficient=other)

    def __rmul__(self, other: Union[complex, float, int]) -> 'alphaclops.LinearCombinationOfGates':
        return self.wrap_in_linear_combination(coefficient=other)

    def __truediv__(self, other: Union[complex, float, int]) -> 'alphaclops.LinearCombinationOfGates':
    ...
    def with_probability(self, probability: 'alphaclops.TParamVal') -> 'alphaclops.Gate':
        ...
        Returns:
            `alphaclops.RandomGateChannel` that applies `self` with probability
    ...
    def controlled(
        ...
        Returns:
            A `alphaclops.Gate` representing `self` controlled by the given control values
                and qubits. This is a `alphaclops.ControlledGate` in the base
    ...
    # to keep backwards compatibility with versions of alphaclops where num_qubits
    ...
    def _mul_with_qubits(self, qubits: Tuple['alphaclops.Qid', ...], other):
        """alphaclops.GateOperation.__mul__ delegates to this method."""
        return NotImplemented

    def _rmul_with_qubits(self, qubits: Tuple['alphaclops.Qid', ...], other):
        """alphaclops.GateOperation.__rmul__ delegates to this method."""
        return NotImplemented
...
class Operation(metaclass=abc.ABCMeta):
    ...
    @property
    def gate(self) -> Optional['alphaclops.Gate']:
        return None
    ...
    @abc.abstractmethod
    def with_qubits(self, *new_qubits: 'alphaclops.Qid') -> Self:
    ...
    @property
    def untagged(self) -> 'alphaclops.Operation':
        """Returns the underlying operation without any tags."""
        return self

    def with_tags(self, *new_tags: Hashable) -> 'alphaclops.Operation':
    ...
    def with_probability(self, probability: 'alphaclops.TParamVal') -> 'alphaclops.Operation':
    ...
    def validate_args(self, qubits: Sequence['alphaclops.Qid']):
    ...
    @property
    def classical_controls(self) -> FrozenSet['alphaclops.Condition']:
        """The classical controls gating this operation."""
        return frozenset()

    def with_classical_controls(
        self, *conditions: Union[str, 'alphaclops.MeasurementKey', 'alphaclops.Condition', sympy.Expr]
    ) -> 'alphaclops.Operation':
        ...
        or a user-specified `alphaclops.Condition`. A measurement key evaluates to
        True iff any qubit in the corresponding measurement operation evaluated
        to a non-zero value; `alphaclops.Condition` supports more complex,
    ...
    def without_classical_controls(self) -> 'alphaclops.Operation':
...
class TaggedOperation(Operation):
    ...
    def __init__(self, sub_operation: 'alphaclops.Operation', *tags: Hashable):
        self._sub_operation = sub_operation
        self._tags = tuple(tags)

    @property
    def sub_operation(self) -> 'alphaclops.Operation':
    ...
    def with_tags(self, *new_tags: Hashable) -> 'alphaclops.TaggedOperation':
    ...
    def __str__(self) -> str:
        ...
        return f'alphaclops.TaggedOperation({repr(self.sub_operation)}, {tag_repr})'
    ...
    def _decompose_with_context_(
        self, context: Optional['alphaclops.DecompositionContext'] = None
    ) -> 'alphaclops.OP_TREE':
    ...
    def _act_on_(self, sim_state: 'alphaclops.SimulationStateBase') -> bool:
    ...
    def _phase_by_(self, phase_turns: float, qubit_index: int) -> 'alphaclops.Operation':
        return protocols.phase_by(self.sub_operation, phase_turns, qubit_index)

    def __pow__(self, exponent: Any) -> 'alphaclops.Operation':
...
class _InverseCompositeGate(Gate):
    ...
    def _decompose_with_context_(
        self, qubits: Sequence['alphaclops.Qid'], context: Optional['alphaclops.DecompositionContext'] = None
    ) -> 'alphaclops.OP_TREE':
    ...
    def _has_unitary_(self):
        from alphaclops import protocols, devices
    ...
    def _circuit_diagram_info_(self, args: 'alphaclops.CircuitDiagramInfoArgs'):
...
def _validate_qid_shape(val: Any, qubits: Sequence['alphaclops.Qid']) -> None:
