...
from alphaclops.ops.linear_combinations import PauliSum, PauliString
...
class BooleanHamiltonianGate(raw_types.Gate):
    ...
    ) -> 'alphaclops.BooleanHamiltonianGate':
    ...
    def __repr__(self) -> str:
        return (
            f'alphaclops.BooleanHamiltonianGate('
...
def _get_gates_from_hamiltonians(
    hamiltonian_polynomial_list: List['alphaclops.PauliSum'],
    qubit_map: Dict[str, 'alphaclops.Qid'],
    theta: float,
) -> Generator['alphaclops.Operation', None, None]:
    ...
    def _apply_cnots(prevh: Tuple[int, ...], currh: Tuple[int, ...]):
        ...
        for gate in (alphaclops.CNOT(qubits[c], qubits[t]) for c, t in cnots):
            yield gate
    ...
    for h in sorted_hamiltonian_keys:
        ...
        if len(h) >= 1:
            yield alphaclops.Rz(rads=(theta * w)).on(qubits[h[-1]])
