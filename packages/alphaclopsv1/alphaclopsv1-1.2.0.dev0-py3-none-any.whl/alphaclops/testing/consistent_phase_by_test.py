...
class SemiBadPhaser:
    ...
    def _unitary_(self):
        a1 = alphaclops.unitary(GoodPhaser(self.e[0]))
        a2 = alphaclops.unitary(BadPhaser(self.e[1]))
...
def test_assert_phase_by_is_consistent_with_unitary():
    alphaclops.testing.assert_phase_by_is_consistent_with_unitary(GoodPhaser(0.5))

    alphaclops.testing.assert_phase_by_is_consistent_with_unitary(GoodQuditPhaser(0.5))

    with pytest.raises(AssertionError, match='Phased unitary was incorrect for index #0'):
        alphaclops.testing.assert_phase_by_is_consistent_with_unitary(BadPhaser(0.5))

    with pytest.raises(AssertionError, match='Phased unitary was incorrect for index #1'):
        alphaclops.testing.assert_phase_by_is_consistent_with_unitary(SemiBadPhaser([0.5, 0.25]))

    # Vacuous success.
    alphaclops.testing.assert_phase_by_is_consistent_with_unitary(NotPhaser())
