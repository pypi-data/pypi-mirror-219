...
__alphaclops_debug__ = contextvars.ContextVar('__alphaclops_debug__', default=__debug__)
document(
    __alphaclops_debug__,
    "A alphaclops specific flag which can be used to conditionally turn off all validations across alphaclops "
...
def with_debug(value: bool) -> Iterator[None]:
    """Sets the value of global constant `alphaclops.__alphaclops_debug__` within the context.

    If `__alphaclops_debug__` is set to False, all validations in alphaclops are disabled to optimize
    performance. Users should use the `alphaclops.with_debug` context manager instead of manually
    mutating the value of `__alphaclops_debug__` flag. On exit, the context manager resets the
    value of `__alphaclops_debug__` flag to what it was before entering the context manager.
    """
    token = __alphaclops_debug__.set(value)
    try:
        yield
    finally:
        __alphaclops_debug__.reset(token)
...
def dataclass_repr(value: Any, namespace: str = 'alphaclops') -> str:
    """Create a alphaclops-style repr for a dataclass.
    ...
    if _called_from_test() and not deprecation_allowed:
        for filename, line_number, function_name, text in reversed(traceback.extract_stack()):
            if (
                ...
                and not filename.endswith(os.path.join("alphaclops", "_compat.py"))
                and "_test.py" in filename
            ):
                break
        raise ValueError(
            f"During testing using alphaclops deprecated functionality is not allowed: {msg}"
            f"Update to non-deprecated functionality, or alternatively, you can quiet "
            f"this error by removing the alphaclops_TESTING environment variable "
            f"temporarily with `@mock.patch.dict(os.environ, clear='alphaclops_TESTING')`.\n"
            f"In case the usage of deprecated alphaclops is intentional, use "
            f"`with alphaclops.testing.assert_deprecated(...):` around this line:\n"
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def decorated_func(*args, **kwargs) -> Any:
            ...
            _warn_or_error(
                ...
                f'It will be removed in alphaclops {deadline}.\n'
                f'{fix}\n'
            )
...
def _called_from_test() -> bool:
    return 'alphaclops_TESTING' in os.environ
...
def _should_dedupe_module_deprecation() -> bool:
    ...
    """
    force_dedupe = "alphaclops_FORCE_DEDUPE_MODULE_DEPRECATION" in os.environ
...
def deprecated_submodule(
    *, new_module_name: str, old_parent: str, old_child: str, deadline: str, create_attribute: bool
    ...
    For `new_module_name` (e.g. alphaclops_google) creates an alias (e.g alphaclops.google) in Python's module
    cache. It also recursively checks for the already imported submodules (e.g. alphaclops_google.api) and
    creates the alias for them too (e.g. alphaclops.google.api). With this method it is possible to create
    an alias that really looks like a module, e.g you can do things like
    `from alphaclops.google import api` - which would be otherwise impossible.
    ...
    Args:
        ...
        deadline: The version of alphaclops where the module will be removed.
    ...
    if create_attribute:
        try:
            ...
            msg = (
                ...
                f"\n 2.) when developing alphaclops, you don't have your PYTHONPATH "
