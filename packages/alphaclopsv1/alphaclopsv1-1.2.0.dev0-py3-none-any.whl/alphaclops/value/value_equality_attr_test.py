...
@alphaclops.value_equality(manual_cls=True)
...
def test_value_equality_manual():
    ...
    eq.add_equality_group(BasicD(-1))
    assert alphaclops.approx_eq(MasqueradePositiveD(3), BasicD(3))
    assert alphaclops.approx_eq(MasqueradePositiveD(4), MasqueradePositiveD(4))


@alphaclops.value_equality(unhashable=True)
class UnhashableC:
    def __init__(self, x):
        self.x = x
...
@alphaclops.value_equality(distinct_child_types=True)
class DistinctC:
    def __init__(self, x):
        self.x = x
...
@alphaclops.value_equality(approximate=True)
class ApproxE:
    def __init__(self, x):
        self.x = x
...
def test_value_equality_approximate():
    assert alphaclops.approx_eq(ApproxE(0.0), ApproxE(0.0), atol=0.1)
    assert alphaclops.approx_eq(ApproxE(0.0), ApproxE(0.2), atol=0.3)
    assert not alphaclops.approx_eq(ApproxE(0.0), ApproxE(0.2), atol=0.1)
...
def test_value_equality_approximate_specialized():
    ...
    assert alphaclops.approx_eq(PeriodicF(1, 4), PeriodicF(5, 4), atol=0.1)
    assert not alphaclops.approx_eq(PeriodicF(1, 4), PeriodicF(6, 4), atol=0.1)


def test_value_equality_approximate_not_supported():
    assert not alphaclops.approx_eq(BasicC(0.0), BasicC(0.1), atol=0.2)
...
@alphaclops.value_equality(distinct_child_types=True, approximate=True)
class ApproxG:
    def __init__(self, x):
        self.x = x
...
def test_value_equality_approximate_typing():
    assert not alphaclops.approx_eq(ApproxE(0.0), PeriodicF(0.0, 1.0), atol=0.1)
    assert alphaclops.approx_eq(ApproxEa(0.0), ApproxEb(0.0), atol=0.1)
    assert alphaclops.approx_eq(ApproxG(0.0), ApproxG(0.0), atol=0.1)
    assert not alphaclops.approx_eq(ApproxGa(0.0), ApproxGb(0.0), atol=0.1)
    assert not alphaclops.approx_eq(ApproxG(0.0), ApproxGb(0.0), atol=0.1)
...
def test_bad_manual_cls_incompatible_args():
    with pytest.raises(ValueError, match='incompatible'):
        ...
        @alphaclops.value_equality(manual_cls=True, distinct_child_types=True)
        class _:
            pass
