...
from alphaclops.protocols import json_serialization, measurement_key_protocol as mkp
from alphaclops.value import measurement_key
...
class Condition(abc.ABC):
    ...
    @abc.abstractmethod
    def replace_key(self, current: 'alphaclops.MeasurementKey', replacement: 'alphaclops.MeasurementKey'):
        """Replaces the control keys."""

    @abc.abstractmethod
    def resolve(self, classical_data: 'alphaclops.ClassicalDataStoreReader') -> bool:
    ...
    def _with_measurement_key_mapping_(self, key_map: Mapping[str, str]) -> 'alphaclops.Condition':
        condition = self
        for k in self.keys:
            condition = condition.replace_key(k, mkp.with_measurement_key_mapping(k, key_map))
        return condition

    def _with_key_path_prefix_(self, path: Tuple[str, ...]) -> 'alphaclops.Condition':
        condition = self
        for k in self.keys:
...
class KeyCondition(Condition):
    ...
    def __repr__(self):
        if self.index != -1:
            return f'alphaclops.KeyCondition({self.key!r}, {self.index})'
        return f'alphaclops.KeyCondition({self.key!r})'
...
class SympyCondition(Condition):
    ...
    def __repr__(self):
        return f'alphaclops.SympyCondition({proper_repr(self.expr)})'
