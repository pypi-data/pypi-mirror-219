...
"""Defines `@alphaclops.value_equality`, for easy __eq__/__hash__ methods."""
...
class _SupportsValueEquality(Protocol):
    ...
    def _value_equality_approximate_values_(self) -> Any:
        ...
        Returns:
            Any type supported by `alphaclops.approx_eq()`.
        """
        # coverage: ignore
        return self._value_equality_values_()

    def _value_equality_values_cls_(self) -> Any:
        """Automatically implemented by the `alphaclops.value_equality` decorator.
...
def _value_equality_approx_eq(
    ...
    if cls_self != cls_other:
        return False
    # Delegate to alphaclops.approx_eq for approximate equality comparison.
    ...
    *,
    ...
    Args:
        cls: The type to decorate. Automatically passed in by python when using
            the @alphaclops.value_equality decorator notation on a class.
    ...
    if values_getter is None:
        raise TypeError(
            'The @alphaclops.value_equality decorator requires a '
