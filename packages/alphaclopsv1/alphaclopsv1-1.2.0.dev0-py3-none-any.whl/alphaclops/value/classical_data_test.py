...
mkey_m = alphaclops.MeasurementKey('m')
mkey_c = alphaclops.MeasurementKey('c')
two_qubits = tuple(alphaclops.LineQubit.range(2))


def test_init():
    cd = alphaclops.ClassicalDataDictionaryStore()
    assert cd.records == {}
    assert cd.keys() == ()
    ...
    assert cd.measurement_types == {
        mkey_m: alphaclops.MeasurementType.MEASUREMENT,
        mkey_c: alphaclops.MeasurementType.CHANNEL,
    }
...
def test_record_measurement_errors():
    ...
    with pytest.raises(
        ...
        cd.record_measurement(mkey_m, (1, 0, 4), tuple(alphaclops.LineQubit.range(3)))
    with pytest.raises(
        ValueError, match=re.escape('Measurement shape (3, 3) does not match (2, 2) in m')
    ):
        cd.record_measurement(mkey_m, (1, 0), tuple(alphaclops.LineQid.range(2, dimension=3)))
...
def test_get_int():
    ...
    assert cd.get_int(mkey_m) == 1
    cd = alphaclops.ClassicalDataDictionaryStore()
    cd.record_measurement(mkey_m, (1, 1), (alphaclops.LineQid.range(2, dimension=3)))
...
def test_repr():
    ...
    alphaclops.testing.assert_equivalent_repr(cd)
