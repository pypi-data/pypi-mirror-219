...
import alphaclops
from alphaclops.value import Duration
...
def test_repr():
    alphaclops.testing.assert_equivalent_repr(Duration(millis=2))
    alphaclops.testing.assert_equivalent_repr(Duration(micros=2))
    alphaclops.testing.assert_equivalent_repr(Duration(picos=1000, nanos=1000))
    alphaclops.testing.assert_equivalent_repr(Duration(picos=5000))
    alphaclops.testing.assert_equivalent_repr(Duration(nanos=1.0))
    alphaclops.testing.assert_equivalent_repr(Duration(micros=sympy.Symbol('t')))
    alphaclops.testing.assert_equivalent_repr(Duration(micros=1.5 * sympy.Symbol('t')))
...
def test_parameterized(resolve_fn):
    t = sympy.Symbol('t')
    assert not alphaclops.is_parameterized(Duration())
    assert not alphaclops.is_parameterized(Duration(nanos=500))
    assert alphaclops.is_parameterized(Duration(nanos=500 * t))
...
def test_bool():
    assert bool(alphaclops.Duration(picos=1))
    assert not bool(alphaclops.Duration())
...
def test_repr_preserves_type_information():
    ...
    assert repr(alphaclops.Duration(micros=1500)) == 'alphaclops.Duration(micros=1500)'
    assert repr(alphaclops.Duration(micros=1500.0)) == 'alphaclops.Duration(micros=1500.0)'
    assert repr(alphaclops.Duration(millis=1.5)) == 'alphaclops.Duration(micros=1500.0)'

    assert repr(alphaclops.Duration(micros=1500 * t)) == (
        "alphaclops.Duration(micros=sympy.Mul(sympy.Integer(1500), sympy.Symbol('t')))"
    )
    assert repr(alphaclops.Duration(micros=1500.0 * t)) == (
        "alphaclops.Duration(micros=sympy.Mul(sympy.Float('1500.0', precision=53), sympy.Symbol('t')))"
    )
    assert repr(alphaclops.Duration(millis=1.5 * t)) == (
        "alphaclops.Duration(micros=sympy.Mul(sympy.Float('1500.0', precision=53), sympy.Symbol('t')))"
    )
