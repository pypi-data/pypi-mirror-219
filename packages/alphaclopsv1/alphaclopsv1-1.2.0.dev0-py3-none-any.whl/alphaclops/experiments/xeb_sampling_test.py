...
def test_sample_2q_xeb_circuits():
    ...
    q1 = alphaclops.NamedQubit('b')
    circuits = [
        rqcg.random_rotations_between_two_qubit_circuit(
            q0, q1, depth=20, two_qubit_op_factory=lambda a, b, _: alphaclops.SQRT_ISWAP(a, b)
        )
        for _ in range(2)
    ]
...
def test_sample_2q_xeb_circuits_error():
    ...
    circuits = [alphaclops.testing.random_circuit(qubits, n_moments=5, op_density=0.8, random_state=52)]
    cycle_depths = np.arange(3, 50, 9)
    with pytest.raises(ValueError):  # three qubit circuits
        _ = sample_2q_xeb_circuits(
            sampler=alphaclops.Simulator(), circuits=circuits, cycle_depths=cycle_depths
        )


def test_sample_2q_xeb_circuits_no_progress(capsys):
    qubits = alphaclops.LineQubit.range(2)
    circuits = [alphaclops.testing.random_circuit(qubits, n_moments=7, op_density=0.8, random_state=52)]
    cycle_depths = np.arange(3, 4)
    _ = sample_2q_xeb_circuits(
        sampler=alphaclops.Simulator(), circuits=circuits, cycle_depths=cycle_depths, progress_bar=None
    )
...
def test_sample_2q_parallel_xeb_circuits(tmpdir):
    circuits = rqcg.generate_library_of_2q_circuits(
        n_library_circuits=5, two_qubit_gate=alphaclops.ISWAP ** 0.5, max_cycle_depth=10
    )
    cycle_depths = [5, 10]
    graph = _gridqubits_to_graph_device(alphaclops.TensorCircuit.rect(3, 2))
    ...
    chunks = [record for fn in glob.glob(f'{tmpdir}/my_dataset/*') for record in alphaclops.read_json(fn)]
...
def test_sample_2q_parallel_xeb_circuits_error_bad_qubits():
    circuits = rqcg.generate_library_of_2q_circuits(
        ...
        max_cycle_depth=10,
        q0=alphaclops.TensorCircuit(0, 0),
