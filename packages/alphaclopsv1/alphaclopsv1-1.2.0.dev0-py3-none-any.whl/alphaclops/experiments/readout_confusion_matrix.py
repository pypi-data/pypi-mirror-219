...
from alphaclops import circuits, ops, vis, study
...
class TensoredConfusionMatrices:
    ...
    Use `alphaclops.measure_confusion_matrix(sampler, qubits, repetitions)` to perform
    an experiment on `sampler` and construct the `alphaclops.TensoredConfusionMatrices` object.
    """

    def __init__(
        self,
        confusion_matrices: Union[np.ndarray, Sequence[np.ndarray]],
        measure_qubits: Union[Sequence['alphaclops.Qid'], Sequence[Sequence['alphaclops.Qid']]],
        *,
        repetitions: int,
        timestamp: float,
    ):
        """Initializes `alphaclops.TensoredConfusionMatrices`.
        ...
        measure_qubits = cast(
            Sequence[Sequence['alphaclops.Qid']],
        ...
        self._cache: Dict[Tuple['alphaclops.Qid', ...], np.ndarray] = {}
    ...
    def from_measurement(
        cls, gate: ops.MeasurementGate, qubits: Sequence['alphaclops.Qid']
    ...
    @property
    def measure_qubits(self) -> Tuple[Tuple['alphaclops.Qid', ...], ...]:
    ...
    def _get_vars(self, qubit_pattern: Sequence['alphaclops.Qid']) -> List[int]:
        in_vars = [2 * self._qubits_to_idx[q] for q in qubit_pattern]
        out_vars = [2 * self._qubits_to_idx[q] + 1 for q in qubit_pattern]
        return in_vars + out_vars

    def _confusion_matrix(self, qubits: Sequence['alphaclops.Qid']) -> np.ndarray:
        ein_input = []
    ...
    def confusion_matrix(self, qubits: Optional[Sequence['alphaclops.Qid']] = None) -> np.ndarray:
    ...
    def correction_matrix(self, qubits: Optional[Sequence['alphaclops.Qid']] = None) -> np.ndarray:
    ...
    def __repr__(self) -> str:
        return (
            f"alphaclops.TensoredConfusionMatrices("
...
def measure_confusion_matrix(
    sampler: 'alphaclops.Sampler',
    qubits: Union[Sequence['alphaclops.Qid'], Sequence[Sequence['alphaclops.Qid']]],
    ...
    """
    qubits = cast(
        Sequence[Sequence['alphaclops.Qid']], [qubits] if isinstance(qubits[0], ops.Qid) else qubits
    )
    confusion_matrices = []
    for qs in qubits:
