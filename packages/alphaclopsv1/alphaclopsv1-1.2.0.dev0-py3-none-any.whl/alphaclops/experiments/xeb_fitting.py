...
import pandas as pd
import sympy
from alphaclops import circuits, ops, protocols, _import
from alphaclops.experiments.xeb_simulation import simulate_2q_xeb_circuits
...
def benchmark_2q_xeb_fidelities(
    ...
    circuits: Sequence['alphaclops.Circuit'],
    ...
    `alphaclops.experiments.fidelity_estimation.least_squares_xeb_fidelity_from_expectations`, but
...
class XEBCharacterizationOptions(ABC):
    @staticmethod
    @abstractmethod
    def should_parameterize(op: 'alphaclops.Operation') -> bool:
        """Whether to replace `op` with a parameterized version."""

    @abstractmethod
    def get_parameterized_gate(self) -> 'alphaclops.Gate':
...
def phased_fsim_angles_from_gate(gate: 'alphaclops.Gate') -> Dict[str, 'alphaclops.TParamVal']:
    """For a given gate, return a dictionary mapping '{angle}_default' to its noiseless value
    for the five PhasedFSim angles."""
    defaults: Dict[str, 'alphaclops.TParamVal'] = {
        'theta_default': 0.0,
        'zeta_default': 0.0,
        'chi_default': 0.0,
        'gamma_default': 0.0,
...
class XEBPhasedFSimCharacterizationOptions(XEBCharacterizationOptions):
    ...
    def get_initial_simplex_and_names(
        ...
        We also return a list of parameter names so the alphaclops `param_resovler`
    ...
    def with_defaults_from_gate(
        self, gate: 'alphaclops.Gate', gate_to_angles_func=phased_fsim_angles_from_gate
    ):
...
def parameterize_circuit(
    circuit: 'alphaclops.Circuit', options: XEBCharacterizationOptions
) -> 'alphaclops.Circuit':
...
QPair_T = Tuple['alphaclops.Qid', 'alphaclops.Qid']
...
def characterize_phased_fsim_parameters_with_xeb(
    sampled_df: pd.DataFrame,
    parameterized_circuits: List['alphaclops.Circuit'],
    ...
    final_params: 'alphaclops.ParamDictType' = dict(zip(names, optimization_result.x))
