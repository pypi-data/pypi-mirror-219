...
from alphaclops import ops, devices, value, protocols
from alphaclops.circuits import Circuit, Moment
from alphaclops.experiments.random_quantum_circuit_generation import CircuitLibraryCombination
...
class _Sample2qXEBTask:
    ...
    layer_i: int
    combination_i: int
    prepared_circuit: 'alphaclops.RandomGrid'
...
def _verify_and_get_two_qubits_from_circuits(circuits: Sequence['alphaclops.Circuit']):
    """Make sure each of the provided circuits uses the same two qubits and return them."""
    all_qubits_set: Set['alphaclops.Qid'] = set()
...
def _verify_two_line_qubits_from_circuits(circuits: Sequence['alphaclops.Circuit']):
...
class _ZippedCircuit:
    ...
    wide_circuit: 'alphaclops.Circuit'
    pairs: List[Tuple['alphaclops.Qid', 'alphaclops.Qid']]
    combination: List[int]
    layer_i: int
    combination_i: int
...
) -> Tuple[List[CircuitLibraryCombination], List['alphaclops.Circuit']]:
...
def _zip_circuits(
    circuits: Sequence['alphaclops.Circuit'], combinations_by_layer: List[CircuitLibraryCombination]
) -> List[_ZippedCircuit]:
...
def sample_2q_xeb_circuits(
    ...
    *,
    ...
    shuffle: Optional['alphaclops.RANDOM_STATE_OR_SEED_LIKE'] = None,
    dataset_directory: Optional[str] = None,
):
    """Sample two-qubit XEB circuits given a sampler.

    Args:
        sampler: A alphaclops sampler for executing circuits.
