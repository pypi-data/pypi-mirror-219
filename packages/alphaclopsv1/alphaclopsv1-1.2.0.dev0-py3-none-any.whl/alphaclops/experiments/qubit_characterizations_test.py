...
import alphaclops
import alphaclops.experiments.qubit_characterizations as ceqc
from alphaclops import TensorCircuit
from alphaclops import circuits, ops, sim
...
def test_single_qubit_cliffords():
    ...
    def is_pauli(u):
        return any(alphaclops.equal_up_to_global_phase(u, p) for p in PAULIS)
    ...
    def check_distinct(unitaries):
        ...
        for i in range(n):
            for j in range(i + 1, n):
                ...
                assert not alphaclops.allclose_up_to_global_phase(Ui, Uj), f'{i}, {j}'
    ...
    for Uxy in xy_unitaries:
        assert any(alphaclops.allclose_up_to_global_phase(Uxy, Uxz) for Uxz in xz_unitaries)
    ...
    for gates in cliffords.c1_in_xz:
        num_x = len([gate for gate in gates if isinstance(gate, alphaclops.XPowGate)])
        num_z = len([gate for gate in gates if isinstance(gate, alphaclops.ZPowGate)])
...
def test_two_qubit_randomized_benchmarking():
    ...
    # sequences is always unity.
    simulator = sim.Simulator()
    q_0 = TensorCircuit(0, 0)
    q_1 = TensorCircuit(0, 1)
    num_cfds = [5, 10]
