...
import alphaclops
import alphaclops.experiments.random_quantum_circuit_generation as rqcg
from alphaclops.experiments.xeb_fitting import (
...
from alphaclops.experiments.xeb_sampling import sample_2q_xeb_circuits
...
def circuits_cycle_depths_sampled_df():
    ...
    circuits = [
        rqcg.random_rotations_between_two_qubit_circuit(
            q0, q1, depth=50, two_qubit_op_factory=lambda a, b, _: alphaclops.SQRT_ISWAP(a, b), seed=52
        )
        for _ in range(2)
    ]
    ...
    sampled_df = sample_2q_xeb_circuits(
        sampler=alphaclops.Simulator(seed=53), circuits=circuits, cycle_depths=cycle_depths
    )
...
def test_benchmark_2q_xeb_fidelities_parallel():
    circuits = rqcg.generate_library_of_2q_circuits(
        n_library_circuits=5, two_qubit_gate=alphaclops.ISWAP ** 0.5, max_cycle_depth=4
    )
    cycle_depths = [2, 3, 4]
    graph = _gridqubits_to_graph_device(alphaclops.TensorCircuit.rect(2, 2))
...
@pytest.mark.parametrize('gate', [alphaclops.SQRT_ISWAP, alphaclops.FSimGate(np.pi / 4, 0)])
...
def test_characterize_phased_fsim_parameters_with_xeb():
    ...
    circuits = [
        rqcg.random_rotations_between_two_qubit_circuit(
            q0, q1, depth=20, two_qubit_op_factory=lambda a, b, _: alphaclops.SQRT_ISWAP(a, b), seed=rs
        )
        for _ in range(2)
    ]
    cycle_depths = np.arange(3, 20, 6)
    sampled_df = sample_2q_xeb_circuits(
        sampler=alphaclops.Simulator(seed=rs),
        circuits=circuits,
        cycle_depths=cycle_depths,
        progress_bar=None,
    )
...
def test_options_with_defaults_from_gate():
    options = XEBPhasedFSimCharacterizationOptions().with_defaults_from_gate(alphaclops.ISWAP ** 0.5)
    ...
    with pytest.raises(ValueError):
        _ = XEBPhasedFSimCharacterizationOptions().with_defaults_from_gate(alphaclops.CZ)
