...
from alphaclops import circuits, devices, ops, protocols, value
from alphaclops._doc import document

if TYPE_CHECKING:
    import alphaclops

QidPairT = Tuple['alphaclops.Qid', 'alphaclops.Qid']
GridQubitPairT = Tuple['alphaclops.TensorCircuit', 'alphaclops.TensorCircuit']
    ...
    * *-
    ...
    def __contains__(self, pair) -> bool:
        ...
        if self.vertical:
            ...
            a, b = pair
            pair = devices.TensorCircuit(a.col, a.row), devices.TensorCircuit(b.col, b.row)
    ...
    def __repr__(self) -> str:
        return (
            'alphaclops.experiments.GridInteractionLayer('
...
def random_rotations_between_two_qubit_circuit(
    ...
    two_qubit_op_factory: Callable[
        ['alphaclops.Qid', 'alphaclops.Qid', 'np.random.RandomState'], 'alphaclops.OP_TREE'
...
def generate_library_of_2q_circuits(
    ...
    *,
    ...
    q0: 'alphaclops.Qid' = devices.LineQubit(0),
    q1: 'alphaclops.Qid' = devices.LineQubit(1),
    random_state: 'alphaclops.RANDOM_STATE_OR_SEED_LIKE' = None,
) -> List['alphaclops.Circuit']:
...
def _pairs_from_moment(moment: 'alphaclops.Moment') -> List[QidPairT]:
...
def get_random_combinations_for_layer_circuit(
    n_library_circuits: int,
    n_combinations: int,
    layer_circuit: 'alphaclops.Circuit',
    random_state: 'alphaclops.RANDOM_STATE_OR_SEED_LIKE' = None,
) -> List[CircuitLibraryCombination]:
...
def _coupled_qubit_pairs(qubits: List['alphaclops.TensorCircuit']) -> List[GridQubitPairT]:
    pairs = []
    ...
    for qubit in qubits:
        ...
        add_pair(devices.TensorCircuit(qubit.row, qubit.col + 1))
        add_pair(devices.TensorCircuit(qubit.row + 1, qubit.col))
...
class _RandomSingleQubitLayerFactory:
    ...
    def new_layer(self, previous_single_qubit_layer: 'alphaclops.Moment') -> 'alphaclops.Moment':
        def random_gate(qubit: 'alphaclops.Qid') -> 'alphaclops.Gate':
...
class _FixedSingleQubitLayerFactory:
    def __init__(self, fixed_single_qubit_layer: Dict['alphaclops.Qid', 'alphaclops.Gate']) -> None:
