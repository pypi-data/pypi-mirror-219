...
from alphaclops._compat import (
...
def test_with_debug():
    assert alphaclops.__alphaclops_debug__.get()
    with alphaclops.with_debug(False):
        assert not alphaclops.__alphaclops_debug__.get()
        with alphaclops.with_debug(True):
            assert alphaclops.__alphaclops_debug__.get()
        assert not alphaclops.__alphaclops_debug__.get()
    assert alphaclops.__alphaclops_debug__.get()
...
def test_dataclass_repr_simple() -> None:
    ...
    assert repr(TestClass1(5, 'hi')) == "alphaclops.TestClass1(x=5, y='hi')"
...
def test_dataclass_repr_numpy() -> None:
    ...
    class TestClass2:
        ...
        def __repr__(self):
            return dataclass_repr(self, namespace='alphaclops.testing')

    tc = TestClass2(np.ones(3))
    assert (
        repr(tc)
        == "alphaclops.testing.TestClass2(x=np.array([1.0, 1.0, 1.0], dtype=np.dtype('float64')))"
    )
...
def test_deprecated_with_name():
    ...
    with alphaclops.testing.assert_deprecated(
        '_compat_test.py:',
        'test_func was used',
        'will be removed in alphaclops v1.2',
        'Roll some dice.',
        deadline='v1.2',
    ):
        assert f(1, 2) == 3
...
def test_deprecated():
    ...
    with pytest.raises(
        ValueError, match='During testing using alphaclops deprecated functionality is not allowed'
    ):
        old_func(1, 2)
...
def test_deprecated_parameter():
    ...
    with alphaclops.testing.assert_logs(count=0):
        assert f(1) == 1
...
def test_wrap_module():
    ...
    with alphaclops.testing.assert_deprecated(
        ...
        'will be removed in alphaclops v0.6',
        'use bar instead',
        deadline='v0.6',
    ):
        _ = wrapped.foo
...
def _test_deprecate_attributes_assert_attributes_in_sys_modules():
    ...
    import alphaclops.testing._compat_test_data.module_a as module_a0

    module_a1 = deprecate_attributes(
        'alphaclops.testing._compat_test_data.module_a',
        {'MODULE_A_ATTRIBUTE': ('v0.6', 'use plain string instead')},
    )

    assert module_a1 is not module_a0
    assert module_a1 is alphaclops.testing._compat_test_data.module_a
    assert module_a1 is sys.modules['alphaclops.testing._compat_test_data.module_a']
...
def _from_parent_import_deprecated():
    from alphaclops.testing._compat_test_data import fake_a

    assert fake_a.MODULE_A_ATTRIBUTE == 'module_a'


def _import_deprecated_assert_sub():
    import alphaclops.testing._compat_test_data.fake_a  # type: ignore

    assert alphaclops.testing._compat_test_data.fake_a.module_b.MODULE_B_ATTRIBUTE == 'module_b'


def _from_deprecated_import_sub():
    from alphaclops.testing._compat_test_data.fake_a import module_b
...
def _import_deprecated_first_new_second():
    ...
    assert 'alphaclops.testing._compat_test_data.fake_a' in sys.modules
    assert 'alphaclops.testing._compat_test_data.module_a' in sys.modules

    assert (
        sys.modules['alphaclops.testing._compat_test_data.fake_a']
        == sys.modules['alphaclops.testing._compat_test_data.module_a']
    )
...
def _import_new_first_deprecated_second():
    ...
    assert 'alphaclops.testing._compat_test_data.fake_a' not in sys.modules
    assert 'alphaclops.testing._compat_test_data.module_a' in sys.modules
...
def _from_deprecated_import_sub_of_sub():
    ...
    from alphaclops.testing._compat_test_data.module_a.module_b import module_c

    assert module_c.MODULE_C_ATTRIBUTE == 'module_c'
    from alphaclops.testing._compat_test_data.fake_a.module_b import module_c  # type: ignore
...
def _import_multiple_deprecated():
    ...
    assert module_c.MODULE_C_ATTRIBUTE == 'module_c'
    from alphaclops.testing._compat_test_data.fake_a.module_b import module_c

    assert module_c.MODULE_C_ATTRIBUTE == 'module_c'
    from alphaclops.testing._compat_test_data.fake_b import module_c  # type: ignore
...
def _deprecate_grandchild_assert_attributes_in_sys_modules():
    ...
    import alphaclops.testing._compat_test_data.module_a.fake_ab  # type: ignore

    assert (
            alphaclops.testing._compat_test_data.module_a.fake_ab
            is sys.modules['alphaclops.testing._compat_test_data.module_a.fake_ab']
    )
    assert (
            alphaclops.testing._compat_test_data.module_a
            is sys.modules['alphaclops.testing._compat_test_data.module_a']
    )
    assert alphaclops.testing._compat_test_data is sys.modules['alphaclops.testing._compat_test_data']


def _new_module_in_different_parent():
    from alphaclops.testing._compat_test_data.fake_ops import raw_types  # type: ignore

    assert raw_types.Qid == alphaclops.Qid
...
def _find_spec_deprecated_multiple_times():
    ...
    assert importlib.util.find_spec('alphaclops.testing._compat_test_data.fake_a')
...
def _import_parent_use_constant_from_deprecated_module_attribute():
    ...
    # sets up the DeprecationFinders
    import alphaclops.testing._compat_test_data

    # the parent should have fake_a set on it as an attribute - just like
    # a regular module import (e.g. alphaclops.ops)
    # should have a DUPE_CONSTANT as its imported from the dupe submodule
    assert alphaclops.testing._compat_test_data.fake_a.DUPE_CONSTANT is False

    assert 'module_a for module deprecation tests' in alphaclops.testing._compat_test_data.fake_a.__doc__
    assert 'Test module for deprecation testing' in alphaclops.testing._compat_test_data.__doc__


def _import_deprecated_sub_use_constant():
    """to ensure that submodule initializations set attributes correctly"""
    # sets up the DeprecationFinders
    import alphaclops.testing._compat_test_data.fake_a.dupe  # type: ignore

    # should have a DUPE_CONSTANT as its defined on it, set to False
    assert alphaclops.testing._compat_test_data.fake_a.dupe.DUPE_CONSTANT is False


def _import_deprecated_same_name_in_earlier_subtree():
    from alphaclops.testing._compat_test_data.fake_a.sub.subsub.dupe import DUPE_CONSTANT  # type: ignore

    assert DUPE_CONSTANT


def _import_top_level_deprecated():
    from alphaclops.testing._compat_test_data.fake_freezegun import api  # type: ignore
    import time
...
def _repeated_import_path():
    ...
    from alphaclops.testing._compat_test_data.repeated_child.alphaclops.testing._compat_test_data.repeated_child import (  # type: ignore
        child,
    )
...
def _type_repr_in_deprecated_module():
    ...
    import alphaclops.testing._compat_test_data.fake_a as mod_a

    expected_repr = "<class 'alphaclops.testing._compat_test_data.module_a.types.SampleType'>"
    assert repr(mod_a.SampleType) == expected_repr


old_parent = 'alphaclops.testing._compat_test_data'
...
# see alphaclops_compat_test_data/__init__.py for the setup code
...
@mock.patch.dict(os.environ, {"alphaclops_FORCE_DEDUPE_MODULE_DEPRECATION": "1"})
...
def _test_deprecated_module_inner(outdated_method, deprecation_messages):
    ...
    with alphaclops.testing.assert_logs(
...
def test_same_name_submodule_earlier_in_subtree():
    ...
    We have two packages with a bool `DUPE_CONSTANT` attribute each:
       1. alphaclops.testing._compat_test_data.module_a.sub.dupe.DUPE_CONSTANT=True # the right one
       2. alphaclops.testing._compat_test_data.module_a.dupe.DUPE_CONSTANT=False # the wrong one

    If the new module's (in this case alphaclops.testing._compat_test_data.module_a) path has precedence
    during module spec resolution, dupe number 2 is going to get resolved.

    You might wonder where this comes up in alphaclops. There was a bug where the lookup path was not in
    the right order. The motivating example is alphaclops.ops.calibration vs the
    alphaclops.ops.engine.calibration packages. The wrong resolution resulted in false circular
    imports!
    """
...
def _test_same_name_submodule_earlier_in_subtree_inner():
    from alphaclops.testing._compat_test_data.module_a.sub.subsub.dupe import DUPE_CONSTANT
...
def _test_metadata_search_path_inner():  # pragma: no cover
    # initialize the DeprecatedModuleFinders
    # pylint: disable=unused-import
    import alphaclops.testing._compat_test_data.module_a
...
def _test_metadata_distributions_after_deprecated_submodule():
    ...
    # See https://github.com/quantumlib/alphaclops/issues/4729
    deprecated_submodule(
        new_module_name='alphaclops.neutral_atoms',
        old_parent='alphaclops',
...
def _test_parent_spec_after_deprecated_submodule():
    ...
    assert alphaclops.testing._compat_test_data.__spec__.name == 'alphaclops.testing._compat_test_data'
...
def _test_type_repr_in_new_module_inner():
    # initialize the DeprecatedModuleFinders
    # pylint: disable=unused-import
    import alphaclops.testing._compat_test_data.module_a as mod_a
...
def _test_broken_module_1_inner():
    with pytest.raises(
        ...
        # pylint: disable=unused-import
        import alphaclops.testing._compat_test_data.broken_ref as br  # type: ignore


def _test_broken_module_2_inner():
    warnings.simplefilter('always')
    with alphaclops.testing.assert_deprecated(deadline="v0.20", count=None):
        with pytest.raises(
            DeprecatedModuleImportError,
            match="missing_module cannot be imported. The typical reasons",
        ):
            # note that this passes
            from alphaclops.testing._compat_test_data import broken_ref
...
def _test_broken_module_3_inner():
    ...
    with alphaclops.testing.assert_deprecated(deadline="v0.20", count=None):
        with pytest.raises(
            ...
            alphaclops.testing._compat_test_data.broken_ref.something()
...
def _test_import_deprecated_with_no_attribute_inner():
    ...
    assert not hasattr(alphaclops.testing._compat_test_data, 'fake_b')
...
def _dir_is_still_valid_inner():
    ...
    import alphaclops.testing._compat_test_data as mod
...
def test_block_overlapping_deprecation():
    ...
    with alphaclops.testing.assert_deprecated('f', deadline='v1000.0', count=1):
        f(5)
