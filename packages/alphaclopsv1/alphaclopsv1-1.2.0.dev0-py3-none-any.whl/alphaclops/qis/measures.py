...
from alphaclops.qis.states import (
    QuantumState,
    infer_qid_shape,
    quantum_state,
...
def _validate_product_state(
    state: 'alphaclops.ProductState', qid_shape: Optional[Tuple[int, ...]]
) -> None:
...
def fidelity(
    state1: 'alphaclops.QUANTUM_STATE_LIKE',
    state2: 'alphaclops.QUANTUM_STATE_LIKE',
        ...
        if state1.shape == state2.shape:
            if qid_shape is None:
                ...
                raise ValueError(
                    ...
                    'using a wrapper function like alphaclops.density_matrix.'
                )
...
def von_neumann_entropy(
    state: 'alphaclops.QUANTUM_STATE_LIKE',
    qid_shape: Optional[Tuple[int, ...]] = None,
    validate: bool = True,
    atol: float = 1e-7,
) -> float:
...
def entanglement_fidelity(operation: 'alphaclops.SupportsKraus') -> float:
