...
from alphaclops.qis import quantum_state_representation
from alphaclops.value import big_endian_int_to_digits, linear_dict, random_state
...
class CliffordTableau(StabilizerState):
    ...
    def __repr__(self) -> str:
        return (
            f"alphaclops.CliffordTableau({self.n},"
    ...
    def _row_to_dense_pauli(self, i: int) -> 'alphaclops.DensePauliString':
    ...
    def stabilizers(self) -> List['alphaclops.DensePauliString']:
        """Returns the stabilizer generators of the state. These
        are n operators {S_1,S_2,...,S_n} such that S_i |psi> = |psi>"""
        return [self._row_to_dense_pauli(i) for i in range(self.n, 2 * self.n)]

    def destabilizers(self) -> List['alphaclops.DensePauliString']:
