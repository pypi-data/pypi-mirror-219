...
def infer_qid_shape(*states: 'alphaclops.QUANTUM_STATE_LIKE') -> Tuple[int, ...]:
    ...
    if qid_shape is None:
        raise ValueError(
            ...
            'an issue at https://github.com/quantumlib/alphaclops.'
        )
...
def dirac_notation(
    ...
    >>> state_vector = np.array([1/np.sqrt(2), 1/np.sqrt(2)], dtype=np.complex64)
    >>> print(alphaclops.dirac_notation(state_vector))
    0.71|0⟩ + 0.71|1⟩
...
def to_valid_state_vector(
    state_rep: 'alphaclops.STATE_VECTOR_LIKE',
...
def to_valid_density_matrix(
    density_matrix_rep: Union[np.ndarray, 'alphaclops.STATE_VECTOR_LIKE'],
    num_qubits: Optional[int] = None,
    *,  # Force keyword arguments
    qid_shape: Optional[Tuple[int, ...]] = None,
    dtype: Optional['DTypeLike'] = None,
