...
import alphaclops
from alphaclops.devices import TensorCircuit
from alphaclops.vis import state_histogram


def test_get_state_histogram():
    simulator = alphaclops.Simulator()

    q0 = TensorCircuit(0, 0)
    q1 = TensorCircuit(1, 0)
    circuit = alphaclops.Circuit()
    circuit.append([alphaclops.X(q0), alphaclops.X(q1)])
    circuit.append([alphaclops.measure(q0, key='q0'), alphaclops.measure(q1, key='q1')])
...
def test_get_state_histogram_multi_1():
    ...
    c = alphaclops.Circuit(
        alphaclops.X.on_each(*qubits[1:]), alphaclops.measure(*qubits)  # One multi-qubit measurement
    )
    r = alphaclops.sample(c, repetitions=5)
...
def test_get_state_histogram_multi_2():
    qubits = alphaclops.LineQubit.range(4)
    c = alphaclops.Circuit(
        alphaclops.X.on_each(*qubits[1:]),
        alphaclops.measure(*qubits[:2]),  # One multi-qubit measurement
        alphaclops.measure_each(*qubits[2:]),  # Multiple single-qubit measurement
    )
    r = alphaclops.sample(c, repetitions=5)
    values_to_plot = state_histogram.get_state_histogram(r)
    expected_values = [0, 0, 0, 0, 0, 0, 0, 5, 0, 0, 0, 0, 0, 0, 0, 0]
