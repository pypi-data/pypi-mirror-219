...
import alphaclops
from alphaclops.contrib.custom_simulators.custom_state_simulator import CustomStateSimulator


class ComputationalBasisState(alphaclops.qis.QuantumStateRepresentation):
    ...
    def measure(self, axes: Sequence[int], seed: 'alphaclops.RANDOM_STATE_OR_SEED_LIKE' = None):
        return [self.basis[i] for i in axes]


class ComputationalBasisSimState(alphaclops.SimulationState[ComputationalBasisState]):
    def __init__(self, initial_state, qubits, classical_data):
        state = ComputationalBasisState(
            alphaclops.big_endian_int_to_bits(initial_state, bit_count=len(qubits))
        )
        super().__init__(state=state, qubits=qubits, classical_data=classical_data)

    def _act_on_fallback_(self, action, qubits: Sequence[alphaclops.Qid], allow_decompose: bool = True):
        gate = action.gate if isinstance(action, alphaclops.Operation) else action
        if isinstance(gate, alphaclops.XPowGate):
...
def create_test_circuit():
    ...
    return alphaclops.Circuit(
        ...
        alphaclops.CircuitOperation(
            alphaclops.FrozenCircuit(x(q1), alphaclops.measure(q1, key='b')),
            repeat_until=alphaclops.SympyCondition(sympy.Eq(sympy.Symbol('b'), 2)),
...
def test_built_in_states():
    ...
    sim = CustomStateSimulator(alphaclops.StateVectorSimulationState)
    circuit = create_test_circuit()
    r = sim.simulate(circuit)
    assert r.measurements == {'a': np.array([1]), 'b': np.array([2])}
    assert np.allclose(
...
def test_product_state_mode_built_in_state():
    sim = CustomStateSimulator(alphaclops.StateVectorSimulationState, split_untangled_states=True)
    circuit = create_test_circuit()
    r = sim.simulate(circuit)
    assert r.measurements == {'a': np.array([1]), 'b': np.array([2])}

    # Ensure the state is in product-state mode, and it's got three states (q0, q1, phase)
    assert isinstance(r._final_simulator_state, alphaclops.SimulationProductState)
...
def test_parameterized_repetitions():
    q = alphaclops.LineQid(0, dimension=5)
    x = alphaclops.XPowGate(dimension=5)
    circuit = alphaclops.Circuit(
        alphaclops.CircuitOperation(
            alphaclops.FrozenCircuit(x(q), alphaclops.measure(q, key='a')),
...
class ComputationalBasisProductState(alphaclops.qis.QuantumStateRepresentation):
    def __init__(self, initial_state: List[int]):
        self.basis = initial_state
...
class ComputationalBasisSimProductState(alphaclops.SimulationState[ComputationalBasisProductState]):
    def __init__(self, initial_state, qubits, classical_data):
