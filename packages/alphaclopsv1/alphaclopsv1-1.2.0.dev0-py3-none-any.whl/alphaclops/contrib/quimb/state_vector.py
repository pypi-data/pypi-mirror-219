...
) -> Tuple[List[qtn.Tensor], Dict['alphaclops.Qid', int], None]:
...
def circuit_for_expectation_value(
    circuit: alphaclops.Circuit, pauli_string: alphaclops.PauliString
) -> alphaclops.Circuit:
    ...
    return alphaclops.Circuit(
        [
            circuit,
            alphaclops.Moment(gate.on(q) for q, gate in pauli_string.items()),
            alphaclops.inverse(circuit),
        ]
    )


def tensor_expectation_value(
    circuit: alphaclops.Circuit, pauli_string: alphaclops.PauliString, max_ram_gb=16, tol=1e-6
) -> float:
    ...
    if QUIMB_VERSION[0] < (1, 3):
        ...
        warnings.warn(
            ...
            'See https://github.com/quantumlib/alphaclops/issues/3263'
        )
    else:
