...
@lru_cache()
def _qpos_y(
    qubits: Union[alphaclops.Qid, Tuple[alphaclops.Qid, ...]], all_qubits: Tuple[alphaclops.Qid, ...]
) -> float:
...
def _add_to_positions(
    ...
    mi: int,
    qubits: Union[alphaclops.Qid, Tuple[alphaclops.Qid]],
    *,
...
def circuit_to_density_matrix_tensors(
    circuit: alphaclops.Circuit, qubits: Optional[Sequence[alphaclops.Qid]] = None
) -> Tuple[List[qtn.Tensor], Dict['alphaclops.Qid', int], Dict[Tuple[str, str], Tuple[float, float]]]:
    ...
    Args:
        circuit: The circuit containing operations that support the
            alphaclops.unitary() or alphaclops.kraus() protocols.
    ...
    qubit_frontier: Dict[alphaclops.Qid, int] = {q: 0 for q in qubits}
    kraus_frontier = 0
    positions: Dict[Tuple[str, str], Tuple[float, float]] = {}
    ...
    for mi, moment in enumerate(circuit.moments):
        for op in moment.operations:
            ...
            if alphaclops.has_unitary(op):
                U = alphaclops.unitary(op).reshape((2,) * 2 * len(op.qubits)).astype(np.complex128)
                tensors.append(
                    qtn.Tensor(
                        data=U,
            ...
            elif alphaclops.has_kraus(op):
                K = np.asarray(alphaclops.kraus(op), dtype=np.complex128)
...
def tensor_density_matrix(
    circuit: alphaclops.Circuit, qubits: Optional[List[alphaclops.Qid]] = None
) -> np.ndarray:
