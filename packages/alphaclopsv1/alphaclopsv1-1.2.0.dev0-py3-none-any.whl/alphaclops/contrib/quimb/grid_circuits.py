...
def get_grid_moments(
    problem_graph: nx.Graph, two_qubit_gate=alphaclops.ZZPowGate
) -> Iterator[alphaclops.Moment]:
        ...
        for row in range(row_start + row_start_offset, row_end + row_end_offset, row_step):
            for col in range(col_start + col_start_offset, col_end + col_end_offset, col_step):
                ...
                yield two_qubit_gate(exponent=weight, global_shift=-0.5).on(
                    alphaclops.TensorCircuit(*node1), alphaclops.TensorCircuit(*node2)
                )

    # Horizontal
    yield alphaclops.Moment(
        _interaction(
            col_start_offset=0,
            col_end_offset=-1,
            col_step=2,
...
def simplify_expectation_value_circuit(circuit_sand: alphaclops.Circuit):
    ...
    while True:
        circuit = alphaclops.merge_k_qubit_unitaries(circuit, k=1)
        circuit = alphaclops.drop_negligible_operations(circuit, atol=1e-6)
        circuit = alphaclops.merge_k_qubit_unitaries(circuit, k=2)
