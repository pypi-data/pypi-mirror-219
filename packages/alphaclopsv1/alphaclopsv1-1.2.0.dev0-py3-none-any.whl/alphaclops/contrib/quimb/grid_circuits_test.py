...
def _get_circuit(width, height, rs, p=2):
    ...
    qubits = [alphaclops.TensorCircuit(*n) for n in graph]
    circuit = alphaclops.Circuit(
        alphaclops.H.on_each(qubits),
        [
            (ccq.get_grid_moments(graph), alphaclops.Moment([alphaclops.rx(0.456).on_each(qubits)]))
            for _ in range(p)
        ],
    )
    ...
    for width in [2, 3]:
        for height in [1, 3]:
            for p in [1, 2]:
                ...
                operator = alphaclops.PauliString(
                    {q: alphaclops.Z for q in rs.choice(qubits, size=2, replace=False)}
                )
...
def test_circuit_to_tensors(simplify):
    ...
    operator = alphaclops.PauliString({q: alphaclops.Z for q in rs.choice(qubits, size=2, replace=False)})
