...
from alphaclops.contrib.routing.greedy import route_circuit_greedily


def test_bad_args():
    """Test zero valued arguments in greedy router."""
    circuit = alphaclops.testing.random_circuit(4, 2, 0.5, random_state=5)
...
def create_circuit_and_device():
    ...
    to test the greedy router. This instance hangs router in alphaclops 8.2.
    """
    num_qubits = 6
    gate_domain = {alphaclops.ops.CNOT: 2}
    circuit = alphaclops.testing.random_circuit(num_qubits, 15, 0.5, gate_domain, random_state=37)
