...
def get_grid_device_graph(*args, **kwargs) -> nx.Graph:
    ...
    See TensorCircuit.rect for argument details."""
    return gridqubits_to_graph_device(alphaclops.TensorCircuit.rect(*args, **kwargs))


def gridqubits_to_graph_device(qubits: Iterable[alphaclops.TensorCircuit]):
...
def nx_qubit_layout(graph: nx.Graph) -> Dict[alphaclops.Qid, Tuple[float, float]]:
    ...
    >>> plt.clf()
    >>> g = ccr.gridqubits_to_graph_device(alphaclops.TensorCircuit.rect(4,5))
    >>> pos = ccr.nx_qubit_layout(g)
    >>> nx.draw_networkx(g, pos=pos)

    """
    pos: Dict[alphaclops.Qid, Tuple[float, float]] = {}

    _node_to_i_cache = None
    for node in graph.nodes:
        if isinstance(node, alphaclops.TensorCircuit):
            pos[node] = (node.col, -node.row)
        elif isinstance(node, alphaclops.LineQubit):
