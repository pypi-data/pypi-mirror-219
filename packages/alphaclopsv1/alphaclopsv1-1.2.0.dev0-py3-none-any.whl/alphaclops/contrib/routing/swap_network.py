...
class SwapNetwork:
    ...
    def __init__(
        self, circuit: 'alphaclops.Circuit', initial_mapping: Dict['alphaclops.Qid', 'alphaclops.Qid']
    ) -> None:
    ...
    def final_mapping(self) -> Dict['alphaclops.Qid', 'alphaclops.Qid']:
        mapping = dict(self.initial_mapping)
        cca.update_mapping(mapping, self.circuit.all_operations())
        return mapping

    def get_logical_operations(self) -> Iterable['alphaclops.Operation']:
