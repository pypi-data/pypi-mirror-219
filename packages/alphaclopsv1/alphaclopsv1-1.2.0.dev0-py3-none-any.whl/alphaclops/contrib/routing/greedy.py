...
from alphaclops import circuits, ops, value
import alphaclops.contrib.acquaintance as cca
from alphaclops.contrib import circuitdag
from alphaclops.contrib.routing.initialization import get_initial_mapping
from alphaclops.contrib.routing.swap_network import SwapNetwork
from alphaclops.contrib.routing.utils import get_time_slices, ops_are_consistent_with_device_graph
...
class _GreedyRouter:
    ...
    def log_to_phys(self, *qubits: 'alphaclops.Qid') -> Iterable[ops.Qid]:
        """Returns an iterator over the physical qubits mapped to by the given
        logical qubits."""
        return (self._log_to_phys[q] for q in qubits)

    def phys_to_log(self, *qubits: 'alphaclops.Qid') -> Iterable[Optional[ops.Qid]]:
