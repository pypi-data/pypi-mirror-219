...
def fixup_text(text: str):
    if '\n' in text:
        # https://github.com/quantumlib/alphaclops/issues/4499
        # TODO: Visualize Custom MatrixGate
        return '?'
    # https://github.com/quantumlib/alphaclops/issues/2905
    text = text.replace('[<virtual>]', '')
    text = text.replace('[alphaclops.VirtualTag()]', '')
...
def _fit_horizontal(
    tdd: 'alphaclops.TextDiagramDrawer', ref_boxwidth: float, col_padding: float
...
def _fit_vertical(
    tdd: 'alphaclops.TextDiagramDrawer', ref_boxheight: float, row_padding: float
...
def tdd_to_svg(
    tdd: 'alphaclops.TextDiagramDrawer',
...
def _validate_circuit(circuit: 'alphaclops.Circuit'):
    if len(circuit) == 0:
        raise ValueError("Can't draw SVG diagram for empty circuits")


class SVGCircuit:
    """A wrapper around alphaclops.Circuit to enable rich display in a Jupyter
    notebook.
    ...
    def __init__(self, circuit: 'alphaclops.Circuit'):
        # coverage: ignore
...
def circuit_to_svg(circuit: 'alphaclops.Circuit') -> str:
