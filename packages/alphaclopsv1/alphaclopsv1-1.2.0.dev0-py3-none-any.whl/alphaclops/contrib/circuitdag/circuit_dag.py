...
class Unique(Generic[T]):
    ...
    of the circuit are wrapped by `alphaclops.Unique(X(q0))` so they are distinct
    nodes in the graph.
    """
    ...
    def __repr__(self) -> str:
        return f'alphaclops.contrib.Unique({id(self)}, {self.val!r})'
...
def _disjoint_qubits(op1: 'alphaclops.Operation', op2: 'alphaclops.Operation') -> bool:
...
class CircuitDag(networkx.DiGraph):
    ...
    def __init__(
        self,
        can_reorder: Callable[['alphaclops.Operation', 'alphaclops.Operation'], bool] = _disjoint_qubits,
    ...
    @staticmethod
    def make_node(op: 'alphaclops.Operation') -> Unique:
        return Unique(op)
    ...
    def append(self, op: 'alphaclops.Operation') -> None:
    ...
    def ordered_nodes(self) -> Iterator[Unique['alphaclops.Operation']]:
        if not self.nodes():
            return
        g = self.copy()

        def get_root_node(some_node: Unique['alphaclops.Operation']) -> Unique['alphaclops.Operation']:
            pred = g.pred
            while pred[some_node]:
                some_node = next(iter(pred[some_node]))
            return some_node

        def get_first_node() -> Unique['alphaclops.Operation']:
            return get_root_node(next(iter(g.nodes())))

        def get_next_node(succ: networkx.classes.coreviews.AtlasView) -> Unique['alphaclops.Operation']:
            if succ:
    ...
    def findall_nodes_until_blocked(
        self, is_blocker: Callable[['alphaclops.Operation'], bool]
    ) -> Iterator[Unique['alphaclops.Operation']]:
