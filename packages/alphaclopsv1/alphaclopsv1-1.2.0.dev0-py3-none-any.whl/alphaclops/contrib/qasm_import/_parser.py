...
from alphaclops import ops, Circuit, NamedQubit, CX
from alphaclops.circuits.qasm_output import QasmUGate
from alphaclops.contrib.qasm_import._lexer import QasmLexer
from alphaclops.contrib.qasm_import.exception import QasmException
...
class QasmGateStatement:
    ...
    to a list of `alphaclops.GateOperation`s.

    Has the responsibility to validate the arguments
    and parameters of the call and to generate a list of corresponding
    `alphaclops.GateOperation`s in the `on` method.
    """

    def __init__(
        self,
        qasm_gate: str,
        alphaclops_gate: Union[ops.Gate, Callable[[List[float]], ops.Gate]],
        num_params: int,
        num_args: int,
    ):
        ...
        Args:
            ...
            alphaclops_gate: The gate class on the alphaclops side.
        ...
        self.alphaclops_gate = alphaclops_gate
    ...
    def on(
        ...
        final_gate: ops.Gate = (
            self.alphaclops_gate if isinstance(self.alphaclops_gate, ops.Gate) else self.alphaclops_gate(params)
        )
        ...
        op_qubits = functools.reduce(
            cast(Callable[[List['alphaclops.Qid'], List['alphaclops.Qid']], List['alphaclops.Qid']], np.broadcast),
            args,
        )
...
class QasmParser:
    ...
    basic_gates: Dict[str, QasmGateStatement] = {
        'CX': QasmGateStatement(qasm_gate='CX', alphaclops_gate=CX, num_params=0, num_args=2),
        'U': QasmGateStatement(
            qasm_gate='U',
            num_params=3,
            num_args=1,
            # QasmUGate expects half turns
            alphaclops_gate=(lambda params: QasmUGate(*[p / np.pi for p in params])),
        ),
    }

    qelib_gates = {
        'rx': QasmGateStatement(
            qasm_gate='rx', alphaclops_gate=(lambda params: ops.rx(params[0])), num_params=1, num_args=1
        ),
        'sx': QasmGateStatement(
            qasm_gate='sx', num_params=0, num_args=1, alphaclops_gate=ops.XPowGate(exponent=0.5)
        ),
        'sxdg': QasmGateStatement(
            qasm_gate='sxdg', num_params=0, num_args=1, alphaclops_gate=ops.XPowGate(exponent=-0.5)
        ),
        'ry': QasmGateStatement(
            qasm_gate='ry', alphaclops_gate=(lambda params: ops.ry(params[0])), num_params=1, num_args=1
        ),
        'rz': QasmGateStatement(
            qasm_gate='rz', alphaclops_gate=(lambda params: ops.rz(params[0])), num_params=1, num_args=1
        ),
        'id': QasmGateStatement(
            qasm_gate='id', alphaclops_gate=ops.IdentityGate(1), num_params=0, num_args=1
        ),
        'u1': QasmGateStatement(
            qasm_gate='u1',
            alphaclops_gate=(lambda params: QasmUGate(0, 0, params[0] / np.pi)),
            num_params=1,
            num_args=1,
        ),
        'u2': QasmGateStatement(
            qasm_gate='u2',
            alphaclops_gate=(lambda params: QasmUGate(0.5, params[0] / np.pi, params[1] / np.pi)),
            num_params=2,
            num_args=1,
        ),
        ...
        'x': QasmGateStatement(qasm_gate='x', num_params=0, num_args=1, alphaclops_gate=ops.X),
        'y': QasmGateStatement(qasm_gate='y', num_params=0, num_args=1, alphaclops_gate=ops.Y),
        'z': QasmGateStatement(qasm_gate='z', num_params=0, num_args=1, alphaclops_gate=ops.Z),
        'h': QasmGateStatement(qasm_gate='h', num_params=0, num_args=1, alphaclops_gate=ops.H),
        's': QasmGateStatement(qasm_gate='s', num_params=0, num_args=1, alphaclops_gate=ops.S),
        't': QasmGateStatement(qasm_gate='t', num_params=0, num_args=1, alphaclops_gate=ops.T),
        'cx': QasmGateStatement(qasm_gate='cx', alphaclops_gate=CX, num_params=0, num_args=2),
        'cy': QasmGateStatement(
            qasm_gate='cy', alphaclops_gate=ops.ControlledGate(ops.Y), num_params=0, num_args=2
        ),
        'cz': QasmGateStatement(qasm_gate='cz', alphaclops_gate=ops.CZ, num_params=0, num_args=2),
        'ch': QasmGateStatement(
            qasm_gate='ch', alphaclops_gate=ops.ControlledGate(ops.H), num_params=0, num_args=2
        ),
        'swap': QasmGateStatement(qasm_gate='swap', alphaclops_gate=ops.SWAP, num_params=0, num_args=2),
        'cswap': QasmGateStatement(
            qasm_gate='cswap', num_params=0, num_args=3, alphaclops_gate=ops.CSWAP
        ),
        'ccx': QasmGateStatement(qasm_gate='ccx', num_params=0, num_args=3, alphaclops_gate=ops.CCX),
        'sdg': QasmGateStatement(qasm_gate='sdg', num_params=0, num_args=1, alphaclops_gate=ops.S**-1),
        'tdg': QasmGateStatement(qasm_gate='tdg', num_params=0, num_args=1, alphaclops_gate=ops.T**-1),
    }
    ...
    def p_format(self, p):
        ...
        if p[1] != "2.0":
            raise QasmException(
                ...
                "only 2.0 is supported currently by alphaclops".format(p[1])
            )
