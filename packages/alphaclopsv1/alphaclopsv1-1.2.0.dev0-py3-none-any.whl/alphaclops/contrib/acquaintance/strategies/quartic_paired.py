...
from alphaclops import circuits, ops
from alphaclops.contrib.acquaintance.gates import acquaint, SwapNetworkGate
...
def qubit_pairs_to_qubit_order(qubit_pairs: Sequence[Sequence['alphaclops.Qid']]) -> List['alphaclops.Qid']:
...
def quartic_paired_acquaintance_strategy(
    qubit_pairs: Iterable[Tuple['alphaclops.Qid', ops.Qid]]
) -> Tuple['alphaclops.Circuit', Sequence['alphaclops.Qid']]:
    ...
    qubit_pairs = tuple(
        cast(Tuple['alphaclops.Qid', ops.Qid], tuple(qubit_pair)) for qubit_pair in qubit_pairs
    )
