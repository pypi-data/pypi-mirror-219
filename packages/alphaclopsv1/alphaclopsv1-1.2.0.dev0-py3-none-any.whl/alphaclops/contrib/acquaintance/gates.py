...
def operations_to_part_lens(
    qubit_order: Sequence['alphaclops.Qid'], op_tree: 'alphaclops.OP_TREE'
) -> Tuple[int, ...]:
    ...
    singletons: List[Tuple['alphaclops.Qid', ...]] = [
...
class AcquaintanceOpportunityGate(ops.Gate, ops.InterchangeableQubitsGate):
    ...
    def _circuit_diagram_info_(self, args: 'alphaclops.CircuitDiagramInfoArgs') -> Iterable[str]:
...
def acquaint(*qubits) -> 'alphaclops.Operation':
...
Layers = NamedTuple(
    'Layers',
    [
        ('prior_interstitial', List['alphaclops.Operation']),
        ('pre', List['alphaclops.Operation']),
        ('intra', List['alphaclops.Operation']),
        ('post', List['alphaclops.Operation']),
        ('posterior_interstitial', List['alphaclops.Operation']),
    ],
)


def new_layers(**kwargs: List['alphaclops.Operation']) -> Layers:
    return Layers._make(kwargs.get(field, []) for field in Layers._fields)


def acquaint_insides(
    swap_gate: 'alphaclops.Gate',
    acquaintance_gate: 'alphaclops.Operation',
...
def acquaint_and_shift(
    parts: Tuple[List['alphaclops.Qid'], List['alphaclops.Qid']],
    layers: Layers,
...
class SwapNetworkGate(PermutationGate):
    ...
    def __init__(
        ...
        swap_gate: 'alphaclops.Gate' = ops.SWAP,
    ) -> None:
    ...
    @staticmethod
    def from_operations(
        qubit_order: Sequence['alphaclops.Qid'],
    ...
    def __repr__(self) -> str:
        return 'alphaclops.contrib.acquaintance.SwapNetworkGate({!r}, {!r})'.format(
