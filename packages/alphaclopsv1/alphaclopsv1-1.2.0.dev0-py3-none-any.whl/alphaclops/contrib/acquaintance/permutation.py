...
from alphaclops import circuits, ops, protocols, transformers, value
...
LogicalIndexSequence = Union[Sequence[int], Sequence['alphaclops.Qid']]
...
class PermutationGate(ops.Gate, metaclass=abc.ABCMeta):
    ...
    def __init__(self, num_qubits: int, swap_gate: 'alphaclops.Gate' = ops.SWAP) -> None:
    ...
    def update_mapping(
        self, mapping: Dict[ops.Qid, LogicalIndex], keys: Sequence['alphaclops.Qid']
    ) -> None:
    ...
    ) -> Union[str, Iterable[str], 'alphaclops.CircuitDiagramInfo']:
...
def display_mapping(circuit: 'alphaclops.Circuit', initial_mapping: LogicalMapping) -> None:
...
class SwapPermutationGate(PermutationGate):
    ...
    def __init__(self, swap_gate: 'alphaclops.Gate' = ops.SWAP):
...
class LinearPermutationGate(PermutationGate):
    ...
    def __init__(
        self, num_qubits: int, permutation: Dict[int, int], swap_gate: 'alphaclops.Gate' = ops.SWAP
    ) -> None:
    ...
    def __repr__(self) -> str:
        return (
            'alphaclops.contrib.acquaintance.LinearPermutationGate('
...
def update_mapping(mapping: Dict[ops.Qid, LogicalIndex], operations: 'alphaclops.OP_TREE') -> None:
...
def get_logical_operations(
    operations: 'alphaclops.OP_TREE', initial_mapping: Dict[ops.Qid, ops.Qid]
) -> Iterable['alphaclops.Operation']:
    ...
    """
    mapping = initial_mapping.copy()
    for op in cast(Iterable['alphaclops.Operation'], ops.flatten_op_tree(operations)):
        if isinstance(op, ops.GateOperation) and isinstance(op.gate, PermutationGate):
            op.gate.update_mapping(mapping, op.qubits)
        else:
            for q in op.qubits:
...
class DecomposePermutationGates:
    ...
    def optimize_circuit(self, circuit: 'alphaclops.Circuit') -> None:
        circuit._moments = [*transformers.expand_composite(circuit, no_decomp=self.no_decomp)]

    def __call__(self, circuit: 'alphaclops.Circuit') -> None:
...
def return_to_initial_mapping(circuit: 'alphaclops.Circuit', swap_gate: 'alphaclops.Gate' = ops.SWAP) -> None:
    qubits = sorted(circuit.all_qubits())
    n_qubits = len(qubits)
...
def uses_consistent_swap_gate(circuit: 'alphaclops.Circuit', swap_gate: 'alphaclops.Gate') -> bool:
