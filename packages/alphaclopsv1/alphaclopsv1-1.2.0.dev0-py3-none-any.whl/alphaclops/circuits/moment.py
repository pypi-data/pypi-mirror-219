...
from alphaclops import protocols, ops, qis, _compat
from alphaclops._import import LazyLoader
from alphaclops.ops import raw_types, op_tree
...
circuits = LazyLoader("circuits", globals(), "alphaclops.circuits.circuit")
op_tree = LazyLoader("op_tree", globals(), "alphaclops.ops.op_tree")
text_diagram_drawer = LazyLoader(
    "text_diagram_drawer", globals(), "alphaclops.circuits.text_diagram_drawer"
)


def _default_breakdown(qid: 'alphaclops.Qid') -> Tuple[Any, Any]:
...
class Moment:
    ...
    *   `moment[qubits]` returns another Moment which consists only of those
    ...
    def __init__(self, *contents: 'alphaclops.OP_TREE', _flatten_contents: bool = True) -> None:
        ...
        self._operations = (
            ...
            else cast(Tuple['alphaclops.Operation'], contents)
        )
        self._sorted_operations: Optional[Tuple['alphaclops.Operation', ...]] = None

        # An internal dictionary to support efficient operation access by qubit.
        self._qubit_to_op: Dict['alphaclops.Qid', 'alphaclops.Operation'] = {}
        ...
        self._measurement_key_objs: Optional[FrozenSet['alphaclops.MeasurementKey']] = None
        self._control_keys: Optional[FrozenSet['alphaclops.MeasurementKey']] = None

    @classmethod
    def from_ops(cls, *ops: 'alphaclops.Operation') -> 'alphaclops.Moment':
    ...
    @property
    def operations(self) -> Tuple['alphaclops.Operation', ...]:
        return self._operations

    @property
    def qubits(self) -> FrozenSet['alphaclops.Qid']:
        return self._qubits

    def operates_on_single_qubit(self, qubit: 'alphaclops.Qid') -> bool:
    ...
    def operates_on(self, qubits: Iterable['alphaclops.Qid']) -> bool:
    ...
    def operation_at(self, qubit: raw_types.Qid) -> Optional['alphaclops.Operation']:
    ...
    def with_operation(self, operation: 'alphaclops.Operation') -> 'alphaclops.Moment':
    ...
    def with_operations(self, *contents: 'alphaclops.OP_TREE') -> 'alphaclops.Moment':
    ...
    def without_operations_touching(self, qubits: Iterable['alphaclops.Qid']) -> 'alphaclops.Moment':
    ...
    def _resolve_parameters_(
        ...
        changed = False
        resolved_ops: List['alphaclops.Operation'] = []
        for op in self:
    ...
    def _sorted_operations_(self) -> Tuple['alphaclops.Operation', ...]:
    ...
    def __iter__(self) -> Iterator['alphaclops.Operation']:
    ...
    def expand_to(self, qubits: Iterable['alphaclops.Qid']) -> 'alphaclops.Moment':
        """Returns self expanded to given superset of qubits by making identities explicit.

        Args:
            qubits: Iterable of `alphaclops.Qid`s to expand this moment to.

        Returns:
            A new `alphaclops.Moment` with identity operations on the new qubits
    ...
    def _kraus_(self) -> Sequence[np.ndarray]:
        ...
        def row_subscripts(qs: Sequence['alphaclops.Qid']) -> str:
            return ''.join(qubit_to_row_subscript[q] for q in qs)

        def col_subscripts(qs: Sequence['alphaclops.Qid']) -> str:
            return ''.join(qubit_to_col_subscript[q] for q in qs)

        def kraus_tensors(op: 'alphaclops.Operation') -> Sequence[np.ndarray]:
    ...
    def __add__(self, other: 'alphaclops.OP_TREE') -> 'alphaclops.Moment':

        if isinstance(other, circuits.RandomGrid):
            return NotImplemented  # Delegate to Circuit.__radd__.
        return self.with_operations(other)

    def __sub__(self, other: 'alphaclops.OP_TREE') -> 'alphaclops.Moment':
    ...
    @overload
    def __getitem__(self, key: raw_types.Qid) -> 'alphaclops.Operation':
        pass

    @overload
    def __getitem__(self, key: Iterable[raw_types.Qid]) -> 'alphaclops.Moment':
        pass
    ...
    def to_text_diagram(
        self: 'alphaclops.Moment',
        *,
        xy_breakdown_func: Callable[['alphaclops.Qid'], Tuple[Any, Any]] = _default_breakdown,
        extra_qubits: Iterable['alphaclops.Qid'] = (),
        ...
        Args:
            xy_breakdown_func: A function to split qubits/qudits into x and y
                ...
                `alphaclops.TensorCircuit(row, col)` into the tuple `(col, row)` and
                `alphaclops.LineQubit(x)` into `(x, 0)`.
