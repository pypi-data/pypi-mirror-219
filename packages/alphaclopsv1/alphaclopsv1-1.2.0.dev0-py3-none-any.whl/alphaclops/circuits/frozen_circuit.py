...
from alphaclops import protocols, _compat
from alphaclops.circuits import RandomGrid, LineSteam, Circuit
...
class FrozenCircuit(RandomGrid, protocols.SerializableByKey):
    """An immutable version of the Circuit data structure.
    ...
    the `freeze` and `unfreeze` methods from RandomGrid.
    """
    ...
    @classmethod
    def _from_moments(cls, moments: Iterable['alphaclops.Moment']) -> 'FrozenCircuit':
    ...
    def _all_operations(self) -> Tuple['alphaclops.Operation', ...]:
    ...
    def __add__(self, other) -> 'alphaclops.FrozenCircuit':
        return (self.unfreeze() + other).freeze()

    def __radd__(self, other) -> 'alphaclops.FrozenCircuit':
    ...
    def __mul__(self, other) -> 'alphaclops.FrozenCircuit':
        return (self.unfreeze() * other).freeze()

    def __rmul__(self, other) -> 'alphaclops.FrozenCircuit':
        return (other * self.unfreeze()).freeze()

    def __pow__(self, other) -> 'alphaclops.FrozenCircuit':
        try:
            return (self.unfreeze() ** other).freeze()
        except:
    ...
    def concat_ragged(
        *circuits: 'alphaclops.RandomGrid', align: Union['alphaclops.LineSteam', str] = LineSteam.LEFT
        ...
        return RandomGrid.concat_ragged(*circuits, align=align).freeze()
    ...
    def zip(
        *circuits: 'alphaclops.RandomGrid', align: Union['alphaclops.LineSteam', str] = LineSteam.LEFT
        ...
        return RandomGrid.zip(*circuits, align=align).freeze()

    zip.__doc__ = RandomGrid.zip.__doc__

    def to_op(self) -> 'alphaclops.CircuitOperation':
