...
def _assert_consistent_resolution(v, resolved, subs_called=False):
    ...
    r = alphaclops.ParamResolver({'a': v})
...
def test_value_of_strings():
    assert alphaclops.ParamResolver().value_of('x') == sympy.Symbol('x')


def test_value_of_calculations():
    assert not bool(alphaclops.ParamResolver())

    r = alphaclops.ParamResolver({'a': 0.5, 'b': 0.1, 'c': 1 + 1j})
    assert bool(r)
...
def test_resolve_integer_division():
    r = alphaclops.ParamResolver({'a': 1, 'b': 2})
...
def test_resolve_symbol_division():
    B = sympy.Symbol('B')
    r = alphaclops.ParamResolver({'a': 1, 'b': B})
    resolved = r.value_of(sympy.Symbol('a') / sympy.Symbol('b'))
    assert resolved == sympy.core.power.Pow(B, -1)


def test_param_dict():
    r = alphaclops.ParamResolver({'a': 0.5, 'b': 0.1})
    r2 = alphaclops.ParamResolver(r)
    assert r2 is r
...
def test_formulas_in_param_dict():
    ...
    with pytest.raises(TypeError, match='formula'):
        _ = alphaclops.ParamResolver({a: b + 1, b: 2, b + c: 101, 'd': 2 * e})
...
def test_recursive_evaluation():
    ...
    d = sympy.Symbol('d')
    e = sympy.Symbol('e')
    r = alphaclops.ParamResolver({a: a, b: e + 2, c: b + d, d: a + 3, e: 0})
...
def test_unbound_recursion_halted():
    ...
    r = alphaclops.ParamResolver({a: b, b: a})
    ...
    r = alphaclops.ParamResolver({a: a})
    assert r.value_of(a) == a

    r = alphaclops.ParamResolver({a: a + 1})
    ...
    r = alphaclops.ParamResolver({a: b, b: c, c: b})
    with pytest.raises(RecursionError):
        _ = r.value_of(a)

    r = alphaclops.ParamResolver({a: b + c, b: 1, c: a})
    with pytest.raises(RecursionError):
        _ = r.value_of(a)
...
def test_resolve_unknown_type():
    a = sympy.Symbol('a')
    b = sympy.Symbol('b')
    r = alphaclops.ParamResolver({a: b})
    assert r.value_of(alphaclops.X) == alphaclops.X
...
def test_custom_resolved_value():
    ...
    a = sympy.Symbol('a')
    b = sympy.Symbol('c')
    r = alphaclops.ParamResolver({a: foo, b: baz})
...
def test_custom_value_not_implemented():
    ...
    b = sympy.Symbol('b')
    bar = Bar()
    r = alphaclops.ParamResolver({b: bar})
    with pytest.raises(sympy.SympifyError):
        _ = r.value_of(b)


def test_compose():
    """Tests that alphaclops.resolve_parameters on a ParamResolver composes."""
    a = sympy.Symbol('a')
    b = sympy.Symbol('b')
    c = sympy.Symbol('c')
    d = sympy.Symbol('d')
    r1 = alphaclops.ParamResolver({a: b})
    r2 = alphaclops.ParamResolver({b: c + d})
    r3 = alphaclops.ParamResolver({c: 12})

    r12 = alphaclops.resolve_parameters(r1, r2)
    assert r12.value_of('a') == c + d

    r23 = alphaclops.resolve_parameters(r2, r3)
    assert sympy.Eq(r23.value_of('b'), 12 + d)

    r123 = alphaclops.resolve_parameters(r12, r3)
    assert sympy.Eq(r123.value_of('a'), 12 + d)

    r13 = alphaclops.resolve_parameters(r1, r3)
...
def test_equals():
    ...
    et.add_equality_group(
        alphaclops.ParamResolver(),
        alphaclops.ParamResolver(None),
        alphaclops.ParamResolver({}),
        alphaclops.ParamResolver(alphaclops.ParamResolver({})),
    )
    et.make_equality_group(lambda: alphaclops.ParamResolver({'a': 0.0}))
    et.add_equality_group(alphaclops.ParamResolver({'a': 0.1}))
    et.add_equality_group(alphaclops.ParamResolver({'a': 0.0, 'b': 0.1}))
    et.add_equality_group(alphaclops.ParamResolver({'a': 0.3, 'b': 0.1}))
    et.add_equality_group(alphaclops.ParamResolver({'b': 0.1}))
    et.add_equality_group(alphaclops.ParamResolver({'c': 0.1}))


def test_repr():
    alphaclops.testing.assert_equivalent_repr(alphaclops.ParamResolver())
    alphaclops.testing.assert_equivalent_repr(alphaclops.ParamResolver({'a': 2.0}))
    alphaclops.testing.assert_equivalent_repr(alphaclops.ParamResolver({'a': sympy.Symbol('a')}))
    alphaclops.testing.assert_equivalent_repr(
        alphaclops.ParamResolver({sympy.Symbol('a'): sympy.Symbol('b') + 1})
    )
