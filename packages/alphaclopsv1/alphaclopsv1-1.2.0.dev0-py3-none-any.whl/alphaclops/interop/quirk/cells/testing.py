...
def assert_url_to_circuit_returns(
    json_text: str,
    circuit: Optional['alphaclops.Circuit'] = None,
    *,
    ...
    Args:
        ...
        output_amplitudes_from_quirk: Optional data copied from Quirk's "export
            simulation data" function, for comparison to alphaclops's simulator
    ...
    if diagram is not None:
        alphaclops.testing.assert_has_diagram(parsed, diagram)

    if circuit is not None:
        alphaclops.testing.assert_same_circuits(parsed, circuit)

    if unitary is not None:
        np.testing.assert_allclose(alphaclops.unitary(parsed), unitary, atol=1e-8)
    ...
    if maps:
        alphaclops.testing.assert_equivalent_computational_basis_map(maps, parsed)
