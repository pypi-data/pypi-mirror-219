...
import alphaclops
from alphaclops import quirk_json_to_circuit
from alphaclops.interop.quirk.cells.testing import assert_url_to_circuit_returns
from alphaclops.interop.quirk.cells.composite_cell import _iterator_to_iterable
...
def test_custom_circuit_gate():
    ...
    assert_url_to_circuit_returns(
        ...
        alphaclops.Circuit(alphaclops.H(a), alphaclops.X(b).controlled_by(a), alphaclops.Y(a)),
    )
    ...
    assert_url_to_circuit_returns(
        ...
        alphaclops.Circuit(
            alphaclops.interop.quirk.QuirkArithmeticGate('+=A1', target=[2], inputs=[[2]]).on(b, a)
        ),
    )
    ...
    assert_url_to_circuit_returns(
        ...
        '"circuit":{"cols":[["X"],["Y","Z"]]}}]}',
        alphaclops.Circuit(
            alphaclops.X(c).controlled_by(a), alphaclops.Y(c).controlled_by(a), alphaclops.Z(d).controlled_by(a)
        ),
    )
    ...
    assert_url_to_circuit_returns(
        ...
        '"circuit":{"cols":[["X"],["⊕","Z"]]}}]}',
        alphaclops.Circuit(
            alphaclops.X(c).controlled_by(a),
            alphaclops.Y(c) ** 0.5,
            alphaclops.Z(d).controlled_by(a, c),
            alphaclops.Y(c) ** -0.5,
        ),
    )
    ...
    assert_url_to_circuit_returns(
        ...
        alphaclops.Circuit(
            alphaclops.interop.quirk.QuirkArithmeticGate('+=A2', target=[2, 2], inputs=[[2, 2]]).on(
                a, b, d, e
            ),
            alphaclops.interop.quirk.QuirkArithmeticGate('+=A2', target=[2, 2], inputs=[[2, 2]]).on(
                b, c, d, e
    ...
    assert_url_to_circuit_returns(
        ...
        alphaclops.Circuit(alphaclops.X(a)) * 4,
    )
