...
import alphaclops
from alphaclops.interop.quirk.cells import arithmetic_cells
...
def test_repr():
    ...
    alphaclops.testing.assert_equivalent_repr(
        alphaclops.interop.quirk.cells.arithmetic_cells.ArithmeticCell(
            '+=A2', alphaclops.LineQubit.range(2), [alphaclops.LineQubit.range(2, 5)]
        )
    )
...
def test_with_registers():
    ...
    op = cast(alphaclops.ArithmeticGate, circuit[0].operations[0].gate)
    ...
    np.testing.assert_allclose(alphaclops.unitary(alphaclops.Circuit(op2())), np.array([[1]]), atol=1e-8)

    op2 = op.with_registers([2, 2, 2], 5, 5)
    np.testing.assert_allclose(
        alphaclops.final_state_vector(alphaclops.Circuit(op2(*alphaclops.LineQubit.range(3))), initial_state=0),
        alphaclops.one_hot(index=25 % 8, shape=8, dtype=np.complex64),
        atol=1e-8,
    )
...
def test_with_line_qubits_mapped_to():
    ...
    a2, b2, c2, d2, e2 = alphaclops.NamedQubit.range(5, prefix='p')

    # After assigned to qubit register.
    cell = alphaclops.interop.quirk.cells.arithmetic_cells.ArithmeticCell('+=A2', [a, b], [(c, d, e)])
    mapped_cell = alphaclops.interop.quirk.cells.arithmetic_cells.ArithmeticCell(
        '+=A2', [a2, b2], [(c2, d2, e2)]
    )
    ...
    # Before assigned.
    cell = alphaclops.interop.quirk.cells.arithmetic_cells.ArithmeticCell('+=A2', [a, b], [None])
    mapped_cell = alphaclops.interop.quirk.cells.arithmetic_cells.ArithmeticCell('+=A2', [a2, b2], [None])
    assert cell != mapped_cell
    assert cell.with_line_qubits_mapped_to([a2, b2, c2, d2, e2]) == mapped_cell

    # After assigned to classical constant.
    cell = alphaclops.interop.quirk.cells.arithmetic_cells.ArithmeticCell('+=A2', [a, b], [42])
    mapped_cell = alphaclops.interop.quirk.cells.arithmetic_cells.ArithmeticCell('+=A2', [a2, b2], [42])
    assert cell != mapped_cell
    assert cell.with_line_qubits_mapped_to([a2, b2, c2, d2, e2]) == mapped_cell
