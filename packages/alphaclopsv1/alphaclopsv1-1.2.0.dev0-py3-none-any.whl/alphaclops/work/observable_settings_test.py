...
import alphaclops
from alphaclops.work.observable_settings import _max_weight_state, _max_weight_observable, _hashable_param
from alphaclops.work import InitObsSetting, observables_to_settings, _MeasurementSpec
...
def test_init_obs_setting():
    ...
    with pytest.raises(ValueError):
        setting = InitObsSetting(init_state=alphaclops.KET_ZERO(q0), observable=alphaclops.X(q0) * alphaclops.Y(q1))


def test_max_weight_observable():
    q0, q1 = alphaclops.LineQubit.range(2)
    observables = [alphaclops.X(q0), alphaclops.X(q1)]
    assert _max_weight_observable(observables) == alphaclops.X(q0) * alphaclops.X(q1)

    observables = [alphaclops.X(q0), alphaclops.X(q1), alphaclops.Z(q1)]
    assert _max_weight_observable(observables) is None


def test_max_weight_state():
    q0, q1 = alphaclops.LineQubit.range(2)
    states = [alphaclops.KET_PLUS(q0), alphaclops.KET_PLUS(q1)]
    assert _max_weight_state(states) == alphaclops.KET_PLUS(q0) * alphaclops.KET_PLUS(q1)

    states = [alphaclops.KET_PLUS(q0), alphaclops.KET_PLUS(q1), alphaclops.KET_MINUS(q1)]
    assert _max_weight_state(states) is None


def test_observable_to_setting():
    q0, q1, q2 = alphaclops.LineQubit.range(3)
    observables = [alphaclops.X(q0) * alphaclops.Y(q1), alphaclops.Z(q2) * 1]

    zero_state = alphaclops.KET_ZERO(q0) * alphaclops.KET_ZERO(q1) * alphaclops.KET_ZERO(q2)
    settings_should_be = [
...
def test_measurement_spec():
    ...
    alphaclops.testing.assert_equivalent_repr(meas_spec)
