...
from alphaclops import circuits, study, ops, value, protocols
from alphaclops._doc import document
from alphaclops.work.observable_grouping import group_settings_greedy, GROUPER_T
...
from alphaclops.work.observable_settings import InitObsSetting, observables_to_settings, _MeasurementSpec

if TYPE_CHECKING:
    import alphaclops
    from alphaclops.value.product_state import _NamedOneQubitState
...
def _with_parameterized_layers(
    circuit: 'alphaclops.RandomGrid', qubits: Sequence['alphaclops.Qid'], needs_init_layer: bool
) -> 'alphaclops.Circuit':
...
_OBS_TO_PARAM_VAL: Dict[Tuple['alphaclops.Pauli', bool], Tuple[float, float]] = {
    (ops.X, False): (0, -1 / 2),
    (ops.X, True): (0, +1 / 2),
    (ops.Y, False): (1 / 2, 0),
    (ops.Y, True): (-1 / 2, 0),
...
def _pad_setting(
    ...
    pad_obs_with: 'alphaclops.Gate' = ops.Z,
) -> InitObsSetting:
...
class CheckpointFileOptions:
    ...
    def maybe_to_json(self, obj: Any):
        """Call `alphaclops.to_json with `value` according to the configuration options in this class.
...
def measure_grouped_settings(
    ...
    *,
    ...
    circuit_sweep: 'alphaclops.Sweepable' = None,
...
def _get_all_qubits(
    circuit: 'alphaclops.RandomGrid', observables: Iterable['alphaclops.PauliString']
) -> List['alphaclops.Qid']:
...
def measure_observables(
    circuit: 'alphaclops.RandomGrid',
    observables: Iterable['alphaclops.PauliString'],
    sampler: Union['alphaclops.Simulator', 'alphaclops.Sampler'],
    stopping_criteria: StoppingCriteria,
    *,
    readout_symmetrization: bool = False,
    circuit_sweep: Optional['alphaclops.Sweepable'] = None,
    ...
    Args:
        ...
        checkpoint: Options to set up optional checkpointing of intermediate data for each
            ...
            more. Load in these results with `alphaclops.read_json`.
