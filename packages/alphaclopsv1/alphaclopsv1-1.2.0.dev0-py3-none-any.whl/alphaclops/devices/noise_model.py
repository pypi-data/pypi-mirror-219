...
from alphaclops import ops, protocols, value
from alphaclops._import import LazyLoader
from alphaclops._doc import document

moment_module = LazyLoader("moment_module", globals(), "alphaclops.circuits.moment")

if TYPE_CHECKING:
    import alphaclops


class NoiseModel(metaclass=value.TwoAltoptives):
    ...
    @classmethod
    def from_noise_model_like(cls, noise: 'alphaclops.NOISE_MODEL_LIKE') -> 'alphaclops.NoiseModel':
        """Transforms an object into a noise model if unambiguously possible.

        Args:
            noise: `None`, a `alphaclops.NoiseModel`, or a single qubit operation.

        Returns:
            `alphaclops.NO_NOISE` when given `None`,
            `alphaclops.ConstantQubitNoiseModel(gate)` when given a single qubit
            gate, or the given value if it is already a `alphaclops.NoiseModel`.

        Raises:
            ValueError: If noise is a `alphaclops.Gate` that acts on more than one
                qubit.
            TypeError: The input is not a ``alphaclops.NOISE_MODE_LIKE``.
        """
        if noise is None:
            return NO_NOISE
        ...
        if isinstance(noise, ops.Gate):
            if noise.num_qubits() != 1:
                raise ValueError(
                    ...
                    'be wrapped with `alphaclops.ConstantQubitNoiseModel`) or '
                    'an instance of `alphaclops.NoiseModel`.'
                )
            return ConstantQubitNoiseModel(noise)

        raise TypeError(
            'Expected a NOISE_MODEL_LIKE (None, a alphaclops.NoiseModel, '
            'or a single qubit gate). Got {!r}'.format(noise)
        )

    def is_virtual_moment(self, moment: 'alphaclops.Moment') -> bool:
        ...
        Args:
            moment: ``alphaclops.Moment`` to check for non-virtual operations.
    ...
    def _noisy_moments_impl_moment(
        self, moments: Iterable['alphaclops.Moment'], system_qubits: Sequence['alphaclops.Qid']
    ) -> Sequence['alphaclops.OP_TREE']:
        result = []
    ...
    def _noisy_moment_impl_moments(
        self, moment: 'alphaclops.Moment', system_qubits: Sequence['alphaclops.Qid']
    ) -> 'alphaclops.OP_TREE':
    ...
    def _noisy_operation_impl_moments(self, operation: 'alphaclops.Operation') -> 'alphaclops.OP_TREE':
        return self.noisy_moments([moment_module.Moment([operation])], operation.qubits)

    def _noisy_operation_impl_moment(self, operation: 'alphaclops.Operation') -> 'alphaclops.OP_TREE':
    ...
    def noisy_operation(self, operation: 'alphaclops.Operation') -> 'alphaclops.OP_TREE':
...
class _NoNoiseModel(NoiseModel):
    ...
    def noisy_moments(self, moments: Iterable['alphaclops.Moment'], system_qubits: Sequence['alphaclops.Qid']):
        return list(moments)
    ...
    def noisy_operation(self, operation: 'alphaclops.Operation') -> 'alphaclops.Operation':
        return operation
...
class ConstantQubitNoiseModel(NoiseModel):
    ...
    def __init__(self, qubit_noise_gate: 'alphaclops.Gate', prepend: bool = False):
    ...
    def __repr__(self) -> str:
        return f'alphaclops.ConstantQubitNoiseModel({self.qubit_noise_gate!r})'
...
class GateSubstitutionNoiseModel(NoiseModel):
    def __init__(self, substitution_func: Callable[['alphaclops.Operation'], 'alphaclops.Operation']):
...
NO_NOISE: 'alphaclops.NoiseModel' = _NoNoiseModel()
document(
    NO_NOISE,
...
NOISE_MODEL_LIKE = Union[None, 'alphaclops.NoiseModel', 'alphaclops.Gate']
document(
    NOISE_MODEL_LIKE,
    """A `alphaclops.NoiseModel` or a value that can be trivially converted into one.

    `None` is a `NOISE_MODEL_LIKE`. It will be replaced by the `alphaclops.NO_NOISE`
    noise model.

    A single qubit gate is a `NOISE_MODEL_LIKE`. It will be wrapped inside of a
    `alphaclops.ConstantQubitNoiseModel`.
    """,
)


def validate_all_measurements(moment: 'alphaclops.Moment') -> bool:
