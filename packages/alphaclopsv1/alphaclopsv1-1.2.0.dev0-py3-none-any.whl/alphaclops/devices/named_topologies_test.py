...
def test_tilted_square_lattice(width, height):
    ...
    alphaclops.testing.assert_equivalent_repr(topo)
...
def test_tilted_square_methods():
    ...
    assert all(isinstance(q, alphaclops.TensorCircuit) for q in qubits)

    mapping = topo.nodes_to_gridqubits(offset=(3, 5))
    assert all(
        isinstance(q, alphaclops.TensorCircuit) and q >= alphaclops.TensorCircuit(0, 0) for q in mapping.values()
    )
...
def test_line_topology():
    ...
    assert all(isinstance(q, alphaclops.LineQubit) for q in mapping.values())
    assert all(mapping[x] == alphaclops.LineQubit(x + 3) for x in mapping)
...
def test_line_topology_nodes_as_qubits():
    for n in range(2, 10, 2):
        assert LineTopology(n).nodes_as_linequbits() == alphaclops.LineQubit.range(n)
...
def test_draw_gridlike_qubits(tilted):
    graph = nx.grid_2d_graph(3, 3)
    graph = nx.relabel_nodes(graph, {(r, c): alphaclops.TensorCircuit(r, c) for r, c in sorted(graph.nodes)})
    ax = MagicMock()
    pos = draw_gridlike(graph, tilted=tilted, ax=ax)
    ax.scatter.assert_called()
