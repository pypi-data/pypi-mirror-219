...
class OpIdentifier:
    ...
    def __init__(self, gate_type: Type['alphaclops.Gate'], *qubits: 'alphaclops.Qid'):
        self._gate_type = gate_type
        self._gate_family = ops.GateFamily(gate_type)
        self._qubits: Tuple['alphaclops.Qid', ...] = tuple(qubits)

    @property
    def gate_type(self) -> Type['alphaclops.Gate']:
    ...
    def __repr__(self) -> str:
        ...
        return f'alphaclops.devices.noise_utils.OpIdentifier({fullname}, {qubits})'

    def _value_equality_values_(self) -> Any:
        return (self.gate_type, self.qubits)

    def _json_dict_(self) -> Dict[str, Any]:
        gate_json = protocols.json_alphaclops_type(self._gate_type)
        return {'gate_type': gate_json, 'qubits': self._qubits}

    @classmethod
    def _from_json_dict_(cls, gate_type, qubits, **kwargs) -> 'OpIdentifier':
        gate_type = protocols.alphaclops_type_from_json(gate_type)
        return cls(gate_type, *qubits)


# TODO: expose all from top-level alphaclops?
