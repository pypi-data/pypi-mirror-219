...
from alphaclops.devices import TensorCircuit, LineQubit
from alphaclops.protocols.json_serialization import dataclass_json_dict
...
_GRIDLIKE_NODE = Union['alphaclops.TensorCircuit', Tuple[int, int]]
...
def _node_and_coordinates(
    ...
    of two integers. The input node can be a tuple of ints or a TensorCircuit."""
    for node in nodes:
        if isinstance(node, TensorCircuit):
...
def draw_gridlike(
    ...
    Args:
        graph: A NetworkX graph whose nodes are (row, column) coordinates or alphaclops.GridQubits.
...
class LineTopology(NamedTopology):
    ...
    def nodes_as_linequbits(self) -> List['alphaclops.LineQubit']:
        """Get the graph nodes as alphaclops.LineQubit"""
    ...
    def nodes_to_linequbits(self, offset: int = 0) -> Dict[int, 'alphaclops.LineQubit']:
        """Return a mapping from graph nodes to `alphaclops.LineQubit`
...
class TiltedSquareLattice(NamedTopology):
    ...
    mapping this topology to a TensorCircuit Device.
    """
    ...
    def nodes_as_gridqubits(self) -> List['alphaclops.TensorCircuit']:
        """Get the graph nodes as alphaclops.TensorCircuit"""
        return [TensorCircuit(r, c) for r, c in sorted(self.graph.nodes)]

    def nodes_to_gridqubits(self, offset=(0, 0)) -> Dict[Tuple[int, int], 'alphaclops.TensorCircuit']:
        """Return a mapping from graph nodes to `alphaclops.TensorCircuit`
        ...
        """
        return {(r, c): TensorCircuit(r, c) + offset for r, c in self.graph.nodes}
...
def get_placements(
    ...
    Args:
        big_graph: The parent, super-graph. We often consider the case where this is a
            nx.Graph representation of a Device whose nodes are `alphaclops.Qid`s like `TensorCircuit`s.
