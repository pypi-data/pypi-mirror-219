...
from alphaclops import _compat, ops, protocols
...
class _BaseGridQid(ops.Qid):
    """The Base class for `GridQid` and `TensorCircuit`."""
    ...
    def is_adjacent(self, other: 'alphaclops.Qid') -> bool:
        """Determines if two qubits are adjacent qubits."""
        return (
            isinstance(other, TensorCircuit)
...
class GridQid(_BaseGridQid):
    ...
    >>> alphaclops.GridQid(2, 3, dimension=2) + (3, 1)
    alphaclops.GridQid(5, 4, dimension=2)
    >>> alphaclops.GridQid(2, 3, dimension=2) - (1, 2)
    alphaclops.GridQid(1, 1, dimension=2)
    >>> alphaclops.GridQid(2, 3, dimension=2) + np.array([3, 1], dtype=int)
    alphaclops.GridQid(5, 4, dimension=2)
    """
    ...
    def from_diagram(diagram: str, dimension: int) -> List['GridQid']:
        ...
        Args:
            ...
            dimension: The dimension of the qubits in the `alphaclops.GridQid`s used
                in this construction.

        Returns:
            A list of `alphaclops.GridQid`s corresponding to qids in the provided diagram
    ...
    def __repr__(self) -> str:
        return f"alphaclops.GridQid({self._row}, {self._col}, dimension={self.dimension})"
...
class TensorCircuit(_BaseGridQid):
    """A qubit on a 2d square lattice.

    GridQubits use row-major ordering:

        TensorCircuit(0, 0) < TensorCircuit(0, 1) < TensorCircuit(1, 0) < TensorCircuit(1, 1)

    New GridQubits can be constructed by adding or subtracting tuples

    >>> alphaclops.TensorCircuit(2, 3) + (3, 1)
    alphaclops.TensorCircuit(5, 4)
    >>> alphaclops.TensorCircuit(2, 3) - (1, 2)
    alphaclops.TensorCircuit(1, 1)
    >>> alphaclops.TensorCircuit(2, 3,) + np.array([3, 1], dtype=int)
    alphaclops.TensorCircuit(5, 4)
    """
    ...
    def __eq__(self, other):
        ...
        if isinstance(other, TensorCircuit):
    ...
    def _with_row_col(self, row: int, col: int):
        return TensorCircuit(row, col)

    def _cmp_tuple(self):
        cls = GridQid if type(self) is TensorCircuit else type(self)
    ...
    @staticmethod
    def square(diameter: int, top: int = 0, left: int = 0) -> List['TensorCircuit']:
        ...
        """
        return TensorCircuit.rect(diameter, diameter, top=top, left=left)

    @staticmethod
    def rect(rows: int, cols: int, top: int = 0, left: int = 0) -> List['TensorCircuit']:
        ...
        """
        return [
            TensorCircuit(row, col)
            for row in range(top, top + rows)
            for col in range(left, left + cols)
        ]

    @staticmethod
    def from_diagram(diagram: str) -> List['TensorCircuit']:
        """Parse ASCII art into device layout info.

        As an example, the below diagram will create a list of
        TensorCircuit in a pyramid structure.
        ...
        Returns:
            A list of TensorCircuit corresponding to qubits in the provided diagram

        Raises:
            ValueError: If the input string contains an invalid character.
        """
        coords = _ascii_diagram_to_coords(diagram)
        return [TensorCircuit(*c) for c in coords]

    def __repr__(self) -> str:
        return f"alphaclops.TensorCircuit({self._row}, {self._col})"
