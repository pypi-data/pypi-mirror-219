...
import alphaclops
from alphaclops.devices.noise_utils import PHYSICAL_GATE_TAG
from alphaclops.devices.thermal_noise_model import (
    _left_mul,
    _right_mul,
    _validate_rates,
    ThermalNoiseModel,
...
def test_create_thermal_noise_per_qubit():
    q0, q1 = alphaclops.LineQubit.range(2)
    gate_durations = {alphaclops.PhasedXZGate: 25.0}
...
def test_noise_from_zero_duration():
    ...
    moment = alphaclops.Moment(alphaclops.Z(q0), alphaclops.Z(q1))
    assert model.noisy_moment(moment, system_qubits=[q0, q1]) == [moment]
...
def test_noise_from_virtual_gates():
    ...
    q0, q1 = alphaclops.LineQubit.range(2)
    gate_durations = {alphaclops.ZPowGate: 25.0}
    heat_rate_GHz = {q1: 1e-5}
    cool_rate_GHz = {q0: 1e-4}
    model = ThermalNoiseModel(
        qubits={q0, q1},
    ...
    part_virtual_moment = alphaclops.Moment(alphaclops.Z(q0), alphaclops.Z(q1).with_tags(PHYSICAL_GATE_TAG))
...
def test_noise_from_measurement():
    ...
    q0, q1 = alphaclops.LineQubit.range(2)
    gate_durations = {alphaclops.ZPowGate: 25.0, alphaclops.MeasurementGate: 4000.0}
    heat_rate_GHz = {q1: 1e-5}
    cool_rate_GHz = {q0: 1e-4}
    model = ThermalNoiseModel(
        qubits={q0, q1},
    ...
    moment = alphaclops.Moment(alphaclops.measure(q0, q1, key='m'))
    assert model.noisy_moment(moment, system_qubits=[q0, q1]) == [moment]

    part_measure_moment = alphaclops.Moment(alphaclops.measure(q0, key='m'), alphaclops.Z(q1))
...
def test_noisy_moment_one_qubit():
    ...
    model = ThermalNoiseModel(
        qubits={q0, q1},
        gate_durations_ns={alphaclops.PhasedXZGate: 25.0, alphaclops.CZPowGate: 25.0},
        heat_rate_GHz={q0: 1e-5, q1: 2e-5},
        cool_rate_GHz={q0: 1e-4, q1: 2e-4},
        dephase_rate_GHz={q0: 3e-4, q1: 4e-4},
        require_physical_tag=False,
    )
    gate = alphaclops.PhasedXZGate(x_exponent=1, z_exponent=0.5, axis_phase_exponent=0.25)
    moment = alphaclops.Moment(gate.on(q0))
    ...
    noisy_choi = alphaclops.kraus_to_choi(alphaclops.kraus(noisy_moment[1].operations[0]))
    assert np.allclose(
        noisy_choi,
        [
...
def test_noisy_moment_one_qubit_prepend():
    ...
    noisy_choi = alphaclops.kraus_to_choi(alphaclops.kraus(noisy_moment[0].operations[0]))
    assert np.allclose(
        noisy_choi,
        [
            [9.99750343e-01, 0, 0, 9.91164267e-01],
...
def test_noise_from_wait():
    ...
    moment = alphaclops.Moment(alphaclops.wait(q0, nanos=100))
...
def test_symbolic_times_for_wait_gate():
    ...
    moment = alphaclops.Moment(alphaclops.wait(q0, nanos=sympy.Symbol('t')))
...
def test_noisy_moment_two_qubit():
    ...
    gate = alphaclops.CZ ** 0.5
    moment = alphaclops.Moment(gate.on(q0, q1))
    noisy_moment = model.noisy_moment(moment, system_qubits=[q0, q1])
    assert noisy_moment[0] == moment
    assert len(noisy_moment[1]) == 2
    noisy_choi_0 = alphaclops.kraus_to_choi(alphaclops.kraus(noisy_moment[1].operations[0]))
    assert np.allclose(
        noisy_choi_0,
        [
            [9.99750343e-01, 0, 0, 9.91164267e-01],
    ...
    noisy_choi_1 = alphaclops.kraus_to_choi(alphaclops.kraus(noisy_moment[1].operations[1]))
    assert np.allclose(
        noisy_choi_1,
        [
