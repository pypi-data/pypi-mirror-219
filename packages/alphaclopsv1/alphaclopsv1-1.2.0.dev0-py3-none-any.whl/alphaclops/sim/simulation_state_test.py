...
import alphaclops
from alphaclops.sim import simulation_state
from alphaclops.testing import PhaseUsingCleanAncilla, PhaseUsingDirtyAncilla
...
class DummySimulationState(alphaclops.SimulationState):
    def __init__(self, qubits=alphaclops.LineQubit.range(2)):
...
class DelegatingAncillaZ(alphaclops.Gate):
...
def test_measurements():
    ...
    args.measure([alphaclops.LineQubit(0)], "test", [False], {})
...
def test_decompose():
    ...
    assert simulation_state.strat_act_on_from_apply_decompose(
        Composite(), args, [alphaclops.LineQubit(0)]
    )


def test_decompose_for_gate_allocating_qubits_raises():
    class Composite(alphaclops.testing.SingleQubitGate):
        def _decompose_(self, qubits):
            anc = alphaclops.NamedQubit("anc")
            yield alphaclops.CNOT(*qubits, anc)

    args = DummySimulationState()

    with pytest.raises(TypeError, match="add_qubits but not remove_qubits"):
        simulation_state.strat_act_on_from_apply_decompose(Composite(), args, [alphaclops.LineQubit(0)])


def test_mapping():
    args = DummySimulationState()
    assert list(iter(args)) == alphaclops.LineQubit.range(2)
    r1 = args[alphaclops.LineQubit(0)]
    assert args is r1
    with pytest.raises(IndexError):
        _ = args[alphaclops.LineQubit(2)]


def test_swap_bad_dimensions():
    q0 = alphaclops.LineQubit(0)
    q1 = alphaclops.LineQid(1, 3)
    args = DummySimulationState()
...
def test_field_getters():
    ...
    assert args.qubit_map == {q: i for i, q in enumerate(alphaclops.LineQubit.range(2))}
...
def test_delegating_gate_unitary(exp):
    ...
    test_circuit = alphaclops.Circuit()
    test_circuit.append(alphaclops.H(q))
    test_circuit.append(DelegatingAncillaZ(exp).on(q))

    control_circuit = alphaclops.Circuit(alphaclops.H(q))
    control_circuit.append(alphaclops.ZPowGate(exponent=exp).on(q))
...
def test_phase_using_dirty_ancilla(num_ancilla: int):
    q = alphaclops.LineQubit(0)
    anc = alphaclops.NamedQubit.range(num_ancilla, prefix='anc')

    u = alphaclops.MatrixGate(alphaclops.testing.random_unitary(2 ** (num_ancilla + 1)))
    test_circuit = alphaclops.Circuit(
        u.on(q, *anc), PhaseUsingDirtyAncilla(ancilla_bitsize=num_ancilla).on(q)
    )
    control_circuit = alphaclops.Circuit(u.on(q, *anc), alphaclops.Z(q))
    assert_test_circuit_for_dm_simulator(test_circuit, control_circuit)
    assert_test_circuit_for_sv_simulator(test_circuit, control_circuit)
...
def test_phase_using_clean_ancilla(num_ancilla: int, theta: float):
    q = alphaclops.LineQubit(0)
    u = alphaclops.MatrixGate(alphaclops.testing.random_unitary(2))
    test_circuit = alphaclops.Circuit(
        u.on(q), PhaseUsingCleanAncilla(theta=theta, ancilla_bitsize=num_ancilla).on(q)
    )
    control_circuit = alphaclops.Circuit(u.on(q), alphaclops.ZPowGate(exponent=theta).on(q))
    assert_test_circuit_for_dm_simulator(test_circuit, control_circuit)
    assert_test_circuit_for_sv_simulator(test_circuit, control_circuit)


def assert_test_circuit_for_dm_simulator(test_circuit, control_circuit) -> None:
    # Density Matrix Simulator: For unitary gates, this fallbacks to `alphaclops.apply_channel`
    # which recursively calls to `alphaclops.apply_unitary(decompose=True)`.
    for split_untangled_states in [True, False]:
        sim = alphaclops.DensityMatrixSimulator(split_untangled_states=split_untangled_states)
...
def assert_test_circuit_for_sv_simulator(test_circuit, control_circuit) -> None:
    ...
    for split_untangled_states in [True, False]:
        sim = alphaclops.Simulator(split_untangled_states=split_untangled_states)
