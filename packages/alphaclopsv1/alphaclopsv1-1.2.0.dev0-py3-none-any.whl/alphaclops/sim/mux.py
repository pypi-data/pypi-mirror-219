...
from alphaclops import circuits, protocols, study, devices, ops, value
from alphaclops._doc import document
from alphaclops.sim import sparse_simulator, density_matrix_simulator
from alphaclops.sim.clifford import clifford_simulator
from alphaclops.transformers import measurement_transformers
...
def _is_clifford_circuit(program: 'alphaclops.Circuit') -> bool:
    return all(
...
def sample(
    ...
    *,
    ...
    param_resolver: Optional['alphaclops.ParamResolver'] = None,
    repetitions: int = 1,
    ...
    Returns:
        A `alphaclops.Result` object containing the requested measurement samples.
    """
...
def _to_circuit(program: 'alphaclops.CIRCUIT_LIKE') -> 'alphaclops.Circuit':
...
def final_state_vector(
    program: 'alphaclops.CIRCUIT_LIKE',
    *,
    ...
    if not protocols.has_unitary(protocols.resolve_parameters(circuit_like, param_resolver)):
        raise ValueError(
            ...
            "Maybe you wanted `alphaclops.final_density_matrix`?\n"
            "\n"
...
def final_density_matrix(
    ...
    *,
    ...
    Note that, unlike `alphaclops.final_state_vector`, terminal measurements
