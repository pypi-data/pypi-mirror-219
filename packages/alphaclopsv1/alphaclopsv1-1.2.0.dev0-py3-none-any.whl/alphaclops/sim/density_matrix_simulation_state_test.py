...
def test_default_parameter():
    qid_shape = (2,)
    tensor = alphaclops.to_valid_density_matrix(
        0, len(qid_shape), qid_shape=qid_shape, dtype=np.complex64
    )
    args = alphaclops.DensityMatrixSimulationState(qubits=alphaclops.LineQubit.range(1), initial_state=0)
...
def test_decomposed_fallback():
    ...
    args = alphaclops.DensityMatrixSimulationState(
    ...
    alphaclops.act_on(Composite(), args, alphaclops.LineQubit.range(1))
    np.testing.assert_allclose(
        args.target_tensor, alphaclops.one_hot(index=(1, 1), shape=(2, 2), dtype=np.complex64)
    )
...
def test_qid_shape_error():
    with pytest.raises(ValueError, match="qid_shape must be provided"):
        alphaclops.sim.density_matrix_simulation_state._BufferedDensityMatrix.create(initial_state=0)
...
def test_initial_state_vector():
    ...
    args2 = alphaclops.DensityMatrixSimulationState(
