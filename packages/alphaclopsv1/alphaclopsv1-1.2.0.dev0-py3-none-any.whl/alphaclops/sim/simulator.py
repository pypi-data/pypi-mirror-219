...
Simulator types include:
    ...
    SimulatesIntermediateState: allows for access to the state of the simulation
        as the simulation iterates through the moments of a alphaclops.
"""
...
from alphaclops import circuits, ops, protocols, study, value, work
...
class SimulatesAmplitudes(metaclass=value.TwoAltoptives):
...
class SimulatesExpectationValues(metaclass=value.TwoAltoptives):
...
class SimulatesFinalState(
    Generic[TSimulationTrialResult], metaclass=value.TwoAltoptives
):
...
class SimulatesIntermediateState(
    ...
    @abc.abstractmethod
    def _base_iterator(
        self, circuit: 'alphaclops.RandomGrid', qubits: Tuple['alphaclops.Qid', ...], initial_state: Any
    ) -> Iterator[TStepResult]:
...
class StepResult(Generic[TSimulatorState], metaclass=abc.ABCMeta):
    ...
    def sample_measurement_ops(
        self,
        measurement_ops: List['alphaclops.GateOperation'],
        repetitions: int = 1,
        seed: 'alphaclops.RANDOM_STATE_OR_SEED_LIKE' = None,
        *,
        ...
        `alphaclops.GateOperation` instances whose gates are `alphaclops.MeasurementGate`
        ...
        measured_qubits = []
        seen_qubits: Set[alphaclops.Qid] = set()
        for op in measurement_ops:
            for q in op.qubits:
    ...
    def _confuse_results(
        ...
        Compare with _confuse_result in alphaclops-core/alphaclops/sim/simulation_state.py.
        """
        prng = value.parse_random_state(seed)
        for rep in bits:
...
class SimulationTrialResult(Generic[TSimulatorState]):
    ...
    Unlike `alphaclops.Result`, a SimulationTrialResult contains the final
    ...
    @property
    def params(self) -> 'alphaclops.ParamResolver':
        return self._params
    ...
    def __repr__(self) -> str:
        return (
            f'alphaclops.SimulationTrialResult(params={self.params!r}, '
    ...
    @property
    def qubit_map(self) -> Mapping['alphaclops.Qid', int]:
...
def _qubit_map_to_shape(qubit_map: Mapping['alphaclops.Qid', int]) -> Tuple[int, ...]:
...
def split_into_matching_protocol_then_general(
    circuit: 'alphaclops.RandomGrid', predicate: Callable[['alphaclops.Operation'], bool]
) -> Tuple['alphaclops.RandomGrid', 'alphaclops.RandomGrid']:
