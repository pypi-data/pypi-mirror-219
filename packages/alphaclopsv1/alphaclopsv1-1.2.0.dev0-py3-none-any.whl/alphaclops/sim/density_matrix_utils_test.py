...
def test_sample_density_matrix_big_endian():
    results = []
    for x in range(8):
        matrix = alphaclops.to_valid_density_matrix(x, 3)
        sample = alphaclops.sample_density_matrix(matrix, [2, 1, 0])
    ...
    for index in range(3):
        for x in range(8):
            ...
            np.testing.assert_equal(
                alphaclops.sample_density_matrix(matrix, [index]), [[bool(1 & (x >> (2 - index)))]]
            )
...
def test_sample_density_matrix_partial_indices_oder():
    for x in range(8):
        ...
        np.testing.assert_equal(alphaclops.sample_density_matrix(matrix, [2, 1]), expected)
...
def test_sample_density_matrix_partial_indices_all_orders():
    for perm in itertools.permutations([0, 1, 2]):
        for x in range(8):
            ...
            np.testing.assert_equal(alphaclops.sample_density_matrix(matrix, perm), expected)
...
def test_sample_density_matrix():
    ...
    state[0] = 1 / np.sqrt(2)
    state[2] = 1 / np.sqrt(2)
    matrix = alphaclops.to_valid_density_matrix(state, num_qubits=3)
    for _ in range(10):
        sample = alphaclops.sample_density_matrix(matrix, [2, 1, 0])
    ...
    for _ in range(10):
        np.testing.assert_equal(alphaclops.sample_density_matrix(matrix, [2]), [[False]])
        np.testing.assert_equal(alphaclops.sample_density_matrix(matrix, [0]), [[False]])


def test_sample_density_matrix_seed():
    density_matrix = 0.5 * np.eye(2)

    samples = alphaclops.sample_density_matrix(density_matrix, [0], repetitions=10, seed=1234)
    assert np.array_equal(
        samples,
        [[False], [True], [False], [True], [True], [False], [False], [True], [True], [True]],
    )

    samples = alphaclops.sample_density_matrix(
...
def test_sample_empty_density_matrix():
    ...
    np.testing.assert_almost_equal(alphaclops.sample_density_matrix(matrix, []), [[]])


def test_sample_density_matrix_no_repetitions():
    matrix = alphaclops.to_valid_density_matrix(0, 3)
    np.testing.assert_almost_equal(
        alphaclops.sample_density_matrix(matrix, [1], repetitions=0), np.zeros(shape=(0, 1))
    )
    np.testing.assert_almost_equal(
        alphaclops.sample_density_matrix(matrix, [0, 1], repetitions=0), np.zeros(shape=(0, 2))
    )
...
def test_sample_density_matrix_repetitions():
    for perm in itertools.permutations([0, 1, 2]):
        for x in range(8):
            ...
            result = alphaclops.sample_density_matrix(matrix, perm, repetitions=3)
            np.testing.assert_equal(result, expected)
...
def test_sample_density_matrix_negative_repetitions():
    ...
    with pytest.raises(ValueError, match='-1'):
        alphaclops.sample_density_matrix(matrix, [1], repetitions=-1)


def test_sample_density_matrix_not_square():
    with pytest.raises(ValueError, match='not square'):
        alphaclops.sample_density_matrix(np.array([1, 0, 0]), [1])


def test_sample_density_matrix_not_power_of_two():
    with pytest.raises(ValueError, match='power of two'):
        alphaclops.sample_density_matrix(np.ones((3, 3)) / 3, [1])
    with pytest.raises(ValueError, match='power of two'):
        alphaclops.sample_density_matrix(np.ones((2, 3, 2, 3)) / 6, [1])


def test_sample_density_matrix_higher_powers_of_two():
    with pytest.raises(ValueError, match='powers of two'):
        alphaclops.sample_density_matrix(np.ones((2, 4, 2, 4)) / 8, [1])


def test_sample_density_matrix_out_of_range():
    matrix = alphaclops.to_valid_density_matrix(0, 3)
    with pytest.raises(IndexError, match='-2'):
        alphaclops.sample_density_matrix(matrix, [-2])
    with pytest.raises(IndexError, match='3'):
        alphaclops.sample_density_matrix(matrix, [3])


def test_sample_density_matrix_no_indices():
    matrix = alphaclops.to_valid_density_matrix(0, 3)
    bits = alphaclops.sample_density_matrix(matrix, [])
    np.testing.assert_almost_equal(bits, np.zeros(shape=(1, 0)))


def test_sample_density_matrix_validate_qid_shape():
    matrix = alphaclops.to_valid_density_matrix(0, 3)
    alphaclops.sample_density_matrix(matrix, [], qid_shape=(2, 2, 2))
    with pytest.raises(ValueError, match='Matrix size does not match qid shape'):
        alphaclops.sample_density_matrix(matrix, [], qid_shape=(2, 2, 1))
    matrix2 = alphaclops.to_valid_density_matrix(0, qid_shape=(1, 2, 3))
    alphaclops.sample_density_matrix(matrix2, [], qid_shape=(1, 2, 3))
    with pytest.raises(ValueError, match='Matrix size does not match qid shape'):
        alphaclops.sample_density_matrix(matrix2, [], qid_shape=(2, 2, 2))
...
def test_measure_density_matrix_computational_basis():
    ...
    for x in range(8):
        matrix = alphaclops.to_valid_density_matrix(x, 3)
        bits, out_matrix = alphaclops.measure_density_matrix(matrix, [2, 1, 0])
        results.append(bits)
...
def test_measure_density_matrix_computational_basis_reversed():
    ...
    for x in range(8):
        matrix = alphaclops.to_valid_density_matrix(x, 3)
        bits, out_matrix = alphaclops.measure_density_matrix(matrix, [0, 1, 2])
        results.append(bits)
        np.testing.assert_almost_equal(out_matrix, matrix)
...
def test_measure_density_matrix_computational_basis_reshaped():
    results = []
    for x in range(8):
        matrix = np.reshape(alphaclops.to_valid_density_matrix(x, 3), (2,) * 6)
        bits, out_matrix = alphaclops.measure_density_matrix(matrix, [2, 1, 0])
        results.append(bits)
        np.testing.assert_almost_equal(out_matrix, matrix)
    expected = [list(reversed(x)) for x in list(itertools.product([False, True], repeat=3))]
...
def test_measure_density_matrix_partial_indices():
    for index in range(3):
        for x in range(8):
            matrix = alphaclops.to_valid_density_matrix(x, 3)
            bits, out_matrix = alphaclops.measure_density_matrix(matrix, [index])
            np.testing.assert_almost_equal(out_matrix, matrix)
...
def test_measure_density_matrix_partial_indices_all_orders():
    for perm in itertools.permutations([0, 1, 2]):
        for x in range(8):
            matrix = alphaclops.to_valid_density_matrix(x, 3)
            bits, out_matrix = alphaclops.measure_density_matrix(matrix, perm)
...
def matrix_000_plus_010():
    ...
    return alphaclops.to_valid_density_matrix(state, num_qubits=3)
...
def test_measure_density_matrix_collapse():
    ...
    # Partial sample is correct.
    for _ in range(10):
        bits, out_matrix = alphaclops.measure_density_matrix(matrix, [2])
        np.testing.assert_almost_equal(out_matrix, matrix)
        assert bits == [False]

        bits, out_matrix = alphaclops.measure_density_matrix(matrix, [0])
        np.testing.assert_almost_equal(out_matrix, matrix)
        assert bits == [False]
...
def test_measure_density_matrix_seed():
    ...
    bits, out_matrix1 = alphaclops.measure_density_matrix(matrix, range(n), seed=1234)
    assert bits == [False, False, True, True, False]

    bits, out_matrix2 = alphaclops.measure_density_matrix(
...
def test_measure_density_matrix_out_is_matrix():
    matrix = matrix_000_plus_010()
    bits, out_matrix = alphaclops.measure_density_matrix(matrix, [2, 1, 0], out=matrix)
...
def test_measure_state_out_is_not_matrix():
    ...
    _, out_matrix = alphaclops.measure_density_matrix(matrix, [2, 1, 0], out=out)
...
def test_measure_density_matrix_not_square():
    with pytest.raises(ValueError, match='not square'):
        alphaclops.measure_density_matrix(np.array([1, 0, 0]), [1])
    with pytest.raises(ValueError, match='not square'):
        alphaclops.measure_density_matrix(
...
def test_measure_density_matrix_not_power_of_two():
    with pytest.raises(ValueError, match='power of two'):
        alphaclops.measure_density_matrix(np.ones((3, 3)) / 3, [1])
    with pytest.raises(ValueError, match='power of two'):
        alphaclops.measure_density_matrix(np.ones((2, 3, 2, 3)) / 6, [1])


def test_measure_density_matrix_higher_powers_of_two():
    with pytest.raises(ValueError, match='powers of two'):
        alphaclops.measure_density_matrix(np.ones((2, 4, 2, 4)) / 8, [1])
...
def test_measure_density_matrix_out_of_range():
    matrix = alphaclops.to_valid_density_matrix(0, 3)
    with pytest.raises(IndexError, match='-2'):
        alphaclops.measure_density_matrix(matrix, [-2])
    with pytest.raises(IndexError, match='3'):
        alphaclops.measure_density_matrix(matrix, [3])


def test_measure_state_no_indices():
    matrix = alphaclops.to_valid_density_matrix(0, 3)
    bits, out_matrix = alphaclops.measure_density_matrix(matrix, [])
    assert [] == bits
    np.testing.assert_almost_equal(out_matrix, matrix)
...
def test_measure_state_no_indices_out_is_matrix():
    matrix = alphaclops.to_valid_density_matrix(0, 3)
    bits, out_matrix = alphaclops.measure_density_matrix(matrix, [], out=matrix)
    assert [] == bits
    np.testing.assert_almost_equal(out_matrix, matrix)
    assert out_matrix is matrix
...
def test_measure_state_no_indices_out_is_not_matrix():
    matrix = alphaclops.to_valid_density_matrix(0, 3)
    out = np.zeros_like(matrix)
    bits, out_matrix = alphaclops.measure_density_matrix(matrix, [], out=out)
    assert [] == bits
    np.testing.assert_almost_equal(out_matrix, matrix)
    assert out is out_matrix
    assert out is not matrix
...
def test_to_valid_density_matrix_on_simulator_output(seed, dtype, split):
    circuit = alphaclops.testing.random_circuit(qubits=5, n_moments=20, op_density=0.9, random_state=seed)
    simulator = alphaclops.DensityMatrixSimulator(split_untangled_states=split, dtype=dtype)
    result = simulator.simulate(circuit)
    _ = alphaclops.to_valid_density_matrix(result.final_density_matrix, num_qubits=5, atol=1e-6)


def test_factor_validation():
    args = alphaclops.DensityMatrixSimulator()._create_simulation_state(0, qubits=alphaclops.LineQubit.range(2))
    args.apply_operation(alphaclops.H(alphaclops.LineQubit(0)))
    t = args.create_merged_state().target_tensor
    alphaclops.linalg.transformations.factor_density_matrix(t, [0])
    alphaclops.linalg.transformations.factor_density_matrix(t, [1])
    args.apply_operation(alphaclops.CNOT(alphaclops.LineQubit(0), alphaclops.LineQubit(1)))
    t = args.create_merged_state().target_tensor
