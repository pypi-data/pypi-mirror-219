...
class CountingState(alphaclops.qis.QuantumStateRepresentation):
    ...
    def measure(
        self, axes: Sequence[int], seed: 'alphaclops.RANDOM_STATE_OR_SEED_LIKE' = None
    ) -> List[int]:
...
class CountingSimulationState(alphaclops.SimulationState[CountingState]):
...
class CountingStepResult(alphaclops.StepResultBase[CountingSimulationState]):
    def sample(
        self,
        qubits: List[alphaclops.Qid],
        repetitions: int = 1,
...
class CountingTrialResult(alphaclops.SimulationTrialResultBase[CountingSimulationState]):
    pass


class CountingSimulator(
    alphaclops.SimulatorBase[CountingStepResult, CountingTrialResult, CountingSimulationState]
):
    ...
    def _create_partial_simulation_state(
        ...
        classical_data: alphaclops.ClassicalDataStore,
    ...
    def _create_simulator_trial_result(
        ...
        final_simulator_state: 'alphaclops.SimulationStateBase[CountingSimulationState]',
    ) -> CountingTrialResult:
    ...
    def _create_step_result(
        self, sim_state: alphaclops.SimulationStateBase[CountingSimulationState]
    ) -> CountingStepResult:
...
class TestOp(alphaclops.Operation):
...
def test_simulate_empty_circuit():
    ...
    r = sim.simulate(alphaclops.Circuit())
...
def test_simulate_one_gate_circuit():
    sim = CountingSimulator()
    r = sim.simulate(alphaclops.Circuit(alphaclops.X(q0)))
...
def test_simulate_one_measurement_circuit():
    sim = CountingSimulator()
    r = sim.simulate(alphaclops.Circuit(alphaclops.measure(q0)))
    assert r._final_simulator_state.gate_count == 0
...
def test_empty_circuit_simulation_has_moment():
    sim = CountingSimulator()
    steps = list(sim.simulate_moment_steps(alphaclops.Circuit()))
    assert len(steps) == 1


def test_noise_applied():
    sim = CountingSimulator(noise=alphaclops.X)
    r = sim.simulate(alphaclops.Circuit(alphaclops.X(q0)))
...
def test_parameterized_copies_all_but_last():
    sim = CountingSimulator()
    n = 4
    rs = sim.simulate_sweep(alphaclops.Circuit(alphaclops.X(q0) ** 'a'), [{'a': i} for i in range(n)])
    for i in range(n):
        r = rs[i]
        assert r._final_simulator_state.gate_count == 1
        assert r._final_simulator_state.measurement_count == 0
...
def test_cannot_act():
    ...
    with pytest.raises(TypeError, match="CountingSimulator doesn't support .*BadOp"):
        sim.simulate(alphaclops.Circuit(BadOp()))


def test_run_one_gate_circuit():
    sim = CountingSimulator()
    r = sim.run(alphaclops.Circuit(alphaclops.X(q0), alphaclops.measure(q0)), repetitions=2)
...
def test_run_non_unitary_circuit():
    sim = CountingSimulator()
    r = sim.run(alphaclops.Circuit(alphaclops.phase_damp(1).on(q0), alphaclops.measure(q0)), repetitions=2)
    assert np.allclose(r.measurements['q(0)'], [[1], [1]])
...
def test_run_non_unitary_circuit_non_unitary_state():
    class DensityCountingSimulator(CountingSimulator):
        def _can_be_in_run_prefix(self, val):
            return not alphaclops.is_measurement(val)
...
def test_run_non_terminal_measurement():
    sim = CountingSimulator()
    r = sim.run(alphaclops.Circuit(alphaclops.X(q0), alphaclops.measure(q0), alphaclops.X(q0)), repetitions=2)
    assert np.allclose(r.measurements['q(0)'], [[1], [1]])
...
def test_entanglement_causes_join():
    ...
    state = sim._create_simulation_state(2, (q0, q1))
    assert len(set(state.values())) == 3
    state.apply_operation(alphaclops.CNOT(q0, q1))
    assert len(set(state.values())) == 2
    assert state[q0] is state[q1]
...
def test_measurement_causes_split():
    ...
    state = sim._create_simulation_state(entangled_state_repr, (q0, q1))
    assert len(set(state.values())) == 2
    state.apply_operation(alphaclops.measure(q0))
    assert len(set(state.values())) == 3
    assert state[q0] is not state[q1]
...
def test_sim_state_instance_unchanged_during_normal_sim(split: bool):
    ...
    circuit = alphaclops.Circuit(alphaclops.H(q0), alphaclops.CNOT(q0, q1), alphaclops.reset(q1))
...
def test_measurements_retained_in_step_results():
    ...
    circuit = alphaclops.Circuit(
        alphaclops.measure(q0, key='a'), alphaclops.measure(q0, key='b'), alphaclops.measure(q0, key='c')
    )
