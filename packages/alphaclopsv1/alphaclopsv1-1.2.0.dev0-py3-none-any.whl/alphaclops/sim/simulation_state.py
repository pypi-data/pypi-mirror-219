...
TState = TypeVar('TState', bound='alphaclops.QuantumStateRepresentation')
...
class SimulationState(SimulationStateBase, Generic[TState], metaclass=abc.ABCMeta):
    ...
    def get_axes(self, qubits: Sequence['alphaclops.Qid']) -> List[int]:
        return [self.qubit_map[q] for q in qubits]

    def _perform_measurement(self, qubits: Sequence['alphaclops.Qid']) -> List[int]:
    ...
    def _confuse_result(
        ...
        Compare with _confuse_results in alphaclops-core/alphaclops/sim/simulator.py.
        """
        confused = list(bits)
    ...
    def add_qubits(self: Self, qubits: Sequence['alphaclops.Qid']) -> Self:
        ...
        Returns:
            ...
            Self: A `alphaclops.SimulationState` with qubits added or `self` if there are no qubits to
                add.
        """
        if not qubits:
            return self
    ...
    def remove_qubits(self: Self, qubits: Sequence['alphaclops.Qid']) -> Self:
        ...
        Returns:
            NotImplemented: If the subclass does not implement this method.
            Self: A `alphaclops.SimulationState` with qubits removed or `self` if there are no qubits to
                remove.
        """
        if not qubits:
            return self
    ...
    def factor(
        self, qubits: Sequence['alphaclops.Qid'], *, validate=True, atol=1e-07, inplace=False
    ) -> Tuple[Self, Self]:
    ...
    def transpose_to_qubit_order(self, qubits: Sequence['alphaclops.Qid'], *, inplace=False) -> Self:
    ...
    def swap(self, q1: 'alphaclops.Qid', q2: 'alphaclops.Qid', *, inplace=False):
        """Swaps two qubits.
    ...
    def rename(self, q1: 'alphaclops.Qid', q2: 'alphaclops.Qid', *, inplace=False):
        """Renames `q1` to `q2`.
    ...
    def __getitem__(self, item: Optional['alphaclops.Qid']) -> Self:
    ...
    def __iter__(self) -> Iterator[Optional['alphaclops.Qid']]:
...
def strat_act_on_from_apply_decompose(
    val: Any, args: 'alphaclops.SimulationState', qubits: Sequence['alphaclops.Qid']
) -> bool:
    ...
    if decomposed is None:
        return NotImplemented
    all_ancilla: Set['alphaclops.Qid'] = set()
