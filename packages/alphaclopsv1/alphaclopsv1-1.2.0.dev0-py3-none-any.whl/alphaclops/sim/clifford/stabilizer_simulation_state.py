...
from alphaclops import linalg, ops, protocols
from alphaclops.ops import common_gates, global_phase_op, matrix_gates, swap_gates
from alphaclops.ops.clifford_gate import SingleQubitCliffordGate
from alphaclops.protocols import has_unitary, num_qubits, unitary
...
TStabilizerState = TypeVar('TStabilizerState', bound='alphaclops.StabilizerState')
...
class StabilizerSimulationState(
    ...
    def _strat_apply_gate(self, val: Any, qubits: Sequence['alphaclops.Qid']) -> bool:
    ...
    def _strat_apply_mixture(self, val: Any, qubits: Sequence['alphaclops.Qid']) -> bool:
    ...
    def _strat_act_from_single_qubit_decompose(
        self, val: Any, qubits: Sequence['alphaclops.Qid']
    ) -> bool:
        if num_qubits(val) == 1:
    ...
    def _strat_decompose(self, val: Any, qubits: Sequence['alphaclops.Qid']) -> bool:
        gate = val.gate if isinstance(val, ops.Operation) else val
