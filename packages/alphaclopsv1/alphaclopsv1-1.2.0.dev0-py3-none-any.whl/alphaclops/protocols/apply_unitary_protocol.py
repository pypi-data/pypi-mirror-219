...
from alphaclops import linalg, qis
...
class SupportsConsistentApplyUnitary(Protocol):
    ...
    def _apply_unitary_(
        ...
        Args:
            args: A `alphaclops.ApplyUnitaryArgs` object with the `args.target_tensor`
    ...
    *,
    ...
    C. Try to use `unitary_value._decompose_()` (if `allow_decompose`).
        ...
        Case b) Method returns an OP_TREE.
            Delegate to `alphaclops.apply_unitaries`.
    ...
    Args:
        ...
        args: A mutable `alphaclops.ApplyUnitaryArgs` object describing the target
    ...
    if default is not RaiseTypeErrorIfNotProvided:
        return default
    raise TypeError(
        "alphaclops.apply_unitary failed. "
...
def apply_unitaries(
    ...
    Uses `alphaclops.apply_unitary` on each of the unitary values, to apply them to
    ...
    for op in unitary_values:
        ...
        if result is None:
            if default is RaiseTypeErrorIfNotProvided:
                raise TypeError(
                    "alphaclops.apply_unitaries failed. "
