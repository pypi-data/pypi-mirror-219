...
from alphaclops.protocols.decompose_protocol import _try_decompose_into_operations_and_qubits
from alphaclops.protocols.mixture_protocol import has_mixture
...
class SupportsKraus(Protocol):
    ...
    def _kraus_(self) -> Union[Sequence[np.ndarray], NotImplementedType]:
        ...
        This method is used by the global `alphaclops.channel` method. If this method
        ...
        (resulting in a TypeError or default result when calling `alphaclops.channel`
    ...
    def _has_kraus_(self) -> bool:
        ...
        This method is used by the global `alphaclops.has_channel` method.  If this
        method is not present, or returns NotImplemented, it will fallback
        to similarly checking `alphaclops.has_mixture` or `alphaclops.has_unitary`. If none
        of these are present or return NotImplemented, then `alphaclops.has_channel`
        will fall back to checking whether `alphaclops.channel` has a non-default
        value. Otherwise `alphaclops.has_channel` returns False.
...
def kraus(
    ...
    if channel_getter is not None:
        warnings.warn(
            '_channel_ is deprecated and will be removed in alphaclops 0.13, rename to _kraus_',
            DeprecationWarning,
        )
