...
class SupportsActOn(Protocol):
    ...
    @doc_private
    def _act_on_(self, sim_state: 'alphaclops.SimulationStateBase') -> Union[NotImplementedType, bool]:
        ...
        checks if `isinstance(sim_state, alphaclops.StateVectorSimulationState)` and,
        if so, apply their unitary effect to the state vector.

        The global `alphaclops.act_on` method looks for whether or not the given
...
def act_on(
    action: Any,
    sim_state: 'alphaclops.SimulationStateBase',
    ...
    *,
    ...
    For example, the action may be a `alphaclops.Operation` and the state argument may
