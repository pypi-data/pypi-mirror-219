...
def apply_channel(val, rho, left_axes, right_axes, assert_result_is_out_buf=False):
    ...
    result = alphaclops.apply_channel(
        val,
        args=alphaclops.ApplyChannelArgs(
            target_tensor=rho,
            left_axes=left_axes,
...
def test_apply_channel_bad_args():
    ...
    with pytest.raises(ValueError, match='Invalid target_tensor shape'):
        alphaclops.apply_channel(
            alphaclops.IdentityGate(3, (1, 2, 3)),
            alphaclops.ApplyChannelArgs(
                target,
...
def test_apply_channel_simple():
    ...
    class HasApplyChannel:
        def _apply_channel_(self, args: alphaclops.ApplyChannelArgs):
            ...
            zero_right = alphaclops.slice_for_qubits_equal_to(args.right_axes, 0)
            one_right = alphaclops.slice_for_qubits_equal_to(args.right_axes, 1)
            args.out_buffer[:] = 0
...
def test_apply_channel_no_protocols_implemented_default():
    ...
    result = alphaclops.apply_channel(NoProtocols(), args, 'alphaclops')
...
def test_apply_channel_unitary():
    ...
    class HasUnitaryButReturnsNotImplemented(HasUnitary):
        def _apply_unitary_(self, args: alphaclops.ApplyChannelArgs):
