...
def test_powerless(val):
    assert alphaclops.pow(val, 5, None) is None
    assert alphaclops.pow(val, 2, NotImplemented) is NotImplemented

    # Don't assume X**1 == X if X doesn't define __pow__.
    assert alphaclops.pow(val, 1, None) is None


def test_pow_error():
    with pytest.raises(TypeError, match="returned NotImplemented"):
        _ = alphaclops.pow(ReturnsNotImplemented(), 3)
    with pytest.raises(TypeError, match="no __pow__ method"):
        _ = alphaclops.pow(NoMethod(), 3)


@pytest.mark.parametrize('val,exponent,out', ((ReturnsExponent(), 2, 2), (1, 2, 1), (2, 3, 8)))
def test_pow_with_result(val, exponent, out):
    assert (
            alphaclops.pow(val, exponent) == alphaclops.pow(val, exponent, default=None) == val ** exponent == out
    )
