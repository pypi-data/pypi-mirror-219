...
def test_equivalent_to_builtin_mul():
    ...
    for a in test_vals:
        for b in test_vals:
            ...
            c = alphaclops.mul(a, b, default=None)
            if c is None:
                with pytest.raises(TypeError):
                    _ = a * b
                with pytest.raises(TypeError):
                    _ = alphaclops.mul(a, b)
            else:
                assert c == a * b
...
def test_symbol_special_case():
    x = sympy.Symbol('x')
    assert alphaclops.mul(x, 1.0) is x
    assert alphaclops.mul(1.0, x) is x
    assert str(alphaclops.mul(-1.0, x)) == '-x'
    assert str(alphaclops.mul(x, -1.0)) == '-x'
