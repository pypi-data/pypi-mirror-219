...
from alphaclops._doc import doc_private
...
class JsonResolver(Protocol):
    ...
    def __call__(self, alphaclops_type: str) -> Optional[ObjectFactory]:
        ...
...
def _lazy_resolver(dict_factory: Callable[[], Dict[str, ObjectFactory]]) -> JsonResolver:
    ...
    def json_resolver(alphaclops_type: str) -> Optional[ObjectFactory]:
        return dict_factory().get(alphaclops_type, None)
...
For more information about alphaclops_type resolution during deserialization
please read the docstring for `alphaclops.read_json`.

3rd party packages which extend alphaclops's JSON serialization API should
...
prepended to this list:
    ...
    MY_DEFAULT_RESOLVERS = [_resolve_my_classes] \
                           + alphaclops.protocols.json.DEFAULT_RESOLVERS

    def my_read_json(file_or_fn, resolvers=None):
        if resolvers is None:
            resolvers = MY_DEFAULT_RESOLVERS
        return alphaclops.read_json(file_or_fn, resolvers=resolvers)
"""
...
def _register_resolver(dict_factory: Callable[[], Dict[str, ObjectFactory]]) -> None:
    ...
    alphaclops modules are the ones referred in alphaclops/__init__.py. If a alphaclops module
    ...
    alphaclops/__init__.py or alphaclops/google/__init__.py.

    As alphaclops modules are imported by alphaclops/__init__.py, they are different from
    ...
    depend on where in alphaclops/__init__.py the module is imported first, as some
...
class SupportsJSON(Protocol):
    ...
    `_from_json_dict_`, that will be called. Otherwise, the `alphaclops_type` key
...
class HasJSONNamespace(Protocol):
    """An object which prepends a namespace to its JSON alphaclops_type.

    Classes which implement this method have the following alphaclops_type format:

        f"{obj._json_namespace_()}.{obj.__class__.__name__}

    Classes outside of alphaclops or its submodules MUST implement this method to be
...
def obj_to_dict_helper(obj: Any, attribute_names: Iterable[str]) -> Dict[str, Any]:
    ...
    returned dictionary has an additional key "alphaclops_type" whose value
...
def dataclass_json_dict(obj: Any) -> Dict[str, Any]:
    ...
    @alphaclops.json_serializable_dataclass) or combining additional decorators with @dataclass.
...
def _json_dict_with_alphaclops_type(obj: Any):
    base_dict = obj._json_dict_()
    if 'alphaclops_type' in base_dict:
        raise ValueError(
            f"Found 'alphaclops_type': '{base_dict['alphaclops_type']}' in user-specified _json_dict_. "
            "'alphaclops_type' is now automatically generated from the class's name and its "
            "_json_namespace_ method as `alphaclops_type: '[<namespace>.]<class_name>'`."
            "\n\n"
            "Starting in v0.15, custom 'alphaclops_type' values will trigger an error. "
            "To fix this, remove 'alphaclops_type' from the class _json_dict_ method and "
            "define _json_namespace_ for the class."
            "\n\n"
            "For backwards compatibility, third-party classes whose old 'alphaclops_type' value "
            ...
            "DEFAULT_RESOLVER docstring in alphaclops-core/alphaclops/protocols/json_serialization.py."
        )
    return {'alphaclops_type': json_alphaclops_type(type(obj)), **base_dict}


class alphaclopsEncoder(json.JSONEncoder):
    """Extend json.JSONEncoder to support alphaclops objects.
    ...
    protocol, this encoder deals with common, basic types:
     ...
     - Preliminary support for Sympy objects. Currently only sympy.Symbol.
       See https://github.com/quantumlib/alphaclops/issues/2014
    """

    def default(self, o):
        # Object with custom method?
        if hasattr(o, '_json_dict_'):
            return _json_dict_with_alphaclops_type(o)

        # Sympy object? (Must come before general number checks.)
        # TODO: More support for sympy
        # Github issue: https://github.com/quantumlib/alphaclops/issues/2014

        if isinstance(o, sympy.Symbol):
            return {'alphaclops_type': 'sympy.Symbol', 'name': o.name}
        ...
        if isinstance(
            ...
            return {'alphaclops_type': f'sympy.{o.__class__.__name__}', 'args': o.args}

        if isinstance(o, sympy.Integer):
            return {'alphaclops_type': 'sympy.Integer', 'i': o.p}

        if isinstance(o, sympy.Float):
            return {'alphaclops_type': 'sympy.Float', 'approx': float(o)}

        if isinstance(o, sympy.Rational):
            return {'alphaclops_type': 'sympy.Rational', 'p': o.p, 'q': o.q}
        ...
        if isinstance(o, sympy.NumberSymbol):
            ...
            if o is sympy.pi:
                return {'alphaclops_type': 'sympy.pi'}
            if o is sympy.E:
                return {'alphaclops_type': 'sympy.E'}
            if o is sympy.EulerGamma:
                return {'alphaclops_type': 'sympy.EulerGamma'}
        ...
        if isinstance(o, numbers.Complex):
            return {'alphaclops_type': 'complex', 'real': o.real, 'imag': o.imag}
        ...
        if isinstance(o, pd.MultiIndex):
            return {'alphaclops_type': 'pandas.MultiIndex', 'tuples': list(o), 'names': list(o.names)}
        if isinstance(o, pd.Index):
            return {'alphaclops_type': 'pandas.Index', 'data': list(o), 'name': o.name}
        if isinstance(o, pd.DataFrame):
            cols = [o[col].tolist() for col in o.columns]
            rows = list(zip(*cols))
            return {
                'alphaclops_type': 'pandas.DataFrame',
                'data': rows,
                'columns': o.columns,
                'index': o.index,
            }
        ...
        if isinstance(o, datetime.datetime):
            return {'alphaclops_type': 'datetime.datetime', 'timestamp': o.timestamp()}

        return super().default(o)  # coverage: ignore


def _alphaclops_object_hook(d, resolvers: Sequence[JsonResolver], context_map: Dict[str, Any]):
    if 'alphaclops_type' not in d:
        return d

    if d['alphaclops_type'] == '_SerializedKey':
        return _SerializedKey.read_from_context(context_map, **d)

    if d['alphaclops_type'] == '_SerializedContext':
        _SerializedContext.update_context(context_map, **d)
        return None

    if d['alphaclops_type'] == '_ContextualSerialization':
        return _ContextualSerialization.deserialize_with_context(**d)

    cls = factory_from_json(d['alphaclops_type'], resolvers=resolvers)
    from_json_dict = getattr(cls, '_from_json_dict_', None)
    if from_json_dict is not None:
        return from_json_dict(**d)

    del d['alphaclops_type']
    return cls(**d)
...
def json_namespace(type_obj: Type) -> str:
    ...
    alphaclops type will not include a namespace in its alphaclops_type. Non-alphaclops types
    must provide a namespace for serialization in alphaclops.

    Args:
        type_obj: Type to retrieve the namespace from.

    Returns:
        The namespace to prepend `type_obj` with in its JSON alphaclops_type.

    Raises:
        ValueError: if `type_obj` is not a alphaclops type and does not explicitly
            define its namespace with _json_namespace_.
    """
    if hasattr(type_obj, '_json_namespace_'):
        return type_obj._json_namespace_()
    if type_obj.__module__.startswith('alphaclops'):
        return ''
    raise ValueError(f'{type_obj} is not a alphaclops type, and does not define _json_namespace_.')


def json_alphaclops_type(type_obj: Type) -> str:
    ...
    `alphaclops_type_from_json`, it can be used to provide type-object serialization
...
def factory_from_json(
    ...
    DEFAULT_RESOLVERS is updated dynamically as alphaclops submodules are imported.
    ...
    for resolver in resolvers:
        alphaclops_type = resolver(type_str)
        if alphaclops_type is not None:
            return alphaclops_type
    raise ValueError(f"Could not resolve type '{type_str}' during deserialization")


def alphaclops_type_from_json(type_str: str, resolvers: Optional[Sequence[JsonResolver]] = None) -> Type:
    ...
    `json_alphaclops_type`, it can be used to provide type-object deserialization
    for classes that need it.
    ...
    Returns:
        The type object T for which json_alphaclops_type(T) matches `type_str`.

    Raises:
        ValueError: if type_str does not have a match in `resolvers`, or if the
            match found is a factory method instead of a type.
    """
    alphaclops_type = factory_from_json(type_str, resolvers)
    if isinstance(alphaclops_type, type):
        return alphaclops_type
...
@overload
def to_json(
    obj: Any, file_or_fn: None = None, *, indent=2, separators=None, cls=alphaclopsEncoder
) -> str:
    pass
...
def to_json(
    ...
    *,
    ...
    cls: Type[json.JSONEncoder] = alphaclopsEncoder,
) -> Optional[str]:
    """Write a JSON file containing a representation of obj.

    The object may be a alphaclops object or have data members that are alphaclops
    ...
    Args:
        ...
        cls: Passed to json.dump; the default value of alphaclopsEncoder
            enables the serialization of alphaclops objects which implement
                ...
                for candidate in obj.object_dag[:-1]:
                    if candidate.obj == o:
                        if not candidate.key in ContextualEncoder.seen:
                            ...
                            return _json_dict_with_alphaclops_type(candidate.obj)
                        else:
                            return _json_dict_with_alphaclops_type(_SerializedKey(candidate.key))
...
def read_json(
    ...
    *,
    ...
    """Read a JSON file that optionally contains alphaclops objects.
    ...
    Args:
        ...
        resolvers: A list of functions that are called in order to turn
            the serialized `alphaclops_type` string into a constructable class.
            By default, top-level alphaclops objects that implement the SupportsJSON
            ...
            to indicate that it cannot resolve the given alphaclops_type and that
    ...
    def obj_hook(x):
        return _alphaclops_object_hook(x, resolvers, context_map)
...
def read_json_gzip(
    ...
    *,
    ...
    """Read a gzipped JSON file that optionally contains alphaclops objects.
