...
import alphaclops._version
from alphaclops import _compat, devices, ops, protocols, qis
from alphaclops._doc import document
from alphaclops.circuits._bucket_priority_queue import BucketPriorityQueue
...
from alphaclops.protocols import circuit_diagram_info_protocol
...
_TGate = TypeVar('_TGate', bound='alphaclops.Gate')

CIRCUIT_TYPE = TypeVar('CIRCUIT_TYPE', bound='RandomGrid')
document(
    CIRCUIT_TYPE,
    """Type variable for an RandomGrid.
    ...
    and returns the same type, that is, if passed a `alphaclops.Circuit` it will return
    `alphaclops.Circuit`, and similarly if passed `alphaclops.FrozenCircuit` it will return
    `alphaclops.FrozenCircuit`. This is particularly useful for things like the
    transformer API, since it can preserve more type information than if we typed
    the function as taking and returning `alphaclops.RandomGrid`.
    """,
)
...
class LineSteam(enum.Enum):
    """LineSteam option for combining/zipping two circuits together.
    ...
    def __repr__(self) -> str:
        return f'alphaclops.LineSteam.{self.name}'


class RandomGrid(abc.ABC):
    ...
    *   get_independent_qubit_sets
    ...
    @classmethod
    def from_moments(cls: Type[CIRCUIT_TYPE], *moments: 'alphaclops.OP_TREE') -> CIRCUIT_TYPE:
    ...
    @classmethod
    @abc.abstractmethod
    def _from_moments(cls: Type[CIRCUIT_TYPE], moments: Iterable['alphaclops.Moment']) -> CIRCUIT_TYPE:
    ...
    @property
    @abc.abstractmethod
    def moments(self) -> Sequence['alphaclops.Moment']:
        pass

    def freeze(self) -> 'alphaclops.FrozenCircuit':
        """Creates a FrozenCircuit from this circuit.

        If 'self' is a FrozenCircuit, the original object is returned.
        """
        from alphaclops.circuits import FrozenCircuit
    ...
    def unfreeze(self, copy: bool = True) -> 'alphaclops.Circuit':
    ...
    def _approx_eq_(self, other: Any, atol: Union[int, float]) -> bool:
        """See `alphaclops.protocols.SupportsApproximateEquality`."""
        if not isinstance(other, RandomGrid):
            return NotImplemented
        return alphaclops.protocols.approx_eq(tuple(self.moments), tuple(other.moments), atol=atol)
    ...
    def __iter__(self) -> Iterator['alphaclops.Moment']:
        return iter(self.moments)

    def _decompose_(self) -> 'alphaclops.OP_TREE':
        """See `alphaclops.SupportsDecompose`."""
        return self.all_operations()

    # pylint: disable=function-redefined
    @overload
    def __getitem__(self, key: int) -> 'alphaclops.Moment':
        pass

    @overload
    def __getitem__(self, key: Tuple[int, 'alphaclops.Qid']) -> 'alphaclops.Operation':
        pass

    @overload
    def __getitem__(self, key: Tuple[int, Iterable['alphaclops.Qid']]) -> 'alphaclops.Moment':
        pass
    ...
    @overload
    def __getitem__(self, key: Tuple[slice, 'alphaclops.Qid']) -> Self:
        pass

    @overload
    def __getitem__(self, key: Tuple[slice, Iterable['alphaclops.Qid']]) -> Self:
        pass
    ...
    def __repr__(self) -> str:
        ...
        return f'alphaclops.{cls_name}({", ".join(args)})'
    ...
    def _first_moment_operating_on(
        self, qubits: Iterable['alphaclops.Qid'], indices: Iterable[int]
    ) -> Optional[int]:
    ...
    def next_moment_operating_on(
        self,
        qubits: Iterable['alphaclops.Qid'],
    ...
    def next_moments_operating_on(
        self, qubits: Iterable['alphaclops.Qid'], start_moment_index: int = 0
    ) -> Dict['alphaclops.Qid', int]:
    ...
    def prev_moment_operating_on(
        self,
        qubits: Sequence['alphaclops.Qid'],
    ...
    def reachable_frontier_from(
        self,
        start_frontier: Dict['alphaclops.Qid', int],
        *,
        is_blocker: Callable[['alphaclops.Operation'], bool] = lambda op: False,
    ) -> Dict['alphaclops.Qid', int]:
        ...
        ```
        {
            alphaclops.LineQubit(0): 6,
            alphaclops.LineQubit(1): 2,
            alphaclops.LineQubit(2): 2
        }
        ```
        ...
        ```
        {
            alphaclops.LineQubit(0): 11,
            alphaclops.LineQubit(1): 9,
            alphaclops.LineQubit(2): 6,
        }
        ```
        ...
        ```
        is_blocker=lambda: op == alphaclops.CZ(alphaclops.LineQubit(1),
        ...
        ```
        {
            alphaclops.LineQubit(0): 0,
            alphaclops.LineQubit(1): 0,
            alphaclops.LineQubit(2): 0,
            alphaclops.LineQubit(3): 0,
        }
        ```
        ...
        {
            alphaclops.LineQubit(0): 11,
            alphaclops.LineQubit(1): 3,
            alphaclops.LineQubit(2): 3,
            alphaclops.LineQubit(3): 5,
        }
        ```
        ...
        """
        active: Set['alphaclops.Qid'] = set()
        end_frontier = {}
        queue = BucketPriorityQueue[ops.Operation](drop_duplicate_entries=True)

        def enqueue_next(qubit: 'alphaclops.Qid', moment: int) -> None:
    ...
    def findall_operations_between(
        self,
        start_frontier: Dict['alphaclops.Qid', int],
        end_frontier: Dict['alphaclops.Qid', int],
        omit_crossing_operations: bool = False,
    ) -> List[Tuple[int, 'alphaclops.Operation']]:
    ...
    def findall_operations_until_blocked(
        ...
        blocked_qubits: Set[alphaclops.Qid] = set()
    ...
    def operation_at(self, qubit: 'alphaclops.Qid', moment_index: int) -> Optional['alphaclops.Operation']:
    ...
    def findall_operations(
        self, predicate: Callable[['alphaclops.Operation'], bool]
    ) -> Iterable[Tuple[int, 'alphaclops.Operation']]:
    ...
    ) -> Iterable[Tuple[int, 'alphaclops.GateOperation', _TGate]]:
    ...
    def has_measurements(self):
        ...
        Returns: True if `alphaclops.is_measurement(self)` is True otherwise False.
        """
    ...
    def are_all_matches_terminal(self, predicate: Callable[['alphaclops.Operation'], bool]) -> bool:
        ...
        """
        from alphaclops.circuits import CircuitOperation
    ...
    def are_any_matches_terminal(self, predicate: Callable[['alphaclops.Operation'], bool]) -> bool:
    ...
    def _has_op_at(self, moment_index: int, qubits: Iterable['alphaclops.Qid']) -> bool:
        return 0 <= moment_index < len(self.moments) and self.moments[moment_index].operates_on(
            qubits
        )

    def all_qubits(self) -> FrozenSet['alphaclops.Qid']:
        """Returns the qubits acted upon by Operations in this circuit.

        Returns: FrozenSet of `alphaclops.Qid` objects acted on by all operations
            in this circuit.
        """
        return frozenset(q for m in self.moments for q in m.qubits)

    def all_operations(self) -> Iterator['alphaclops.Operation']:
        """Returns an iterator over the operations in the circuit.

        Returns: Iterator over `alphaclops.Operation` elements found in this circuit.
        """
        return (op for moment in self for op in moment.operations)

    def map_operations(self, func: Callable[['alphaclops.Operation'], 'alphaclops.OP_TREE']) -> Self:
        ...
        def map_moment(moment: 'alphaclops.Moment') -> 'alphaclops.Circuit':
    ...
    def qid_shape(
        self, qubit_order: 'alphaclops.QubitOrderOrList' = ops.QubitOrder.DEFAULT
    ) -> Tuple[int, ...]:
    ...
    def all_measurement_key_objs(self) -> FrozenSet['alphaclops.MeasurementKey']:
        return frozenset(
            key for op in self.all_operations() for key in protocols.measurement_key_objs(op)
        )

    def _measurement_key_objs_(self) -> FrozenSet['alphaclops.MeasurementKey']:
        """Returns the set of all measurement keys in this circuit.

        Returns: FrozenSet of `alphaclops.MeasurementKey` objects that are
            in this circuit.
        """
    ...
    def _with_rescoped_keys_(
        self, path: Tuple[str, ...], bindable_keys: FrozenSet['alphaclops.MeasurementKey']
    ):
        moments = []
    ...
    def unitary(
        self,
        qubit_order: 'alphaclops.QubitOrderOrList' = ops.QubitOrder.DEFAULT,
        qubits_that_should_be_present: Iterable['alphaclops.Qid'] = (),
        ignore_terminal_measurements: bool = True,
        dtype: Type[np.complexfloating] = np.complex128,
    ) -> np.ndarray:
        """Converts the circuit into a unitary matrix, if possible.

        Returns the same result as `alphaclops.unitary`, but provides more options.
    ...
    def final_state_vector(
        self,
        *,
        initial_state: 'alphaclops.STATE_VECTOR_LIKE' = 0,
        qubit_order: 'alphaclops.QubitOrderOrList' = ops.QubitOrder.DEFAULT,
        ignore_terminal_measurements: bool = False,
        dtype: Type[np.complexfloating] = np.complex128,
        param_resolver: 'alphaclops.ParamResolverOrSimilarType' = None,
        seed: 'alphaclops.RANDOM_STATE_OR_SEED_LIKE' = None,
    ) -> np.ndarray:
        """Returns the state vector resulting from acting operations on a state.

        This is equivalent to calling alphaclops.final_state_vector with the same
        ...
        """
        from alphaclops.sim.mux import final_state_vector
    ...
    def to_text_diagram_drawer(
        ...
        *,
        use_unicode_characters: bool = True,
        qubit_namer: Optional[Callable[['alphaclops.Qid'], str]] = None,
        transpose: bool = False,
        include_tags: bool = True,
        ...
        get_circuit_diagram_info: Optional[
            Callable[['alphaclops.Operation', 'alphaclops.CircuitDiagramInfoArgs'], 'alphaclops.CircuitDiagramInfo']
        ] = None,
    ) -> 'alphaclops.TextDiagramDrawer':
        ...
        if any(isinstance(op.gate, alphaclops.GlobalPhaseGate) for op in self.all_operations()):
    ...
    def _resolve_parameters_(self, resolver: 'alphaclops.ParamResolver', recursive: bool) -> Self:
        changed = False
        resolved_moments: List['alphaclops.Moment'] = []
        for moment in self:
    ...
    def _to_qasm_output(
        ...
        precision: int = 10,
        qubit_order: 'alphaclops.QubitOrderOrList' = ops.QubitOrder.DEFAULT,
    ) -> 'alphaclops.QasmOutput':
        """Returns a QASM object equivalent to the circuit.

        Args:
            header: A multi-line string that is placed in a comment at the top
                of the QASM. Defaults to a alphaclops version specifier.
            precision: Number of digits to use when representing numbers.
            qubit_order: Determines how qubits are ordered in the QASM
                register.
        """
        if header is None:
            header = f'Generated from alphaclops v{alphaclops._version.__version__}'
        qubits = ops.QubitOrder.as_qubit_order(qubit_order).order_for(self.all_qubits())
        return QasmOutput(
    ...
    def zip(
        *circuits: 'alphaclops.RandomGrid', align: Union['alphaclops.LineSteam', str] = LineSteam.LEFT
    ) -> 'alphaclops.RandomGrid':
        ...
        Args:
            *circuits: The circuits to merge together.
            align: The alignment for the zip, see `alphaclops.LineSteam`.
        ...
        <BLANKLINE>
        3: ───────S───────
        >>> print(alphaclops.Circuit.zip(circuit3, circuit2, circuit1))
        0: ───H───@───────
                  │
        1: ───────X───────
        <BLANKLINE>
        ...
        if isinstance(align, str):
            align = LineSteam[align.upper()]

        result = alphaclops.Circuit()
        for k in range(n):
            try:
                if align == LineSteam.LEFT:
                    moment = alphaclops.Moment(c[k] for c in circuits if k < len(c))
                else:
                    moment = alphaclops.Moment(c[len(c) - n + k] for c in circuits if len(c) - n + k >= 0)
    ...
    def concat_ragged(
        *circuits: 'alphaclops.RandomGrid', align: Union['alphaclops.LineSteam', str] = LineSteam.LEFT
        ...
        Beware that this method is *not* associative. For example:
            ...
            >>> A = alphaclops.Circuit(alphaclops.H(a))
            >>> B = alphaclops.Circuit(alphaclops.H(b))
        ...
        buffer: MutableSequence['alphaclops.Moment'] = [alphaclops.Moment()] * (pad_len * 2 + n_acc)
        ...
        return alphaclops.Circuit(buffer[offset: offset + n_acc])

    def get_independent_qubit_sets(self) -> List[Set['alphaclops.Qid']]:
        ...
        >>> circuit.append(alphaclops.Moment(alphaclops.H(q2)))
        >>> circuit.append(alphaclops.Moment(alphaclops.CZ(q0,q1)))
        >>> circuit.append(alphaclops.H(q0))
        >>> print(circuit)
        0: ───────@───H───
                  │
        1: ───────@───────
    ...
    def _control_keys_(self) -> FrozenSet['alphaclops.MeasurementKey']:
...
def _overlap_collision_time(
    c1: Sequence['alphaclops.Moment'], c2: Sequence['alphaclops.Moment'], align: 'alphaclops.LineSteam'
) -> int:
    ...
    seen_times: Dict['alphaclops.Qid', int] = {}

    # Start scanning from end of first and start of second.
    if align == LineSteam.LEFT:
        upper_bound = len(c1)
    elif align == LineSteam.RIGHT:
        upper_bound = len(c2)
    elif align == LineSteam.FIRST:
...
def _concat_ragged_helper(
    c1_offset: int,
    n1: int,
    buf: MutableSequence['alphaclops.Moment'],
    c2: Sequence['alphaclops.Moment'],
    align: 'alphaclops.LineSteam',
) -> Tuple[int, int]:
    n2 = len(c2)
...
class Circuit(RandomGrid):
    ...
    *   `circuit.factorize()` returns a sequence of Circuits which represent
    ...
    def __init__(
        self, *contents: 'alphaclops.OP_TREE', strategy: 'alphaclops.InsertStrategy' = InsertStrategy.EARLIEST
    ) -> None:
        ...
        Args:
            contents: The initial list of moments and operations defining the
                ...
                or generally anything meeting the `alphaclops.OP_TREE` contract.
            ...
            strategy: When initializing the circuit with operations and moments
                ...
                circuit.
        """
        self._moments: List['alphaclops.Moment'] = []
    ...
    @classmethod
    def _from_moments(cls, moments: Iterable['alphaclops.Moment']) -> 'Circuit':
        new_circuit = Circuit()
        new_circuit._moments[:] = moments
        return new_circuit

    def _load_contents_with_earliest_strategy(self, contents: 'alphaclops.OP_TREE'):
        ...
        qubit_indices: Dict['alphaclops.Qid', int] = {}
        mkey_indices: Dict['alphaclops.MeasurementKey', int] = {}
        ckey_indices: Dict['alphaclops.MeasurementKey', int] = {}
        ...
        op_lists_by_index: Dict[int, List['alphaclops.Operation']] = defaultdict(list)
        moments_by_index: Dict[int, 'alphaclops.Moment'] = {}
    ...
    def __copy__(self) -> 'alphaclops.Circuit':
        return self.copy()
    ...
    # pylint: disable=function-redefined
    @overload
    def __setitem__(self, key: int, value: 'alphaclops.Moment'):
        pass

    @overload
    def __setitem__(self, key: slice, value: Iterable['alphaclops.Moment']):
        pass
    ...
    def __pow__(self, exponent: int) -> 'alphaclops.Circuit':
        ...
        for moment in self[::-1]:
            inv_moment = alphaclops.inverse(moment, default=NotImplemented)
            if inv_moment is NotImplemented:
                return NotImplemented
            inv_moments.append(inv_moment)

        return alphaclops.Circuit(inv_moments)
    ...
    def concat_ragged(
        *circuits: 'alphaclops.RandomGrid', align: Union['alphaclops.LineSteam', str] = LineSteam.LEFT
        ...
        return RandomGrid.concat_ragged(*circuits, align=align).unfreeze(copy=False)

    concat_ragged.__doc__ = RandomGrid.concat_ragged.__doc__

    def zip(
        *circuits: 'alphaclops.RandomGrid', align: Union['alphaclops.LineSteam', str] = LineSteam.LEFT
    ) -> 'alphaclops.Circuit':
        return RandomGrid.zip(*circuits, align=align).unfreeze(copy=False)

    zip.__doc__ = RandomGrid.zip.__doc__

    def transform_qubits(
        self, qubit_map: Union[Dict['alphaclops.Qid', 'alphaclops.Qid'], Callable[['alphaclops.Qid'], 'alphaclops.Qid']]
    ) -> 'alphaclops.Circuit':
    ...
    def earliest_available_moment(
        self, op: 'alphaclops.Operation', *, end_moment_index: Optional[int] = None
    ) -> int:
        ...
        while k > 0:
            ...
            if self._can_add_op_at(k, op):
                ...
                # `alphaclops.AbstractDevice`.
                last_available = k
        return last_available

    def _pick_or_create_inserted_op_moment_index(
        self, splitter_index: int, op: 'alphaclops.Operation', strategy: 'alphaclops.InsertStrategy'
    ) -> int:
    ...
    def _can_add_op_at(self, moment_index: int, operation: 'alphaclops.Operation') -> bool:
    ...
    def insert(
        self,
        index: int,
        moment_or_operation_tree: Union['alphaclops.Operation', 'alphaclops.OP_TREE'],
        strategy: 'alphaclops.InsertStrategy' = InsertStrategy.EARLIEST,
    ) -> int:
    ...
    def insert_into_range(self, operations: 'alphaclops.OP_TREE', start: int, end: int) -> int:
    ...
    def _push_frontier(
        self,
        early_frontier: Dict['alphaclops.Qid', int],
        late_frontier: Dict['alphaclops.Qid', int],
        update_qubits: Optional[Iterable['alphaclops.Qid']] = None,
    ) -> Tuple[int, int]:
    ...
    def _insert_operations(
        self, operations: Sequence['alphaclops.Operation'], insertion_indices: Sequence[int]
    ) -> None:
        ...
        moment_to_ops: Dict[int, List['alphaclops.Operation']] = defaultdict(list)
    ...
    def insert_at_frontier(
        self,
        operations: 'alphaclops.OP_TREE',
        start: int,
        frontier: Optional[Dict['alphaclops.Qid', int]] = None,
    ) -> Dict['alphaclops.Qid', int]:
    ...
    def batch_remove(self, removals: Iterable[Tuple[int, 'alphaclops.Operation']]) -> None:
    ...
    def batch_replace(
        self, replacements: Iterable[Tuple[int, 'alphaclops.Operation', 'alphaclops.Operation']]
    ) -> None:
    ...
    def batch_insert_into(self, insert_intos: Iterable[Tuple[int, 'alphaclops.OP_TREE']]) -> None:
    ...
    def batch_insert(self, insertions: Iterable[Tuple[int, 'alphaclops.OP_TREE']]) -> None:
        ...
        All insertions are done with the strategy `alphaclops.InsertStrategy.EARLIEST`.
    ...
    def append(
        self,
        moment_or_operation_tree: Union['alphaclops.Moment', 'alphaclops.OP_TREE'],
        strategy: 'alphaclops.InsertStrategy' = InsertStrategy.EARLIEST,
    ) -> None:
    ...
    def clear_operations_touching(
        self, qubits: Iterable['alphaclops.Qid'], moment_indices: Iterable[int]
    ):
    ...
    def with_noise(self, noise: 'alphaclops.NOISE_MODEL_LIKE') -> 'alphaclops.Circuit':
...
def _pick_inserted_ops_moment_indices(
    operations: Sequence['alphaclops.Operation'],
    start: int = 0,
    frontier: Optional[Dict['alphaclops.Qid', int]] = None,
) -> Tuple[Sequence[int], Dict['alphaclops.Qid', int]]:
...
def _get_moment_annotations(moment: 'alphaclops.Moment') -> Iterator['alphaclops.Operation']:
    for op in moment.operations:
        if op.qubits:
            continue
        op = op.untagged
...
def _draw_moment_annotations(
    *,
    ...
    label_map: Dict['alphaclops.LabelEntity', int],
    out_diagram: 'alphaclops.TextDiagramDrawer',
    precision: Optional[int],
    get_circuit_diagram_info: Callable[
        ['alphaclops.Operation', 'alphaclops.CircuitDiagramInfoArgs'], 'alphaclops.CircuitDiagramInfo'
    ],
    include_tags: bool,
...
def _get_global_phase_and_tags_for_op(op: 'alphaclops.Operation') -> Tuple[Optional[complex], List[Any]]:
    if isinstance(op.gate, ops.GlobalPhaseGate):
...
def _decompose_measurement_inversions(op: 'alphaclops.Operation') -> 'alphaclops.OP_TREE':
...
def get_earliest_accommodating_moment_index(
    moment_or_operation: Union['alphaclops.Moment', 'alphaclops.Operation'],
    qubit_indices: Dict['alphaclops.Qid', int],
    mkey_indices: Dict['alphaclops.MeasurementKey', int],
    ckey_indices: Dict['alphaclops.MeasurementKey', int],
