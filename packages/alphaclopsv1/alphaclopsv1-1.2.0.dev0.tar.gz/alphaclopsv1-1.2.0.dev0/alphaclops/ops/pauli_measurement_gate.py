...
class PauliMeasurementGate(raw_types.Gate):
    ...
    def __init__(
        self,
        observable: Union['alphaclops.BaseDensePauliString', Iterable['alphaclops.Pauli']],
        key: Union[str, 'alphaclops.MeasurementKey'] = '',
    ) -> None:
        """Inits PauliMeasurementGate.

        Args:
            observable: Pauli observable to measure. Any `Iterable[alphaclops.Pauli]`
                is a valid Pauli observable (with a +1 coefficient by default).
                If you wish to measure pauli observables with coefficient -1,
                then pass a `alphaclops.DensePauliString` as observable.
        ...
        if not all(
            isinstance(p, pauli_gates.Pauli) for p in cast(Iterable['alphaclops.Gate'], observable)
        ):
            raise ValueError(f'Pauli observable {observable} must be Iterable[`alphaclops.Pauli`].')
        coefficient = (
            observable.coefficient if isinstance(observable, dps.BaseDensePauliString) else 1
        )
        if coefficient not in [+1, -1]:
            raise ValueError(
                f'`alphaclops.DensePauliString` observable {observable} must have coefficient +1/-1.'
            )
    ...
    @property
    def mkey(self) -> 'alphaclops.MeasurementKey':
        return self._mkey
    ...
    def with_key(self, key: Union[str, 'alphaclops.MeasurementKey']) -> 'PauliMeasurementGate':
    ...
    def with_observable(
        self, observable: Union['alphaclops.BaseDensePauliString', Iterable['alphaclops.Pauli']]
    ) -> 'PauliMeasurementGate':
    ...
    def _measurement_key_obj_(self) -> 'alphaclops.MeasurementKey':
        return self.mkey

    def observable(self) -> 'alphaclops.DensePauliString':
        """Pauli observable which should be measured by the gate."""
        return self._observable

    def _decompose_(
        self, qubits: Tuple['alphaclops.Qid', ...]
    ...
    def _op_repr_(self, qubits: Sequence['alphaclops.Qid']) -> str:
        ...
        arg_list = ', '.join(args)
        return f'alphaclops.measure_single_paulistring({arg_list})'

    def __repr__(self) -> str:
        return f'alphaclops.PauliMeasurementGate({self._observable!r}, {self.mkey!r})'
