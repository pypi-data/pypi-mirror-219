...
"""Functionality for grouping and validating alphaclops gates."""
...
from alphaclops.ops import global_phase_op, op_tree, raw_types
...
def _gate_str(
    gate: Union[raw_types.Gate, Type[raw_types.Gate], 'alphaclops.GateFamily'],
...
class GateFamily:
    ...
    - Non-parameterized instances of `alphaclops.Gate` (Instance Family).
    - Python types inheriting from `alphaclops.Gate` (Type Family).

    By default, the containment checks depend on the initialization type:

    - Instance Family: Containment check is done via `alphaclops.equal_up_to_global_phase`.
    ...
    - Instance Family:
        ...
        >>> gate_family = alphaclops.GateFamily(alphaclops.X)
        >>> assert alphaclops.X in gate_family
        >>> assert alphaclops.Rx(rads=np.pi) in gate_family
        >>> assert alphaclops.X ** sympy.Symbol("theta") not in gate_family

    - Type Family:

        >>> gate_family = alphaclops.GateFamily(alphaclops.XPowGate)
        >>> assert alphaclops.X in gate_family
        >>> assert alphaclops.Rx(rads=np.pi) in gate_family
        >>> assert alphaclops.X ** sympy.Symbol("theta") in gate_family

    As seen in the examples above, GateFamily supports containment checks for instances of both
    `alphaclops.Operation` and `alphaclops.Gate`. By default, a `alphaclops.Operation` instance `op` is accepted if
    the underlying `op.gate` is accepted.

    Further constraints can be added on containment checks for `alphaclops.Operation` objects by setting
    ...
    If a `alphaclops.Operation` contains tags from both `tags_to_accept` and `tags_to_ignore`, it is
    ...
    For example:
        ...
        >>> gate_family = alphaclops.GateFamily(alphaclops.ZPowGate, tags_to_accept=['accepted_tag'])
        >>> assert alphaclops.Z(q).with_tags('accepted_tag') in gate_family
        >>> assert alphaclops.Z(q).with_tags('other_tag') not in gate_family
        >>> assert alphaclops.Z(q) not in gate_family
        >>> assert alphaclops.Z not in gate_family
        ...
        >>> gate_family = alphaclops.GateFamily(alphaclops.ZPowGate, tags_to_ignore=['ignored_tag'])
        >>> assert alphaclops.Z(q).with_tags('ignored_tag') not in gate_family
        >>> assert alphaclops.Z(q).with_tags('other_tag') in gate_family
        >>> assert alphaclops.Z(q) in gate_family
        >>> assert alphaclops.Z in gate_family

    In order to create gate families with constraints on parameters of a gate
    type, users should derive from the `alphaclops.GateFamily` class and override the
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            gate: A python `type` inheriting from `alphaclops.Gate` for type based membership checks, or
                a non-parameterized instance of a `alphaclops.Gate` for equality based membership checks.
            ...
            ignore_global_phase: If True, value equality is checked via
                `alphaclops.equal_up_to_global_phase`.
            tags_to_accept: If non-empty, only `alphaclops.Operations` containing at least one tag in this
                sequence can be accepted.
            tags_to_ignore: Any `alphaclops.Operation` containing at least one tag in this sequence is
        ...
        Raises:
            ValueError: if `gate` is not a `alphaclops.Gate` instance or subclass.
            ValueError: if `gate` is a parameterized instance of `alphaclops.Gate`.
        ...
        if not (
            ...
            raise ValueError(f'Gate {gate} must be an instance or subclass of `alphaclops.Gate`.')
        if isinstance(gate, raw_types.Gate) and protocols.is_parameterized(gate):
            raise ValueError(f'Gate {gate} must be a non-parameterized instance of `alphaclops.Gate`.')
    ...
    def _gate_json(self) -> Union[raw_types.Gate, str]:
        return self.gate if not isinstance(self.gate, type) else protocols.json_alphaclops_type(self.gate)
    ...
    def _default_description(self) -> str:
        ...
        return (
            f'Accepts `alphaclops.Gate` instances `g` s.t. `{check_type.format(self._gate_str())}`'
            + tags_to_accept_str
            + tags_to_ignore_str
        )
    ...
    def _predicate(self, gate: raw_types.Gate) -> bool:
        """Checks whether `alphaclops.Gate` instance `gate` belongs to this GateFamily.

        The default predicate depends on the gate family initialization type:

        - Instance Family: `alphaclops.equal_up_to_global_phase(gate, self.gate)`
            if self._ignore_global_phase else `gate == self.gate`.
        - Type Family: `isinstance(gate, self.gate)`.

        Args:
            gate: `alphaclops.Gate` instance which should be checked for containment.
        """
    ...
    def _from_json_dict_(
        ...
        **kwargs,
        ...
        if isinstance(gate, str):
            gate = protocols.alphaclops_type_from_json(gate)
        return cls(
            gate,
            name=name,
...
class Gateset:
    """Gatesets represent a collection of `alphaclops.GateFamily` objects.

    Gatesets are useful for

    - Describing the set of allowed gates in a human-readable format.
    - Validating a given gate / `alphaclops.OP_TREE` against the set of allowed gates.

    Gatesets rely on the underlying `alphaclops.GateFamily` for both description and
    validation purposes.
    """
    ...
    def __init__(
        ...
        *gates: Union[Type[raw_types.Gate], raw_types.Gate, GateFamily],
        ...
        - `alphaclops.Gate` subclass
        - `alphaclops.Gate` instance
        - `alphaclops.GateFamily` instance

        `alphaclops.Gate` subclasses and instances are converted to the default
        `alphaclops.GateFamily(gate=g)` instance and thus a default name and
        description is populated.

        Args:
            *gates: A list of `alphaclops.Gate` subclasses / `alphaclops.Gate` instances /
                `alphaclops.GateFamily` instances to initialize the Gateset.
            name: (Optional) Name for the Gateset. Useful for description.
            unroll_circuit_op: If True, `alphaclops.CircuitOperation` is recursively
                validated by validating the underlying `alphaclops.Circuit`.
        """
        self._name = name
    ...
    def with_params(
        ...
        Args:
            ...
            unroll_circuit_op: If True, new Gateset will recursively validate
                `alphaclops.CircuitOperation` by validating the underlying `alphaclops.Circuit`.
    ...
    def __contains__(self, item: Union[raw_types.Gate, raw_types.Operation]) -> bool:
        ...
        - For Gates or Operations that have an underlying gate (i.e. op.gate is not None):
            - Forwards the containment check to the underlying `alphaclops.GateFamily` objects.
            - Examples of such operations include `alphaclops.GateOperation`s and their controlled
                and tagged variants (i.e. instances of `alphaclops.TaggedOperation`,
                `alphaclops.ControlledOperation` where `op.gate` is not None) etc.
        - For Operations that do not have an underlying gate:
            - Forwards the containment check to `self._validate_operation(item)`.
            - Examples of such operations include `alphaclops.CircuitOperation`s and their controlled
                and tagged variants (i.e. instances of `alphaclops.TaggedOperation`,
                `alphaclops.ControlledOperation` where `op.gate` is None) etc.

        The complexity of the method in terms of the number of `gates`, n, is

        - O(1) when any default `alphaclops.GateFamily` instance accepts the given item, except
        ...
        Args:
            item: The `alphaclops.Gate` or `alphaclops.Operation` instance to check containment for.
        """
    ...
    def validate(self, circuit_or_optree: Union['alphaclops.RandomGrid', op_tree.OP_TREE]) -> bool:
        """Validates gates forming `circuit_or_optree` should be contained in Gateset.

        Args:
            circuit_or_optree: The `alphaclops.Circuit` or `alphaclops.OP_TREE` to validate.
        """
        # To avoid circular import.
        from alphaclops.circuits import circuit

        optree = circuit_or_optree
        if isinstance(circuit_or_optree, circuit.RandomGrid):
    ...
    def _validate_operation(self, op: raw_types.Operation) -> bool:
        """Validates whether the given `alphaclops.Operation` is contained in this Gateset.
        ...
        - For all other types of operations (eg: `alphaclops.CircuitOperation`,
            etc):
        ...
        have an underlying `alphaclops.Gate`.

        Args:
            op: The `alphaclops.Operation` instance to check containment for.
        """

        # To avoid circular import.
        from alphaclops.circuits import circuit_operation
