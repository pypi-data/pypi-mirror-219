...
from alphaclops import protocols, linalg, value
from alphaclops._compat import proper_repr
from alphaclops.ops import raw_types, identity, pauli_gates, global_phase_op, pauli_string
...
PAULI_CHARS = 'IXYZ'
PAULI_GATES: List[Union['alphaclops.Pauli', 'alphaclops.IdentityGate']] = [
    identity.I,
    pauli_gates.X,
    pauli_gates.Y,
    pauli_gates.Z,
...
class BaseDensePauliString(raw_types.Gate, metaclass=abc.ABCMeta):
    """Parent class for `alphaclops.DensePauliString` and `alphaclops.MutableDensePauliString`.

    `alphaclops.BaseDensePauliString` is an abstract base class, which is used to implement
    `alphaclops.DensePauliString` and `alphaclops.MutableDensePauliString`. The non-mutable version
    is used as the corresponding gate for `alphaclops.PauliString` operation and the mutable
    version is mainly used for efficiently manipulating dense pauli strings.

    See the docstrings of `alphaclops.DensePauliString` and `alphaclops.MutableDensePauliString` for more
    details.

    Examples:
    >>> print(alphaclops.DensePauliString('XXIY'))
    +XXIY

    >>> print(alphaclops.MutableDensePauliString('IZII', coefficient=-1))
    -IZII (mutable)

    >>> print(alphaclops.DensePauliString([0, 1, 2, 3],
    ...
    def __init__(
        self,
        pauli_mask: Union[Iterable['alphaclops.PAULI_GATE_LIKE'], np.ndarray],
        *,
    ...
    @classmethod
    def one_hot(cls, *, index: int, length: int, pauli: 'alphaclops.PAULI_GATE_LIKE') -> Self:
        ...
        Args:
            ...
            pauli: The pauli gate to put at the hot index. Can be set to either
                a string ('X', 'Y', 'Z', 'I'), a alphaclops gate (`alphaclops.X`,
                `alphaclops.Y`, `alphaclops.Z`, or `alphaclops.I`), or an integer (0=I, 1=X, 2=Y,
                3=Z).
        """
    ...
    def _apply_unitary_(self, args) -> Union[np.ndarray, None, NotImplementedType]:
        if not self._has_unitary_():
            return NotImplemented
        from alphaclops import devices

        qubits = devices.LineQubit.range(len(self))
        decomposed_ops = cast(Iterable['alphaclops.OP_TREE'], self._decompose_(qubits))
        return protocols.apply_unitaries(decomposed_ops, qubits, args)

    def _decompose_(
        self, qubits: Sequence['alphaclops.Qid']
    ) -> Union[NotImplementedType, 'alphaclops.OP_TREE']:
        if not self._has_unitary_():
            return NotImplemented
    ...
    @overload
    def __getitem__(self, item: int) -> Union['alphaclops.Pauli', 'alphaclops.IdentityGate']:
        pass
    ...
    def __iter__(self) -> Iterator[Union['alphaclops.Pauli', 'alphaclops.IdentityGate']]:
    ...
    def on(self, *qubits: 'alphaclops.Qid') -> 'alphaclops.PauliString':
        return self.sparse(qubits)

    def sparse(self, qubits: Optional[Sequence['alphaclops.Qid']] = None) -> 'alphaclops.PauliString':
        """A `alphaclops.PauliString` version of this dense pauli string.

        Args:
            qubits: The qubits to apply the Paulis to. Defaults to
                `alphaclops.LineQubit.range(len(self))`.

        Returns:
            A `alphaclops.PauliString` with the non-identity operations from
    ...
    def __repr__(self) -> str:
        ...
        return (
            f'alphaclops.{type(self).__name__}({repr(paulis)}, '
    ...
    def frozen(self) -> 'DensePauliString':
        """A `alphaclops.DensePauliString` with the same contents."""
    ...
    def mutable_copy(self) -> 'MutableDensePauliString':
        """A `alphaclops.MutableDensePauliString` with the same contents."""
...
class DensePauliString(BaseDensePauliString):
    ...
    qubits Pauli gates (including the `alphaclops.IdentityGate`), each of which would act on a
    different qubit. When applied on qubits, a `DensePauliString` results in `alphaclops.PauliString`
    as an operation.

    Note that `alphaclops.PauliString` only stores a tensor product of non-identity `alphaclops.Pauli`
    operations whereas `alphaclops.DensePauliString` also supports storing the `alphaclops.IdentityGate`.

    For example,

    >>> dps = alphaclops.DensePauliString('XXIY')
    >>> print(dps) # 4 qubit pauli operator with 'X' on first 2 qubits, 'I' on 3rd and 'Y' on 4th.
    +XXIY
    >>> ps = dps.on(*alphaclops.LineQubit.range(4)) # When applied on qubits, we get a `alphaclops.PauliString`.
    >>> print(ps) # Note that `alphaclops.PauliString` only preserves non-identity operations.
    X(q(0))*X(q(1))*Y(q(3))

    This can optionally take a coefficient, for example:

    >>> dps = alphaclops.DensePauliString("XX", coefficient=3)
    >>> print(dps) # Represents 3 times the operator XX acting on two qubits.
    (3+0j)*XX
    >>> print(dps.on(*alphaclops.LineQubit.range(2))) # Coefficient is propagated to `alphaclops.PauliString`.
    (3+0j)*X(q(0))*X(q(1))

    If the coefficient has magnitude of 1, the resulting operator is a unitary and thus is
    also a `alphaclops.Gate`.

    Note that `DensePauliString` is an immutable object. If you need a mutable version of
    dense pauli strings, see `alphaclops.MutableDensePauliString`.
    """
...
class MutableDensePauliString(BaseDensePauliString):
    ...
    `alphaclops.MutableDensePauliString` is a mutable version of `alphaclops.DensePauliString`.
    It exists mainly to help mutate dense pauli strings efficiently, instead of always creating
    a copy, and then converting back to a frozen `alphaclops.DensePauliString` representation.

    For example:

    >>> mutable_dps = alphaclops.MutableDensePauliString('XXZZ')
    >>> mutable_dps[:2] = 'YY' # `alphaclops.MutableDensePauliString` supports item assignment.
    >>> print(mutable_dps)
    +YYZZ (mutable)

    See docstrings of `alphaclops.DensePauliString` for more details on dense pauli strings.
    """

    @overload
    def __setitem__(self, key: int, value: 'alphaclops.PAULI_GATE_LIKE') -> Self:
        pass
    ...
    def __setitem__(
        self,
        key: slice,
        value: Union[Iterable['alphaclops.PAULI_GATE_LIKE'], np.ndarray, BaseDensePauliString],
    ) -> Self:
        pass
...
def _pauli_index(val: 'alphaclops.PAULI_GATE_LIKE') -> int:
    m = pauli_string.PAULI_GATE_LIKE_TO_INDEX_MAP
    if val not in m:
        raise TypeError(
            f'Expected a alphaclops.PAULI_GATE_LIKE (any of alphaclops.I alphaclops.X, alphaclops.Y, '
            f'alphaclops.Z, "I", "X", "Y", "Z", "i", "x", "y", "z", 0, 1, 2, 3) but '
            f'got {repr(val)}.'
        )
    return m[val]


def _as_pauli_mask(val: Union[Iterable['alphaclops.PAULI_GATE_LIKE'], np.ndarray]) -> np.ndarray:
    if isinstance(val, np.ndarray):
        return np.asarray(val, dtype=np.uint8)
    return np.array([_pauli_index(v) for v in val], dtype=np.uint8)


def _attempt_value_to_pauli_index(v: 'alphaclops.Operation') -> Optional[Tuple[int, int]]:
    ...
    if not isinstance(q, devices.LineQubit):
        raise ValueError(
            ...
            'other than `alphaclops.LineQubit` so its dense index is ambiguous.\n'
            f'v={repr(v)}.'
        )
