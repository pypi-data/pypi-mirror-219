...
from alphaclops.ops import common_gates, raw_types, identity
from alphaclops.type_workarounds import NotImplementedType


if TYPE_CHECKING:
    import alphaclops
    from alphaclops.ops.pauli_string import SingleQubitPauliStringGateOperation
...
class Pauli(raw_types.Gate, metaclass=abc.ABCMeta):
    ...
    def phased_pauli_product(
        self, other: Union['alphaclops.Pauli', 'identity.IdentityGate']
    ) -> Tuple[complex, Union['alphaclops.Pauli', 'identity.IdentityGate']]:
        if self == other:
            return 1, identity.I
    ...
    def on(self, *qubits: 'alphaclops.Qid') -> 'SingleQubitPauliStringGateOperation':
...
class _PauliX(Pauli, common_gates.XPowGate):
    ...
    def __pow__(self, exponent: 'alphaclops.TParamVal') -> common_gates.XPowGate:
        return common_gates.XPowGate(exponent=exponent) if exponent != 1 else _PauliX()

    def _with_exponent(self, exponent: 'alphaclops.TParamVal') -> common_gates.XPowGate:
    ...
    def basis(self) -> Dict[int, '_XEigenState']:
        from alphaclops.value.product_state import _XEigenState
...
class _PauliY(Pauli, common_gates.YPowGate):
    ...
    def __pow__(self, exponent: 'alphaclops.TParamVal') -> common_gates.YPowGate:
        return common_gates.YPowGate(exponent=exponent) if exponent != 1 else _PauliY()

    def _with_exponent(self, exponent: 'alphaclops.TParamVal') -> common_gates.YPowGate:
        return self.__pow__(exponent)
    ...
    def basis(self) -> Dict[int, '_YEigenState']:
        from alphaclops.value.product_state import _YEigenState
...
class _PauliZ(Pauli, common_gates.ZPowGate):
    ...
    def __pow__(self, exponent: 'alphaclops.TParamVal') -> common_gates.ZPowGate:
        return common_gates.ZPowGate(exponent=exponent) if exponent != 1 else _PauliZ()

    def _with_exponent(self, exponent: 'alphaclops.TParamVal') -> common_gates.ZPowGate:
        return self.__pow__(exponent)
    ...
    def basis(self) -> Dict[int, '_ZEigenState']:
        from alphaclops.value.product_state import _ZEigenState
...
document(
    ...
    This is the `exponent=1` instance of the `alphaclops.XPowGate`.

    The untary matrix of `alphaclops.X` is:
    $$
    \begin{bmatrix}
        0 & 1 \\
        1 & 0
...
document(
    ...
    This is the `exponent=1` instance of the `alphaclops.YPowGate`.

    The unitary matrix of `alphaclops.Y` is:
    $$
    \begin{bmatrix}
        0 & -i \\
        i & 0
...
document(
    ...
    This is the `exponent=1` instance of the `alphaclops.ZPowGate`.

    The unitary matrix of `alphaclops.Z` is:
    $$
    \begin{bmatrix}
        1 & 0 \\
        0 & -1
