...
from alphaclops import linalg, protocols, _import
from alphaclops._compat import proper_repr
from alphaclops.ops import raw_types, phased_x_z_gate

if TYPE_CHECKING:
    import alphaclops

single_qubit_decompositions = _import.LazyLoader(
    'single_qubit_decompositions', globals(), 'alphaclops.transformers.analytical_decompositions'
)
two_qubit_to_cz = _import.LazyLoader(
    'two_qubit_to_cz', globals(), 'alphaclops.transformers.analytical_decompositions'
)
three_qubit_decomposition = _import.LazyLoader(
    'three_qubit_decomposition', globals(), 'alphaclops.transformers.analytical_decompositions'
)
...
class MatrixGate(raw_types.Gate):
    ...
    For example `alphaclops.MatrixGate(np.array([[0, 1j], [1, 0]]))` has the unitary matrix:
        ...
        *,
        ...
        Args:
            ...
            unitary_check_rtol: The relative tolerance for checking whether the supplied matrix
                is unitary. See `alphaclops.is_unitary`.
    ...
    def __repr__(self) -> str:
        if all(e == 2 for e in self._qid_shape):
            return f'alphaclops.MatrixGate({proper_repr(self._matrix)})'
        return f'alphaclops.MatrixGate({proper_repr(self._matrix)}, qid_shape={self._qid_shape})'
