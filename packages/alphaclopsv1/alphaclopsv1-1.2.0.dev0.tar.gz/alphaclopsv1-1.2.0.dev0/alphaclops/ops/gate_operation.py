...
from alphaclops.ops import raw_types, gate_features, control_values as cv
...
class GateOperation(raw_types.Operation):
    ...
    def __init__(self, gate: 'alphaclops.Gate', qubits: Sequence['alphaclops.Qid']) -> None:
        """Inits GateOperation.
    ...
    def with_gate(self, new_gate: 'alphaclops.Gate') -> 'alphaclops.Operation':
    ...
    def __repr__(self):
        ...
        return f'alphaclops.GateOperation(gate={self.gate!r}, qubits=[{qubit_args_repr}])'
    ...
    def _group_interchangeable_qubits(
        self,
    ) -> Tuple[Union['alphaclops.Qid', Tuple[int, FrozenSet['alphaclops.Qid']]], ...]:
        if not isinstance(self.gate, gate_features.InterchangeableQubitsGate):
            return self.qubits
        groups: Dict[int, List['alphaclops.Qid']] = {}
    ...
    def _measurement_key_obj_(self) -> Optional['alphaclops.MeasurementKey']:
    ...
    def _measurement_key_objs_(
        self,
    ) -> Union[FrozenSet['alphaclops.MeasurementKey'], NotImplementedType, None]:
        getter = getattr(self.gate, '_measurement_key_objs_', None)
        if getter is not None:
            return getter()
        return NotImplemented

    def _act_on_(self, sim_state: 'alphaclops.SimulationStateBase'):
    ...
    def __pow__(self, exponent: Any) -> 'alphaclops.Operation':
        ...
        Only works if the gate implements alphaclops.ExtrapolatableEffect.
