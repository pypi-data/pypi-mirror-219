...
class QubitManager(metaclass=abc.ABCMeta):
    @abc.abstractmethod
    def qalloc(self, n: int, dim: int = 2) -> List['alphaclops.Qid']:
        """Allocate `n` clean qubits, i.e. qubits guaranteed to be in state |0>."""

    @abc.abstractmethod
    def qborrow(self, n: int, dim: int = 2) -> List['alphaclops.Qid']:
        """Allocate `n` dirty qubits, i.e. the returned qubits can be in any state."""

    @abc.abstractmethod
    def qfree(self, qubits: Iterable['alphaclops.Qid']) -> None:
...
class _BaseAncillaQid(raw_types.Qid):
    ...
    def __repr__(self) -> str:
        ...
        return f"alphaclops.ops.{type(self).__name__}({self.id}{dim_str}{prefix_str})"
