...
from alphaclops.ops import gate_features, eigen_gate, common_gates, pauli_gates
...
class XXPowGate(gate_features.InterchangeableQubitsGate, eigen_gate.EigenGate):
    ...
    See also: `alphaclops.ops.MSGate` (the Mølmer–Sørensen gate), which is
    ...
    def _decompose_(self, qubits: Tuple['alphaclops.Qid', ...]) -> 'alphaclops.OP_TREE':
    ...
    def __repr__(self) -> str:
        if self._global_shift == 0:
            if self._exponent == 1:
                return 'alphaclops.XX'
            return f'(alphaclops.XX**{proper_repr(self._exponent)})'
        return (
            f'alphaclops.XXPowGate(exponent={proper_repr(self._exponent)}, '
...
class YYPowGate(gate_features.InterchangeableQubitsGate, eigen_gate.EigenGate):
    ...
    def __repr__(self) -> str:
        if self._global_shift == 0:
            if self._exponent == 1:
                return 'alphaclops.YY'
            return f'(alphaclops.YY**{proper_repr(self._exponent)})'
        return (
            f'alphaclops.YYPowGate(exponent={proper_repr(self._exponent)}, '
            f'global_shift={self._global_shift!r})'
        )
...
class ZZPowGate(gate_features.InterchangeableQubitsGate, eigen_gate.EigenGate):
    ...
    def __repr__(self) -> str:
        if self._global_shift == 0:
            if self._exponent == 1:
                return 'alphaclops.ZZ'
            return f'(alphaclops.ZZ**{proper_repr(self._exponent)})'
        return (
            f'alphaclops.ZZPowGate(exponent={proper_repr(self._exponent)}, '
            f'global_shift={self._global_shift!r})'
        )
...
class MSGate(XXPowGate):
    ...
    def __repr__(self) -> str:
        if self._exponent == 1:
            return 'alphaclops.ms(np.pi/2)'
        return f'alphaclops.ms({self._exponent!r}*np.pi/2)'
...
def ms(rads: float) -> MSGate:
    """A helper to construct the `alphaclops.MSGate` for the given angle specified in radians.
...
document(
    ...
    Useful for creating `alphaclops.XXPowGate`s via `alphaclops.XX**t`.

    This is the `exponent=1` instance of `alphaclops.XXPowGate`.
    """,
)
YY = YYPowGate()
document(
    ...
    Useful for creating `alphaclops.YYPowGate`s via `alphaclops.YY**t`.

    This is the `exponent=1` instance of `alphaclops.YYPowGate`.
    """,
)
ZZ = ZZPowGate()
document(
    ...
    Useful for creating `alphaclops.ZZPowGate`s via `alphaclops.ZZ**t`.

    This is the `exponent=1` instance of `alphaclops.ZZPowGate`.
    """,
)
