...
from alphaclops.linalg import transformations
from alphaclops.ops import raw_types, common_gates, pauli_gates, identity
...
class AsymmetricDepolarizingChannel(raw_types.Gate):
    ...
    Note: prior to alphaclops v0.14, this class contained `num_qubits` as a property.
    This violates the contract of `alphaclops.Gate` so it was removed.  One can
    ...
    def __repr__(self) -> str:
        return 'alphaclops.asymmetric_depolarize(' + f"error_probabilities={self._error_probabilities})"
...
class DepolarizingChannel(raw_types.Gate):
    ...
    def __repr__(self) -> str:
        if self._n_qubits == 1:
            return f"alphaclops.depolarize(p={self._p})"
        return f"alphaclops.depolarize(p={self._p},n_qubits={self._n_qubits})"
...
class GeneralizedAmplitudeDampingChannel(raw_types.Gate):
    ...
    def __repr__(self) -> str:
        return f'alphaclops.generalized_amplitude_damp(p={self._p!r},gamma={self._gamma!r})'
...
class AmplitudeDampingChannel(raw_types.Gate):
    ...
    def __repr__(self) -> str:
        return f'alphaclops.amplitude_damp(gamma={self._gamma!r})'
...
class ResetChannel(raw_types.Gate):
    ...
    def _act_on_(self, sim_state: 'alphaclops.SimulationStateBase', qubits: Sequence['alphaclops.Qid']):
        if len(qubits) != 1:
    ...
    def __repr__(self) -> str:
        if self._dimension == 2:
            return 'alphaclops.ResetChannel()'
        else:
            return f'alphaclops.ResetChannel(dimension={self._dimension!r})'
...
def reset(qubit: 'alphaclops.Qid') -> raw_types.Operation:
    """Returns a `alphaclops.ResetChannel` on the given qubit.

    This can also be used with the alias `alphaclops.R`.
    """
...
def reset_each(*qubits: 'alphaclops.Qid') -> List[raw_types.Operation]:
    """Returns a list of `alphaclops.ResetChannel` instances on the given qubits."""
...
class PhaseDampingChannel(raw_types.Gate):
    ...
    def __repr__(self) -> str:
        return f'alphaclops.phase_damp(gamma={self._gamma!r})'
...
class PhaseFlipChannel(raw_types.Gate):
    ...
    def __repr__(self) -> str:
        return f'alphaclops.phase_flip(p={self._p!r})'
...
def _phase_flip_Z() -> common_gates.ZPowGate:
    """Returns a alphaclops.Z which corresponds to a guaranteed phase flip."""
...
class BitFlipChannel(raw_types.Gate):
    ...
    def __repr__(self) -> str:
        return f'alphaclops.bit_flip(p={self._p!r})'
