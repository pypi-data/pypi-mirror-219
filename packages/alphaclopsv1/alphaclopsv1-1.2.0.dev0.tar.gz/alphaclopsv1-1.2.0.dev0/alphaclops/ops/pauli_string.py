...
import alphaclops
from alphaclops import value, protocols, linalg, qis
from alphaclops._doc import document
from alphaclops._import import LazyLoader
from alphaclops.ops import (
    clifford_gate,
    common_gates,
    gate_operation,
    global_phase_op,
...
linear_combinations = LazyLoader("linear_combinations", globals(), "alphaclops.ops.linear_combinations")
...
# A value that can be unambiguously converted into a `alphaclops.PauliString`.

PAULI_STRING_LIKE = Union[
    complex,
    'alphaclops.OP_TREE',
    Mapping[TKey, 'alphaclops.PAULI_GATE_LIKE'],
    Iterable,  # of PAULI_STRING_LIKE, but mypy doesn't do recursive types yet.
]
document(
    PAULI_STRING_LIKE,
    """A `alphaclops.PauliString` or a value that can easily be converted into one.
    ...
    Each Pauli operation can be specified as a alphaclops object (e.g. `alphaclops.X`) or as
...
PAULI_GATE_LIKE = Union['alphaclops.Pauli', 'alphaclops.IdentityGate', str, int,]
document(
    PAULI_GATE_LIKE,
    ...
    1. alphaclops gates: `alphaclops.I`, `alphaclops.X`, `alphaclops.Y`, `alphaclops.Z`.
...
class PauliString(raw_types.Operation, Generic[TKey]):
    ...
    `alphaclops.PauliString` represents a multi-qubit pauli operator, i.e.
    ...
    - X(0) * Y(1) * Z(2): Represents a pauli string which is a tensor product of
                          `alphaclops.X(q0)`, `alphaclops.Y(q1)` and `alphaclops.Z(q2)`.
    ...
    >>> print(alphaclops.X(a) * alphaclops.Y(b)) # Tensor product of Pauli's acting on different qubits.
    X(q(0))*Y(q(1))
    >>> print(alphaclops.X(a) * alphaclops.Y(a)) # Composition is reduced to an equivalent PauliString.
    1j*Z(q(0))
    >>> print(alphaclops.X(a) * alphaclops.I(b)) # Identity operations are dropped by default.
    X(q(0))
    >>> print(alphaclops.PauliString()) # String representation of an "empty" PaulString is "I".
    I

    `alphaclops.PauliString` is often used to represent:
    - Pauli operators: Can be inserted into circuits as multi qubit operations.
    - Pauli observables: Can be measured using either `alphaclops.measure_single_paulistring`/
                        `alphaclops.measure_paulistring_terms`; or using the observable
                        measurement framework in `alphaclops.measure_observables`.

    PauliStrings can be constructed via various different ways, some examples are
    given as follows:

    >>> a, b, c = alphaclops.LineQubit.range(3)
    >>> print(alphaclops.PauliString([alphaclops.X(a), alphaclops.X(a)]))
    I
    >>> print(alphaclops.PauliString(-1, alphaclops.X(a), alphaclops.Y(b), alphaclops.Z(c)))
    -X(q(0))*Y(q(1))*Z(q(2))
    >>> print(-1 * alphaclops.X(a) * alphaclops.Y(b) * alphaclops.Z(c))
    -X(q(0))*Y(q(1))*Z(q(2))
    >>> print(alphaclops.PauliString({a: alphaclops.X}, [-2, 3, alphaclops.Y(a)]))
    -6j*Z(q(0))
    >>> print(alphaclops.PauliString({a: alphaclops.I, b: alphaclops.X}))
    X(q(1))
    >>> print(alphaclops.PauliString({a: alphaclops.Y}, qubit_pauli_map={a: alphaclops.X}))
    1j*Z(q(0))

    Note that `alphaclops.PauliString`s are immutable objects. If you need a mutable version
    of pauli strings, see `alphaclops.MutablePauliString`.
    """

    def __init__(
        self,
        *contents: 'alphaclops.PAULI_STRING_LIKE',
        qubit_pauli_map: Optional[Dict[TKey, 'alphaclops.Pauli']] = None,
        coefficient: 'alphaclops.TParamValComplex' = 1,
    ):
        ...
        self._qubit_pauli_map: Dict[TKey, 'alphaclops.Pauli'] = qubit_pauli_map or {}
        self._coefficient: Union['alphaclops.TParamValComplex', sympy.Expr] = (
    ...
    def equal_up_to_coefficient(self, other: 'alphaclops.PauliString') -> bool:
    ...
    def get(
        ...
        """Returns the `alphaclops.Pauli` operation acting on qubit `key` or `default` if none exists."""
        return self._qubit_pauli_map.get(key, default)

    @overload
    def __mul__(
        self, other: 'alphaclops.PauliString[TKeyOther]'
    ) -> 'alphaclops.PauliString[Union[TKey, TKeyOther]]':
        pass

    @overload
    def __mul__(
        self, other: Mapping[TKeyOther, 'alphaclops.PAULI_GATE_LIKE']
    ) -> 'alphaclops.PauliString[Union[TKey, TKeyOther]]':
        pass

    @overload
    def __mul__(
        self, other: Iterable['alphaclops.PAULI_STRING_LIKE']
    ) -> 'alphaclops.PauliString[Union[TKey, alphaclops.Qid]]':
        pass

    @overload
    def __mul__(self, other: 'alphaclops.Operation') -> 'alphaclops.PauliString[Union[TKey, alphaclops.Qid]]':
        pass

    @overload
    def __mul__(
        self, other: Union[complex, int, float, numbers.Number]
    ) -> 'alphaclops.PauliString[TKey]':
        pass
    ...
    @property
    def gate(self) -> 'alphaclops.DensePauliString':
        """Returns a `alphaclops.DensePauliString`"""
        ...
        from alphaclops.ops.dense_pauli_string import DensePauliString
    ...
    def _circuit_diagram_info_(self, args: 'alphaclops.CircuitDiagramInfoArgs') -> List[str]:
    ...
    def with_qubits(self, *new_qubits: 'alphaclops.Qid') -> 'PauliString':
    ...
    def with_coefficient(self, new_coefficient: 'alphaclops.TParamValComplex') -> 'PauliString':
    ...
    def values(self) -> ValuesView[pauli_gates.Pauli]:
        """Ordered sequence of `alphaclops.Pauli` gates acting on `self.keys()`."""
        return self._qubit_pauli_map.values()

    def items(self) -> ItemsView[TKey, pauli_gates.Pauli]:
        """Returns (alphaclops.Qid, alphaclops.Pauli) pairs representing 1-qubit operations of pauli string."""
        return self._qubit_pauli_map.items()

    def frozen(self) -> 'alphaclops.PauliString':
        """Returns a `alphaclops.PauliString` with the same contents."""
        return self

    def mutable_copy(self) -> 'alphaclops.MutablePauliString':
        """Returns a new `alphaclops.MutablePauliString` with the same contents."""
    ...
    def __repr__(self) -> str:
        ...
        if not ordered_qubits:
            factors.append('alphaclops.PauliString()')
    ...
    def matrix(self, qubits: Optional[Iterable[TKey]] = None) -> np.ndarray:
        ...
        if alphaclops.is_parameterized(self):
    ...
    def expectation_from_state_vector(
        ...
        *,
        ...
        $|0\rangle |+\rangle$ and `q0, q1 = alphaclops.LineQubit.range(2)` then:

            alphaclops.X(q0).expectation(state, qubit_map={q0: 0, q1: 1}) = 0
            alphaclops.X(q0).expectation(state, qubit_map={q0: 1, q1: 0}) = 1
    ...
    def zip_items(
        self, other: 'alphaclops.PauliString[TKey]'
        ...
        For every qubit that has a `alphaclops.Pauli` operation acting on it in both `self` and `other`,
        the method yields a tuple corresponding to `(qubit, (pauli_in_self, pauli_in_other))`.

        Args:
            other: The other `alphaclops.PauliString` to zip pauli operations with.

        Returns:
            A sequence of `(qubit, (pauli_in_self, pauli_in_other))` tuples for every `qubit`
            that has a `alphaclops.Pauli` operation acting on it in both `self` and `other.
        """
    ...
    def zip_paulis(
        self, other: 'alphaclops.PauliString'
    ...
    def __pow__(self, power):
        ...
        if isinstance(power, (int, float)):
            ...
            from alphaclops.ops import pauli_string_phasor
    ...
    def map_qubits(self, qubit_map: Dict[TKey, TKeyNew]) -> 'alphaclops.PauliString[TKeyNew]':
    ...
    def dense(self, qubits: Sequence[TKey]) -> 'alphaclops.DensePauliString':
        """Returns a `alphaclops.DensePauliString` version of this Pauli string.
        ...
        Returns:
            A `alphaclops.DensePauliString` instance `D` such that `D.on(*qubits)`
            equals the receiving `alphaclops.PauliString` instance `P`.
    ...
    def conjugated_by(self, clifford: 'alphaclops.OP_TREE') -> 'PauliString':
        ...
        Examples:
            >>> a, b = alphaclops.LineQubit.range(2)
            >>> print(alphaclops.X(a).conjugated_by(alphaclops.CZ(a, b)))
            X(q(0))*Z(q(1))
            >>> print(alphaclops.X(a).conjugated_by(alphaclops.S(a)))
            -Y(q(0))
            >>> print(alphaclops.X(a).conjugated_by([alphaclops.H(a), alphaclops.CNOT(a, b)]))
            Z(q(0))*X(q(1))
    ...
    def after(self, ops: 'alphaclops.OP_TREE') -> 'alphaclops.PauliString':
    ...
    def before(self, ops: 'alphaclops.OP_TREE') -> 'alphaclops.PauliString':
    ...
    def pass_operations_over(
        self, ops: Iterable['alphaclops.Operation'], after_to_before: bool = False
    ) -> 'PauliString':
...
class SingleQubitPauliStringGateOperation(  # type: ignore
    ...
    Satisfies the contract of both `alphaclops.GateOperation` and `alphaclops.PauliString`. Relies
    implicitly on the fact that PauliString({q: X}) compares as equal to
    GateOperation(X, [q]).
    """

    def __init__(self, pauli: pauli_gates.Pauli, qubit: 'alphaclops.Qid'):
    ...
    def with_qubits(self, *new_qubits: 'alphaclops.Qid') -> 'SingleQubitPauliStringGateOperation':
        if len(new_qubits) != 1:
    ...
    def _from_json_dict_(  # type: ignore
        cls, pauli: pauli_gates.Pauli, qubit: 'alphaclops.Qid', **kwargs
    ):
...
class MutablePauliString(Generic[TKey]):
    """Mutable version of `alphaclops.PauliString`, used mainly for efficiently mutating pauli strings.

    `alphaclops.MutablePauliString` is a mutable version of `alphaclops.PauliString`, which is often
    useful for mutating pauli strings efficiently instead of always creating a copy. Note
    that, unlike `alphaclops.PauliString`, `MutablePauliString` is not a `alphaclops.Operation`.

    It exists mainly to help mutate pauli strings efficiently and then convert back to a
    frozen `alphaclops.PauliString` representation, which can then be used as operators or
    observables.
    """
    ...
    def __init__(
        ...
        *contents: 'alphaclops.PAULI_STRING_LIKE',
        ...
        """
        self.coefficient: Union[sympy.Expr, 'alphaclops.TParamValComplex'] = (
            coefficient if isinstance(coefficient, sympy.Expr) else complex(coefficient)
        )
    ...
    def values(self) -> Iterator['alphaclops.Pauli']:
        """Ordered sequence of `alphaclops.Pauli` gates acting on `self.keys()`."""
    ...
    def frozen(self) -> 'alphaclops.PauliString':
        ...
        For example, this is useful because `alphaclops.PauliString` is an operation
        whereas `alphaclops.MutablePauliString` is not.
        """
        return PauliString(
            coefficient=self.coefficient,
    ...
    def items(self) -> Iterator[Tuple[TKey, 'alphaclops.Pauli']]:
        """Returns (alphaclops.Qid, alphaclops.Pauli) pairs representing 1-qubit operations of pauli string."""
    ...
    def __getitem__(self, item: Any) -> 'alphaclops.Pauli':
        return _INT_TO_PAULI[self.pauli_int_dict[item] - 1]

    def __setitem__(self, key: TKey, value: 'alphaclops.PAULI_GATE_LIKE'):
    ...
    # pylint: disable=function-redefined
    @overload
    def get(self, key: TKey, default: None = None) -> Union['alphaclops.Pauli', None]:
        pass

    @overload
    def get(self, key: TKey, default: TDefault) -> Union['alphaclops.Pauli', TDefault]:
        pass

    def get(self, key: TKey, default=None) -> Union['alphaclops.Pauli', TDefault, None]:
        """Returns the `alphaclops.Pauli` operation acting on qubit `key` or `default` if none exists."""
        result = self.pauli_int_dict.get(key, None)
        return default if result is None else _INT_TO_PAULI[result - 1]

    # pylint: enable=function-redefined
    def inplace_before(self, ops: 'alphaclops.OP_TREE') -> 'alphaclops.MutablePauliString':
    ...
    def inplace_after(self, ops: 'alphaclops.OP_TREE') -> 'alphaclops.MutablePauliString':
    ...
    def _imul_helper(self, other: 'alphaclops.PAULI_STRING_LIKE', sign: int):
    ...
    def _imul_helper_checkpoint(self, other: 'alphaclops.PAULI_STRING_LIKE', sign: int):
    ...
    def inplace_left_multiply_by(
        self, other: 'alphaclops.PAULI_STRING_LIKE'
    ) -> 'alphaclops.MutablePauliString':
        """Left-multiplies a pauli string into this pauli string.

        Args:
            other: A pauli string or `alphaclops.PAULI_STRING_LIKE` to left-multiply
                into `self`.
        ...
        Raises:
            TypeError: `other` was not a `alphaclops.PAULI_STRING_LIKE`. `self`
                was not mutated.
        """
        if self._imul_helper_checkpoint(other, -1) is NotImplemented:
            raise TypeError(f"{other!r} is not alphaclops.PAULI_STRING_LIKE.")
        return self
    ...
    def inplace_right_multiply_by(
        ...
        Args:
            other: A pauli string or `alphaclops.PAULI_STRING_LIKE` to right-multiply
                into `self`.
    ...
    def __neg__(self) -> 'alphaclops.MutablePauliString':
        result = self.mutable_copy()
        result.coefficient *= -1
        return result

    def __pos__(self) -> 'alphaclops.MutablePauliString':
        return self.mutable_copy()

    def transform_qubits(
        self, func: Callable[[TKey], TKeyNew], *, inplace: bool = False
    ) -> 'alphaclops.MutablePauliString[TKeyNew]':
        ...
        Args:
            ...
            inplace: If false (the default), creates a new mutable pauli string
                ...
                `alphaclops.PauliString.transform_qubits` in situations where the
        ...
        result = cast('alphaclops.MutablePauliString[TKeyNew]', self)
        result.pauli_int_dict = new_dict
        return result

    def __imul__(self, other: 'alphaclops.PAULI_STRING_LIKE') -> 'alphaclops.MutablePauliString':
        """Left-multiplies a pauli string into this pauli string.
        ...
        Returns:
            ...
            If `other` is not a `alphaclops.PAULI_STRING_LIKE`, `self` is not mutated
            and `NotImplemented` is returned.

        """
        return self._imul_helper_checkpoint(other, +1)

    def __mul__(self, other: 'alphaclops.PAULI_STRING_LIKE') -> 'alphaclops.PauliString':
    ...
    def __rmul__(self, other: 'alphaclops.PAULI_STRING_LIKE') -> 'alphaclops.PauliString':
        """Multiplies two pauli-string-likes together.
...
def _decompose_into_cliffords(op: 'alphaclops.Operation') -> List['alphaclops.Operation']:
...
def _pass_operation_over(
    pauli_map: Dict[TKey, pauli_gates.Pauli], op: 'alphaclops.Operation', after_to_before: bool = False
) -> bool:
...
PAULI_GATE_LIKE_TO_INDEX_MAP: Dict['alphaclops.PAULI_GATE_LIKE', int] = {
    _i: 0,
    _x: 1,
    _y: 2,
    _z: 3,
...
_INT_TO_PAULI_OR_IDENTITY: List[Union['alphaclops.Pauli', 'alphaclops.IdentityGate']] = [_i, _x, _y, _z]
_INT_TO_PAULI: List['alphaclops.Pauli'] = [_x, _y, _z]


PAULI_GATE_LIKE_TO_GATE_MAP: Dict[
    'alphaclops.PAULI_GATE_LIKE', Union['alphaclops.Pauli', 'alphaclops.IdentityGate']
...
def _pauli_like_to_pauli_int(key: Any, pauli_gate_like: PAULI_GATE_LIKE):
    ...
    if pauli_int is None:
        raise TypeError(
            ...
            f'alphaclops.PAULI_GATE_LIKE value. '
