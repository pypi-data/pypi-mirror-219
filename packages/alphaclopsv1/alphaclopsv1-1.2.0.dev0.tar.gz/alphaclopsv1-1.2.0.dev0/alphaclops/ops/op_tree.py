...
from alphaclops.ops.raw_types import Operation

if TYPE_CHECKING:
    import alphaclops

moment = LazyLoader("moment", globals(), "alphaclops.circuits.moment")
...
document(
    ...
    `alphaclops.OP_TREE` argument:

    - A single operation (a `alphaclops.Operation`).
    - A list of operations (a `List[alphaclops.Operation]`).
    - A list of lists of operations (a `List[List[alphaclops.Operation]]`).
    - A list mixing operations and generators of operations
        (a `List[Union[alphaclops.Operation, Iterator[alphaclops.Operation]]]`).
...
) -> Iterator[Union[Operation, 'alphaclops.Moment']]:
...
def flatten_to_ops_or_moments(root: OP_TREE) -> Iterator[Union[Operation, 'alphaclops.Moment']]:
