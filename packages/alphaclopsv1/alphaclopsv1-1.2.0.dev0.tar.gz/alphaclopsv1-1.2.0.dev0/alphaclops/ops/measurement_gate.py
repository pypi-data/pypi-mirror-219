...
from alphaclops import _compat, protocols, value
...
class MeasurementGate(raw_types.Gate):
    ...
    Instead of constructing this directly, consider using the `alphaclops.measure`
    helper method.
    """
    ...
    def with_key(self, key: Union[str, 'alphaclops.MeasurementKey']) -> 'MeasurementGate':
    ...
    def _op_repr_(self, qubits: Sequence['alphaclops.Qid']) -> str:
        ...
        arg_list = ', '.join(args)
        return f'alphaclops.measure({arg_list})'
    ...
    def __repr__(self):
        ...
        if self.confusion_map:
            ...
            args.append(f'confusion_map={{{proper_map_str}}}')
        return f'alphaclops.MeasurementGate({", ".join(args)})'
    ...
    def _act_on_(self, sim_state: 'alphaclops.SimulationStateBase', qubits: Sequence['alphaclops.Qid']) -> bool:
        from alphaclops.sim import SimulationState
