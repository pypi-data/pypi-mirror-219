...
from alphaclops import TwoAltoptives, alternative


def test_regular_abstract():
    class RegularAbc(metaclass=TwoAltoptives):
        @abc.abstractmethod
...
def test_single_alternative():
    class SingleAlternative(metaclass=TwoAltoptives):
...
def test_bad_alternative():
    with pytest.raises(TypeError, match='not exist'):
        ...
        class _(metaclass=TwoAltoptives):
...
def test_two_alternatives():
    class TwoAlternatives(metaclass=TwoAltoptives):
...
def test_implement_any_one():
    ...
    class AnyOneAbc(metaclass=TwoAltoptives):
