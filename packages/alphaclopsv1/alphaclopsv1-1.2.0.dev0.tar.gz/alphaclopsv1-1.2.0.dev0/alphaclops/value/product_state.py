...
class _NamedOneQubitState(metaclass=abc.ABCMeta):
    ...
    def on(self, qubit: 'alphaclops.Qid') -> 'ProductState':
...
class ProductState:
    ...
    For example, the |00⟩ state is `alphaclops.KET_ZERO(q0) * alphaclops.KET_ZERO(q1)`.
    The |+⟩ state is a length-1 tensor product state and can be constructed
    with `alphaclops.KET_PLUS(q0)`.
    """

    states: Dict['alphaclops.Qid', _NamedOneQubitState]
    ...
    @property
    def qubits(self) -> Sequence['alphaclops.Qid']:
        return sorted(self.states.keys())

    def __mul__(self, other: 'alphaclops.ProductState') -> 'alphaclops.ProductState':
    ...
    def __repr__(self) -> str:
        ...
        return 'alphaclops.ProductState({%s})' % states_dict_repr

    def __getitem__(self, qubit: 'alphaclops.Qid') -> _NamedOneQubitState:
        """Return the _NamedOneQubitState at the given qubit."""
        return self.states[qubit]

    def __iter__(self) -> Iterator[Tuple['alphaclops.Qid', _NamedOneQubitState]]:
    ...
    def state_vector(self, qubit_order: Optional['alphaclops.QubitOrder'] = None) -> np.ndarray:
    ...
    def projector(self, qubit_order: Optional['alphaclops.QubitOrder'] = None) -> np.ndarray:
...
class _PauliEigenState(_NamedOneQubitState):
    ...
    def __repr__(self) -> str:
        return f'alphaclops.{self._symbol}.basis[{self.eigenvalue:+d}]'

    @abc.abstractmethod
    def stabilized_by(self) -> Tuple[int, 'alphaclops.Pauli']:
        pass
