...
DURATION_LIKE = Union[None, datetime.timedelta, 'alphaclops.Duration']
document(
    DURATION_LIKE,
    """A `alphaclops.Duration` or value that can trivially converted to one.

    A `datetime.timedelta` is a `alphaclops.DURATION_LIKE`. It is converted while
    preserving its duration.

    `None` is a `alphaclops.DURATION_LIKE` that converts into a zero-length duration.
...
class Duration:
    ...
    def __init__(
        ...
        *,  # Force keyword args.
        ...
        Raises:
            TypeError: If the given value is not of a `alphaclops.DURATION_LIKE` type.

        Examples:
            >>> print(alphaclops.Duration(nanos=100))
            100 ns
            >>> print(alphaclops.Duration(micros=1.5 * sympy.Symbol('t')))
            (1500.0*t) ns
        """
        if value is not None and value != 0:
            if isinstance(value, datetime.timedelta):
                ...
                raise TypeError(f'Not a `alphaclops.DURATION_LIKE`: {repr(value)}.')
    ...
    def _resolve_parameters_(self, resolver: 'alphaclops.ParamResolver', recursive: bool) -> 'Duration':
    ...
    def __repr__(self) -> str:
        ...
        return f'alphaclops.Duration({unit}={proper_repr(amount)})'
