...
def test_single_qubit_readout_result_repr():
    result = alphaclops.experiments.SingleQubitReadoutCalibrationResult(
        zero_state_errors={alphaclops.LineQubit(0): 0.1},
        one_state_errors={alphaclops.LineQubit(0): 0.2},
        repetitions=1000,
        timestamp=0.3,
    )
...
class NoisySingleQubitReadoutSampler(alphaclops.Sampler):
    def __init__(self, p0: float, p1: float, seed: 'alphaclops.RANDOM_STATE_OR_SEED_LIKE' = None):
        ...
        self.p0 = p0
        self.p1 = p1
        self.prng = alphaclops.value.parse_random_state(seed)
        self.simulator = alphaclops.Simulator(seed=self.prng, split_untangled_states=False)
...
def test_estimate_single_qubit_readout_errors_no_noise():
    ...
    repetitions = 1000
    result = alphaclops.estimate_single_qubit_readout_errors(
...
def test_estimate_parallel_readout_errors_no_noise():
    ...
    sampler = alphaclops.Simulator()
    repetitions = 1000
    result = alphaclops.estimate_parallel_single_qubit_readout_errors(
        sampler, qubits=qubits, repetitions=repetitions
    )
    assert result.zero_state_errors == {q: 0 for q in qubits}
    assert result.one_state_errors == {q: 0 for q in qubits}
...
def test_estimate_parallel_readout_errors_bad_bit_string():
    ...
    with pytest.raises(ValueError, match='but was None'):
        _ = alphaclops.estimate_parallel_single_qubit_readout_errors(
            alphaclops.ZerosSampler(),
            qubits=qubits,
            repetitions=1000,
            trials=35,
            trials_per_batch=10,
...
def test_estimate_parallel_readout_errors_zero_reps():
    ...
    with pytest.raises(ValueError, match='non-zero repetition'):
        _ = alphaclops.estimate_parallel_single_qubit_readout_errors(
            alphaclops.ZerosSampler(), qubits=qubits, repetitions=0, trials=35, trials_per_batch=10
        )
...
def test_estimate_parallel_readout_errors_zero_trials():
    ...
    with pytest.raises(ValueError, match='non-zero trials'):
        _ = alphaclops.estimate_parallel_single_qubit_readout_errors(
            alphaclops.ZerosSampler(), qubits=qubits, repetitions=1000, trials=0, trials_per_batch=10
        )
...
def test_estimate_parallel_readout_errors_zero_batch():
    ...
    with pytest.raises(ValueError, match='non-zero trials_per_batch'):
        _ = alphaclops.estimate_parallel_single_qubit_readout_errors(
            alphaclops.ZerosSampler(), qubits=qubits, repetitions=1000, trials=10, trials_per_batch=0
        )
