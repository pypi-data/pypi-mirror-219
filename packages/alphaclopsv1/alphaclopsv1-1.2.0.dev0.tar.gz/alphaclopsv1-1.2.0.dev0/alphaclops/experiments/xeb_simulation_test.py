...
def test_simulate_2q_xeb_circuits():
    ...
    circuits = [
        rqcg.random_rotations_between_two_qubit_circuit(
            q0, q1, depth=50, two_qubit_op_factory=lambda a, b, _: alphaclops.SQRT_ISWAP(a, b)
        )
        for _ in range(2)
    ]
...
def test_simulate_circuit_length_validation():
    ...
    circuits = [
        rqcg.random_rotations_between_two_qubit_circuit(
            ...
            two_qubit_op_factory=lambda a, b, _: alphaclops.SQRT_ISWAP(a, b),
        )
        for _ in range(2)
    ]
...
def _ref_simulate_2q_xeb_circuit(task: Dict[str, Any]):
    ...
    tcircuit = alphaclops.resolve_parameters_once(tcircuit, param_resolver=param_resolver)

    pure_sim = alphaclops.Simulator()
    psi = pure_sim.simulate(tcircuit)
    psi_vector = psi.final_state_vector
    pure_probs = alphaclops.state_vector_to_probabilities(psi_vector)
...
def test_incremental_simulate(multiprocess):
    ...
    circuits = [
        rqcg.random_rotations_between_two_qubit_circuit(
            q0, q1, depth=100, two_qubit_op_factory=lambda a, b, _: alphaclops.SQRT_ISWAP(a, b)
        )
        for _ in range(20)
    ]
