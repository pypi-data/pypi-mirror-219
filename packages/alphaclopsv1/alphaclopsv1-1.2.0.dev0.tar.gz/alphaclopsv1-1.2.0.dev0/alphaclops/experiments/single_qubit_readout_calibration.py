...
import sympy
import numpy as np
from alphaclops import circuits, ops, study
...
class SingleQubitReadoutCalibrationResult:
    ...
    zero_state_errors: Dict['alphaclops.Qid', float]
    one_state_errors: Dict['alphaclops.Qid', float]
    repetitions: int
    timestamp: float
    ...
    def __repr__(self) -> str:
        return (
            'alphaclops.experiments.SingleQubitReadoutCalibrationResult('
...
def estimate_single_qubit_readout_errors(
    sampler: 'alphaclops.Sampler', *, qubits: Iterable['alphaclops.Qid'], repetitions: int = 1000
...
def estimate_parallel_single_qubit_readout_errors(
    ...
    *,
    ...
    Args:
        sampler: The `alphaclops.Sampler` used to run the circuits.
        qubits: The qubits being tested.
