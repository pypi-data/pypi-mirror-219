...
from alphaclops import circuits, ops, study, value
...
def t2_decay(
    ...
    *,
    ...
    num_points: int,
    max_delay: 'alphaclops.DURATION_LIKE',
    min_delay: 'alphaclops.DURATION_LIKE' = None,
    repetitions: int = 1000,
    delay_sweep: Optional[study.Sweep] = None,
    num_pulses: Optional[List[int]] = None,
) -> Union['alphaclops.experiments.T2DecayResult', List['alphaclops.experiments.T2DecayResult']]:
    ...
    prepared with a square root Y gate (`alphaclops.Y ** 0.5`) and then waits for
    ...
    by performing either a `alphaclops.Y ** -0.5` or `alphaclops.X ** 0.5`.  The square of
    ...
    will also be prepared with a square root Y gate (`alphaclops.Y ** 0.5`).
    However, during the mid-point of the delay time being measured, a pi-pulse
    (`alphaclops.X`) gate will be applied to cancel out inhomogeneous dephasing.
...
def _cpmg_circuit(qubit: 'alphaclops.Qid', delay_var: sympy.Symbol, max_pulses: int) -> 'alphaclops.Circuit':
...
class T2DecayResult:
    ...
    def __repr__(self):
        return (
            f'alphaclops.experiments.T2DecayResult('
