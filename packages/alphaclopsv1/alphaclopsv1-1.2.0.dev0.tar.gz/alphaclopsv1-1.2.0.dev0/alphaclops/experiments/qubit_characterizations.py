...
def two_qubit_randomized_benchmarking(
    sampler: 'alphaclops.Sampler',
    first_qubit: 'alphaclops.Qid',
    second_qubit: 'alphaclops.Qid',
    *,
...
def _two_qubit_clifford_matrices(
    q_0: 'alphaclops.Qid', q_1: 'alphaclops.Qid', cliffords: Cliffords
) -> np.ndarray:
    mats = []
...
def _random_single_q_clifford(
    qubit: 'alphaclops.Qid',
    num_cfds: int,
    cfds: Sequence[Sequence['alphaclops.Gate']],
...
def _random_two_q_clifford(
    q_0: 'alphaclops.Qid', q_1: 'alphaclops.Qid', num_cfds: int, cfd_matrices: np.ndarray, cliffords: Cliffords
) -> 'alphaclops.Circuit':
    clifford_group_size = 11520
...
def _gate_seq_to_mats(gate_seq: Sequence['alphaclops.Gate']) -> np.ndarray:
...
def _two_qubit_clifford(
    q_0: 'alphaclops.Qid', q_1: 'alphaclops.Qid', idx: int, cliffords: Cliffords
) -> Iterator['alphaclops.OP_TREE']:
...
def _two_qubit_clifford_starters(
    q_0: 'alphaclops.Qid', q_1: 'alphaclops.Qid', idx_0: int, idx_1: int, cliffords: Cliffords
) -> Iterator['alphaclops.OP_TREE']:
...
def _two_qubit_clifford_mixers(
    q_0: 'alphaclops.Qid', q_1: 'alphaclops.Qid', idx_2: int, cliffords: Cliffords
) -> Iterator['alphaclops.OP_TREE']:
...
def _single_qubit_gates(
    gate_seq: Sequence['alphaclops.Gate'], qubit: 'alphaclops.Qid'
) -> Iterator['alphaclops.OP_TREE']:
...
def _single_qubit_cliffords() -> Cliffords:
    ...
    c1_in_xy: List[List['alphaclops.Gate']] = []
    c1_in_xz: List[List['alphaclops.Gate']] = []
    ...
    s1: List[List['alphaclops.Gate']] = [[X ** 0.0], [Y ** 0.5, X ** 0.5], [X ** -0.5, Y ** -0.5]]
    s1_x: List[List['alphaclops.Gate']] = [[X ** 0.5], [X ** 0.5, Y ** 0.5, X ** 0.5], [Y ** -0.5]]
    s1_y: List[List['alphaclops.Gate']] = [[Y ** 0.5], [X ** -0.5, Y ** -0.5, X ** 0.5], [Y, X ** 0.5]]
