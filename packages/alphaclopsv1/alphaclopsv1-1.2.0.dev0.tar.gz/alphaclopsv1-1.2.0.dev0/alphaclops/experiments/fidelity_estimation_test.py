...
def sample_noisy_bitstrings(
    circuit: alphaclops.Circuit, qubit_order: Sequence[alphaclops.Qid], depolarization: float, repetitions: int
) -> np.ndarray:
    ...
    circuit_with_measurements = alphaclops.Circuit(circuit, alphaclops.measure(*qubit_order, key='m'))
    r = alphaclops.sample(circuit_with_measurements, repetitions=n_coherent)
    coherent_samples = r.data['m'].to_numpy()
    return np.concatenate((coherent_samples, incoherent_samples))


def make_random_quantum_circuit(qubits: Sequence[alphaclops.Qid], depth: int) -> alphaclops.Circuit:
    SQ_GATES = [alphaclops.X ** 0.5, alphaclops.Y ** 0.5, alphaclops.T]
    circuit = alphaclops.Circuit()
    cz_start = 0
    for q in qubits:
        circuit.append(alphaclops.H(q))
...
@pytest.mark.parametrize(
    ...
    itertools.product(
        (0.0, 0.2, 0.7, 1.0),
        (
                alphaclops.hog_score_xeb_fidelity_from_probabilities,
                alphaclops.linear_xeb_fidelity_from_probabilities,
                alphaclops.log_xeb_fidelity_from_probabilities,
        ),
    ),
)
def test_xeb_fidelity(depolarization, estimator):
    ...
    for _ in range(10):
        ...
        f = alphaclops.xeb_fidelity(circuit, bitstrings, qubits, estimator=estimator)
        amplitudes = alphaclops.final_state_vector(circuit)
        f2 = alphaclops.xeb_fidelity(
...
def test_linear_and_log_xeb_fidelity():
    ...
    for _ in range(10):
        ...
        f_log = alphaclops.log_xeb_fidelity(circuit, bitstrings, qubits)
        f_lin = alphaclops.linear_xeb_fidelity(circuit, bitstrings, qubits)
...
def test_xeb_fidelity_invalid_qubits():
    ...
    with pytest.raises(ValueError):
        alphaclops.xeb_fidelity(circuit, bitstrings, (q0, q2))
...
def test_xeb_fidelity_invalid_bitstrings():
    ...
    with pytest.raises(ValueError):
        alphaclops.xeb_fidelity(circuit, bitstrings, (q0, q1))
...
def test_xeb_fidelity_tuple_input():
    ...
    circuit = alphaclops.Circuit(alphaclops.H(q0), alphaclops.CNOT(q0, q1))
    bitstrings = [0, 1, 2]
    f1 = alphaclops.xeb_fidelity(circuit, bitstrings, (q0, q1))
    f2 = alphaclops.xeb_fidelity(circuit, tuple(bitstrings), (q0, q1))
    assert f1 == f2
