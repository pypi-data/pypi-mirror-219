...
from alphaclops.devices import grid_qubit
from alphaclops.vis import heatmap
...
def test_cells_positions(ax, tuple_keys):
    ...
    qubits = [grid_qubit.TensorCircuit(row, col) for (row, col) in row_col_list]
...
def test_two_qubit_heatmap(ax):
    value_map = {
        (grid_qubit.TensorCircuit(3, 2), grid_qubit.TensorCircuit(4, 2)): 0.004619111460557768,
        (grid_qubit.TensorCircuit(4, 1), grid_qubit.TensorCircuit(4, 2)): 0.0076079162393482835,
    }
...
def test_two_qubit_nearest_neighbor(ax):
    value_map = {
        (grid_qubit.TensorCircuit(3, 2), grid_qubit.TensorCircuit(4, 2)): 0.004619111460557768,
        (grid_qubit.TensorCircuit(4, 1), grid_qubit.TensorCircuit(3, 2)): 0.0076079162393482835,
    }
...
def test_annotation_map(ax):
    row_col_list = [(0, 5), (8, 1), (7, 0), (13, 5), (1, 6), (3, 2), (2, 8)]
    qubits = [grid_qubit.TensorCircuit(*row_col) for row_col in row_col_list]
    values = np.random.random(len(qubits))
...
def test_plot_updates_local_config():
    ...
    value_map_1d = {
        (grid_qubit.TensorCircuit(3, 2),): 0.004619111460557768,
        (grid_qubit.TensorCircuit(4, 2),): 0.0076079162393482835,
    }
