...
from alphaclops import devices, circuits, ops, protocols
from alphaclops.interop.quirk.cells import (
    Cell,
    CellMaker,
    CellMakerArgs,
    CompositeCell,
...
from alphaclops.interop.quirk.cells.parse import parse_matrix
...
def quirk_url_to_circuit(
    ...
    *,
    ...
    extra_cell_makers: Union[
        Dict[str, 'alphaclops.Gate'], Iterable['alphaclops.interop.quirk.cells.CellMaker']
    ] = (),
    max_operation_count: int = 10**6,
) -> 'alphaclops.Circuit':
    """Parses a alphaclops circuit out of a Quirk URL.
    ...
    Args:
        ...
        qubits: Qubits to use in the circuit. The length of the list must be
            ...
            This argument defaults to `alphaclops.LineQubit.range(16)` when not
            specified.
        extra_cell_makers: Non-standard Quirk cells to accept. This can be
            ...
            as either a list of `alphaclops.interop.quirk.cells.CellMaker` instances,
            or for more simple cases as a dictionary from a Quirk id string
            to a alphaclops Gate.
    ...
    Examples:
        >>> print(alphaclops.quirk_url_to_circuit(
        ...
        >>> print(alphaclops.quirk_url_to_circuit(
        ...     'http://algassert.com/quirk#circuit={"cols":[["H"],["•","X"]]}',
        ...     qubits=[alphaclops.NamedQubit('Alice'), alphaclops.NamedQubit('Bob')]
        ... ))
        Alice: ───H───@───
                      │
        Bob: ─────────X───
        ...
        ...             identifier='iswap',
        ...             size=2,
        ...             maker=lambda args: alphaclops.ISWAP(*args.qubits))
        ...     ]))
        0: ───iSwap───
              │
        1: ───iSwap───
...
def quirk_json_to_circuit(
    ...
    *,
    ...
    """Constructs a alphaclops circuit from Quirk's JSON format.
    ...
    Examples:
        >>> print(alphaclops.quirk_json_to_circuit(
        ...     {"cols":[["H"], ["•", "X"]]}
        ... ))
        0: ───H───@───
                  │
    ...
    if qubits is not None:
        ...
        def map_qubit(qubit: 'alphaclops.Qid') -> 'alphaclops.Qid':
...
def _init_ops(data: Dict[str, Any]) -> 'alphaclops.OP_TREE':
    if 'init' not in data:
        return []
    init = data['init']
