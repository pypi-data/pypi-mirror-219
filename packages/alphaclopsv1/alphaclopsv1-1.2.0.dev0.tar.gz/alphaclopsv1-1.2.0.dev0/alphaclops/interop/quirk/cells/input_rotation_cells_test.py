...
def test_validation():
    with pytest.raises(ValueError, match='sign'):
        _ = alphaclops.interop.quirk.QuirkInputRotationOperation(
            identifier='xxx',
            register=alphaclops.LineQubit.range(4),
            base_operation=alphaclops.X.on(alphaclops.LineQubit(5)),
            exponent_sign=2,
        )
...
def test_input_rotation_with_qubits():
    ...
    x, y, z, t, w = alphaclops.LineQubit.range(10, 15)
    op = alphaclops.interop.quirk.QuirkInputRotationOperation(
        identifier='test',
        register=[a, b, c],
        base_operation=alphaclops.X(d).controlled_by(e),
        exponent_sign=-1,
    )
    assert op.qubits == (e, d, a, b, c)
    assert op.with_qubits(x, y, z, t, w) == (
        alphaclops.interop.quirk.QuirkInputRotationOperation(
            identifier='test',
            register=[z, t, w],
            base_operation=alphaclops.X(y).controlled_by(x),
            exponent_sign=-1,
        )
    )
...
def test_input_rotation_cell_with_qubits():
    a, b, c, d, e = alphaclops.LineQubit.range(5)
    x, y, z, t, w = alphaclops.LineQubit.range(10, 15)
    cell = alphaclops.interop.quirk.cells.input_rotation_cells.InputRotationCell(
        identifier='test',
        register=[a, b, c],
        base_operation=alphaclops.X(d).controlled_by(e),
        exponent_sign=-1,
    ...
    assert cell.with_line_qubits_mapped_to([x, y, z, t, w]) == (
        alphaclops.interop.quirk.cells.input_rotation_cells.InputRotationCell(
            identifier='test',
            register=[x, y, z],
            base_operation=alphaclops.X(t).controlled_by(w),
            exponent_sign=-1,
        )
    )
...
def test_input_rotation_cell_with_qubits_before_register_specified():
    d, e = alphaclops.LineQubit.range(3, 5)
    x, y, z, t, w = alphaclops.LineQubit.range(10, 15)
    cell = alphaclops.interop.quirk.cells.input_rotation_cells.InputRotationCell(
        identifier='test',
        register=None,
