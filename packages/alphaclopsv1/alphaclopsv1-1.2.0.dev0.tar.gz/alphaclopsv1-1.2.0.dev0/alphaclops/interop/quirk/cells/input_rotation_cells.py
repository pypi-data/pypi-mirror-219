...
import alphaclops
from alphaclops import ops, linalg, value
from alphaclops.interop.quirk.cells.cell import Cell, CellMaker
...
class InputRotationCell(Cell):
    ...
    def __init__(
        self,
        identifier: str,
        register: Optional[Sequence['alphaclops.Qid']],
        base_operation: 'alphaclops.Operation',
        exponent_sign: int,
    ):
    ...
    def __repr__(self) -> str:
        return (
            f'alphaclops.interop.quirk.cells.input_rotation_cells.InputRotationCell('
            f'\n    {self.identifier!r},'
            f'\n    {self.register!r},'
    ...
    def controlled_by(self, qubit: 'alphaclops.Qid'):
        return InputRotationCell(
            self.identifier,
            self.register,
...
class QuirkInputRotationOperation(ops.Operation):
    ...
    def __init__(
        self,
        identifier: str,
        register: Iterable['alphaclops.Qid'],
        base_operation: 'alphaclops.Operation',
        exponent_sign: int,
    ):
    ...
    def _circuit_diagram_info_(self, args: 'alphaclops.CircuitDiagramInfoArgs'):
        sub_result = alphaclops.circuit_diagram_info(self.base_operation)
    ...
    def _apply_unitary_(self, args: 'alphaclops.ApplyUnitaryArgs'):
        ...
        for i in range(control_max):
            assert isinstance(self.base_operation, alphaclops.GateOperation)
            ...
            sub_args = alphaclops.ApplyUnitaryArgs(
                transposed_args.target_tensor[control_index],
                transposed_args.available_buffer[control_index],
                target_axes,
            )
            sub_result = alphaclops.apply_unitary(operation, sub_args)
    ...
    def __repr__(self) -> str:
        return (
            f'alphaclops.interop.quirk.QuirkInputRotationOperation('
...
def _input_rotation_gate(identifier: str, gate: 'alphaclops.Gate', exponent_sign: int) -> CellMaker:
    return CellMaker(
        identifier,
        gate.num_qubits(),
