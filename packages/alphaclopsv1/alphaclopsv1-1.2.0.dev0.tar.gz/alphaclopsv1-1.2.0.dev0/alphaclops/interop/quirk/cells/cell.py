...
from alphaclops import ops, value, devices
...
class Cell(metaclass=abc.ABCMeta):
    ...
    @classmethod
    def _replace_qubit(cls, old_qubit: 'alphaclops.Qid', qubits: List['alphaclops.Qid']) -> 'alphaclops.Qid':
    ...
    @classmethod
    def _replace_qubits(
        cls, old_qubits: Iterable['alphaclops.Qid'], qubits: List['alphaclops.Qid']
    ) -> Tuple['alphaclops.Qid', ...]:
        return tuple(Cell._replace_qubit(e, qubits) for e in old_qubits)

    @abc.abstractmethod
    def with_line_qubits_mapped_to(self, qubits: List['alphaclops.Qid']) -> 'Cell':
    ...
    def with_input(self, letter: str, register: Union[Sequence['alphaclops.Qid'], int]) -> 'Cell':
    ...
    def controlled_by(self, qubit: 'alphaclops.Qid') -> 'Cell':
    ...
    def operations(self) -> 'alphaclops.OP_TREE':
        """Returns operations that implement the cell's main action.

        Returns:
            A `alphaclops.OP_TREE` of operations implementing the cell.
    ...
    def basis_change(self) -> 'alphaclops.OP_TREE':
        ...
        Returns:
            A `alphaclops.OP_TREE` of basis change operations.
        """
        return ()
...
class ExplicitOperationsCell(Cell):
    ...
    def controlled_by(self, qubit: 'alphaclops.Qid') -> 'ExplicitOperationsCell':
        return ExplicitOperationsCell(
...
CellMakerArgs = NamedTuple(
    'CellMakerArgs', [('qubits', Sequence['alphaclops.Qid']), ('value', Any), ('row', int), ('col', int)]
)
...
CellMaker = NamedTuple(
    ...
    [
        ('identifier', str),
        ('size', int),
        ('maker', Callable[[CellMakerArgs], Union[None, 'Cell', 'alphaclops.Operation']]),
    ],
)
CellMaker.__doc__ = """Turns Quirk identifiers into alphaclops operations.
...
Attributes:
    ...
    maker: A function that takes a `alphaclops.interop.quirk.cells.CellMakerArgs` and
        returns either a `alphaclops.Operation` or a `alphaclops.interop.quirk.cells.Cell`.
