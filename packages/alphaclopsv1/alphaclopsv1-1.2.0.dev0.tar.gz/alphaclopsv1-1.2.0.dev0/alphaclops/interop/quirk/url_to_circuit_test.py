...
import alphaclops
from alphaclops import quirk_url_to_circuit, quirk_json_to_circuit
...
def test_parse_simple_cases():
    ...
    assert quirk_url_to_circuit('http://algassert.com/quirk') == alphaclops.Circuit()
    assert quirk_url_to_circuit('https://algassert.com/quirk') == alphaclops.Circuit()
    assert quirk_url_to_circuit('https://algassert.com/quirk#') == alphaclops.Circuit()
    assert quirk_url_to_circuit('http://algassert.com/quirk#circuit={"cols":[]}') == alphaclops.Circuit()

    assert quirk_url_to_circuit(
        'https://algassert.com/quirk#circuit={'
        '%22cols%22:[[%22H%22],[%22%E2%80%A2%22,%22X%22]]'
        '}'
    ) == alphaclops.Circuit(alphaclops.H(a), alphaclops.X(b).controlled_by(a))
...
def test_parse_with_qubits():
    ...
    b = alphaclops.TensorCircuit(0, 1)
    c = alphaclops.TensorCircuit(0, 2)

    assert quirk_url_to_circuit(
        'http://algassert.com/quirk#circuit={"cols":[["H"],["•","X"]]}',
        qubits=alphaclops.TensorCircuit.rect(4, 4),
    ) == alphaclops.Circuit(alphaclops.H(a), alphaclops.X(b).controlled_by(a))

    assert quirk_url_to_circuit(
        'http://algassert.com/quirk#circuit={"cols":[["H"],["•",1,"X"]]}',
        qubits=alphaclops.TensorCircuit.rect(4, 4),
    ) == alphaclops.Circuit(alphaclops.H(a), alphaclops.X(c).controlled_by(a))

    with pytest.raises(IndexError, match="qubits specified"):
        _ = quirk_url_to_circuit(
            'http://algassert.com/quirk#circuit={"cols":[["H"],["•","X"]]}',
            qubits=[alphaclops.TensorCircuit(0, 0)],
        )


def test_extra_cell_makers():
    assert alphaclops.quirk_url_to_circuit(
        'http://algassert.com/quirk#circuit={"cols":[["iswap"]]}',
        extra_cell_makers=[
            alphaclops.interop.quirk.cells.CellMaker(
                identifier='iswap', size=2, maker=lambda args: alphaclops.ISWAP(*args.qubits)
            )
        ],
    ) == alphaclops.Circuit(alphaclops.ISWAP(*alphaclops.LineQubit.range(2)))

    assert alphaclops.quirk_url_to_circuit(
        'http://algassert.com/quirk#circuit={"cols":[["iswap"]]}',
        extra_cell_makers={'iswap': alphaclops.ISWAP},
    ) == alphaclops.Circuit(alphaclops.ISWAP(*alphaclops.LineQubit.range(2)))
    ...
    assert alphaclops.quirk_url_to_circuit(
        ...
        extra_cell_makers=[
            ...
            alphaclops.interop.quirk.cells.CellMaker(
                identifier='toffoli', size=3, maker=lambda args: alphaclops.TOFFOLI(*args.qubits)
            ),
        ],
    ) == alphaclops.Circuit(
        [alphaclops.ISWAP(*alphaclops.LineQubit.range(2)), alphaclops.TOFFOLI(*alphaclops.LineQubit.range(3))]
    )

    assert alphaclops.quirk_url_to_circuit(
        'http://algassert.com/quirk#circuit={"cols":[["iswap"], ["toffoli"]]}',
        extra_cell_makers={'iswap': alphaclops.ISWAP, 'toffoli': alphaclops.TOFFOLI},
    ) == alphaclops.Circuit(
        [alphaclops.ISWAP(*alphaclops.LineQubit.range(2)), alphaclops.TOFFOLI(*alphaclops.LineQubit.range(3))]
    )
...
def test_init():
    b, c, d, e, f = alphaclops.LineQubit.range(1, 6)
    assert_url_to_circuit_returns(
        '{"cols":[],"init":[0,1,"+","-","i","-i"]}',
        alphaclops.Circuit(
            alphaclops.X(b),
            alphaclops.ry(np.pi / 2).on(c),
            alphaclops.ry(-np.pi / 2).on(d),
            alphaclops.rx(-np.pi / 2).on(e),
            alphaclops.rx(np.pi / 2).on(f),
        ),
    )
    ...
    with pytest.raises(ValueError, match="init must be a list"):
        _ = alphaclops.quirk_url_to_circuit('http://algassert.com/quirk#circuit={"cols":[],"init":0}')

    with pytest.raises(ValueError, match="Unrecognized init state"):
        _ = alphaclops.quirk_url_to_circuit('http://algassert.com/quirk#circuit={"cols":[],"init":[2]}')
...
def test_custom_matrix_gate():
    ...
    assert_url_to_circuit_returns(
        ...
        alphaclops.Circuit(alphaclops.MatrixGate(np.array([[0, 1], [1, 0]])).on(a)),
    )

    # With name.
    assert_url_to_circuit_returns(
        '{"cols":[["~cv0d"]],"gates":[{"id":"~cv0d","name":"test","matrix":"{{0,i},{1,0}}"}]}',
        alphaclops.Circuit(alphaclops.MatrixGate(np.array([[0, 1j], [1, 0]])).on(a)),
    )
    ...
    assert_url_to_circuit_returns(
        ...
        alphaclops.Circuit(alphaclops.X(a), alphaclops.MatrixGate(np.diag([-1, 1j, 1, -1j])).on(b, a)),
        output_amplitudes_from_quirk=[
            {"r": 0, "i": 0},
            {"r": 0, "i": 1},
            {"r": 0, "i": 0},
...
def test_survives_a_billion_laughs():
    ...
    with pytest.raises(ValueError, match=f'{4**26} operations'):
        alphaclops.quirk_url_to_circuit(
            'https://algassert.com/quirk#circuit={'
            '"cols":[["~z"]],'
            '"gates":['
...
def test_completes_weight_zero_billion_laughs():
    circuit = alphaclops.quirk_url_to_circuit(
        'https://algassert.com/quirk#circuit={'
        '"cols":[["~z"]],'
        '"gates":['
        '{"id":"~a","circuit":{"cols":['
