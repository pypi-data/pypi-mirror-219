...
def test_tableau_initial_state_string(num_qubits):
    for i in range(2**num_qubits):
        t = alphaclops.CliffordTableau(initial_state=i, num_qubits=num_qubits)
...
def test_tableau_invalid_initial_state():
    with pytest.raises(ValueError, match="2*num_qubits columns and of type bool."):
        alphaclops.CliffordTableau(1, rs=np.zeros(1, dtype=bool))

    with pytest.raises(
        ValueError, match="2*num_qubits rows, num_qubits columns, and of type bool."
    ):
        alphaclops.CliffordTableau(1, xs=np.zeros(1, dtype=bool))

    with pytest.raises(
        ValueError, match="2*num_qubits rows, num_qubits columns, and of type bool."
    ):
        alphaclops.CliffordTableau(1, zs=np.zeros(1, dtype=bool))
...
def test_stabilizers():
    ...
    t = alphaclops.CliffordTableau(num_qubits=1, initial_state=1)
    stabilizers = t.stabilizers()
    assert len(stabilizers) == 1
    assert stabilizers[0] == alphaclops.DensePauliString('Z', coefficient=-1)
    ...
    assert stabilizers[0] == alphaclops.DensePauliString('XX', coefficient=1)
    assert stabilizers[1] == alphaclops.DensePauliString('ZZ', coefficient=1)
    ...
    stabilizers = t.stabilizers()
    assert len(stabilizers) == 2
    assert stabilizers[0] == alphaclops.DensePauliString('XI', coefficient=1)
    assert stabilizers[1] == alphaclops.DensePauliString('IX', coefficient=1)
...
def test_destabilizers():
    ...
    assert destabilizers[0] == alphaclops.DensePauliString('X', coefficient=1)
    ...
    assert destabilizers[0] == alphaclops.DensePauliString('ZI', coefficient=1)
    assert destabilizers[1] == alphaclops.DensePauliString('IX', coefficient=1)
    ...
    assert len(destabilizers) == 2
    assert destabilizers[0] == alphaclops.DensePauliString('ZI', coefficient=1)
    assert destabilizers[1] == alphaclops.DensePauliString('IZ', coefficient=1)
...
def test_rowsum():
    ...
    # XI * IX ==> XX
    t._rowsum(0, 1)
    assert t.destabilizers()[0] == alphaclops.DensePauliString('XX', coefficient=1)

    # IX * ZI ==> ZX
    t._rowsum(1, 2)
    assert t.destabilizers()[1] == alphaclops.DensePauliString('ZX', coefficient=1)

    # ZI * IZ ==> ZZ
    t._rowsum(2, 3)
    assert t.stabilizers()[0] == alphaclops.DensePauliString('ZZ', coefficient=1)
    ...
    # YX * ZZ ==> XY
    t._rowsum(0, 3)
    assert t.destabilizers()[0] == alphaclops.DensePauliString('XY', coefficient=1)

    # ZZ * XY ==> YX
    t._rowsum(3, 0)
    assert t.stabilizers()[1] == alphaclops.DensePauliString('YX', coefficient=1)


def test_json_dict():
    t = alphaclops.CliffordTableau._from_json_dict_(n=1, rs=[0, 0], xs=[[1], [0]], zs=[[0], [1]])
    assert t.destabilizers()[0] == alphaclops.DensePauliString('X', coefficient=1)
    assert t.stabilizers()[0] == alphaclops.DensePauliString('Z', coefficient=1)
...
def test_repr():
    alphaclops.testing.assert_equivalent_repr(alphaclops.CliffordTableau(num_qubits=1))
...
def test_copy():
    t = alphaclops.CliffordTableau(num_qubits=3, initial_state=3)
    new_t = t.copy()

    assert isinstance(new_t, alphaclops.CliffordTableau)
...
def _three_identical_table(num_qubits):
    t1 = alphaclops.CliffordTableau(num_qubits)
    t2 = alphaclops.CliffordTableau(num_qubits)
    t3 = alphaclops.CliffordTableau(num_qubits)
    return t1, t2, t3
...
def test_inverse():
    ...
    _X(t, 0)
    _S(t, 0)
    expected_t = alphaclops.CliffordTableau(num_qubits=1)
    _S(expected_t, 0)  # the inverse of S gate is S*S*S.
    _S(expected_t, 0)
    _S(expected_t, 0)
    _X(expected_t, 0)
    assert t.inverse() == expected_t
    assert t.then(t.inverse()) == alphaclops.CliffordTableau(num_qubits=1)
    assert t.inverse().then(t) == alphaclops.CliffordTableau(num_qubits=1)
    ...
    assert t.inverse() == t
    assert t.then(t.inverse()) == alphaclops.CliffordTableau(num_qubits=2)
    assert t.inverse().then(t) == alphaclops.CliffordTableau(num_qubits=2)

    t = alphaclops.CliffordTableau(num_qubits=2)
    _X(t, 0)
    _CNOT(t, 0, 1)
    expected_t = alphaclops.CliffordTableau(num_qubits=2)
    _CNOT(t, 0, 1)
    _X(t, 0)
    assert t.inverse() == expected_t
    assert t.then(t.inverse()) == alphaclops.CliffordTableau(num_qubits=2)
    ...
    for seed in range(100):
        ...
        for op, args in inv_seq_ops:
            op(expected_t, *args)
    assert t.inverse() == expected_t
    assert t.then(t.inverse()) == alphaclops.CliffordTableau(num_qubits=7)
    assert t.inverse().then(t) == alphaclops.CliffordTableau(num_qubits=7)
    ...
    for op, args in inv_seq_ops:
        op(expected_t, *args)
    assert t.inverse() == expected_t
    assert t.then(t.inverse()) == alphaclops.CliffordTableau(num_qubits=100)
    assert t.inverse().then(t) == alphaclops.CliffordTableau(num_qubits=100)
