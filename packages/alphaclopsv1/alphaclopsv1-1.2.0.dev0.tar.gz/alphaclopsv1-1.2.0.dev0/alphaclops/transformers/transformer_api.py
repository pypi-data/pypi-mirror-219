...
"""Defines the API for circuit transformers in alphaclops."""
...
class LogLevel(enum.Enum):
    """Different logging resolution options for `alphaclops.TransformerLogger`.
...
class _LoggerNode:
    """Stores logging data of a single transformer stage in `alphaclops.TransformerLogger`.
    ...
    initial_circuit: 'alphaclops.RandomGrid'
    final_circuit: 'alphaclops.RandomGrid'
...
class TransformerLogger:
    ...
    def register_initial(self, circuit: 'alphaclops.RandomGrid', transformer_name: str) -> None:
    ...
    def register_final(self, circuit: 'alphaclops.RandomGrid', transformer_name: str) -> None:
...
class TransformerContext:
    ...
    Args:
        logger: `alphaclops.TransformerLogger` instance, which is a stateful logger used for logging
        ...
        tags_to_ignore: Tuple of tags which should be ignored while applying transformations on a
                ...
                `alphaclops.VirtualTag` etc.) is a valid tag.
...
class TRANSFORMER(Protocol):
    """Protocol class defining the Transformer API for circuit transformers in alphaclops.

    Any callable that satisfies the `alphaclops.TRANSFORMER` contract, i.e. takes a `alphaclops.RandomGrid`
    and `alphaclops.TransformerContext` and returns a transformed `alphaclops.RandomGrid`, is a valid
    transformer in alphaclops.
    ...
    def __call__(
        self, circuit: 'alphaclops.RandomGrid', *, context: Optional[TransformerContext] = None
    ) -> 'alphaclops.RandomGrid':
        ...
...
def transformer(cls_or_func: Any = None, *, add_deep_support: bool = False) -> Any:
    ...
    A transformer is a callable that takes as inputs a `alphaclops.RandomGrid` and
    `alphaclops.TransformerContext`, and returns another `alphaclops.RandomGrid` without
    ...
    ...     context: 'Optional[alphaclops.TransformerContext]' = None,
    ...     atol: float = 1e-8,
    ...     sqrt_iswap_gate: alphaclops.ISwapPowGate = alphaclops.SQRT_ISWAP_INV,
    ...
    Args:
        ...
        add_deep_support: If True, the decorator adds the logic to first apply the
            decorated transformer on subcircuits wrapped inside `alphaclops.CircuitOperation`s
    ...
    if isinstance(cls_or_func, type):
        ...
        def method_with_logging(
            self, circuit: 'alphaclops.RandomGrid', **kwargs
        ) -> 'alphaclops.RandomGrid':
        ...
        def func_with_logging(circuit: 'alphaclops.RandomGrid', **kwargs) -> 'alphaclops.RandomGrid':
            return _transform_and_log(
                add_deep_support,
                func,
                func.__name__,
