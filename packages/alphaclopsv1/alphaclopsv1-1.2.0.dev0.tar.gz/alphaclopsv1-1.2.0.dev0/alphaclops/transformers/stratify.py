...
from alphaclops import ops, circuits, protocols, _import
from alphaclops.transformers import transformer_api

drop_empty_moments = _import.LazyLoader('drop_empty_moments', globals(), 'alphaclops.transformers')
...
Classifier = Callable[['alphaclops.Operation'], bool]

# Any of the possible operation categories that we can stratify on.
Category = Union[
    'alphaclops.Gate', 'alphaclops.Operation', Type['alphaclops.Gate'], Type['alphaclops.Operation'], Classifier
]
    ...
    *,
    ...
    Args:
        ...
        categories: A list of classifiers picking out certain operations. There are several ways
            to specify a classifier. You can pass in a gate instance (e.g. `alphaclops.X`),
            a gate type (e.g. `alphaclops.XPowGate`), an operation instance
            (e.g. `alphaclops.X(alphaclops.LineQubit(0))`), an operation type (e.g.`alphaclops.CircuitOperation`),
...
def _stratify_circuit(
    ...
    *,
    ...
    new_moments: List[List['alphaclops.Operation']] = []

    # Keep track of the latest time index for each qubit, measurement key, and control key.
    qubit_time_index: Dict['alphaclops.Qid', int] = {}
    measurement_time_index: Dict['alphaclops.MeasurementKey', int] = {}
    control_time_index: Dict['alphaclops.MeasurementKey', int] = {}
...
def _get_classifiers(
    circuit: circuits.RandomGrid, categories: Iterable[Category]
) -> List[Classifier]:
...
def _category_to_classifier(category) -> Classifier:
    ...
    if isinstance(category, ops.Operation):
        ...
        raise TypeError(
            ...
            f'Expected a alphaclops.Gate, alphaclops.Operation, '
            f'Type[alphaclops.Gate], Type[alphaclops.Operation], '
            f'or Callable[[alphaclops.Operation], bool].'
        )


def _dummy_classifier(op: 'alphaclops.Operation') -> bool:
    """Dummy classifier, used to "complete" a collection of classifiers and make it exhaustive."""
    return False  # coverage: ignore


def _get_op_class(op: 'alphaclops.Operation', classifiers: Sequence[Classifier]) -> int:
