...
from alphaclops import linalg, ops, protocols, value
from alphaclops.linalg import transformations
from alphaclops.transformers import transformer_api, transformer_primitives
from alphaclops.transformers.synchronize_terminal_measurements import find_terminal_measurements
...
class _MeasurementQid(ops.Qid):
    ...
    def __init__(self, key: Union[str, 'alphaclops.MeasurementKey'], qid: 'alphaclops.Qid', index: int = 0):
...
def defer_measurements(
    ...
    Args:
        ...
        context: `alphaclops.TransformerContext` storing common configurable options
            for transformers.
    Returns:
    ...
    Raises:
        NotImplementedError: When attempting to defer a measurement with a
            confusion map. (https://github.com/quantumlib/alphaclops/issues/5482)
    """
    ...
    measurement_qubits: Dict['alphaclops.MeasurementKey', List[Tuple['alphaclops.Qid', ...]]] = defaultdict(
        list
    )

    def defer(op: 'alphaclops.Operation', _) -> 'alphaclops.OP_TREE':
...
def _all_possible_datastore_states(
    keys: Iterable[Tuple['alphaclops.MeasurementKey', int]],
    measurement_qubits: Dict['alphaclops.MeasurementKey', List[Tuple['alphaclops.Qid', ...]]],
) -> Iterable['alphaclops.ClassicalDataStoreReader']:
...
def dephase_measurements(
    ...
    *,
    context: Optional['alphaclops.TransformerContext'] = transformer_api.TransformerContext(deep=True),
) -> 'alphaclops.Circuit':
    ...
    Raises:
        ValueError: If the circuit contains classical controls. In this case,
            ...
            `alphaclops.defer_measurements` first. Since deferral adds ancilla qubits
            to the circuit, this is not done automatically, to prevent
            surprises.
    """

    def dephase(op: 'alphaclops.Operation', _) -> 'alphaclops.OP_TREE':
        gate = op.gate
        if isinstance(gate, ops.MeasurementGate):
            key = value.MeasurementKey.parse_serialized(gate.key)
            ...
            raise ValueError('Use alphaclops.defer_measurements first to remove classical controls.')
        return op
...
def drop_terminal_measurements(
    ...
    *,
    ...
    def flip_inversion(op: 'alphaclops.Operation', _) -> 'alphaclops.OP_TREE':
...
def _mod_add(source: 'alphaclops.Qid', target: 'alphaclops.Qid') -> 'alphaclops.Operation':
