...
from alphaclops.protocols import decompose_protocol as dp
...
def _create_on_stuck_raise_error(gateset: 'alphaclops.Gateset'):
...
def _decompose_operations_to_target_gateset(
    ...
    *,
    ...
    gateset: Optional['alphaclops.Gateset'] = None,
    decomposer: Callable[['alphaclops.Operation', int], dp.DecomposeResult] = lambda *_: NotImplemented,
    ignore_failures: bool = True,
    tags_to_decompose: Sequence[Hashable] = (),
) -> 'alphaclops.Circuit':
    """Decomposes every operation to `gateset` using `alphaclops.decompose` and `decomposer`.

    This transformer attempts to decompose every operation `op` in the given circuit to `gateset`
    using `alphaclops.decompose` protocol with `decomposer` used as an intercepting decomposer. This
    ...
    Args:
        ...
        decomposer: A callable type which accepts an (operation, moment_index) and returns
            - An equivalent `alphaclops.OP_TREE` implementing `op` using gates from `gateset`.
        ...
        tags_to_decompose: `alphaclops.CircuitOperation`s tagged with any of `tags_to_decompose` will
    ...
    def map_func(op: 'alphaclops.Operation', moment_index: int):
        if (
            context
            and context.deep
...
def optimize_for_target_gateset(
    ...
    *,
    context: Optional['alphaclops.TransformerContext'] = None,
    gateset: Optional['alphaclops.CompilationTargetGateset'] = None,
    ignore_failures: bool = True,
) -> 'alphaclops.Circuit':
    ...
    2. Convert operations using built-in alphaclops decompose + `gateset.decompose_to_target_gateset`.
    3. Run all `gateset.postprocess_transformers`

    Args:
        circuit: Input circuit to transform. It will not be modified.
        context: `alphaclops.TransformerContext` storing common configurable options for transformers.
        gateset: Target gateset, which should be an instance of `alphaclops.CompilationTargetGateset`.
        ignore_failures: If set, operations that fail to convert are left unchanged. If not set,
            conversion failures raise a ValueError.
