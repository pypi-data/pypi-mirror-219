...
from alphaclops.transformers.heuristic_decompositions.two_qubit_gate_tabulation import (
...
from alphaclops.testing import random_special_unitary, assert_equivalent_repr

_rng = value.parse_random_state(11)  # for determinism

sycamore_tabulation = two_qubit_gate_product_tabulation(
    alphaclops.unitary(alphaclops.FSimGate(np.pi / 2, np.pi / 6)), 0.2, random_state=_rng
)

sqrt_iswap_tabulation = two_qubit_gate_product_tabulation(
    alphaclops.unitary(alphaclops.FSimGate(np.pi / 4, np.pi / 24)), 0.1, random_state=_rng
)
...
def test_sycamore_gate_tabulation(seed):
    base_gate = alphaclops.unitary(alphaclops.FSimGate(np.pi / 2, np.pi / 6))
