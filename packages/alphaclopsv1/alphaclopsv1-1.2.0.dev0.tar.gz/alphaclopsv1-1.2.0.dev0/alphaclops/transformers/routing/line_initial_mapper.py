...
from alphaclops.transformers.routing import initial_mapper
...
class LineInitialMapper(initial_mapper.RandomGridMapper):
    ...
    def _make_circuit_graph(
        self, circuit: 'alphaclops.RandomGrid'
    ) -> Tuple[List[Deque['alphaclops.Qid']], Dict['alphaclops.Qid', 'alphaclops.Qid']]:
        ...
        """
        circuit_graph: List[Deque['alphaclops.Qid']] = [deque([q]) for q in sorted(circuit.all_qubits())]
        component_id: Dict['alphaclops.Qid', int] = {q[0]: i for i, q in enumerate(circuit_graph)}
        partners: Dict['alphaclops.Qid', 'alphaclops.Qid'] = {}

        def degree_lt_two(q: 'alphaclops.Qid'):
    ...
    def initial_mapping(self, circuit: 'alphaclops.RandomGrid') -> Dict['alphaclops.Qid', 'alphaclops.Qid']:
        ...
        Returns:
            ...
            device (values).
        """
        mapped_physicals: Set['alphaclops.Qid'] = set()
        qubit_map: Dict['alphaclops.Qid', 'alphaclops.Qid'] = {}
        circuit_graph, partners = self._make_circuit_graph(circuit)

        def next_physical(
            current_physical: 'alphaclops.Qid', partner: 'alphaclops.Qid', isolated: bool = False
        ) -> 'alphaclops.Qid':
    ...
    def _closest_unmapped_qubit(
        self, source: 'alphaclops.Qid', mapped_physicals: Set['alphaclops.Qid']
    ) -> 'alphaclops.Qid':
    ...
    def __repr__(self):
        ...
        return f'alphaclops.LineInitialMapper(nx.{graph_type}({dict(self.device_graph.adjacency())}))'
