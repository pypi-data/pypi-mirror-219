...
def construct_device_graph_and_mapping():
    device_graph = nx.Graph(
        [
            (alphaclops.NamedQubit("a"), alphaclops.NamedQubit("b")),
            (alphaclops.NamedQubit("b"), alphaclops.NamedQubit("c")),
            (alphaclops.NamedQubit("c"), alphaclops.NamedQubit("d")),
            (alphaclops.NamedQubit("a"), alphaclops.NamedQubit("e")),
            (alphaclops.NamedQubit("e"), alphaclops.NamedQubit("d")),
        ]
    )
    q = alphaclops.LineQubit.range(5)
    initial_mapping = {
        q[1]: alphaclops.NamedQubit("a"),
        q[3]: alphaclops.NamedQubit("b"),
        q[2]: alphaclops.NamedQubit("c"),
        q[4]: alphaclops.NamedQubit("d"),
    }
...
def test_induced_subgraph():
    ...
    mm = alphaclops.MappingManager(device_graph, initial_mapping)
...
def test_mapped_op():
    ...
    assert mm.mapped_op(alphaclops.CNOT(q[1], q[3])).qubits == (
        alphaclops.NamedQubit("a"),
        alphaclops.NamedQubit("b"),
    )
    # does not fail if qubits non-adjacent
    assert mm.mapped_op(alphaclops.CNOT(q[3], q[4])).qubits == (
        alphaclops.NamedQubit("b"),
        alphaclops.NamedQubit("d"),
    )

    # correctly changes mapped qubits when swapped
    mm.apply_swap(q_int[2], q_int[3])
    assert mm.mapped_op(alphaclops.CNOT(q[1], q[2])).qubits == (
        alphaclops.NamedQubit("a"),
        alphaclops.NamedQubit("b"),
    )
...
def test_distance_on_device_and_is_adjacent():
    ...
    # 'dist_on_device' does not use alphaclops.NamedQubit("e") to find shorter shortest path
