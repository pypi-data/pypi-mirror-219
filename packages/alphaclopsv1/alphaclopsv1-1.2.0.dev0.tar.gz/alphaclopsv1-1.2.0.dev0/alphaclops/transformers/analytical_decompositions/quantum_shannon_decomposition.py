...
from alphaclops.linalg import decompositions, predicates

if TYPE_CHECKING:
    import alphaclops
    from alphaclops.ops import op_tree


def quantum_shannon_decomposition(qubits: 'List[alphaclops.Qid]', u: np.ndarray) -> 'op_tree.OpTree':
    ...
    if not predicates.is_unitary(u):  # Check that u is unitary
        raise ValueError(
            "Expected input matrix u to be unitary, \
                but it fails alphaclops.is_unitary check"
        )
...
def _single_qubit_decomposition(qubit: 'alphaclops.Qid', u: np.ndarray) -> 'op_tree.OpTree':
...
def _msb_demuxer(
    demux_qubits: 'List[alphaclops.Qid]', u1: np.ndarray, u2: np.ndarray
) -> 'op_tree.OpTree':
...
def _multiplexed_cossin(
    cossin_qubits: 'List[alphaclops.Qid]', angles: List[float], rot_func: Callable = ops.ry
) -> 'op_tree.OpTree':
    ...
    Args:
        ...
        rot_func: Rotation function used for this multiplexing implementation
                    (alphaclops.ry or alphaclops.rz)
