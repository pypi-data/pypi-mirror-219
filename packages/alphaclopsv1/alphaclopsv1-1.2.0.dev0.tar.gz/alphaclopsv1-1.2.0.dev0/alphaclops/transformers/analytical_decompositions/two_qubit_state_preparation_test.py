...
def random_state(seed: float):
    return alphaclops.testing.random_superposition(4, random_state=seed)
...
def states_with_phases(st: np.ndarray):
    ...
    for _ in range(3):
        ...
        alphaclops.to_valid_state_vector(curr_st, num_qubits=2)
        for i in range(4):
...
def test_prepare_two_qubit_state_using_cz(state):
    state = alphaclops.to_valid_state_vector(state, num_qubits=2)
    q = alphaclops.LineQubit.range(2)
    circuit = alphaclops.Circuit(alphaclops.prepare_two_qubit_state_using_cz(*q, state))
    ops_cz = [*circuit.findall_operations(lambda op: op.gate == alphaclops.CZ)]
    ops_2q = [*circuit.findall_operations(lambda op: alphaclops.num_qubits(op) > 1)]
    assert ops_cz == ops_2q
    assert len(ops_cz) <= 1
    assert alphaclops.allclose_up_to_global_phase(
...
def test_prepare_two_qubit_state_using_sqrt_iswap(state, use_sqrt_iswap_inv):
    ...
    circuit = alphaclops.Circuit(
        alphaclops.prepare_two_qubit_state_using_sqrt_iswap(
