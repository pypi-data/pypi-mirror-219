# Copyright 2023 The alphaclops Developers
#
...
import alphaclops
from alphaclops.ops import common_gates
...
def test_random_qsd_n_qubit(n_qubits):
    ...
    qubits = [alphaclops.NamedQubit(f'q{i}') for i in range(n_qubits)]
    circuit = alphaclops.Circuit(quantum_shannon_decomposition(qubits, U))
    # Test return is equal to inital unitary
    assert alphaclops.approx_eq(U, circuit.unitary(), atol=1e-9)
...
def test_qsd_n_qubit_errors():
    qubits = [alphaclops.NamedQubit(f'q{i}') for i in range(3)]
    with pytest.raises(ValueError, match="shaped numpy array"):
        alphaclops.Circuit(quantum_shannon_decomposition(qubits, np.eye(9)))
    with pytest.raises(ValueError, match="is_unitary"):
        alphaclops.Circuit(quantum_shannon_decomposition(qubits, np.ones((8, 8))))


def test_random_single_qubit_decomposition():
    U = unitary_group.rvs(2)
    qubit = alphaclops.NamedQubit('q0')
    circuit = alphaclops.Circuit(_single_qubit_decomposition(qubit, U))
    # Test return is equal to inital unitary
    assert alphaclops.approx_eq(U, circuit.unitary(), atol=1e-9)
    # Test all operations in gate set
    gates = (common_gates.Rz, common_gates.Ry, common_gates.ZPowGate, common_gates.CXPowGate)
...
def test_msb_demuxer():
    ...
    qubits = [alphaclops.NamedQubit(f'q{i}') for i in range(3)]
    circuit = alphaclops.Circuit(_msb_demuxer(qubits, U1, U2))
    # Test return is equal to inital unitary
    assert alphaclops.approx_eq(U_full, circuit.unitary(), atol=1e-9)
    # Test all operations in gate set
    gates = (common_gates.Rz, common_gates.Ry, common_gates.ZPowGate, common_gates.CXPowGate)
    assert all(isinstance(op.gate, gates) for op in circuit.all_operations())
...
def test_multiplexed_cossin():
    ...
    qubits = [alphaclops.NamedQubit(f'q{i}') for i in range(2)]
    circuit = alphaclops.Circuit(_multiplexed_cossin(qubits, [angle_1, angle_2]))
    # Test return is equal to inital unitary
    assert alphaclops.approx_eq(multiplexed_ry, circuit.unitary(), atol=1e-9)
    # Test all operations in gate set
    gates = (common_gates.Rz, common_gates.Ry, common_gates.ZPowGate, common_gates.CXPowGate)
    assert all(isinstance(op.gate, gates) for op in circuit.all_operations())
