...
from alphaclops.linalg import is_unitary, is_special_unitary, map_eigenvalues
from alphaclops.protocols import unitary
...
def _decompose_single_ctrl(
    matrix: np.ndarray, control: 'alphaclops.Qid', target: 'alphaclops.Qid'
...
def _ccnot_congruent(c0: 'alphaclops.Qid', c1: 'alphaclops.Qid', target: 'alphaclops.Qid') -> List[
...
def decompose_multi_controlled_x(
    controls: List['alphaclops.Qid'], target: 'alphaclops.Qid', free_qubits: List['alphaclops.Qid']
) -> List['alphaclops.Operation']:
...
def _decompose_su(
    matrix: np.ndarray, controls: List['alphaclops.Qid'], target: 'alphaclops.Qid'
) -> List['alphaclops.Operation']:
...
def _decompose_recursive(
    matrix: np.ndarray,
    power: float,
    controls: List['alphaclops.Qid'],
    target: 'alphaclops.Qid',
    free_qubits: List['alphaclops.Qid'],
) -> List['alphaclops.Operation']:
