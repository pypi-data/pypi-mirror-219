...
from alphaclops.protocols.apply_unitary_protocol import apply_unitary, ApplyUnitaryArgs

from alphaclops.protocols.mixture_protocol import mixture
...
class SupportsApplyMixture(Protocol):
    ...
    def _apply_mixture_(
        ...
        Args:
            args: A `alphaclops.ApplyMixtureArgs` object with the `args.target_tensor`
...
def apply_mixture(
    ...
    *,
    ...
    B. Construct an ApplyUnitaryArgs object `uargs` from `args` and then
        try to use `alphaclops.apply_unitary(val, uargs, None)`.
    ...
    C. Try to use `val._mixture_()`.
        ...
        3. If '_mixture_' returns a list of tuples, loop over the list and
            ...
            attempt to use `alphaclops.apply_unitary(op, uargs, None)`. If this
    ...
    Args:
        ...
        args: A mutable `alphaclops.ApplyMixtureArgs` object describing the target
    ...
    # Possibly use `alphaclops.apply_unitary`. (STEP B)
