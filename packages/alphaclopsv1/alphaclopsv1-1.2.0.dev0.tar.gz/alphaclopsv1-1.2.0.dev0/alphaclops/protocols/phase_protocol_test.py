...
def test_phase_by():
    ...
    with pytest.raises(TypeError, match='no _phase_by_ method'):
        _ = alphaclops.phase_by(n, 1, 0)
    with pytest.raises(TypeError, match='returned NotImplemented'):
        _ = alphaclops.phase_by(rin, 1, 0)

    # With default
    assert alphaclops.phase_by(n, 1, 0, default=None) is None
    assert alphaclops.phase_by(rin, 1, 0, default=None) is None

    test = PhaseIsAddition(3)
    assert test.phase == [0, 0, 0]
    test = alphaclops.phase_by(test, 0.25, 0)
    assert test.phase == [0.25, 0, 0]
    test = alphaclops.phase_by(test, 0.25, 0)
    assert test.phase == [0.50, 0, 0]
    test = alphaclops.phase_by(test, 0.40, 1)
    assert test.phase == [0.50, 0.40, 0]
    test = alphaclops.phase_by(test, 0.40, 4)
    assert test.phase == [0.50, 0.40, 0]
