...
def assert_apply_mixture_returns(
    ...
    result = alphaclops.apply_mixture(
        val,
        args=alphaclops.ApplyMixtureArgs(
            target_tensor=rho,
            left_axes=left_axes,
...
def test_apply_mixture_bad_args():
    ...
    with pytest.raises(ValueError, match='Invalid target_tensor shape'):
        alphaclops.apply_mixture(
            alphaclops.IdentityGate(3, (1, 2, 3)),
            alphaclops.ApplyMixtureArgs(
                target,
...
def test_apply_mixture_simple():
    ...
    class HasApplyMixture:
        def _apply_mixture_(self, args: alphaclops.ApplyMixtureArgs):
            zero_left = alphaclops.slice_for_qubits_equal_to(args.left_axes, 0)
            one_left = alphaclops.slice_for_qubits_equal_to(args.left_axes, 1)
            zero_right = alphaclops.slice_for_qubits_equal_to(cast(Tuple[int], args.right_axes), 0)
            one_right = alphaclops.slice_for_qubits_equal_to(cast(Tuple[int], args.right_axes), 1)
            args.out_buffer[:] = 0
...
def test_apply_mixture_simple_state_vector():
    for _ in range(25):
        state = alphaclops.testing.random_superposition(2)
        u1 = alphaclops.testing.random_unitary(2)
...
def test_apply_mixture_simple_split_fallback():
    ...
    class HasMixture:
        def _mixture_(self):
            return ((0.5, np.eye(2, dtype=np.complex128)), (0.5, alphaclops.X))
...
def test_apply_mixture_fallback_two_qubit_random():
    for _ in range(25):
        state = alphaclops.testing.random_superposition(4)
        rho = np.outer(np.conjugate(state), state)
...
def test_apply_mixture_no_protocols_implemented_default():
    ...
    result = alphaclops.apply_mixture(NoProtocols(), args, default='alphaclops')
    assert result == 'alphaclops'
...
def test_apply_mixture_unitary():
    ...
    class HasUnitaryButReturnsNotImplemented(HasUnitary):
        def _apply_unitary_(self, args: alphaclops.ApplyMixtureArgs):
