...
import alphaclops
from alphaclops._compat import proper_eq
from alphaclops.protocols import json_serialization
from alphaclops.testing.json import ModuleJsonTestSpec, spec_for, assert_json_roundtrip_works
...
TESTED_MODULES: Dict[str, Optional[_ModuleDeprecation]] = {
    'alphaclops_aqt': None,
    'alphaclops_ionq': None,
    'alphaclops_google': None,
    'alphaclops_pasqal': None,
    'alphaclops_rigetti': None,
    'alphaclops.protocols': None,
    'non_existent_should_be_fine': None,
}


# pyQuil 3.0, necessary for alphaclops_rigetti module requires
# python >= 3.9
if sys.version_info < (3, 9):  # pragma: no cover
    del TESTED_MODULES['alphaclops_rigetti']
...
def test_deprecated_alphaclops_type_in_json_dict():
    class HasOldJsonDict:
        # Required for testing serialization of non-alphaclops objects.
        __module__ = 'test.nonalphaclops.namespace'

        def __eq__(self, other):
            return isinstance(other, HasOldJsonDict)

        def _json_dict_(self):
            return {'alphaclops_type': 'test.nonalphaclops.namespace.HasOldJsonDict'}

        @classmethod
        def _from_json_dict_(cls, **kwargs):
            return cls()

    with pytest.raises(ValueError, match='not a alphaclops type'):
        _ = alphaclops.json_alphaclops_type(HasOldJsonDict)

    def custom_resolver(name):
        if name == 'test.nonalphaclops.namespace.HasOldJsonDict':
            return HasOldJsonDict

    test_resolvers = [custom_resolver] + alphaclops.DEFAULT_RESOLVERS
    with pytest.raises(ValueError, match="Found 'alphaclops_type'"):
        assert_json_roundtrip_works(HasOldJsonDict(), resolvers=test_resolvers)


def test_line_qubit_roundtrip():
    q1 = alphaclops.LineQubit(12)
...
def test_gridqubit_roundtrip():
    q = alphaclops.TensorCircuit(15, 18)
    assert_json_roundtrip_works(
        q,
        text_should_be="""{
  "alphaclops_type": "TensorCircuit",
  "row": 15,
  "col": 18
}""",
    )
...
def test_op_roundtrip():
    ...
    op1 = alphaclops.rx(0.123).on(q)
    assert_json_roundtrip_works(
        op1,
        text_should_be="""{
  "alphaclops_type": "GateOperation",
  "gate": {
    "alphaclops_type": "Rx",
    "rads": 0.123
  },
...
def test_op_roundtrip_filename(tmpdir):
    ...
    q = alphaclops.LineQubit(5)
    op1 = alphaclops.rx(0.123).on(q)
    alphaclops.to_json(op1, filename)
    assert os.path.exists(filename)
    op2 = alphaclops.read_json(filename)
    assert op1 == op2

    gzip_filename = f'{tmpdir}/op.gz'
    alphaclops.to_json_gzip(op1, gzip_filename)
    assert os.path.exists(gzip_filename)
    op3 = alphaclops.read_json_gzip(gzip_filename)
    assert op1 == op3
...
def test_op_roundtrip_file_obj(tmpdir):
    ...
    with open(filename, 'w+') as file:
        alphaclops.to_json(op1, file)
        assert os.path.exists(filename)
        file.seek(0)
        op2 = alphaclops.read_json(file)
        assert op1 == op2

    gzip_filename = f'{tmpdir}/op.gz'
    with open(gzip_filename, 'w+b') as gzip_file:
        alphaclops.to_json_gzip(op1, gzip_file)
        assert os.path.exists(gzip_filename)
        gzip_file.seek(0)
        op3 = alphaclops.read_json_gzip(gzip_file)
        assert op1 == op3
...
@mock.patch.dict(os.environ, clear='alphaclops_TESTING')
...
class SBKImpl(alphaclops.SerializableByKey):
...
def test_context_serialization():
    ...
    assert (
        final_obj
        == """{
      "alphaclops_type": "_SerializedKey",
      "key": 4
    }"""
    )
...
@pytest.mark.parametrize('mod_spec,alphaclops_obj_name,cls', _list_public_classes_for_tested_modules())
def test_json_test_data_coverage(mod_spec: ModuleJsonTestSpec, alphaclops_obj_name: str, cls):
    if alphaclops_obj_name in mod_spec.tested_elsewhere:
        pytest.skip("Tested elsewhere.")

    if alphaclops_obj_name in mod_spec.not_yet_serializable:
    ...
    json_path = test_data_path / f'{alphaclops_obj_name}.json'
    json_path2 = test_data_path / f'{alphaclops_obj_name}.json_inward'
    deprecation_deadline = mod_spec.deprecated.get(alphaclops_obj_name)

    if not json_path.exists() and not json_path2.exists():
        # coverage: ignore
        pytest.fail(
            f"Hello intrepid developer. There is a new public or "
            f"serializable object named '{alphaclops_obj_name}' in the module '{mod_spec.name}' "
            f"that does not have associated test data.\n"
            f"\n"
            f"You must create the file\n"
            f"    {rel_path}/{alphaclops_obj_name}.json\n"
            f"and the file\n"
            f"    {rel_path}/{alphaclops_obj_name}.repr\n"
            ...
            f"by `repr(obj)` where `obj` is a test {alphaclops_obj_name} value "
            f"or list of such values. To get this to work you may need to "
            f"implement a __repr__ method for {alphaclops_obj_name}. The repr "
            ...
            f"by `alphaclops.to_json(obj)` where `obj` is the same object or "
            f"list of test objects.\n"
            f"To get this to work you likely need "
            f"to add {alphaclops_obj_name} to the "
            ...
            f"{alphaclops_obj_name}. In some cases you will also need to add a "
            f"_from_json_dict_ class method to the {alphaclops_obj_name} class."
            f"\n"
            f"For more information on JSON serialization, please read the "
            f"docstring for alphaclops.protocols.SupportsJSON. If this object or "
    ...
    repr_file = test_data_path / f'{alphaclops_obj_name}.repr'
    if repr_file.exists() and cls is not None:
        ...
        for obj in objs:
            assert type(obj) == cls, (
                f"Value in {test_data_path}/{alphaclops_obj_name}.repr must be of "
                ...
                f"value to {test_data_path}/{alphaclops_obj_name}.repr_inward\n"
                f"\n"
...
class SerializableTypeObject:
    ...
    def _json_dict_(self):
        return {'test_type': json_serialization.json_alphaclops_type(self.test_type)}

    @classmethod
    def _from_json_dict_(cls, test_type, **kwargs):
        return cls(json_serialization.alphaclops_type_from_json(test_type))


@pytest.mark.parametrize('mod_spec,alphaclops_obj_name,cls', _list_public_classes_for_tested_modules())
def test_type_serialization(mod_spec: ModuleJsonTestSpec, alphaclops_obj_name: str, cls):
    if alphaclops_obj_name in mod_spec.tested_elsewhere:
        pytest.skip("Tested elsewhere.")
    ...
    if cls is None:
        pytest.skip(f'No serialization for None-mapped type: {alphaclops_obj_name}')

    try:
        typename = alphaclops.json_alphaclops_type(cls)
    except ValueError as e:
        pytest.skip(f'No serialization for non-alphaclops type: {str(e)}')
    ...
    test_resolvers = [custom_resolver] + alphaclops.DEFAULT_RESOLVERS
    expected_json = (
        f'{{\n  "alphaclops_type": "SerializableTypeObject",\n' f'  "test_type": "{typename}"\n}}'
    )
    assert alphaclops.to_json(sto) == expected_json
    assert alphaclops.read_json(json_text=expected_json, resolvers=test_resolvers) == sto
...
def test_invalid_type_deserialize():
    ...
    with pytest.raises(ValueError, match='Could not resolve type'):
        _ = alphaclops.read_json(json_text=invalid_json, resolvers=test_resolvers)

    factory_json = '{\n  "alphaclops_type": "SerializableTypeObject",\n  "test_type": "sympy.Add"\n}'
    with pytest.raises(ValueError, match='maps to a factory method'):
        _ = alphaclops.read_json(json_text=factory_json, resolvers=test_resolvers)
...
def test_to_from_strings():
  ...
  "global_shift": 0.0
    ...
    assert alphaclops.to_json(alphaclops.X) == x_json_text
    assert alphaclops.read_json(json_text=x_json_text) == alphaclops.X

    with pytest.raises(ValueError, match='specify ONE'):
        alphaclops.read_json(io.StringIO(), json_text=x_json_text)


def test_to_from_json_gzip():
    a, b = alphaclops.LineQubit.range(2)
    test_circuit = alphaclops.Circuit(alphaclops.H(a), alphaclops.CX(a, b))
    gzip_data = alphaclops.to_json_gzip(test_circuit)
    unzip_circuit = alphaclops.read_json_gzip(gzip_raw=gzip_data)
    assert test_circuit == unzip_circuit

    with pytest.raises(ValueError):
        _ = alphaclops.read_json_gzip(io.StringIO(), gzip_raw=gzip_data)
    with pytest.raises(ValueError):
        _ = alphaclops.read_json_gzip()
...
def _eval_repr_data_file(path: pathlib.Path, deprecation_deadline: Optional[str]):
    ...
    if deprecation_deadline:
        ...
        # modules
        # coverage: ignore
        ctx_managers = [alphaclops.testing.assert_deprecated(deadline=deprecation_deadline, count=None)]
    ...
    imports = {'alphaclops': alphaclops, 'pd': pd, 'sympy': sympy, 'np': np, 'datetime': datetime, 'nx': nx}
...
def assert_repr_and_json_test_data_agree(
    ...
    try:
        ...
        ctx_manager = (
            alphaclops.testing.assert_deprecated(deadline=deprecation_deadline, count=None)
            if deprecation_deadline
            else contextlib.suppress()
        )
        with ctx_manager:
            json_obj = alphaclops.read_json(json_text=json_from_file)
    ...
    if not inward_only:
        json_from_alphaclops = alphaclops.to_json(repr_obj)
        json_from_alphaclops_obj = json.loads(json_from_alphaclops)
        json_from_file_obj = json.loads(json_from_file)

        assert json_from_alphaclops_obj == json_from_file_obj, (
            f'The json produced by alphaclops no longer agrees with the json in the '
            f'{rel_json_path} test data file.\n'
            f'\n'
            f'You must either fix the alphaclops code to continue to produce the '
            ...
            f'{json_from_file}\n'
            f'\n'
            f'alphaclops produced json:\n'
            f'{json_from_alphaclops}\n'
        )
...
def test_pathlib_paths(tmpdir):
    ...
    alphaclops.to_json(alphaclops.X, path)
    assert alphaclops.read_json(path) == alphaclops.X

    gzip_path = pathlib.Path(tmpdir) / 'op.gz'
    alphaclops.to_json_gzip(alphaclops.X, gzip_path)
    assert alphaclops.read_json_gzip(gzip_path) == alphaclops.X


def test_dataclass_json_dict() -> None:
    @dataclasses.dataclass(frozen=True)
    class MyDC:
        q: alphaclops.LineQubit
        desc: str

        def _json_dict_(self):
            return alphaclops.dataclass_json_dict(self)

    def custom_resolver(name):
        if name == 'MyDC':
            return MyDC

    my_dc = MyDC(alphaclops.LineQubit(4), 'hi mom')

    assert_json_roundtrip_works(my_dc, resolvers=[custom_resolver, *alphaclops.DEFAULT_RESOLVERS])


def test_numpy_values():
    assert (
            alphaclops.to_json({'value': np.array(1)})
            == """{
  "value": 1
}"""
    )
...
def test_datetime():
    ...
    re_naive_dt = alphaclops.read_json(json_text=alphaclops.to_json(naive_dt))
    assert re_naive_dt != naive_dt, 'loads in with timezone'
    assert re_naive_dt.timestamp() == naive_dt.timestamp()

    utc_dt = naive_dt.astimezone(datetime.timezone.utc)
    re_utc_dt = alphaclops.read_json(json_text=alphaclops.to_json(utc_dt))
    ...
    re_pst_dt = alphaclops.read_json(json_text=alphaclops.to_json(pst_dt))
