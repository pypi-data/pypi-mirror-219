...
from alphaclops import devices, ops
from alphaclops._doc import doc_private
from alphaclops.protocols import qid_shape_protocol
...
DecomposeResult = Union[None, NotImplementedType, 'alphaclops.OP_TREE']
...
class OpDecomposerWithContext(Protocol):
    def __call__(
        self, __op: 'alphaclops.Operation', *, context: Optional['alphaclops.DecompositionContext'] = None
    ) -> DecomposeResult:
        ...


OpDecomposer = Union[Callable[['alphaclops.Operation'], DecomposeResult], OpDecomposerWithContext]
...
def _value_error_describing_bad_operation(op: 'alphaclops.Operation') -> ValueError:
...
class DecompositionContext:
    ...
    Args:
        qubit_manager: A `alphaclops.QubitManager` instance to allocate clean / dirty ancilla qubits as
            part of the decompose protocol.
    """

    qubit_manager: 'alphaclops.QubitManager'
...
class SupportsDecompose(Protocol):
    ...
    2-qubit gates included by default in alphaclops. alphaclops does not make any guarantees
    about what the final gate set is. Currently, decompositions within alphaclops
    ...
    give an `intercepting_decomposer` to `alphaclops.decompose` that attempts to
    target a specific gate set).

    For example, `alphaclops.TOFFOLI` has a `_decompose_` method that returns a pair
    of Hadamard gates surrounding a `alphaclops.CCZ`. Although `alphaclops.CCZ` is not a
    ...
    decomposing `alphaclops.TOFFOLI` terminates in 1-qubit and 2-qubit operations, and
    so almost all decomposition-aware code will be able to handle `alphaclops.TOFFOLI`
    instances.

    Callers are responsible for iteratively decomposing until they are given
    operations that they understand. The `alphaclops.decompose` method is a simple way
    ...
    in infinite loops when calling `alphaclops.decompose`.
    ...
    2-qubit operations. When this happens, `alphaclops.decompose` will raise
...
class SupportsDecomposeWithQubits(Protocol):
    ...
    def _decompose_(self, qubits: Tuple['alphaclops.Qid', ...]) -> DecomposeResult:
        pass

    def _decompose_with_context_(
        self, qubits: Tuple['alphaclops.Qid', ...], *, context: Optional[DecompositionContext] = None
    ) -> DecomposeResult:
        pass
...
class _DecomposeArgs:
    ...
    keep: Optional[Callable[['alphaclops.Operation'], bool]]
    on_stuck_raise: Union[None, Exception, Callable[['alphaclops.Operation'], Optional[Exception]]]
    preserve_structure: bool


def _decompose_dfs(item: Any, args: _DecomposeArgs) -> Iterator['alphaclops.Operation']:
    from alphaclops.circuits import CircuitOperation, FrozenCircuit
...
def decompose(
    ...
    *,
    ...
    keep: Optional[Callable[['alphaclops.Operation'], bool]] = None,
    on_stuck_raise: Union[
        None, Exception, Callable[['alphaclops.Operation'], Optional[Exception]]
    ] = _value_error_describing_bad_operation,
    preserve_structure: bool = False,
    context: Optional[DecompositionContext] = None,
) -> List['alphaclops.Operation']:
    """Recursively decomposes a value into `alphaclops.Operation`s meeting a criteria.
    ...
    Args:
        ...
        intercepting_decomposer: An optional method that is called before the
            ...
            if `val` isn't a `alphaclops.Operation`.
    ...
    Raises:
        TypeError:
            `val` isn't a `alphaclops.Operation` and can't be decomposed even once.
...
@overload
def decompose_once(val: Any, **kwargs) -> List['alphaclops.Operation']:
    pass
...
) -> Union[TDefault, List['alphaclops.Operation']]:
    pass
...
def decompose_once(
    ...
    **kwargs,
    ...
    until some criteria is met (which is what `alphaclops.decompose` does).
...
def _try_decompose_into_operations_and_qubits(
    val: Any,
) -> Tuple[Optional[List['alphaclops.Operation']], Sequence['alphaclops.Qid'], Tuple[int, ...]]:
    ...
    if isinstance(val, ops.Gate):
        ...
        qubits: Sequence[alphaclops.Qid] = devices.LineQid.for_qid_shape(qid_shape)
    ...
    if result is not None:
        qubit_set = set()
        qid_shape_dict: Dict[alphaclops.Qid, int] = defaultdict(lambda: 1)
        for op in result:
