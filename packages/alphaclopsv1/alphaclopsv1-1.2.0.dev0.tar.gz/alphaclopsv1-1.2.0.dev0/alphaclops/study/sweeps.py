...
from alphaclops.study import resolver

if TYPE_CHECKING:
    import alphaclops

Params = Iterable[Tuple['alphaclops.TParamKey', 'alphaclops.TParamVal']]
ProductOrZipSweepLike = Dict['alphaclops.TParamKey', Union['alphaclops.TParamVal', Sequence['alphaclops.TParamVal']]]
...
class Sweep(metaclass=abc.ABCMeta):
    ...
    @property
    @abc.abstractmethod
    def keys(self) -> List['alphaclops.TParamKey']:
...
class Product(Sweep):
    ...
    def __repr__(self) -> str:
        ...
        return f'alphaclops.Product({factors_repr})'
...
class Zip(Sweep):
    ...
    def __repr__(self) -> str:
        ...
        return f'alphaclops.Zip({sweeps_repr})'
...
class ZipLongest(Zip):
    ...
    def __repr__(self) -> str:
        sweeps_repr = ', '.join(repr(s) for s in self.sweeps)
        return f'alphaclops_google.ZipLongest({sweeps_repr})'
...
class SingleSweep(Sweep):
    ...
    def __init__(self, key: 'alphaclops.TParamKey') -> None:
...
class Points(SingleSweep):
    ...
    def __init__(
        self, key: 'alphaclops.TParamKey', points: Sequence[float], metadata: Optional[Any] = None
    ) -> None:
    ...
    def __repr__(self) -> str:
        ...
        return f'alphaclops.Points({self.key!r}, {self.points!r}{metadata_repr})'
...
class Linspace(SingleSweep):
    ...
    def __repr__(self) -> str:
        metadata_repr = f', metadata={self.metadata!r}' if self.metadata is not None else ""
        return (
            f'alphaclops.Linspace({self.key!r}, start={self.start!r}, '
...
class ListSweep(Sweep):
    ...
    def __init__(self, resolver_list: Iterable[resolver.ParamResolverOrSimilarType]):
        ...
        Raises:
            TypeError: If `resolver_list` is not a `alphaclops.ParamResolver` or a
                dict.
        """
    ...
    def __repr__(self) -> str:
        return f'alphaclops.ListSweep({self.resolver_list!r})'
