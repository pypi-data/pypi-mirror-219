...
from alphaclops.study.resolver import ParamResolver, ParamResolverOrSimilarType
from alphaclops.study.sweeps import ListSweep, Points, Sweep, UnitSweep, Zip, dict_to_product_sweep
    ...
    if isinstance(sweepable, dict):
        if any(isinstance(val, Sequence) for val in sweepable.values()):
            warnings.warn(
                ...
                'of sweeps is deprecated and will be removed in alphaclops 0.10. '
                'Instead, expand the sweep explicitly using '
                '`alphaclops.dict_to_product_sweep`.',
                DeprecationWarning,
                stacklevel=2,
            )
...
def to_sweep(
    ...
    """Converts the argument into a ``alphaclops.Sweep``.

    Args:
        sweep_or_resolver_list: The object to try to turn into a
            ``alphaclops.Sweep`` . A ``alphaclops.Sweep``, a single ``alphaclops.ParamResolver``,
            or a list of ``alphaclops.ParamResolver`` s.
