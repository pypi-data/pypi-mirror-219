...
import sympy
import alphaclops
from alphaclops.study import flatten_expressions
...
def test_flattener_value_of():
    ...
    assert alphaclops.flatten([sympy.Symbol('c') / 2 + 1, sympy.Symbol('c/2') + 1])[0] == [
        sympy.Symbol('<c/2 + 1>'),
...
def test_expression_map_repr():
    alphaclops.testing.assert_equivalent_repr(alphaclops.ExpressionMap({'a': 'b'}))


def test_flatten_circuit():
    qubit = alphaclops.LineQubit(0)
    a = sympy.Symbol('a')
    circuit = alphaclops.Circuit(alphaclops.X(qubit) ** a, alphaclops.X(qubit) ** (1 + a / 2))

    c_flat, expr_map = alphaclops.flatten(circuit)

    c_expected = alphaclops.Circuit(alphaclops.X(qubit) ** a, alphaclops.X(qubit) ** sympy.Symbol('<a/2 + 1>'))
    assert c_flat == c_expected
    assert isinstance(expr_map, alphaclops.ExpressionMap)
...
def test_transform_params():
    qubit = alphaclops.LineQubit(0)
    a = sympy.Symbol('a')
    circuit = alphaclops.Circuit(alphaclops.X(qubit) ** (a / 4), alphaclops.X(qubit) ** (1 + a / 2))
    params = {'a': 3}

    _, new_params = alphaclops.flatten_with_params(circuit, params)
...
def test_transform_sweep():
    ...
    a = sympy.Symbol('a')
    circuit = alphaclops.Circuit(alphaclops.X(qubit) ** (a / 4), alphaclops.X(qubit) ** (1 + a / 2))
    sweep = alphaclops.Linspace(a, start=0, stop=3, length=4)

    _, new_sweep = alphaclops.flatten_with_sweep(circuit, sweep)
    assert isinstance(new_sweep, alphaclops.Sweep)
    resolvers = list(new_sweep)

    expected_resolvers = [
        alphaclops.ParamResolver({'<a/4>': 0.0, '<a/2 + 1>': 1.0}),
        alphaclops.ParamResolver({'<a/4>': 0.25, '<a/2 + 1>': 1.5}),
        alphaclops.ParamResolver({'<a/4>': 0.5, '<a/2 + 1>': 2}),
        alphaclops.ParamResolver({'<a/4>': 0.75, '<a/2 + 1>': 2.5}),
    ]
...
def test_resolver_new():
    flattener = flatten_expressions._ParamFlattener({'a': 'b'})
    flattener2 = alphaclops.ParamResolver(flattener)
...
def test_transformed_sweep():
    a = sympy.Symbol('a')
    sweep = alphaclops.Linspace('a', start=0, stop=3, length=4)
    expr_map = alphaclops.ExpressionMap({a / 4: 'x0', 1 - a / 2: 'x1'})
...
def test_transformed_sweep_equality():
    a = sympy.Symbol('a')
    sweep = alphaclops.Linspace('a', start=0, stop=3, length=4)
    expr_map = alphaclops.ExpressionMap({a / 4: 'x0', 1 - a / 4: 'x1'})

    sweep2 = alphaclops.Linspace(a, start=0, stop=3, length=4)
    expr_map2 = alphaclops.ExpressionMap({a / 4: 'x0', 1 - a / 4: 'x1'})

    sweep3 = alphaclops.Linspace(a, start=0, stop=3, length=20)
    expr_map3 = alphaclops.ExpressionMap({a / 20: 'x0', 1 - a / 20: 'x1'})
