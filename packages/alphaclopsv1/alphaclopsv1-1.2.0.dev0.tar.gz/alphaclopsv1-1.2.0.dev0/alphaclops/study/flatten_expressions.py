...
from alphaclops.study import resolver, sweeps, sweepable
...
def flatten(val: Any) -> Tuple[Any, 'ExpressionMap']:
    ...
    `alphaclops.ExpressionMap({sympy.Symbol('x')+1: sympy.Symbol('<x + 1>')})`.  This
    ...
    ...     alphaclops.X(qubit) ** (a/4),
    ...     alphaclops.Y(qubit) ** (1-a/2),
    ... )
    >>> print(circuit)
    0: ───X^(a/4)───Y^(1 - a/2)───

    >>> sweep = alphaclops.Linspace(a, start=0, stop=3, length=4)
    >>> print(alphaclops.ListSweep(sweep))
    Sweep:
    {'a': 0.0}
    {'a': 1.0}
    {'a': 2.0}
    ...
    0: ───X^(<a/4>)───Y^(<1 - a/2>)───
    >>> expr_map
    alphaclops.ExpressionMap({a/4: <a/4>, 1 - a/2: <1 - a/2>})
    ...
    ...     print(c_flat, '=>', end=' ')
    ...     print(alphaclops.resolve_parameters(c_flat, params))
    0: ───X^(<a/4>)───Y^(<1 - a/2>)─── => 0: ───X^0───Y───
    0: ───X^(<a/4>)───Y^(<1 - a/2>)─── => 0: ───X^0.25───Y^0.5───
    0: ───X^(<a/4>)───Y^(<1 - a/2>)─── => 0: ───X^0.5───Y^0───
    0: ───X^(<a/4>)───Y^(<1 - a/2>)─── => 0: ───X^0.75───Y^-0.5───
...
class _ParamFlattener(resolver.ParamResolver):
    ...
    when it is used to resolve parameters with `alphaclops.resolve_parameters` or
    ...
    def value_of(
        self, value: Union['alphaclops.TParamKey', 'alphaclops.TParamValComplex'], recursive: bool = False
    ) -> 'alphaclops.TParamValComplex':
...
class ExpressionMap(dict):
    ...
    This is returned by `alphaclops.flatten`.  See `ExpressionMap.transform_sweep` and
    ...
    def transform_sweep(
        ...
        See `alphaclops.flatten` for an example.
    ...
    def __repr__(self) -> str:
        ...
        return f'alphaclops.ExpressionMap({super_repr})'


def _ensure_not_str(
    param: Union[sympy.Expr, 'alphaclops.TParamValComplex', str]
) -> Union[sympy.Expr, 'alphaclops.TParamValComplex']:
