...
from alphaclops.study.result import _pack_digits


def test_result_init():
    assert alphaclops.ResultDict(params=alphaclops.ParamResolver({}), measurements=None).repetitions == 0
    assert alphaclops.ResultDict(params=alphaclops.ParamResolver({}), measurements={}).repetitions == 0


def test_default_repetitions():
    class MyResult(alphaclops.Result):
...
def test_repr():
    v = alphaclops.ResultDict(
        params=alphaclops.ParamResolver({'a': 2}), measurements={'xy': np.array([[1, 0], [0, 1]])}
    )
    alphaclops.testing.assert_equivalent_repr(v)

    v = alphaclops.ResultDict(
        params=alphaclops.ParamResolver({'a': 2}),
...
def test_empty_measurements():
    assert alphaclops.ResultDict(params=None).repetitions == 0
    assert alphaclops.ResultDict(params=None, measurements={}).repetitions == 0
    assert alphaclops.ResultDict(params=None, records={}).repetitions == 0
...
def test_result_equality():
    ...
    et.add_equality_group(
        alphaclops.ResultDict(params=alphaclops.ParamResolver({}), measurements={'a': np.array([[0]] * 5)}),
        alphaclops.ResultDict(params=alphaclops.ParamResolver({}), records={'a': np.array([[[0]]] * 5)}),
    )
    et.add_equality_group(
        alphaclops.ResultDict(params=alphaclops.ParamResolver({}), measurements={'a': np.array([[0]] * 6)})
    )
    et.add_equality_group(
        alphaclops.ResultDict(params=alphaclops.ParamResolver({}), measurements={'a': np.array([[1]] * 5)})
    )
    et.add_equality_group(
        alphaclops.ResultDict(params=alphaclops.ParamResolver({}), records={'a': np.array([[[0], [1]]] * 5)})
    )


def test_result_addition_valid():
    a = alphaclops.ResultDict(
        params=alphaclops.ParamResolver({'ax': 1}),
        measurements={
            'q0': np.array([[0, 1], [1, 0], [0, 1]], dtype=bool),
            'q1': np.array([[0], [0], [1]], dtype=bool),
        },
    )
    b = alphaclops.ResultDict(
        params=alphaclops.ParamResolver({'ax': 1}),
...
def test_result_addition_invalid():
    ...
    b = alphaclops.ResultDict(
        params=alphaclops.ParamResolver({'bad': 1}),
        measurements={
            'q0': np.array([[0, 1], [1, 0], [0, 1]], dtype=bool),
            'q1': np.array([[0], [0], [1]], dtype=bool),
        },
    )
    c = alphaclops.ResultDict(
        params=alphaclops.ParamResolver({'ax': 1}),
        measurements={
...
def test_qubit_keys_for_histogram():
    ...
    circuit = alphaclops.Circuit(alphaclops.measure(a, b), alphaclops.X(c), alphaclops.measure(c))
    results = alphaclops.Simulator().run(program=circuit, repetitions=100)
...
def test_json_bit_packing_and_dtype(use_records: bool) -> None:
    ...
    params = alphaclops.ParamResolver({})
    if use_records:
        bits_result = alphaclops.ResultDict(params=params, records={'m': bits})
        digits_result = alphaclops.ResultDict(params=params, records={'m': digits})
    else:
        bits_result = alphaclops.ResultDict(params=params, measurements={'m': bits})
        digits_result = alphaclops.ResultDict(params=params, measurements={'m': digits})

    bits_json = alphaclops.to_json(bits_result)
    digits_json = alphaclops.to_json(digits_result)

    loaded_bits_result = alphaclops.read_json(json_text=bits_json)
    loaded_digits_result = alphaclops.read_json(json_text=digits_json)
...
def test_json_unpack_compat():
    ...
    old_json = """
        {
            "alphaclops_type": "ResultDict",
            "params": {
                "alphaclops_type": "ParamResolver",
                "param_dict": []
            },
            "measurements": {
                "m": {
    ...
    """
    result = alphaclops.read_json(json_text=old_json)
    assert result == alphaclops.ResultDict(
        params=alphaclops.ParamResolver({}),
        measurements={
            'm': np.array(
                [
