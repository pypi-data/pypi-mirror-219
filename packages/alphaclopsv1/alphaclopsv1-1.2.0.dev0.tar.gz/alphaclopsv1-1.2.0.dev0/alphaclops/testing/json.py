...
import alphaclops
from alphaclops._import import ModuleType

# This is the testing framework for json serialization
# The actual tests live in alphaclops.protocols.json_serialization_test.py.
#
...
#       alphaclops/google/json_test_data
...
#       alphaclops/google/json_test_data/spec.py
#  - resolver cache: a resolver cache for the exposed types - see for example
#       alphaclops/json_resolver_cache.py or alphaclops/google/json_resolver_cache.py
...
class ModuleJsonTestSpec:
    ...
    # The unqualified public name is different from the alphaclops_type field of the json object,
    # usually due to namespacing.
    custom_class_name_to_alphaclops_type: Dict[str, str] = dataclasses.field(default_factory=dict)
    ...
    def _get_all_public_classes(self) -> Iterator[Tuple[str, Type]]:
        for module in self.packages:
            for name, obj in inspect.getmembers(module):
                ...
                name = self.custom_class_name_to_alphaclops_type.get(name, name)
                yield name, obj
...
def assert_json_roundtrip_works(obj, text_should_be=None, resolvers=None):
    ...
    """
    buffer = io.StringIO()
    alphaclops.protocols.to_json(obj, buffer)
    ...
    buffer.seek(0)
    restored_obj = alphaclops.protocols.read_json(buffer, resolvers=resolvers)
