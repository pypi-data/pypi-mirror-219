...
def assert_consistent_channel(gate: Any, rtol: float = 1e-5, atol: float = 1e-8):
    ...
    assert alphaclops.has_kraus(gate), f"Given gate {gate!r} does not return True for alphaclops.has_kraus."
    kraus_ops = alphaclops.kraus(gate)
    assert alphaclops.is_cptp(kraus_ops=kraus_ops, rtol=rtol, atol=atol), (
...
def assert_consistent_mixture(gate: Any, rtol: float = 1e-5, atol: float = 1e-8):
    ...
    assert alphaclops.has_mixture(gate), f"Give gate {gate!r} does not return for alphaclops.has_mixture."
    mixture = alphaclops.mixture(gate)
