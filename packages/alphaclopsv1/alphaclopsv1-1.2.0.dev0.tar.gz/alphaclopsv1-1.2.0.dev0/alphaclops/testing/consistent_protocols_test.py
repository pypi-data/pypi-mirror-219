...
import alphaclops.testing.consistent_controlled_gate_op_test as controlled_gate_op_test


class GoodGate(alphaclops.testing.SingleQubitGate):
    def __init__(
        self, *, phase_exponent: Union[float, sympy.Expr], exponent: Union[float, sympy.Expr] = 1.0
    ) -> None:
        self.phase_exponent = alphaclops.canonicalize_half_turns(phase_exponent)
    ...
    def _unitary_(self) -> Union[np.ndarray, NotImplementedType]:
        ...
        z = alphaclops.unitary(alphaclops.Z ** self.phase_exponent)
        x = alphaclops.unitary(alphaclops.X ** self.exponent)
    ...
    def _apply_unitary_(self, args: alphaclops.ApplyUnitaryArgs) -> Union[np.ndarray, NotImplementedType]:
        if self.exponent != 1 or alphaclops.is_parameterized(self):
            return NotImplemented

        zero = alphaclops.slice_for_qubits_equal_to(args.axes, 0)
        one = alphaclops.slice_for_qubits_equal_to(args.axes, 1)
    ...
    def _decompose_(self, qubits: Sequence[alphaclops.Qid]) -> alphaclops.OP_TREE:
        assert len(qubits) == 1
        q = qubits[0]
        z = alphaclops.Z(q) ** self.phase_exponent
        x = alphaclops.X(q) ** self.exponent
        return z**-1, x, z
    ...
    def __pow__(self, exponent: Union[float, sympy.Expr]) -> 'GoodGate':
        new_exponent = alphaclops.mul(self.exponent, exponent, NotImplemented)
    ...
    def _is_parameterized_(self) -> bool:
        return alphaclops.is_parameterized(self.exponent) or alphaclops.is_parameterized(self.phase_exponent)

    def _parameter_names_(self) -> AbstractSet[str]:
        return alphaclops.parameter_names(self.exponent) | alphaclops.parameter_names(self.phase_exponent)
...
class BadGateDecompose(GoodGate):
    def _decompose_(self, qubits: Sequence[alphaclops.Qid]) -> alphaclops.OP_TREE:
        ...
        q = qubits[0]
        z = alphaclops.Z(q) ** self.phase_exponent
        x = alphaclops.X(q) ** (2 * self.exponent)
        if alphaclops.is_parameterized(z):
            # coverage: ignore
            return NotImplemented
        return z**-1, x, z
...
class BadGatePauliExpansion(GoodGate):
    def _pauli_expansion_(self) -> alphaclops.LinearDict[str]:
        return alphaclops.LinearDict({'I': 10})
...
class GoodEigenGate(alphaclops.EigenGate, alphaclops.testing.SingleQubitGate):
...
def test_assert_implements_consistent_protocols():
    ...
    with pytest.raises(AssertionError):
        alphaclops.testing.assert_implements_consistent_protocols(BadGateDecompose(phase_exponent=0.25))
    ...
    with pytest.raises(AssertionError):
        alphaclops.testing.assert_implements_consistent_protocols(BadGatePhaseBy(phase_exponent=0.25))
    ...
    with pytest.raises(AssertionError):
        alphaclops.testing.assert_implements_consistent_protocols(controlled_gate_op_test.BadGate())


def test_assert_eigengate_implements_consistent_protocols():
    alphaclops.testing.assert_eigengate_implements_consistent_protocols(
        GoodEigenGate,
...
def test_assert_commutes_magic_method_consistent_with_unitaries():
    gate_op = alphaclops.CNOT(*alphaclops.LineQubit.range(2))
    with pytest.raises(TypeError):
        alphaclops.testing.assert_commutes_magic_method_consistent_with_unitaries(gate_op)

    exponents = [sympy.Symbol('s'), 0.1, 0.2]
    gates = [alphaclops.ZPowGate(exponent=e) for e in exponents]
    alphaclops.testing.assert_commutes_magic_method_consistent_with_unitaries(*gates)

    alphaclops.testing.assert_commutes_magic_method_consistent_with_unitaries(alphaclops.Z, alphaclops.CNOT)
