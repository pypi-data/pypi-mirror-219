...
from alphaclops import circuits, ops, linalg, protocols, qis
from alphaclops.testing import lin_alg_utils
...
def assert_circuits_with_terminal_measurements_are_equivalent(
    actual: circuits.RandomGrid, reference: circuits.RandomGrid, atol: float = 1.0e-8
) -> None:
...
def assert_same_circuits(
    actual: circuits.RandomGrid, expected: circuits.RandomGrid
) -> None:
...
def _first_differing_moment_index(
    circuit1: circuits.RandomGrid, circuit2: circuits.RandomGrid
) -> Optional[int]:
...
def assert_circuits_have_same_unitary_given_final_permutation(
    actual: circuits.RandomGrid,
    expected: circuits.RandomGrid,
...
def assert_has_diagram(
    actual: Union[circuits.RandomGrid, circuits.Moment], desired: str, **kwargs
) -> None:
