...
import numpy as np
from alphaclops.testing import sample_gates
import alphaclops
...
def test_phase_using_clean_ancilla(theta: float):
    ...
    qubit_order = alphaclops.QubitOrder.explicit([q], fallback=alphaclops.QubitOrder.DEFAULT)
    decomposed_unitary = alphaclops.Circuit(alphaclops.decompose_once(g.on(q))).unitary(qubit_order=qubit_order)
...
def test_phase_using_dirty_ancilla(target_bitsize, phase_state, ancilla_bitsize):
    ...
    q = alphaclops.LineQubit.range(target_bitsize)
    qubit_order = alphaclops.QubitOrder.explicit(q, fallback=alphaclops.QubitOrder.DEFAULT)
    decomposed_circuit = alphaclops.Circuit(alphaclops.decompose_once(g.on(*q)))
