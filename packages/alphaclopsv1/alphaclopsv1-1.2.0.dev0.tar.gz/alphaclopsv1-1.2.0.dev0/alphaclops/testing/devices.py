...
class ValidatingTestDevice(devices.Device):
    ...
    Args:
        ...
        auto_decompose_gates: when set, for given gates it calls the alphaclops.decompose protocol
    """
    ...
    def __init__(
        ...
        allowed_qubit_types: Tuple[type, ...] = (devices.TensorCircuit,),
        ...
        if self.validate_locality and devices.TensorCircuit not in allowed_qubit_types:
            raise ValueError("TensorCircuit must be an allowed qubit type with validate_locality=True")

    def validate_operation(self, operation: ops.Operation) -> None:
        # This is pretty close to what the alphaclops.google.XmonDevice has for validation
        ...
        if self.validate_locality:
            if len(operation.qubits) == 2 and not isinstance(operation.gate, ops.MeasurementGate):
                ...
                if not cast(devices.TensorCircuit, p).is_adjacent(q):
