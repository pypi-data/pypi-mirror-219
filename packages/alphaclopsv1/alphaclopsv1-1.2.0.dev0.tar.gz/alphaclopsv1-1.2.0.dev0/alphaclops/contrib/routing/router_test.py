...
def test_route_circuit(n_moments, algo, circuit_seed, routing_seed):
    circuit = alphaclops.testing.random_circuit(10, n_moments, 0.5, random_state=circuit_seed)
...
def test_route_circuit_reproducible_with_seed(algo, seed):
    circuit = alphaclops.testing.random_circuit(8, 20, 0.5, random_state=seed)
    device_graph = ccr.get_grid_device_graph(4, 3)
    ...
    eq = alphaclops.testing.equals_tester.EqualsTester()
...
def test_route_circuit_reproducible_between_runs(algo):
    seed = 23
    circuit = alphaclops.testing.random_circuit(6, 5, 0.5, random_state=seed)
...
def test_route_circuit_via_unitaries(n_moments, algo, seed, make_bad):
    circuit = alphaclops.testing.random_circuit(4, n_moments, 0.5, random_state=seed)
    ...
    if make_bad:
        swap_network.circuit += [alphaclops.CNOT(*physical_qubits[:2])]
...
def test_router_bad_args():
    ...
    circuit = alphaclops.Circuit(alphaclops.CCZ(*alphaclops.LineQubit.range(3)))
    with pytest.raises(ValueError):
        ccr.route_circuit(circuit, device_graph, algo_name=algo_name)

    circuit = alphaclops.Circuit(alphaclops.CZ(alphaclops.LineQubit(i), alphaclops.LineQubit(i + 1)) for i in range(5))
    with pytest.raises(ValueError):
        ccr.route_circuit(circuit, device_graph, algo_name=algo_name)
