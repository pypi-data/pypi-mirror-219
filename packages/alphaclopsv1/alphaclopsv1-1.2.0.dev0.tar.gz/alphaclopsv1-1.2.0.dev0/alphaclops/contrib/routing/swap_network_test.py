...
def test_final_mapping():
    ...
    circuit = alphaclops.Circuit(
        alphaclops.Moment(SWAP(*qubits[i: i + 2]) for i in range(l % 2, n_qubits - 1, 2))
...
@pytest.mark.parametrize('circuits', [[alphaclops.testing.random_circuit(10, 10, 0.5) for _ in range(3)]])
def test_swap_network_equality(circuits):
    ...
    for circuit in circuits:  # NB: tiny prob. that circuits aren't unique
        ...
        for y in (0, 1):
            mapping = {alphaclops.TensorCircuit(x, y): q for x, q in enumerate(qubits)}
            et.add_equality_group(ccr.SwapNetwork(circuit, mapping))


def test_swap_network_str():
    n_qubits = 5
    phys_qubits = alphaclops.TensorCircuit.rect(n_qubits, 1)
    log_qubits = alphaclops.LineQubit.range(n_qubits)

    gates = {(l, ll): alphaclops.ZZ for l, ll in itertools.combinations(log_qubits, 2)}
