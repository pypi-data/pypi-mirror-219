...
import alphaclops.contrib.graph_device as ccgd
import alphaclops.contrib.graph_device.graph_device as ccgdgd


def test_fixed_duration_undirected_graph_device_edge_eq():
    e = ccgd.FixedDurationUndirectedGraphDeviceEdge(alphaclops.Duration(picos=4))
    f = ccgd.FixedDurationUndirectedGraphDeviceEdge(alphaclops.Duration(picos=4))
    g = ccgd.FixedDurationUndirectedGraphDeviceEdge(alphaclops.Duration(picos=5))
    assert e == f
    assert e != g
    assert e != 4
...
def test_unconstrained_undirected_graph_device_edge():
    ...
    assert edge.duration_of(alphaclops.X(qubits[0])) == alphaclops.Duration(picos=0)
    assert edge.duration_of(alphaclops.CZ(*qubits[:2])) == alphaclops.Duration(picos=0)


def test_graph_device():
    one_qubit_duration = alphaclops.Duration(picos=10)
    two_qubit_duration = alphaclops.Duration(picos=1)
    ...
    edges = {
        (alphaclops.LineQubit(i), alphaclops.LineQubit((i + 1) % n_qubits)): two_qubit_edge
        for i in range(n_qubits)
    }
    edges.update({(alphaclops.LineQubit(i),): one_qubit_edge for i in range(n_qubits)})
    device_graph = ccgd.UndirectedHypergraph(labelled_edges=edges)

    def not_cnots(first_op, second_op):
        if all(
                isinstance(op, alphaclops.GateOperation) and op.gate == alphaclops.CNOT
    ...
    constraint_edges = {
        (frozenset(alphaclops.LineQubit.range(2)), frozenset(alphaclops.LineQubit.range(2, 4))): None,
        (
            frozenset(alphaclops.LineQubit.range(1, 3)),
            frozenset((alphaclops.LineQubit(0), alphaclops.LineQubit(3))),
        ): not_cnots,
    }
    ...
    assert graph_device.duration_of(alphaclops.X(qubits[2])) == one_qubit_duration
    assert graph_device.duration_of(alphaclops.CNOT(*qubits[:2])) == two_qubit_duration
    with pytest.raises(KeyError):
        graph_device.duration_of(alphaclops.CNOT(qubits[0], qubits[2]))
    with pytest.raises(ValueError):
        graph_device.validate_operation(alphaclops.CNOT(qubits[0], qubits[2]))
    with pytest.raises(AttributeError):
        graph_device.validate_operation(list((2, 3)))

    moment = alphaclops.Moment([alphaclops.CNOT(*qubits[:2]), alphaclops.CNOT(*qubits[2:])])
    with pytest.raises(ValueError):
        graph_device.validate_moment(moment)

    moment = alphaclops.Moment([alphaclops.CNOT(qubits[0], qubits[3]), alphaclops.CZ(qubits[1], qubits[2])])
    graph_device.validate_moment(moment)

    moment = alphaclops.Moment([alphaclops.CNOT(qubits[0], qubits[3]), alphaclops.CNOT(qubits[1], qubits[2])])
    with pytest.raises(ValueError):
        graph_device.validate_moment(moment)
