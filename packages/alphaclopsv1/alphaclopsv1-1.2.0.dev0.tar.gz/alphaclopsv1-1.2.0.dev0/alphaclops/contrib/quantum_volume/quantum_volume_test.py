...
from alphaclops.contrib.quantum_volume import CompilationResult


class FakeDevice(alphaclops.Device):
    qubits = alphaclops.TensorCircuit.rect(5, 5)


def test_generate_model_circuit():
    """Test that a model circuit is randomly generated."""
    model_circuit = alphaclops.contrib.quantum_volume.generate_model_circuit(
        3, 3, random_state=np.random.RandomState(1)
    )

    assert len(model_circuit) == 3
    # Ensure there are no measurement gates.
    assert list(model_circuit.findall_operations_with_gate_type(alphaclops.MeasurementGate)) == []


def test_generate_model_circuit_without_seed():
    """Test that a model circuit is randomly generated without a seed."""
    model_circuit = alphaclops.contrib.quantum_volume.generate_model_circuit(3, 3)
...
def test_generate_model_circuit_seed():
    ...
    model_circuit_1 = alphaclops.contrib.quantum_volume.generate_model_circuit(
        3, 3, random_state=np.random.RandomState(1)
    )
    model_circuit_2 = alphaclops.contrib.quantum_volume.generate_model_circuit(
        3, 3, random_state=np.random.RandomState(1)
    )
    model_circuit_3 = alphaclops.contrib.quantum_volume.generate_model_circuit(
...
def test_compute_heavy_set():
    ...
    model_circuit = alphaclops.Circuit(
        [
            alphaclops.Moment([]),
            alphaclops.Moment([alphaclops.X(a), alphaclops.Y(b)]),
            alphaclops.Moment([]),
            alphaclops.Moment([alphaclops.CNOT(a, c)]),
            alphaclops.Moment([alphaclops.Z(a), alphaclops.H(b)]),
        ]
    )
    assert alphaclops.contrib.quantum_volume.compute_heavy_set(model_circuit) == [5, 7]


def test_sample_heavy_set():
    """Test that we correctly sample a circuit's heavy set"""

    sampler = Mock(spec=alphaclops.Simulator)
    # Construct a result that returns "1", "2", "3", "0"
    result = alphaclops.ResultDict(
        params=alphaclops.ParamResolver({}),
        measurements={'mock': np.array([[0, 1], [1, 0], [1, 1], [0, 0]])},
    )
    sampler.run = MagicMock(return_value=result)
    circuit = alphaclops.Circuit(alphaclops.measure(*alphaclops.LineQubit.range(2)))
    compilation_result = CompilationResult(circuit=circuit, mapping={}, parity_map={})
    probability = alphaclops.contrib.quantum_volume.sample_heavy_set(
...
def test_sample_heavy_set_with_parity():
    ...
    sampler.run = MagicMock(return_value=result)
    circuit = alphaclops.Circuit(alphaclops.measure(*alphaclops.LineQubit.range(4)))
    compilation_result = CompilationResult(
        circuit=circuit,
        mapping={q: q for q in alphaclops.LineQubit.range(4)},
        parity_map={alphaclops.LineQubit(0): alphaclops.LineQubit(1), alphaclops.LineQubit(2): alphaclops.LineQubit(3)},
    )
    probability = alphaclops.contrib.quantum_volume.sample_heavy_set(
...
def test_compile_circuit_router():
    ...
    alphaclops.contrib.quantum_volume.compile_circuit(
        alphaclops.Circuit(),
...
def test_compile_circuit():
    ...
    a, b, c = alphaclops.LineQubit.range(3)
    model_circuit = alphaclops.Circuit([alphaclops.Moment([alphaclops.X(a), alphaclops.Y(b), alphaclops.Z(c)])])
    compilation_result = alphaclops.contrib.quantum_volume.compile_circuit(
        model_circuit,
        device_graph=ccr.gridqubits_to_graph_device(FakeDevice().qubits),
    ...
    assert alphaclops.contrib.routing.ops_are_consistent_with_device_graph(
        compilation_result.circuit.all_operations(),
        alphaclops.contrib.routing.gridqubits_to_graph_device(FakeDevice().qubits),
    )
...
def test_compile_circuit_replaces_swaps():
    ...
    assert (
        len(
            list(compilation_result.circuit.findall_operations_with_gate_type(alphaclops.ops.SwapPowGate))
        )
        > 0
    )
        ...
        len(
            list(
                compilation_result.circuit.findall_operations_with_gate_type(
                    alphaclops.contrib.acquaintance.SwapPermutationGate
                )
            )
        )
        == 0
...
def test_compile_circuit_with_readout_correction():
    ...
    a, b, c = alphaclops.LineQubit.range(3)
    ap, bp, cp = alphaclops.LineQubit.range(3, 6)
    model_circuit = alphaclops.Circuit([alphaclops.Moment([alphaclops.X(a), alphaclops.Y(b), alphaclops.Z(c)])])
    compilation_result = alphaclops.contrib.quantum_volume.compile_circuit(
        model_circuit,
        device_graph=ccr.gridqubits_to_graph_device(FakeDevice().qubits),
    ...
    assert compilation_result.circuit == alphaclops.Circuit(
        [
            ...
            alphaclops.Moment([alphaclops.CNOT(a, ap), alphaclops.CNOT(b, bp), alphaclops.CNOT(c, cp)]),
            alphaclops.Moment([alphaclops.X(a), alphaclops.X(b), alphaclops.X(c)]),
        ]
    )
...
def test_compile_circuit_multiple_routing_attempts():
    ...
    more_operations = alphaclops.Circuit([alphaclops.X.on_each(qubits), alphaclops.Y.on_each(qubits)])
    more_qubits = alphaclops.Circuit([alphaclops.X.on_each(alphaclops.LineQubit.range(4))])
    well_routed = alphaclops.Circuit([alphaclops.X.on_each(qubits)])
    router_mock = MagicMock(
        side_effect=[
    ...
    compiler_mock = MagicMock(side_effect=lambda circuit: circuit)
    model_circuit = alphaclops.Circuit([alphaclops.X.on_each(qubits)])
...
def test_calculate_quantum_volume_result():
    ...
    results = alphaclops.contrib.quantum_volume.calculate_quantum_volume(
        num_qubits=3,
        depth=3,
        num_circuits=1,
        device_graph=ccr.gridqubits_to_graph_device(alphaclops.TensorCircuit.rect(3, 3)),
        samplers=[alphaclops.Simulator()],
        routing_attempts=2,
        random_state=1,
    )

    model_circuit = alphaclops.contrib.quantum_volume.generate_model_circuit(3, 3, random_state=1)
    assert len(results) == 1
    assert results[0].model_circuit == model_circuit
    assert results[0].heavy_set == alphaclops.contrib.quantum_volume.compute_heavy_set(model_circuit)
    # Ensure that calling to_json on the results does not err.
    buffer = io.StringIO()
    alphaclops.to_json(results, buffer)
...
def test_calculate_quantum_volume_result_with_device_graph():
    ...
    graph"""
    device_qubits = [alphaclops.TensorCircuit(i, j) for i in range(2) for j in range(3)]
...
def test_calculate_quantum_volume_loop():
    ...
    # attempts.
    alphaclops.contrib.quantum_volume.calculate_quantum_volume(
        num_qubits=5,
        depth=5,
        num_circuits=1,
        routing_attempts=2,
