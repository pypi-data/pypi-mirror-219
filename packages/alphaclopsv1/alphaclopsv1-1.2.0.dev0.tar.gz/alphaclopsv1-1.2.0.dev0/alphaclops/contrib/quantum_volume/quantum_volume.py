...
def generate_model_circuit(
    num_qubits: int, depth: int, *, random_state: 'alphaclops.RANDOM_STATE_OR_SEED_LIKE' = None
) -> alphaclops.Circuit:
    ...
    random_state = alphaclops.value.parse_random_state(random_state)
    ...
    for _ in range(depth):
        ...
        for k in range(0, num_qubits - 1, 2):
            ...
            special_unitary = alphaclops.testing.random_special_unitary(4, random_state=random_state)

            # Convert the decomposed unitary to alphaclops operations and add them to
            # the circuit.
            circuit.append(
                alphaclops.MatrixGate(special_unitary).on(
...
def compute_heavy_set(circuit: alphaclops.Circuit) -> List[int]:
...
class CompilationResult:
    circuit: alphaclops.Circuit
    mapping: Dict[alphaclops.Qid, alphaclops.Qid]
    parity_map: Dict[alphaclops.Qid, alphaclops.Qid]
...
def sample_heavy_set(
    ...
    *,
    repetitions=10_000,
    sampler: alphaclops.Sampler = alphaclops.Simulator(),
) -> float:
    ...
    circuit_copy = circuit + [alphaclops.measure(q) for q in sorted_qubits]
    ...
    results = results.agg(lambda meas: alphaclops.value.big_endian_bits_to_int(meas), axis=1)
...
def process_results(
    mapping: Dict[alphaclops.Qid, alphaclops.Qid],
    parity_mapping: Dict[alphaclops.Qid, alphaclops.Qid],
    trial_result: alphaclops.Result,
) -> pd.DataFrame:
    ...
    inverse_mapping: Dict[alphaclops.Qid, alphaclops.Qid] = {v: k for k, v in mapping.items()}
...
def compile_circuit(
    ...
    *,
    ...
    compiler: Optional[Callable[[alphaclops.Circuit], alphaclops.Circuit]] = None,
    ...
    parity_map: Dict[alphaclops.Qid, alphaclops.Qid] = {}  # original -> parity
    if add_readout_error_correction:
        ...
        for idx, qubit in enumerate(sorted(compiled_circuit.all_qubits())):
            ...
            parity_qubit = alphaclops.LineQubit(qubit_num)
            compiled_circuit.append(alphaclops.X(qubit))
            compiled_circuit.append(alphaclops.CNOT(qubit, parity_qubit))
            compiled_circuit.append(alphaclops.X(qubit))
    ...
    if router is None and routing_algo_name is None:
        ...
        # Github issue: https://github.com/quantumlib/alphaclops/issues/2967
    ...
    def replace_swap_permutation_gate(op: 'alphaclops.Operation', _):
        if isinstance(op.gate, alphaclops.contrib.acquaintance.SwapPermutationGate):
            return [op.gate.swap_gate.on(*op.qubits)]
        return op

    routed_circuit = alphaclops.map_operations_and_unroll(
...
def prepare_circuits(
    *,
    ...
    depth: int,
    num_circuits: int,
    random_state: 'alphaclops.RANDOM_STATE_OR_SEED_LIKE' = None,
) -> List[Tuple[alphaclops.Circuit, List[int]]]:
...
def execute_circuits(
    *,
    device_graph: nx.Graph,
    samplers: List[alphaclops.Sampler],
    circuits: List[Tuple[alphaclops.Circuit, List[int]]],
    routing_attempts: int,
    compiler: Optional[Callable[[alphaclops.Circuit], alphaclops.Circuit]] = None,
