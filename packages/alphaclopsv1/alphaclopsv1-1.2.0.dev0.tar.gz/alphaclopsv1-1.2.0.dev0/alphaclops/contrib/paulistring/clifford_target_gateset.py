...
from alphaclops import ops, transformers, protocols, linalg
...
def _matrix_to_clifford_op(
    mat: np.ndarray, qubit: 'alphaclops.Qid', *, atol: float
...
def _matrix_to_pauli_string_phasors(
    mat: np.ndarray, qubit: 'alphaclops.Qid', *, keep_clifford: bool, atol: float
) -> ops.OP_TREE:
    rotations = transformers.single_qubit_matrix_to_pauli_rotations(mat, atol)
...
class CliffordTargetGateset(transformers.TwoQubitCompilationTargetGateset):
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            single_qubit_target: Specifies the decomposition strategy for single qubit gates.
                SINGLE_QUBIT_CLIFFORDS: Decompose all single qubit gates to
                    `alphaclops.SingleQubitCliffordGate`.
                PAULI_STRING_PHASORS_AND_CLIFFORDS: Accept both `alphaclops.SingleQubitCliffordGate` and
                    `alphaclops.PauliStringPhasorGate`; but decompose unknown gates into
        ...
        gates: List[Union['alphaclops.Gate', Type['alphaclops.Gate']]] = [ops.CZ, ops.MeasurementGate]
    ...
    def _decompose_single_qubit_operation(
        self, op: 'alphaclops.Operation', _
    ...
    def postprocess_transformers(self) -> List['alphaclops.TRANSFORMER']:
        ...
        def rewriter(o: 'alphaclops.CircuitOperation'):
