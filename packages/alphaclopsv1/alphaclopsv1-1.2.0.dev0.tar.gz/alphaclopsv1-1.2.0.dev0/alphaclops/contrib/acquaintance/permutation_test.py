...
def test_swap_permutation_gate():
    ...
    a, b = alphaclops.NamedQubit('a'), alphaclops.NamedQubit('b')
    gate = cca.SwapPermutationGate()
    assert gate.num_qubits() == 2
    circuit = alphaclops.Circuit(gate(a, b))
    circuit = alphaclops.expand_composite(circuit, no_decomp=no_decomp)
    assert tuple(circuit.all_operations()) == (alphaclops.SWAP(a, b),)

    no_decomp = lambda op: (isinstance(op, alphaclops.GateOperation) and op.gate == alphaclops.CZ)
    circuit = alphaclops.Circuit(cca.SwapPermutationGate(alphaclops.CZ)(a, b))
    circuit = alphaclops.expand_composite(circuit, no_decomp=no_decomp)
    assert tuple(circuit.all_operations()) == (alphaclops.CZ(a, b),)

    assert alphaclops.commutes(gate, alphaclops.ZZ)
    with pytest.raises(TypeError):
        alphaclops.commutes(gate, alphaclops.CCZ)
...
def test_validate_permutation_errors():
    ...
    gate = cca.SwapPermutationGate()
    assert alphaclops.circuit_diagram_info(gate, default=None) is None
...
def test_get_logical_operations():
    a, b, c, d = qubits = alphaclops.LineQubit.range(4)
...
def test_linear_permutation_gate(n_elements, n_permuted):
    qubits = alphaclops.LineQubit.range(n_elements)
    ...
    for swap in alphaclops.flatten_op_tree(alphaclops.decompose_once_with_qubits(gate, qubits)):
        assert isinstance(swap, alphaclops.GateOperation)
...
def test_linear_permutation_gate_equality(permutation_sets):
    swap_gates = [alphaclops.SWAP, alphaclops.CNOT]
...
def test_linear_permutation_gate_pow_inverse(num_qubits, permutation, inverse):
    ...
    assert alphaclops.inverse(permutation_gate) == inverse_gate


def test_display_mapping():
    indices = [4, 2, 0, 1, 3]
    qubits = alphaclops.LineQubit.range(len(indices))
...
@pytest.mark.parametrize(
    ...
    [
        alphaclops.Circuit(
            ...
            for qubit_pair in [random.sample(alphaclops.LineQubit.range(10), 2) for _ in range(20)]
        )
        for _ in range(4)
    ],
)
...
def test_uses_consistent_swap_gate():
    ...
    circuit = alphaclops.Circuit([cca.SwapPermutationGate()(a, b), cca.SwapPermutationGate()(a, b)])
    assert cca.uses_consistent_swap_gate(circuit, alphaclops.SWAP)
    assert not cca.uses_consistent_swap_gate(circuit, alphaclops.CZ)
    circuit = alphaclops.Circuit(
        [cca.SwapPermutationGate(alphaclops.CZ)(a, b), cca.SwapPermutationGate(alphaclops.CZ)(a, b)]
    )
    assert cca.uses_consistent_swap_gate(circuit, alphaclops.CZ)
    assert not cca.uses_consistent_swap_gate(circuit, alphaclops.SWAP)
    circuit = alphaclops.Circuit(
        [cca.SwapPermutationGate()(a, b), cca.SwapPermutationGate(alphaclops.CZ)(a, b)]
    )
    assert not cca.uses_consistent_swap_gate(circuit, alphaclops.SWAP)
    assert not cca.uses_consistent_swap_gate(circuit, alphaclops.CZ)
...
def test_swap_gate_eq():
    assert cca.SwapPermutationGate() == cca.SwapPermutationGate(alphaclops.SWAP)
    assert cca.SwapPermutationGate() != cca.SwapPermutationGate(alphaclops.CZ)
    assert cca.SwapPermutationGate(alphaclops.CZ) == cca.SwapPermutationGate(alphaclops.CZ)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        cca.SwapPermutationGate(alphaclops.SWAP),
        cca.SwapPermutationGate(alphaclops.CZ),
    ],
)
