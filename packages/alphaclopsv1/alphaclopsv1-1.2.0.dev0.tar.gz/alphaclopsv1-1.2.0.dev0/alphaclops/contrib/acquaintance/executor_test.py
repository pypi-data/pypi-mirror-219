...
def test_executor_explicit():
    ...
    with pytest.raises(TypeError):
        bad_strategy = alphaclops.Circuit(alphaclops.X(qubits[0]))
...
) -> Dict[Tuple[alphaclops.Qid, ...], alphaclops.Gate]:

    return {
        Q: alphaclops.DiagonalGate(np.random.random(2 ** acquaintance_size))
        for Q in combinations(alphaclops.LineQubit.range(num_qubits), acquaintance_size)
    }
...
def test_executor_random(
    num_qubits: int, acquaintance_size: int, gates: Dict[Tuple[alphaclops.Qid, ...], alphaclops.Gate]
):
    qubits = alphaclops.LineQubit.range(num_qubits)
    circuit = cca.complete_acquaintance_strategy(qubits, acquaintance_size)

    logical_circuit = alphaclops.Circuit([g(*Q) for Q, g in gates.items()])
...
def test_acquaintance_operation():
    n = 5
    physical_qubits = tuple(alphaclops.LineQubit.range(n))
    logical_qubits = tuple(alphaclops.NamedQubit(s) for s in ascii_lowercase[:n])
    ...
    for logical_indices in (logical_qubits, int_indices):
        ...
        assert alphaclops.circuit_diagram_info(op) == alphaclops.CircuitDiagramInfo(wire_symbols=wire_symbols)
