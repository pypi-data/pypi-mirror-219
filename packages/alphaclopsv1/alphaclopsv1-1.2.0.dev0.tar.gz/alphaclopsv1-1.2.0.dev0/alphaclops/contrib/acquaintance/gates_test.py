...
def test_acquaintance_gate_repr():
    assert (
        ...
        == 'alphaclops.contrib.acquaintance.AcquaintanceOpportunityGate'
        '(num_qubits=2)'
    )


def test_acquaintance_gate_text_diagram_info():
    qubits = [alphaclops.NamedQubit(s) for s in 'xyz']
    circuit = alphaclops.Circuit([alphaclops.Moment([cca.acquaint(*qubits)])])
...
def test_acquaintance_gate_unknown_qubit_count():
    assert alphaclops.circuit_diagram_info(cca.acquaint, default=None) is None


def test_swap_network_gate():
    qubits = tuple(alphaclops.NamedQubit(s) for s in alphabet)
    ...
    swap_network = alphaclops.Circuit(swap_network_op)
    expected_text_diagram = """
a: ───×(0,0)───
      │
b: ───×(0,1)───
...
f: ───×(2,1)───
    ...
    swap_network = alphaclops.expand_composite(swap_network, no_decomp=no_decomp)
    expected_text_diagram = """
a: ───█───────╲0╱───█─────────────────█───────────╲0╱───█───────0↦1───
      │       │     │                 │           │     │       │
b: ───█───█───╲1╱───█───█─────────────█───█───────╲1╱───█───█───1↦0───
...
def test_swap_network_gate_from_ops():
    ...
    operations = [
        ...
        alphaclops.CZ(*qubits[1:3]),
        alphaclops.CCZ(*qubits[4:7]),
        alphaclops.CCZ(*qubits[7:]),
    ]
    acquaintance_size = 3
    ...
    swap_network = cca.SwapNetworkGate.from_operations(
        qubits, operations, acquaintance_size, alphaclops.ZZ
    )
    circuit = alphaclops.Circuit(swap_network(*qubits))
...
def test_swap_network_decomposition():
    ...
    operations = alphaclops.decompose_once_with_qubits(swap_network_gate, qubits)
...
class OtherOperation(alphaclops.Operation):
    def __init__(self, qubits: Sequence[alphaclops.Qid]) -> None:
    ...
    def with_qubits(self, *new_qubits: alphaclops.Qid) -> 'OtherOperation':
...
def test_operations_to_part_lens():
    ...
    ops = [alphaclops.CZ(*qubits[1:3]), alphaclops.XX(*qubits[3:5])]
    part_lens = cca.gates.operations_to_part_lens(qubits, ops)
    assert part_lens == (1, 2, 2, 1)

    ops = alphaclops.CZ(qubits[1], qubits[3])
    with pytest.raises(ValueError):
        cca.gates.operations_to_part_lens(qubits, ops)

    ops = [alphaclops.CZ(*qubits[1:3]), alphaclops.CZ(*qubits[2:4])]
    with pytest.raises(ValueError):
        cca.gates.operations_to_part_lens(qubits, ops)
