...
from alphaclops.work import _MeasurementSpec, BitstringAccumulator, group_settings_greedy, InitObsSetting
...
def test_with_parameterized_layers():
    ...
    circuit = alphaclops.Circuit([alphaclops.H.on_each(*qs), alphaclops.CZ(qs[0], qs[1]), alphaclops.CZ(qs[1], qs[2])])
    ...
    *_, xlayer, ylayer, measurelayer = circuit2.moments
    for op in xlayer.operations:
        assert isinstance(op.gate, alphaclops.XPowGate)
        assert op.gate.exponent.name.endswith('-Xf')
    for op in ylayer.operations:
        assert isinstance(op.gate, alphaclops.YPowGate)
        assert op.gate.exponent.name.endswith('-Yf')
    for op in measurelayer:
        assert isinstance(op.gate, alphaclops.MeasurementGate)
...
def test_get_params_for_setting():
    ...
    init_state = alphaclops.KET_PLUS(a) * alphaclops.KET_ZERO(b)
    observable = alphaclops.X(a) * alphaclops.Y(b)
    setting = cw.InitObsSetting(init_state=init_state, observable=observable)
    padded_setting = _pad_setting(setting, qubits=qubits)
    assert padded_setting.init_state == alphaclops.KET_PLUS(a) * alphaclops.KET_ZERO(b) * alphaclops.KET_ZERO(c)
    assert padded_setting.observable == alphaclops.X(a) * alphaclops.Y(b) * alphaclops.Z(c)
    assert init_state == alphaclops.KET_PLUS(a) * alphaclops.KET_ZERO(b)
    assert observable == alphaclops.X(a) * alphaclops.Y(b)
    ...
    circuit = alphaclops.Circuit(alphaclops.I.on_each(*qubits))
    ...
    psi = alphaclops.Simulator().simulate(circuit, param_resolver=params)
    ma = alphaclops.Z(a).expectation_from_state_vector(psi.final_state_vector, qubit_map=psi.qubit_map)
    mb = alphaclops.Z(b).expectation_from_state_vector(psi.final_state_vector, qubit_map=psi.qubit_map)
    mc = alphaclops.Z(c).expectation_from_state_vector(psi.final_state_vector, qubit_map=psi.qubit_map)
...
def test_params_and_settings():
    ...
    (q,) = qubits
    tests = [
        (alphaclops.KET_ZERO, alphaclops.Z, 1),
        (alphaclops.KET_ONE, alphaclops.Z, -1),
        (alphaclops.KET_PLUS, alphaclops.X, 1),
        (alphaclops.KET_MINUS, alphaclops.X, -1),
        (alphaclops.KET_IMAG, alphaclops.Y, 1),
        (alphaclops.KET_MINUS_IMAG, alphaclops.Y, -1),
        (alphaclops.KET_ZERO, alphaclops.Y, 0),
    ]
    ...
    for init, obs, coef in tests:
        ...
        circuit = circuit[:-1]  # remove measurement so we can compute <Z>
        psi = alphaclops.Simulator().simulate(circuit, param_resolver=params)
        z = alphaclops.Z(q).expectation_from_state_vector(psi.final_state_vector, qubit_map=psi.qubit_map)
...
def test_subdivide_meas_specs():
    ...
    setting = cw.InitObsSetting(
        init_state=alphaclops.KET_ZERO(q0) * alphaclops.KET_ZERO(q1), observable=alphaclops.X(q0) * alphaclops.Y(q1)
    )
...
class _MockBitstringAccumulator(BitstringAccumulator):
    def __init__(self):
        super().__init__(
            meas_spec=None,
            simul_settings=None,
            qubit_to_index={q: i for i, q in enumerate(alphaclops.LineQubit.range(5))},
        )
...
def _set_up_meas_specs_for_testing():
    ...
    bsa = cw.BitstringAccumulator(
        meas_spec, [], {q: i for i, q in enumerate(alphaclops.LineQubit.range(3))}
    )
...
def test_measure_grouped_settings(with_circuit_sweep, checkpoint, tmpdir):
    ...
    if with_circuit_sweep:
        ss = alphaclops.Linspace('a', 0, 1, 12)
    else:
        ss = None
...
def test_measure_grouped_settings_read_checkpoint(tmpdir):
    ...
    setting = cw.InitObsSetting(init_state=alphaclops.KET_ZERO(q), observable=alphaclops.Z(q))
    grouped_settings = {setting: [setting]}
    circuit = alphaclops.Circuit(alphaclops.I.on_each(*qubits))
    ...
    results = alphaclops.read_json(f'{tmpdir}/obs.json')
    (result,) = results  # one group
...
@pytest.mark.parametrize(
    ['circuit', 'observable'],
    [
        (alphaclops.Circuit(alphaclops.X(Q) ** 0.2), alphaclops.Z(Q)),
        (alphaclops.Circuit(alphaclops.X(Q) ** -0.5, alphaclops.Z(Q) ** 0.2), alphaclops.Y(Q)),
        (alphaclops.Circuit(alphaclops.Y(Q) ** 0.5, alphaclops.Z(Q) ** 0.2), alphaclops.X(Q)),
    ],
)
...
def test_measure_observable_grouper(grouper):
    circuit = alphaclops.Circuit(alphaclops.X(Q) ** 0.2)
    observables = [alphaclops.Z(Q), alphaclops.Z(alphaclops.NamedQubit('q2'))]
