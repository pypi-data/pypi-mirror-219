...
from alphaclops.work.observable_settings import _MeasurementSpec


def test_get_real_coef():
    q0 = alphaclops.LineQubit(0)
    assert _check_and_get_real_coef(alphaclops.Z(q0) * 2, atol=1e-8) == 2
    assert _check_and_get_real_coef(alphaclops.Z(q0) * complex(2.0), atol=1e-8) == 2
    with pytest.raises(ValueError):
        _check_and_get_real_coef(alphaclops.Z(q0) * 2.0j, atol=1e-8)
...
def test_obs_vals_from_measurements():
    ...
    obs = alphaclops.Z(a) * alphaclops.Z(b) * 10
...
def test_observable_measured_result():
    ...
    omr = cw.ObservableMeasuredResult(
        setting=cw.InitObsSetting(
            init_state=alphaclops.Z(a) * alphaclops.Z(b), observable=alphaclops.Y(a) * alphaclops.Y(b)
        ),
        mean=0,
        variance=5**2,
        repetitions=4,
    ...
    assert omr.stddev == 5
    assert omr.observable == alphaclops.Y(a) * alphaclops.Y(b)
    assert omr.init_state == alphaclops.Z(a) * alphaclops.Z(b)

    alphaclops.testing.assert_equivalent_repr(omr)

    assert omr.as_dict() == {
        'init_state': alphaclops.Z(a) * alphaclops.Z(b),
        'observable': alphaclops.Y(a) * alphaclops.Y(b),
        'mean': 0,
        'variance': 25,
        'repetitions': 4,
        'param.phi': 52,
...
def example_bsa() -> 'cw.BitstringAccumulator':
    ...
    bsa = cw.BitstringAccumulator(
        ...
        simul_settings=[
            setting,
            cw.InitObsSetting(init_state=setting.init_state, observable=alphaclops.X(q0)),
            cw.InitObsSetting(init_state=setting.init_state, observable=alphaclops.Y(q1)),
        ],
...
def test_bitstring_accumulator_strings(example_bsa):
    ...
    settings = cw.observables_to_settings(
        [alphaclops.X(q0), alphaclops.Y(q1), alphaclops.X(q0) * alphaclops.Y(q1)], qubits=[q0, q1]
    )
...
def test_bitstring_accumulator_equality():
    ...
    qubit_to_index = {a: 0, b: 1}
    obs = alphaclops.Z(a) * alphaclops.Z(b) * 10
    setting = cw.InitObsSetting(init_state=alphaclops.Z(a) * alphaclops.Z(b), observable=obs)
...
def _get_ZZ_Z_Z_bsa_constructor_args():
    ...
    settings = list(
        cw.observables_to_settings(
            [alphaclops.Z(a) * alphaclops.Z(b) * 7, alphaclops.Z(a) * 5, alphaclops.Z(b) * 3], qubits=[a, b]
        )
    )
...
def test_bitstring_accumulator_stats():
    ...
    bad_obs = [alphaclops.X(a) * alphaclops.X(b)]
...
def test_bitstring_accumulator_stats_2():
    ...
    b = alphaclops.NamedQubit('b')
    qubit_to_index = {a: 0, b: 1}
    settings = list(cw.observables_to_settings([alphaclops.Z(a) * 5, alphaclops.Z(b) * 3], qubits=[a, b]))
    meas_spec = _MeasurementSpec(settings[0], {})
...
def test_bitstring_accumulator_errors():
    q0, q1 = alphaclops.LineQubit.range(2)
    settings = cw.observables_to_settings(
        [alphaclops.X(q0), alphaclops.Y(q0), alphaclops.Z(q0), alphaclops.Z(q0) * alphaclops.Z(q1)], qubits=[q0, q1]
    )
...
def _get_mock_readout_calibration(qa_0=90, qa_1=10, qb_0=91, qb_1=9):
    ...
    b = alphaclops.NamedQubit('b')
    qubit_to_index = {a: 0, b: 1}
    ro_settings = list(cw.observables_to_settings([alphaclops.Z(a), alphaclops.Z(b)], qubits=[a, b]))
    (ro_meas_spec_setting,) = list(
        cw.observables_to_settings([alphaclops.Z(a) * alphaclops.Z(b)], qubits=[a, b])
    )
...
def test_readout_correction():
    ...
    qubit_to_index = {a: 0, b: 1}
    settings = list(
        cw.observables_to_settings([alphaclops.X(a) * alphaclops.Y(b), alphaclops.X(a), alphaclops.Y(b)], qubits=[a, b])
    )
    meas_spec = _MeasurementSpec(settings[0], {})
