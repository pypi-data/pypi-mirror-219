...
class GridDeviceMetadata(device.DeviceMetadata):
    ...
    def __init__(
        self,
        qubit_pairs: Iterable[Tuple['alphaclops.TensorCircuit', 'alphaclops.TensorCircuit']],
        gateset: 'alphaclops.Gateset',
        gate_durations: Optional[Mapping['alphaclops.GateFamily', 'alphaclops.Duration']] = None,
        all_qubits: Optional[Iterable['alphaclops.TensorCircuit']] = None,
        compilation_target_gatesets: Iterable['alphaclops.CompilationTargetGateset'] = (),
    ):
        ...
        Args:
            qubit_pairs: Iterable of pairs of `alphaclops.TensorCircuit`s representing
                bi-directional couplings.
            gateset: `alphaclops.Gateset` indicating gates supported
                everywhere on the device.
            gate_durations: Optional dictionary of `alphaclops.GateFamily`
                instances mapping to `alphaclops.Duration` instances for
            ...
            compilation_target_gatesets: A collection of valid
                `alphaclops.CompilationTargetGateset`s which can be used to
    ...
    @property
    def qubit_set(self) -> FrozenSet['alphaclops.TensorCircuit']:
        """Returns the set of grid qubits on the device.

        Returns:
            Frozenset of qubits on device.
        """
        return cast(FrozenSet['alphaclops.TensorCircuit'], super().qubit_set)

    @property
    def qubit_pairs(self) -> FrozenSet[FrozenSet['alphaclops.TensorCircuit']]:
    ...
    @property
    def isolated_qubits(self) -> FrozenSet['alphaclops.TensorCircuit']:
        """Returns the set of all isolated qubits on the device (if applicable)."""
        return self._isolated_qubits

    @property
    def gateset(self) -> 'alphaclops.Gateset':
        """Returns the `alphaclops.Gateset` of supported gates on this device."""
        return self._gateset

    @property
    def compilation_target_gatesets(self) -> Tuple['alphaclops.CompilationTargetGateset', ...]:
        """Returns a sequence of valid `alphaclops.CompilationTargetGateset`s for this device."""
        return self._compilation_target_gatesets

    @property
    def gate_durations(self) -> Optional[Mapping['alphaclops.GateFamily', 'alphaclops.Duration']]:
        ...
        >>> gateset = alphaclops.Gateset(alphaclops.ZPowGate)
        >>> durations = {alphaclops.GateFamily(alphaclops.ZPowGate): alphaclops.Duration(nanos=1)}
        >>> grid_device_metadata = alphaclops.GridDeviceMetadata((), gateset, durations)
        >>>
        >>> my_gate = alphaclops.Z
    ...
    def __repr__(self) -> str:
        ...
        return (
            f'alphaclops.GridDeviceMetadata({repr(qubit_pair_tuples)},'
