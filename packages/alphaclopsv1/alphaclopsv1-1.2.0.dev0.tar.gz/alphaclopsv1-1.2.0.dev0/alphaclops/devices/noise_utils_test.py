...
def test_op_identifier():
    op_id = OpIdentifier(alphaclops.XPowGate)
    assert alphaclops.X(alphaclops.LineQubit(1)) in op_id
    assert alphaclops.Rx(rads=1) in op_id


def test_op_identifier_subtypes():
    gate_id = OpIdentifier(alphaclops.Gate)
    xpow_id = OpIdentifier(alphaclops.XPowGate)
    x_on_q0_id = OpIdentifier(alphaclops.XPowGate, alphaclops.LineQubit(0))
...
def test_op_id_str():
    op_id = OpIdentifier(alphaclops.XPowGate, alphaclops.LineQubit(0))
    assert str(op_id) == "<class 'alphaclops.ops.common_gates.XPowGate'>(alphaclops.LineQubit(0),)"
    assert repr(op_id) == (
        "alphaclops.devices.noise_utils.OpIdentifier(alphaclops.ops.common_gates.XPowGate, alphaclops.LineQubit(0))"
    )


def test_op_id_swap():
    q0, q1 = alphaclops.LineQubit.range(2)
    base_id = OpIdentifier(alphaclops.CZPowGate, q0, q1)
    swap_id = OpIdentifier(base_id.gate_type, *base_id.qubits[::-1])
    assert alphaclops.CZ(q0, q1) in base_id
    assert alphaclops.CZ(q0, q1) not in swap_id
    assert alphaclops.CZ(q1, q0) not in base_id
    assert alphaclops.CZ(q1, q0) in swap_id
