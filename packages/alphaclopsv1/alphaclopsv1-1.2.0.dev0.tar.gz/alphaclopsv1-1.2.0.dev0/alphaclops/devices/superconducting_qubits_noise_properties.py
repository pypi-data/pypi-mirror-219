...
from alphaclops import _compat, ops, devices
from alphaclops.devices import noise_utils
...
class SuperconductingQubitsNoiseProperties(devices.NoiseProperties, abc.ABC):
    ...
    Args:
        ...
        t1_ns: Dict[alphaclops.Qid, float] of qubits to their T_1 time, in ns.
        tphi_ns: Dict[alphaclops.Qid, float] of qubits to their T_phi time, in ns.
        readout_errors: Dict[alphaclops.Qid, np.ndarray] of qubits to their readout
    ...
    t1_ns: Dict['alphaclops.Qid', float]
    tphi_ns: Dict['alphaclops.Qid', float]
    readout_errors: Dict['alphaclops.Qid', List[float]]
    gate_pauli_errors: Dict[noise_utils.OpIdentifier, float]

    validate: bool = True
    _qubits: List['alphaclops.Qid'] = field(init=False, default_factory=list)
    ...
    @property
    def qubits(self) -> List['alphaclops.Qid']:
    ...
    def build_noise_models(self) -> List['alphaclops.NoiseModel']:
        noise_models: List['alphaclops.NoiseModel'] = []
        ...
        if self.readout_errors:
            added_measure_errors: Dict[noise_utils.OpIdentifier, 'alphaclops.Operation'] = {}
