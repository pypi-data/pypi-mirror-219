...
class _BaseLineQid(ops.Qid):
    ...
    def is_adjacent(self, other: 'alphaclops.Qid') -> bool:
        ...
        Args:
            other: `alphaclops.Qid` to test for adjacency.
...
class LineQid(_BaseLineQid):
    ...
    One can construct new `alphaclops.LineQid`s by adding or subtracting integers:

    >>> alphaclops.LineQid(1, dimension=2) + 3
    alphaclops.LineQid(4, dimension=2)

    >>> alphaclops.LineQid(2, dimension=3) - 1
    ...
    def for_gate(val: Any, start: int = 0, step: int = 1) -> List['LineQid']:
        ...
        Args:
            val: Any value that supports the `alphaclops.qid_shape` protocol.  Usually
                a gate.
            start: The x coordinate of the first `LineQid`.
            step: The amount to increment each x coordinate.
        """
        # Avoids circular import.
        from alphaclops.protocols.qid_shape_protocol import qid_shape

        return LineQid.for_qid_shape(qid_shape(val), start=start, step=step)

    def __repr__(self) -> str:
        return f"alphaclops.LineQid({self.x}, dimension={self.dimension})"
...
class LineQubit(_BaseLineQid):
    ...
    One can construct new `alphaclops.LineQubit`s by adding or subtracting integers:

    >>> alphaclops.LineQubit(1) + 3
    alphaclops.LineQubit(4)

    >>> alphaclops.LineQubit(2) - 1
    ...
    def __repr__(self) -> str:
        return f"alphaclops.LineQubit({self.x})"
