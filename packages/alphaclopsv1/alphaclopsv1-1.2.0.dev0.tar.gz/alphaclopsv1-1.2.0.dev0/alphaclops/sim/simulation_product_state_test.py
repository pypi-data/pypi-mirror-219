...
class EmptyQuantumState(alphaclops.QuantumStateRepresentation):
...
class EmptySimulationState(alphaclops.SimulationState):
...
q0, q1, q2 = qs3 = alphaclops.LineQubit.range(3)
qs2 = alphaclops.LineQubit.range(2)


def create_container(
    qubits: Sequence['alphaclops.Qid'], split_untangled_states=True
) -> alphaclops.SimulationProductState[EmptySimulationState]:
    state_map: Dict[Optional['alphaclops.Qid'], EmptySimulationState] = {}
    log = alphaclops.ClassicalDataDictionaryStore()
    ...
    return alphaclops.SimulationProductState(
...
def test_subcircuit_entanglement_causes_join():
    state = create_container(qs2)
    assert len(set(state.values())) == 3
    state.apply_operation(alphaclops.CircuitOperation(alphaclops.FrozenCircuit(alphaclops.CNOT(q0, q1))))
    assert len(set(state.values())) == 2
    assert state[q0] is state[q1]
...
def test_subcircuit_entanglement_causes_join_in_subset():
    ...
    assert len(set(state.values())) == 3
    assert state[q0] is state[q1]
    state.apply_operation(alphaclops.CircuitOperation(alphaclops.FrozenCircuit(alphaclops.CNOT(q0, q2))))
    assert len(set(state.values())) == 2
...
def test_identity_does_not_join():
    state = create_container(qs2)
    assert len(set(state.values())) == 3
    state.apply_operation(alphaclops.IdentityGate(2)(q0, q1))
    assert len(set(state.values())) == 3
...
def test_identity_fallback_does_not_join():
    state = create_container(qs2)
    assert len(set(state.values())) == 3
    state._act_on_fallback_(alphaclops.I, (q0, q1))
    assert len(set(state.values())) == 3
    assert state[q0] is not state[q1]
    assert state[q0] is not state[None]
...
def test_subcircuit_identity_does_not_join():
    state = create_container(qs2)
    assert len(set(state.values())) == 3
    state.apply_operation(alphaclops.CircuitOperation(alphaclops.FrozenCircuit(alphaclops.IdentityGate(2)(q0, q1))))
    assert len(set(state.values())) == 3
    assert state[q0] is not state[q1]
...
def test_subcircuit_measurement_causes_split():
    ...
    state.apply_operation(alphaclops.CNOT(q0, q1))
    assert len(set(state.values())) == 2
    state.apply_operation(alphaclops.CircuitOperation(alphaclops.FrozenCircuit(alphaclops.measure(q0))))
    assert len(set(state.values())) == 3
    assert state[q0] is not state[q1]
...
def test_subcircuit_measurement_causes_split_in_subset():
    state = create_container(qs3)
    state.apply_operation(alphaclops.CNOT(q0, q1))
    state.apply_operation(alphaclops.CNOT(q0, q2))
    assert len(set(state.values())) == 2
    state.apply_operation(alphaclops.CircuitOperation(alphaclops.FrozenCircuit(alphaclops.measure(q0))))
    assert len(set(state.values())) == 3
    assert state[q0] is not state[q1]
    state.apply_operation(alphaclops.CircuitOperation(alphaclops.FrozenCircuit(alphaclops.measure(q1))))
    assert len(set(state.values())) == 4
    assert state[q0] is not state[q1]
...
def test_reset_causes_split():
    ...
    state.apply_operation(alphaclops.CNOT(q0, q1))
    assert len(set(state.values())) == 2
    state.apply_operation(alphaclops.reset(q0))
    assert len(set(state.values())) == 3
    assert state[q0] is not state[q1]
    assert state[q0] is not state[None]
...
def test_measurement_of_all_qubits_causes_split():
    ...
    state.apply_operation(alphaclops.CNOT(q0, q1))
    assert len(set(state.values())) == 2
    state.apply_operation(alphaclops.measure(q0, q1))
    assert len(set(state.values())) == 3
    assert state[q0] is not state[q1]
    assert state[q0] is not state[None]
...
def test_swap_does_not_merge():
    ...
    old_q0 = state[q0]
    old_q1 = state[q1]
    state.apply_operation(alphaclops.SWAP(q0, q1))
    assert len(set(state.values())) == 3
...
def test_half_swap_does_merge():
    state = create_container(qs2)
    state.apply_operation(alphaclops.SWAP(q0, q1) ** 0.5)
    assert len(set(state.values())) == 2
    assert state[q0] is state[q1]
...
def test_swap_after_entangle_reorders():
    state = create_container(qs2)
    state.apply_operation(alphaclops.CX(q0, q1))
    assert len(set(state.values())) == 2
...
def test_act_on_gate_does_not_join():
    state = create_container(qs2)
    assert len(set(state.values())) == 3
    alphaclops.act_on(alphaclops.X, state, [q0])
    assert len(set(state.values())) == 3
    assert state[q0] is not state[q1]
    assert state[q0] is not state[None]
