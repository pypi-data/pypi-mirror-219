...
def test_unitary_fallback():
    class UnitaryXGate(alphaclops.testing.SingleQubitGate):
        def _unitary_(self):
    ...
    original_tableau = alphaclops.CliffordTableau(num_qubits=3)
    ...
    alphaclops.act_on(UnitaryXGate(), args, [alphaclops.LineQubit(1)])
    assert args.tableau == alphaclops.CliffordTableau(num_qubits=3, initial_state=2)
...
def test_cannot_act():
    ...
    class NoDetailsSingleQubitGate(alphaclops.testing.SingleQubitGate):
        pass
    ...
    with pytest.raises(TypeError, match="no _num_qubits_ or _qid_shape_"):
        alphaclops.act_on(NoDetails(), args, [alphaclops.LineQubit(1)])

    with pytest.raises(TypeError, match="Failed to act"):
        alphaclops.act_on(NoDetailsSingleQubitGate(), args, [alphaclops.LineQubit(1)])
...
def test_copy():
    ...
    args1 = args.copy()
    assert isinstance(args1, alphaclops.CliffordTableauSimulationState)
