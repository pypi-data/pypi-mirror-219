...
class ChatBase(object):
    ...
    @diskcache(location=os.getenv('INSERT_VECTOR_CACHE', '~/.cache/insert_vector_cache'))
    def create_index(self, docs, **kwargs):
        Milvus.from_documents(docs, **{**self.vdb_kwargs, **kwargs})

    def llm_qa(self, query: str, k: int = 4, threshold: float = 0.5, **kwargs: Any):
