...
from ..utils import patch, patchable, NoelmodConfig
...
class Client:
    ...
    async def listen(
        ...
        if type(listener_type) != ListenerTypes:
            raise TypeError(
                ...
                " value from noelmode.listen.ListenerTypes"
            )
        ...
        try:
            ...
            if callable(NoelmodConfig.timeout_handler):
                NoelmodConfig.timeout_handler(
                    identifier, listener_data, timeout
                )
            elif NoelmodConfig.throw_exceptions:
    ...
    def stop_listening(
        ...
        if callable(NoelmodConfig.stopped_handler):
            NoelmodConfig.stopped_handler(identifier, listener)
        elif NoelmodConfig.throw_exceptions:
...
@patch(noelgram.handlers.message_handler.MessageHandler)
    ...
    async def resolve_future(self, client, message, *args):
        ...
        if listener_does_match:
            if not listener["future"].done():
                ...
                raise noelgram.StopPropagation
        else:
            await self.registered_handler(client, message, *args)


@patch(noelgram.handlers.callback_query_handler.CallbackQueryHandler)
class CallbackQueryHandler:
    ...
    async def check(self, client, query):
        ...
        if NoelmodConfig.unallowed_click_alert:
            ...
            if (permissive_listener and not listener) and permissive_listener[
                ...
                alert = (
                    ...
                    == str
                    else NoelmodConfig.unallowed_click_alert_text
                )
...
@patch(noelgram.types.messages_and_media.message.Message)
class Message(noelgram.types.messages_and_media.message.Message):
    @patchable
...
@patch(noelgram.types.user_and_chats.chat.Chat)
class Chat(noelgram.types.Chat):
    @patchable
...
@patch(noelgram.types.user_and_chats.user.User)
class User(noelgram.types.User):
    @patchable
    def listen(self, *args, **kwargs):
