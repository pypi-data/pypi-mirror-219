...
#
# Utility package from https://github.com/thuml/CDAN/blob/master/pytorch/
#
...
def image_train(resize_size=256, crop_size=224, alexnet=False, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]):
  if not alexnet:
...
def image_test(resize_size=256, crop_size=224, alexnet=False, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]):
  if not alexnet:
    normalize = transforms.Normalize(mean=mean,
                                   std=std)
  else:
...
def image_test_10crop(resize_size=256, crop_size=224, alexnet=False, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]):
    if not alexnet:
        normalize = transforms.Normalize(mean=mean,
                                   std=std)
    else:
