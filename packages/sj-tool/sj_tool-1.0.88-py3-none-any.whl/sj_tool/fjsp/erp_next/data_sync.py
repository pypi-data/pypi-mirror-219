...
def transfer_sale_order(conn, owner="Administrator", **kwargs):
    ...
    # 销售订单
    sale_orders = select_data_from_table(conn, table_sale, columns=["name", "customer", "urgent_order"])
    # 销售订单明细
    ...
    for sale_order in sale_orders:
        ...
        urgent_order = sale_order["urgent_order"] == 1
...
def transfer_inventory_old(conn, owner="Administrator", **kwargs):
    ...
    purchase_data = select_data_from_table(
        ...
        columns=["item_code", "item_name", "qty", "received_qty", "schedule_date", "expected_delivery_date"],
        where_clause=f"owner = %s",
        where_params=(owner,),
    )
    # 出库
...
def transfer_process_table(conn, owner="Administrator", **kwargs):
    ...
    process_table = select_data_from_table(
        conn,
        table_process,
        columns=["operation_number", "name", "operation_name"],
        where_clause=f"owner = %s",
        where_params=(owner,),
    )
    results = []
...
def transfer_machine_table(conn, owner="Administrator", **kwargs):
    ...
    machine_table = select_data_from_table(
        conn,
        table_machine,
        columns=["workstation_id", "workstation_name"],
        where_clause=f"owner = %s",
        where_params=(owner,),
    )
...
def transfer_atb_schedule(conn, owner="Administrator", **kwargs):
    print("同步排程ATB数据。。。")
    st = time.perf_counter()

    atb_keymap = get_atb_key_map()
    df_keys = [item[0] for item in atb_keymap]
    db_keys = [item[1] for item in atb_keymap]

    table_name = "tabATB schedule"
    data = select_data_from_table(conn, table_name, columns="*", where_clause=f"owner = %s", where_params=(owner,))
    df = pd.DataFrame(data)
    df.rename(columns=dict(zip(db_keys, df_keys)), inplace=True)
...
def transfer_atb_plan(conn, owner="Administrator", **kwargs):
    df_sale_order = kwargs.get('')
    print("同步生产计划数据")
    st = time.perf_counter()
...
def get_atb_key_map():
    # 每个元素的第一个表示算法用的字段，第二个表示mysql中的对应字段名
    keymap = [
        # ("ss_number", "ss_number"),
        ("orderId", "order_id"),
        ("worderID", "worder_id"),
...
def get_data(owner, need_data_list: List[str], **kwargs):
    """
    注意：获取atb时需要传入 is_schedule=True/False，表示是否为排程数据还是生产计划数据
    :param owner：用户邮箱
    ...
    key_dict = {
        ...
        "atb_schedule": transfer_atb_schedule,
        "atb_plan": transfer_atb_schedule,
    }
    result_dict = {}
    conn = connect_erp_db()
    try:
