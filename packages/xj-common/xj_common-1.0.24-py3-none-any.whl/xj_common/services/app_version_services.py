...
class AppVersionServices():
    ...
    def validate_version(system: str = None, version: str = None, version_type=None):
        ...
        # 检查当前版本
        app_version_obj = AppVersion.objects.filter(app_system=system, version=version, version_type=version_type).values(
        ...
        # 检查最新版本
        news_version = AppVersion.objects.filter(app_system=system).order_by("-push_time").values(
            "id", "app_system", "version", "version_type", "store_name", "store_url",
            "is_force_update", "push_time", "created_time", "version_detail"
        ).first()

        if app_version_obj.get("version") == news_version.get("version"):
            app_version_obj["is_newest"] = True
            app_version_obj["is_force_update"] = False
        else:
            app_version_obj["is_newest"] = False
            app_version_obj["is_force_update"] = news_version.get("is_force_update", False)
            app_version_obj["newest_version"] = news_version.get("version")
    ...
    def app_version_add(params: dict = None, **kwargs):
        ...
        try:
            params = format_params_handle(
                ...
                filter_filed_list=[
                    "app_system|str", "version|str", "version_type|str", "store_name|str", "store_url|str",
