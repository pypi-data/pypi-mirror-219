...
class ConnectionUsedTooOften(SteadyDBError):
    """Database cursor is invalid."""


def connect(
        creator, datasource=None, maxusage=None, setsession=None, standalone=True, manage=False,
        write=None, failures=None, ping=1, closeable=True, *args, **kwargs):
    ...
    return SteadyDBConnection(
        creator, datasource, maxusage, setsession, standalone, manage, write,
...
class SteadyDBConnection:
    ...
    def __init__(
            self, creator, datasource=None, maxusage=None, setsession=None, standalone=True, manage=False, write=None,
        ...
        if self._standalone and not callable(self._creator):
        ...
        try:
            if self._standalone:
                ...
                if self._write is None:
                    con = self._creator.connection(self.manage)
                elif self._write:
                    con = self._creator.write_connection(self.manage)
                else:
                    con = self._creator.read_connection(self.manage)
