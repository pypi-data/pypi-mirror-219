...
class KrxKindWeb:
    ...
    def fetch_list_search(self, keyword: str, from_dt: datetime, to_dt: datetime,
                          company_id: str = None, company_name: str = "", time_sleep=0.3):
        results = []
        page = 1
        total_page = 1

        while page <= total_page:
            items = self._fetch_list_search(keyword=keyword, from_dt=from_dt, to_dt=to_dt,
                                            company_id=company_id, company_name=company_name, page=page)
            ...
            if items is not None:
                ...
                if self.cache is not None:
                    ## check cache
                    cache_key = f"krxweb_list_search_{keyword}_{from_dt}_{to_dt}_{company_id}"
                    keys = [x.get("doc_id") for x in result]
                    diff = self.cache.differential(cache_key, keys)
                    self.logger.debug(f"diff : {diff}, cached keys : {len(self.cache.keys())}")
                    diff_ratio = float(len(diff)) / float(len(result)) * 100 if len(result) > 0 else float(0)
    ...
    def _fetch_list_search(self, keyword: str, from_dt: datetime, to_dt: datetime,
                           company_id: str = None, company_name: str = '', page: int = 1):
        headers = {
            'authority': 'kind.krx.co.kr',
            'accept': 'text/html, */*; q=0.01',
            'accept-language': 'en-US,en;q=0.9',
        ...
        acode = 'A' + company_id[-6:] if company_id is not None else ''
        ...
        data = {
            ...
            'forward': 'details_sub',
            'repIsuSrtCd': acode,
            'oldSearchCorpName': company_name,
