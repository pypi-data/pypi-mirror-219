...
class Trainer:
    ...
    Args:
        ...
        by_epoch, warmup_t, warmup_by_epoch, warmup_mode, warmup_init_lr, warmup_factor: Refer to
            the documentation of :class:`cdu.lr_scheduler.LRWarmupScheduler`.
    ...
    def _default_setup(self) -> None:
        ...
        setup_logger("cdu", output_dir=self.work_dir, rank=get_rank())
    ...
    def train_one_iter(self) -> None:
        ...
        Subclass :class:`cdu.trainer.Trainer` and implement your own :meth:`train_one_iter`
    ...
    def load_checkpoint(self, path: Optional[str] = None, auto_resume: bool = False):
        ...
        if path:
            ...
            checkpoint = torch.load(path, map_location="cdu")
        else:
