...
class HelpStyles(Styles):
    ...
    TEXT_ACCENT = FS(bold=True)                            # *important*   | * | needs to be wrapped in *        # noqa
                                                             #  '--option' | ' | auto-detection if wrapped in '  # noqa
                                                             #  "--option" | " | ⤒ static-length version         # noqa
                                                             #  “* / +”    |“ ”| ⤒ static-length any* version    # noqa
# fmt: on
...
class HelpFormatter(click.HelpFormatter):
    ...
    def __init__(
        ...
        self.help_filters: t.List[pt.StringReplacer] = [
            ...
            pt.filter.StringReplacerChain(
                compile(R"“(.+?)”", flags=re.DOTALL),
                pt.OmniPadder(),
                NamedGroupsRefilter(
                    compile(R'( )“(.+?)”( )', flags=re.DOTALL),
                    {"": HelpStyles.TEXT_EXAMPLE},  # any
                ),
            ),
            RegexValRefilter(
            ...
            NamedGroupsRefilter(  # &process-&(gre)p
