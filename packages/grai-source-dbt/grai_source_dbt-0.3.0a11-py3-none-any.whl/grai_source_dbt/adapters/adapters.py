...
def build_grai_metadata_from_node(
    current: AllDbtNodeTypes, version: Literal["v1"] = "v1"
) -> TableMetadata:
    """
...
def build_grai_metadata_from_edge(
    ...
    if current.edge_type == EdgeMetadataTypeLabels.table_to_table:
        ...
        raise NotImplementedError(
            f"No supported metadata implementation for edge_type {current.edge_type.value}"
        )
...
def adapt_to_client(current: Any, version: Any) -> None:
    ...
    """
    raise NotImplementedError(
        f"No objects of type `{type(current)}` have a `{version}` client adapter."
    )


@adapt_to_client.register
def adapt_table_to_client(
    current: AllDbtNodeTypes, source: SourceSpec, version: Literal["v1"]
) -> SourcedNodeV1:
    """
