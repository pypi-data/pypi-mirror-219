        ...
        for frame in frames:
            ...
            if message:
                if MessageIds.has_value(message.header.messageId):
                    ret.append(decode_to_specific_message(message.header, decoded, reference_voltage=self.re))
...
class AMSSerialModule():
    ...
    def processing_oversampling(self, slot_id:int, ratio:int, output_samples:int):
        ...
        Args:
            ...
            :ratio (int): Ratio of oversampling. For example value 4 means that there will be 4 times less samples in the output buffer.
