...
use prompt_graph_core::proto2::{ChangeValue, ChangeValueWithCounter, DispatchResult, File, InputProposal, Item, MemoryAction, NodeWillExecute, PromptGraphConstant, PromptGraphMap, PromptGraphNodeCode, PromptGraphNodeCodeSourceCode, PromptGraphNodeComponent, PromptGraphNodeMemory, PromptGraphNodeObservation, PromptGraphNodePrompt, PromptGraphParameterNode, SerializedValue, SupportedEmebddingModel, SupportedSourceCodeLanguages, SupportedVectorDatabase, PromptGraphNodeCustom};
...
use tokio::time;
use prompt_graph_core::execution_router_wasm::ExecutionRouter;

use crate::initialize_nodes;
...
use crate::db_operations::update_head_counter_for_branch;
...
pub struct Executor {
    ...
    // pub running_code: HashMap<u64, ScheduledEvent>,
}
...
impl Executor {
    ...
    pub async fn run(&mut self) -> Result<()> {
        ...
        loop {
            ...
            match idx {
                ...
                0 => {
                    debug!("Handling graph mutation: {:?}", event);
                ...
                // Pending change
                1 => {
                    debug!("Handling change event: {:?}", event);
                    if let sled::Event::Insert { key, value } = event {
                        self.progress().await?;
                    }
                }
    ...
    async fn handle_graph_mutations(&mut self, file: &mut File, branch: u64, counter: u64) -> Result<()> {
        ...
        // TODO: need to record where
        // TODO: branch the point of execution and use that to branch
        // TODO: should we record changes to graphs in the database on a node level?
        // TODO: updates to the graph must include the identity of each node changed

        // Merge each of the nodes into clean file
        let updated_nodes = self.clean_definition_graph.merge_file(file).unwrap();
        // TODO: if a node is updated, we need to branch from that point

        // Propagate changes based on node types that run at time of introduction
        for item in self.clean_definition_graph.node_by_name.values() {
            let (changes, input_proposals) = match (&item.core.as_ref().unwrap(), &item.item.as_ref().unwrap()) {
                (core, item::Item::NodeParameter(n))  => initialize_nodes::node_parameter_init(n, core, branch, counter),
                (core, item::Item::NodeObservation(n)) => initialize_nodes::node_observation_init(n, core, branch, counter),
                (core, item::Item::Constant(c)) => initialize_nodes::node_constant_init(c, core, branch, counter),
                (core, item::Item::NodeComponent(n)) => initialize_nodes::node_component_init(&n, core, branch, counter),
                _ => { continue }
            };

            for input_proposal in input_proposals {
                insert_input_proposal(&self.tree, input_proposal);
            }
            for change in changes {
                insert_new_change_value_with_counter(&self.tree, change);
            }
        }
        ...
        for node in &file.nodes {
            ...
            for (idx, opt_query_path) in query_paths.iter().enumerate() {
                if let Some(query_paths) = opt_query_path {
                    ...
                    // TODO: get this introduced node's query
                    // TODO: if we do have a query instead we want to query the current system state as of the introduced horizon
                    if let Some(change_values_used_in_execution) = evaluate_changes_against_node(&mut state, &query_paths) {
                        ...
                        self.process_node_will_execute(branch, &node_will_execute, node.clone()).await?;
                    }
                } else {
    ...
    pub async fn progress(&mut self) -> Result<()> {
        ...
        // This is handling the mutations generated by the above before exiting
    ...
    pub async fn progress_mutations(&mut self) -> Result<()> {
        ...
        if let Some(((branch, counter), mut file)) = get_next_pending_graph_mutation_on_branch(&self.tree, 0) {
            println!("Found pending graph mutation: {:?}", file);
    ...
    pub async fn progress_changes(&mut self) -> Result<()> {
        if let Some(is_playing) = get_is_playing_status(&self.tree) {
            if !is_playing {
                return Ok(());
            }
        ...
        if let Some(change) = get_next_pending_change_on_branch(&self.tree, 0) {
            println!("Found pending change: {:?}", change);

            // Resolve change against the current branch first
            resolve_pending_change(&self.tree, change.branch, change.monotonic_counter);

            // WARNING: Order matters here
            // We evaluate divergent branches first in order to avoid the case where they reflect against their source and think the change is already resolved

            // For each divergent branch from the change's home branch where the divergence occurred at or after the change's counter
            for divergent_branch in get_branch(&self.tree, change.branch).unwrap().divergent_branches.iter() {
                if divergent_branch.diverges_at_counter < change.monotonic_counter {
                    continue;
                }
                self.exec_change(change.clone(), divergent_branch.branch).await.unwrap();
            }

            // Exec the change in the context of its home branch
    ...
    async fn process_node_will_execute(&mut self, branch: u64, node_will_execute: &NodeWillExecute, item: Item) -> Result<()> {
        ...
        let result_counter = update_head_counter_for_branch(&self.tree, branch).unwrap();
        ...
        let filled_values = match (item.core.as_ref().unwrap(), item.item.as_ref().unwrap()) {
            ...
            (c, item::Item::NodeCode(n)) => {
                node_code::node::execute_node_code(&node_will_execute, &n, c, &output_namespaces)
            }
            (c, item::Item::NodePrompt(n)) => {
                node_prompt::node::execute_node_prompt(&node_will_execute, &n, c, &output_namespaces).await
            }
            (c, item::Item::NodeMemory(n)) => {
                node_memory::node::execute_node_memory(&node_will_execute, &n, c, &output_namespaces).await?
            }
            ...
            (c, item::Item::NodeLoader(n)) => {
                node_loader::node::execute_node_loader(&node_will_execute, &n, c, &output_namespaces)?
            }
...
mod tests {
    ...
    use crate::db_operations::get_head_counter_for_branch;
    ...
    // TODO: test that we can branch execution and re-run a node on both branches
    #[tokio::test]
    async fn test_branching_execution_and_isolation_between_branches() {
        let db = Config::new().temporary(true).flush_every_ms(None).open().unwrap();
        let tree = db.open_tree("test").unwrap();
        let mut file = File {
            id: "test".to_string(),
            nodes: vec![
                ...
                gen_item_non_deterministic()
            ],
        };

        // TODO: consider a setting on nodes that they can introduce x number of branches
        // TODO: create a branch after pausing execution after the first node execution
        let mut executor = Executor::new(tree);
        executor.handle_graph_mutations(&mut file, 0, 0).await;
        let v: Vec<_> = scan_all_pending_changes(&executor.tree).collect();
        executor.progress_changes().await;
        let current_head_counter = get_head_counter_for_branch(&executor.tree, 0);

        pause_execution_at_frame(&executor.tree, 1);
        let new_branch_id = create_branch(&executor.tree, 0, current_head_counter);
        play_execution_at_frame(&executor.tree, 0);
        ...
        assert_ne!(v[0].filled_values[0].value.as_ref().unwrap().val, v[1].filled_values[0].value.as_ref().unwrap().val);
    }
