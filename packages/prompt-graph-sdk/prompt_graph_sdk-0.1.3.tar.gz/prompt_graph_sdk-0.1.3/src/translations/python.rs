...
impl Chidori {
    ...
    fn start_server<'a>(mut self_: PyRefMut<'_, Self>, py: Python<'a>) -> PyResult<&'a PyAny> {
        let url_server = self_.url.clone();
        std::thread::spawn(move || {
            let no_protocol = url_server.replace("http://", "").replace("https://", "").replace("localhost", "127.0.0.1");
            let result = run_server(no_protocol);
            match result {
                Ok(_) => { },
                Err(e) => {
