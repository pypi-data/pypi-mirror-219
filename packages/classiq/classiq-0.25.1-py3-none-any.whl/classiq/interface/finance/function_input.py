...
from classiq.interface.generator.expressions.finance_functions import FinanceFunction
...
class FinanceFunctionInput(pydantic.BaseModel):
    f: Union[int, str, FinanceFunction] = pydantic.Field(
    ...
    def _validate_finance_function(
        cls, f: Union[int, str, FinanceFunction]
    ) -> FinanceFunction:
        if isinstance(f, FinanceFunction):
            return f
        if isinstance(f, int):
            return FinanceFunction(f)
        return FinanceFunction.from_string(f)
    ...
    def _validate_tail_probability_assignment_for_shortfall(
        ...
        if values.get("f") == FinanceFunction.SHORTFALL and not tail_probability:
