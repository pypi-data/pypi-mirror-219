...
class Client():
    ...
    def putfo(self, files):
        ...
        for (path, content) in files:
            ...
            yield client.putfo(content, str(path))
...
def distcrab(download=False, dump=False, ip='192.168.1.200', port=22, username='root', password=None, firmware=None, version=None, branch=None):
    ...
    if download:
        ...
        yield from client.putfo(src)
        yield from client.exec_command([
            '/bin/rm -rf /usr/local/crab/ && /bin/mkdir -p /usr/local/crab/ && /bin/sync && /bin/tar -xvJf /tmp/firmware.bin -C /usr/local/crab/ && /bin/rm -rf /tmp/firmware.bin && /bin/sync && /usr/local/bin/elite_local_start.sh',
        ])
