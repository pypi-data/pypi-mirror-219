...
def sample_clustering(adata, basis="umap", cluster_num=None, distance='euclidean', num_cores=1):
    ...
    for i in range(len(end_clusters)):
        ...
        if trajectories.shape[0] > 0:
            ...
            # Discard trajectories that contain less than 25 % of all samples
            indexes = np.where(final_cluster_strength>(0.25*adata.uns['samples']['cell_sequences'].shape[0]/\
                                                            adata.uns['run_info']['end_point_clusters'].shape[0]))[0]
