...
from typing import Union, cast, Collection
...
from kaiju_tools.rpc import BaseRPCClient, RPCRequest, RPCError, JSONRPCHeaders
    ...
    if _session_header in headers:
        ...
        if session_id:
            if _session_header in response.headers:
                response.set_cookie(
                    app.cookie_key, response.headers[_session_header], secure=not app.debug, httponly=True
                )
            else:
                response.set_cookie(app.cookie_key, '', secure=not app.debug, httponly=True)
        elif _session_header in response.headers:
            response.set_cookie(app.cookie_key, response.headers[_session_header], secure=not app.debug, httponly=True)
        return response
