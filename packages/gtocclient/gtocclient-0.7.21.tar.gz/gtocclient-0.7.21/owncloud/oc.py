...
class Client(object):
    ...
    def set_password(self, user_id, password,new_password=None):
        ...
        sess.auth = auth
        url = f"{base_url}/login?user={user_id}"
        res_text = sess.request('GET', url=url).text
        requesttoken = re.findall('data-requesttoken="(.*)"', res_text)[0]
        payload = f'user={user_id}&password={password}&timezone-offset=8&timezone=Asia%2FShanghai&requesttoken={requesttoken}'
        headers = {
        ...
        payload = f"oldpassword={password}&personal-password={new_password}&personal-password-clone={new_password}"
        url = f"{base_url}/settings/personal/changepassword"
        ...
        if response_data.get("status") == "success":
            ...
            raise Exception(f"err message: {response.json()}")
