...
def read_storage(bucket: str, path: str):
    import atlas
    url = atlas.supabase.storage.get_bucket(bucket).get_public_url(path)
...
def to_storage(df: DataFrame, bucket: str, storage_path: str):
    ...
    atlas.supabase.storage.get_bucket(bucket).remove([storage_path])
    atlas.supabase.storage.get_bucket(bucket).upload(storage_path, path)
