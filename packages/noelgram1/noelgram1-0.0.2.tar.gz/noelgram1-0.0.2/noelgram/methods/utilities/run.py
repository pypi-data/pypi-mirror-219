...
import noelgram
from noelgram.methods.utilities.idle import idle
...
class Run:
    def run(
        ...
        :meth:`~noelgram.Client.start`, :meth:`~noelgram.idle` and :meth:`~noelgram.Client.stop` in sequence.
        ...
        operation. This is almost the same as :py:obj:`asyncio.run` except for the fact that Noelgram's ``run`` uses the
        current event loop instead of a new one.

        If you want to run multiple clients at once, see :meth:`noelgram.compose`.
