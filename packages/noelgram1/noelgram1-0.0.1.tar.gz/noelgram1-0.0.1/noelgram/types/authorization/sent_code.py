...
class SentCode(Object):
    ...
    Parameters:
        type (:obj:`~noelgram.enums.SentCodeType`):
            Type of the current sent code.

        phone_code_hash (``str``):
            Confirmation code identifier useful for the next authorization steps (either
            :meth:`~noelgram.Client.sign_in` or :meth:`~noelgram.Client.sign_up`).

        next_type (:obj:`~noelgram.enums.NextCodeType`, *optional*):
            Type of the next code to be sent with :meth:`~noelgram.Client.resend_code`.

        timeout (``int``, *optional*):
            Delay in seconds before calling :meth:`~noelgram.Client.resend_code`.
    """
