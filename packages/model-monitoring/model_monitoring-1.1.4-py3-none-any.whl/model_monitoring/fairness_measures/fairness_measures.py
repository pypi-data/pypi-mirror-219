...
def predictive_parity_difference(y_true, y_pred, fair_attr, unpriv_group=0, multiclass_agg_mode="max", **kwargs):
...
def equal_opportunity_difference(y_true, y_pred, fair_attr, unpriv_group=0, multiclass_agg_mode="max", **kwargs):
    ...
    if len(np.unique(y_true)) > 2:
        ...
        try:
            ...
            pr_rec_group = recall_score(y_true[fair_attr != unpriv_group], y_pred[fair_attr != unpriv_group], **kwargs)
        # More Fairness Attributes
        except Exception:
            unpr_rec_group = recall_score(
                y_true[(fair_attr == unpriv_group).all(axis=1)],
...
def average_odds_difference(y_true, y_pred, fair_attr, unpriv_group=0, multiclass_agg_mode="max", **kwargs):
...
def false_positive_rate(y_true, y_pred, **kwargs):
    ...
    """
    return 1 - specificity_score(y_true, y_pred, **kwargs)
