...
# ? RULE-BASED PATH
RULE_BASED_PATH = f"{CURRENT_PATH}/../rule_base/students"

NICKNAME_PATH = f"{CURRENT_PATH}/../name_split/nicknames_boundary.parquet"
...
PRONOUN_GENDER_RB_PATH = (
    f"{CURRENT_PATH}/../gender_model/rule_base/pronoun_gender_dict.parquet"
)
...
NAME_ELEMENT_PATH = f"{CURRENT_PATH}/../name_split/name_elements.parquet"

MF_NAME_GENDER_RULE = pd.read_parquet(
    f"{CURRENT_PATH}/../rule_base/gender/mfname.parquet"
)

BEFORE_FNAME_GENDER_RULE = pd.read_parquet(
    f"{CURRENT_PATH}/../rule_base/gender/before_fname.parquet"
)
...
WITHOUT_ACCENT_ELEMENT_POPULARITY = (
    pd.read_parquet(
        f"{CURRENT_PATH}/../name_split/popular_name_without_accent_elements.parquet"
    )
...
WITH_ACCENT_ELEMENT_POPULARITY = (
    pd.read_parquet(
        f"{CURRENT_PATH}/../name_split/popular_name_with_accent_elements.parquet"
    )
...
LASTNAME_IS_FIRSTNAME = pd.read_parquet(
    f"{CURRENT_PATH}/../name_split/lastname_is_firstname.parquet"
)["with_accent"].to_list()
