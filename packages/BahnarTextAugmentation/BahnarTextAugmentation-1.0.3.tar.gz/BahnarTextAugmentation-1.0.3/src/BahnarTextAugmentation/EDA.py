...
def da_eda(proportion, num_aug, synonym_dictionary, stopwords, keep_old=False):
    def wrap(sentences, *args):
        alpha = proportion
        da_pairs = []
        for i in range(len(sentences)):
            ba_sentence = sentences[i][1]
            vi_sentence = sentences[i][0]

            augmented_ba_sentences = eda(ba_sentence, synonym_dictionary, stopwords,alpha_sr=alpha, alpha_ri=alpha, alpha_rs=alpha, p_rd=alpha, num_aug=num_aug)
            augmented_sentences = list(set(augmented_ba_sentences))
            for augmented_sentence in augmented_sentences:
                da_pairs.append([vi_sentence, augmented_sentence])
