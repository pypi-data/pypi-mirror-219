...
class  Transformer(nn.Module):
    ...
    def get_blocks(self, token, E, cfg):
        ...
        if  token == 'mlp' and ( len(token) == 3 or token[3].isdigit() ):
            parts    = token[3:].split('_')  if len(token[3:]) > 1 else []
            E2       = int(parts[0]) if len(parts) > 0 else E
            stretch  = int(parts[1]) if len(parts) > 1 else 4
            block    = Residual(
                MLP(input=E, output=E2, stretch=stretch),
                E=E, Eout=E2, mult = cfg.mult, scale=cfg.scale,
                norm_before= cfg.norm_before, norm_align = cfg.norm_align,
                drop_d_after = cfg.drop_d_after, dim=1, shift_after=cfg.shift_after,
        ...
        if  token == 'att' and ( len(token) == 3 or token[3].isdigit() ):
            block = Residual(
                Attention(E=E, H=cfg.H, causal=cfg.causal, T_max=cfg.T_max),
    ...
    def unit_test():
        B, T, E, H = 1, 10, 128, 16
        cfg = Config(E=E, H=H, blocks="2[att mlp]")
        m = Transformer(cfg)
