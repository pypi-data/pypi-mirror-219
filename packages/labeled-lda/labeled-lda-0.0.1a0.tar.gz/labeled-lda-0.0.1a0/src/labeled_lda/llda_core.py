...
from labeled_lda.preprocess import *
try:
    import copy_reg
except Exception:
...
class LldaModel:
    ...
    """
    def __init__(self, alpha_vector="50_div_K", eta_vector=None, labeled_documents=None,language='en',stopwords_path=None):
        """
        ...
        if language=='en':
            print("This Is English Topic Model")
        if language=='cn':
            print("这是中文主题模型！")
    ...
    def _load_labeled_documents(self, labeled_documents):
        ...
        for document, labels in labeled_documents:
            ...
            if self.language=='en':
                doc_words=get_text_english([document])[0]
            elif self.language=='cn':
                doc_words=get_text_chinese([document],stopwords_path=self.stopwords_path)[0]
            else:
                raise Exception("Error: no language is given!")
            print(doc_words)
