...
class Image(Head):
    def __init__(
        self, 
        event_shape, 
        num_layers=tuning.HyperInt(1, 3), 
        kernel_size=tuning.HyperChoice([3, 5, 7]), 
        hidden_dim=tuning.HyperChoice([8, 16, 32, 64, 128]),
    ...
    def get_parameters(self, hp):
        ...
        config.update({
            ...
            'kernel_size' : self._get_hp(None, 'kernel_size', hp),
            'hidden_dim' : self._get_hp(None, 'hidden_dim', hp),
        })
        return config
    
    def body(self, inputs, num_layers, kernel_size, hidden_dim, **kwargs):
        return layers.Conv2DTransposeBlock(
...
class Logistic(Head):
    def __init__(self, event_shape=(1,), sparse_response=True, multi_label=False, name='Logistic', **kwargs):
        super().__init__(name, **kwargs)
