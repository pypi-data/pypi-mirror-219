...
def main(inputfile, outputfile, environment, title, nthtikz, scale, line_width):
    ...
    with open(path_main, 'w') as file:
        ...
        with open(files[nthtikz-1], 'r') as f:
            for i, line in enumerate(f):
                if i == 1:
                    file.write(line)
                    file.write(f'\n[scale={scale}, line width={line_width} mm]')
                else:
                    file.write(line)
        
        if len(files) >= 1:
            file.write(f'{thumbnail_title.replace("JEE", title).replace("YEAR", extract_year(inputfile))}\n')
