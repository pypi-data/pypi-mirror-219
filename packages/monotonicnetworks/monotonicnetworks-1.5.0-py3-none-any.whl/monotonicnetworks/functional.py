...
def direct_norm(
    ...
    Args:
        ...
        parameter_name (str, optional): Name of the weight matrix in the layer.
        Defaults to "weight".
        vectorwise (bool, optional): Normalize the matrix vectors instead of the matrix itself.
            The vector norm is a weaker constraint and gives better results. See the paper for
    ...
    class Normalize(nn.Module):
        def forward(self, W):
            return get_normed_weights(
                W, kind, always_norm, max_norm, vectorwise
            )
...
def project_norm(
    ...
    """
    Constrain the norm of a layer's weight matrix by projecting the weight matrix to
    the correct norm.
    This function is meant to be used as a wrapper around `torch.nn.Linear`. It adds a hook to the
    forward pass which projects to the current weight matrix to a normalized matrix
    but the gradients are not backpropagated through normalization.
    ...
    Args:
        ...
        always_norm (bool, optional): Always normalize the weights to max_norm or allow them
            to have a norm smaller than 1. Defaults to False.
        max_norm (float, optional): The maximum norm to constrain the weights to.
        Defaults to 1.
        parameter_name (str, optional): Name of the weight matrix in the layer.
        Defaults to "weight".
        vectorwise (bool, optional): Normalize the matrix vectors instead of
        the matrix itself.The vector norm is a weaker constraint and gives better results.
    ...
    def normalize_weight(layer: nn.Linear, _) -> None:
        ...
        weight = get_normed_weights(
            weight, kind, always_norm, max_norm, vectorwise
        )
