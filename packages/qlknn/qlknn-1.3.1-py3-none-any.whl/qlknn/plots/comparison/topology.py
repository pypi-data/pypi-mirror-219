...
def find_similar_topology(network_id):
    ...
    query &= (
        Network.select().where(Network.target_names == Param(train_dim))
    ...
    for res in query:
        df.append(
            (
                res.id,
                res.hyperparameters.get().hidden_neurons,
    ...
    labels = [
        (line[0], "$topo = " + str(line[1]) + "$") for line in df_trim[["id", "topo"]].values
    ]
