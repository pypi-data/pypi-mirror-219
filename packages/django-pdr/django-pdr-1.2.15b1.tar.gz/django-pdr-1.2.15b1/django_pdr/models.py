...
class Database(models.Model):
    ...
    def mount(self, max_attempts=5, sleep_interval=10):
        ...
        If the engine is not connected, it will try to reconnect a number of times
        defined by max_attempts, with a sleep interval between each attempt defined
        by sleep_interval. If it cannot connect after max_attempts times, it will raise an error.
        """
        if str(self.pk) in database_engines:
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    result = database_engines[str(self.pk)].execute("SELECT 1")
            ...
            raise OperationalError("Could not connect to database {0} after {1} attempts.".format(self, max_attempts))
