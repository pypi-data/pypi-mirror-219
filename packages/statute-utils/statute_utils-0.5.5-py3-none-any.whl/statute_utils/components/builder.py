...
def is_excluded(text: str) -> bool:
    return any([text.strip().lower().startswith(excluded) for excluded in excludeables])


def is_special(text: str) -> bool:
    return any([text.strip().lower().startswith(special) for special in special_starts])
