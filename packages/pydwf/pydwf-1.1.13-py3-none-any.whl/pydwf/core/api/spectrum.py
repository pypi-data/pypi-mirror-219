...
class Spectrum(AbstractDwfLibrarySubApi):
    ...
    def window(self, n: int, winfunc: DwfWindow, beta: Optional[float] = None) -> Tuple[np.ndarray, float]:
        """Return a window function and its noise-equivalent bandwidth.
        ...
        Returns:
           Tuple[np.ndarray, float]: The first element of the tuple is a numpy array of length `n`, containing the
           window. All windows supported are symmetric, and they are scaled to have the sum of their values equal to
           `n`.

           The second element of the tuple is the noise equivalent bandwidth of the signal.
        """
    ...
    def fft(self, data: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        Returns:
           Tuple[np.ndarray, np.ndarray]: A tuple (magnitude, phase); but the magnitude and phase arrays are 1D numpy
               arrays with the same length as the input array `data`. The first array contains the non-negative
               magnitude of the signal for this particular frequency; the second array contains its phase.
