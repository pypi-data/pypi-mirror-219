...
class DigitalOut(AbstractDwfDeviceSubApi):
    ...
    def repetitionInfo(self) -> int:
        ...
        result = self.lib.FDwfAnalogInRepetitionInfo(self.hdwf, c_repetition_max)
        if result != RESULT_SUCCESS:
            raise self.dwf.exception()
    ...
    def repetitionSet(self, channel_index: int, repeat: int) -> None:
        ...
        result = self.lib.FDwfAnalogInRepetitionSet(self.hdwf, channel_index, repeat)
        if result != RESULT_SUCCESS:
            raise self.dwf.exception()
    ...
    def repetitionGet(self, channel_index: int) -> int:
        ...
        c_repeat = typespec_ctypes.c_unsigned_int()
        result = self.lib.FDwfAnalogInRepetitionGet(self.hdwf, channel_index, c_repeat)
        if result != RESULT_SUCCESS:
            raise self.dwf.exception()
        repeat = c_repeat.value
        return repeat
