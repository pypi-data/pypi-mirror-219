...
def openDwfDevice(
    ...
    if serial_number_filter is not None:
        ...
        candidate_devices = [
            device_index for device_index in candidate_devices
            if dwf.deviceEnum.serialNumber(device_index).casefold().endswith(serial_number_filter.casefold())]

        if len(candidate_devices) == 0:
            raise PyDwfError("No Digilent Waveforms device found"
                " in device class '{}' with serial number filter '{}'.".format(enum_filter.name, serial_number_filter))

        if len(candidate_devices) > 1:
            # Multiple devices found with a matching serial number.
            raise PyDwfError("Multiple Digilent Waveforms devices found"
                " in device class '{}' with serial number filter '{}'.".format(enum_filter.name, serial_number_filter))
        ...
        if len(candidate_devices) > 1:
            raise PyDwfError(
                ...
                " specify a serial number filter to select one.".format(
