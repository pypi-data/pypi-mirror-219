...
def main():
    ...
    Console.debug_b(
        '''
            shoot-chatgpt-Cloud - A web interface to ChatGPT
            Github: https://github.com/shoot82003/shoot-chatgpt
    ...
    try:
        from shoot-chatgpt_cloud.server import ChatBot as CloudServer

        return CloudServer(args.proxy, args.verbose, args.sentry, True).run(args.server, args.threads)
    except (ImportError, ModuleNotFoundError):
        Console.error_bh('### You need `pip install shoot-chatgpt-ChatGPT[cloud]` to support cloud mode.')
