...
@app.get("/videos")
def videos():
    list_files(VID_FOLDER, '.mp4')

@app.get("/timelapse/date/{date}")
def timelapse(date: str):
    os.system(f'ffmpeg -framerate 30 -pattern_type glob -i "./Pictures/{date}*.jpg" -s:v 1440x1080 -c:v libx264 -crf 17 -pix_fmt yuv420p ./Videos/{date}.mp4')
    return {
        "name": date,
...
def timelapse():
    ...
    os.system(f'ffmpeg -framerate 30 -pattern_type glob -i "./Pictures/{yesterday}*.jpg" -s:v 1440x1080 -c:v libx264 -crf 17 -pix_fmt yuv420p ./Videos/{yesterday}.mp4')
    return {
        "name": yesterday,
...
def schedule_timelapse():
    ...
    job = cron.new(command='curl localhost:8000/timelapse/yesterday', comment='take timelapse')
    job.every().day()
    cron.write()
