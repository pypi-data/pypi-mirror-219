...
class PlanarDepthCamTurtlebot(PlanarDepthCam):
    def __init__(self, index, observerManager, obstacleManager, mean = None, sd = None):
        ...
        Inputs:
            ...
            sd (float): standard deviation for measurement noise
        """
        super().__init__(index, observerManager, obstacleManager, mean, sd)
    ...
    def calc_ptcloud_world(self):
        ...
        arr = super().calc_ptcloud_world()

        #shift the YZ coordinates to XY and set Z coords to 0
        arr = np.vstack((arr[1:, :], np.zeros((1, arr.shape[1]))))

        #assemlble and return the array
        return arr
    ...
    def calc_ptcloud(self):
        ...
        """
        #use the observer to get the position and angle
        state = self.observer.get_state() #(x, y, phi)

        #get position and angle of the quadrotor
        q, theta = np.vstack((state[0:2].reshape((2, 1)), 0)), state[2, 0]

        #compute rotation matrix to spatial frame
        Rse = self.compute_rotation(theta)

        #compute pointcloud in world frame
        ptcloudWorld = self.calc_ptcloud_world()

        #transform pointcloud into ego frame
        ptcloudEgo = Rse.T @ (ptcloudWorld - q)

        #store in the pointcloud attribute
        self._ptcloudData["ptcloud"] = ptcloudEgo
        self._ptcloudData["stateVec"] = state
        return self._ptcloudData
