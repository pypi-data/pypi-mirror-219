"""This module features the StreamlinedDLInference class, that extends the StreamlinedInference
interface, and can be used to run end-to-end inference on a wrapped Inference subclass.
"""
...
class StreamlinedDLInference(StreamlinedInference):
    ...
    Attributes:
        ...
        - wrappee: The Inference subclass that is wrapped by this StreamlinedDLInference.
        - data_validator: An DataValidator instance that is used to validate the input
        data.
        - preprocessor: A BasePreprocessor instance that is used to preprocess the input data.
        - postprocessor: A Postprocessor instance that is used to postprocess the output data.
    """
    ...
    def _preprocess(self, data: InferenceData) -> Any:
        ...
        if self._data_validator.expected_keys:  # type: ignore [union-attr, attr-defined]
            self._preprocessor.expected_keys = (  # type: ignore [union-attr, attr-defined]
                self._data_validator.expected_keys  # type: ignore [union-attr, attr-defined]
            )
