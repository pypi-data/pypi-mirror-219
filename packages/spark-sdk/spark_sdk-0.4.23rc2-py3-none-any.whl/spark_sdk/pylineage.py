...
def McpJobInfo(flow_id, job_id, lineage, _type = 'python'):
    ...
    ############# ADD LINEAGE DATAJOB #############
    logging.warning("DEBUG python job")
    logging.warning(builder.make_data_job_urn(orchestrator=_type, flow_id=flow_id, job_id = job_id))
    mcpJobIO = MetadataChangeProposalWrapper(
...
def SQLQueryExecStartEvent(flow_id, app_id, lineage):    
    ...
    for urn in lineage.sink:
        if not check_exist_dataset(urn):
            mcpDataset.append(McpDataset(urn))
    mcpDataset.extend((mcpDataset,mcpFlowInfo,mcpJobIO,mcpJobInfo))
    print("mcpDataset", mcpDataset)
    return mcpDataset
    ...
    for mcp in mcpws:
        if mcp:
            try:
                ...
                logging.warning("Error to emit", mcp)
                logging.warning("Error to emit", e)
...
def scanPythonFile():
    ...
    if 'requests.post(' in txt or 'request("post"' in txt or 'requests.get(' in txt or 'request("get"' in txt:
        pattern_api = r'url\s*=\s*["\']([^"\']+)["\']'
        matches_api = re.findall(pattern_api, txt)
        from urllib.parse import urlparse
        matches_api = [urlparse(x).netloc for x in matches_api]
        logging.warn("matches_api")
        logging.warn(matches_api)
        matches.extend(matches_api)
        
    pattern_pandas = r'pd\.read_csv\(["\']([^"\']+)["\']'
    matches_pandas = re.findall(pattern_pandas, txt)
    logging.warn("matches_pandas")
    logging.warn(matches_pandas)
    matches.extend(matches_pandas)
    return matches
...
def addLineageAfterToDwh(full_table_name):
    ...
    if job_id:
        ...
        logging.warning("lineage")
        logging.warning(lineage.sink)
        logging.warning(lineage.source)
    return mcpws
