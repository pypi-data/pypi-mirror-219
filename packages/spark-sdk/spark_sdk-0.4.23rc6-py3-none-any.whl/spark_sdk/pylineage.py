...
def get_emitter():
    ...
    log("GMS_AUTH_TOKEN", GMS_AUTH_TOKEN)
...
def scanPythonFile():
    ...
    if 'requests.post(' in txt or 'request("post"' in txt or 'requests.get(' in txt or 'request("get"' in txt:
        ...
        matches_api1 = [urlparse(x).netloc for x in matches_api1]
        matches.extend(matches_api1)
        log("matches_api1", matches_api1)
        
        pattern_api2 = r'url\s*=\s*f["\']([^"\']+)["\']'
        matches_api2 = re.findall(pattern_api2, txt)
        matches_api2 = [urlparse(x).netloc for x in matches_api2]
        log("matches_api2", matches_api2)
        matches_api2 = [value.replace("{", "").replace("}", "") for value in matches_api2 if value.startswith('{') and value.endswith('}')]
        for variable_name in matches_api2:
            pattern_var = '{x}\s*=\s*["\']([^"\']+)["\']'.format(x = variable_name)
            matches_api3 = re.findall(pattern_var, txt)
            matches.extend(matches_api3)
            log("matches_api3", matches_api3)
