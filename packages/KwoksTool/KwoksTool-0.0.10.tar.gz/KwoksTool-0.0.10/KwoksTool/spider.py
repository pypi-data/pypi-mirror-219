    ...
    for i in range(0, len(a['代码'])):
        try:
            ...
            for son in code:
                ...
                # ============================================
                for ii in range(0, len(data['trade_date'])):
                    data.loc[ii, 'trade_date'] = datetime.strptime(str(data.loc[ii, 'trade_date']), '%Y%m%d')
                data = data.astype({'trade_date': 'datetime64[ns]'})  # 处理时间数据
                data = data.rename(
                ...
                # ==========================================
                data.to_excel(r'{}\{}-{}.xlsx'.format(url, a['名称'][i], a['代码'][i]))
                ProgressBar(i, len(a['代码']), a['名称'][i])
        except:
            date=pd.DataFrame()
            data.to_excel(r'{}\{}-{}错误.xlsx'.format(url, a['名称'][i], a['代码'][i]))
            continue