...
class Functions(Parameters):
    ...
    class Encryptor:
        ...
        def project_file_exists():
            ...
            if os.path.exists(ENVIROMENT_PROJ_FILE_PATH) is True:
                project_exists = True
        ...
        def backup_file_exists():
            ...
            if os.path.exists(ENVIRONMENT_BACKUP_FILE_NAME):
                ignored_exists = True
        ...
        def get_data_from_proj_file(self, father_attribute, atribute_to_search, dato_a_buscar, inner_search):
            return_data = None
            try:
                read_xml_file, project_tree = self.get_xml_root(ENVIROMENT_PROJ_FILE_PATH)
        ...
        def get_data_from_backup_file(self, father_attribute, atribute_to_search, dato_a_buscar, inner_search):
            return_data = None
            try:
                read_xml_file, backup_tree = self.get_xml_root(ENVIRONMENT_BACKUP_FILE_NAME)
                for element in read_xml_file.findall(f"./{father_attribute}[@{atribute_to_search}='{dato_a_buscar}']/"):
                    if element.tag == inner_search and (element.text is not None or element.text != ""
                                                        or element.text != " "):
                        return_data = element.text
        ...
        def add_data_to_proj_file(self, father_attribute, atribute_to_search, dato_a_buscar):
            ...
            backup_xml, backup_tree = self.get_xml_root(ENVIRONMENT_BACKUP_FILE_NAME)
            project_xml, project_tree = self.get_xml_root(ENVIROMENT_PROJ_FILE_PATH)
            ...
            project_tree.write(ENVIROMENT_PROJ_FILE_PATH)
            self.compress_and_encrypt_xml(ENVIROMENT_PROJ_FILE_PATH)
        ...
        def create_backup_file(self):
            ...
            with open(ENVIRONMENT_BACKUP_FILE_NAME, 'wb') as output_file:
                output_file.write(encripted_xml)
            output_file.close()

        def create_project_file(self):
            with open(rf"{ENVIROMENT_PROJ_FILE_PATH}", "wb") as env_file:
                env_file.write(b'<root>\n')
                env_file.write(b'</root>\n')
            tree = Et.parse(ENVIROMENT_PROJ_FILE_PATH)
            root = tree.getroot()
            Et.indent(root, "   ")
            tree.write(ENVIROMENT_PROJ_FILE_PATH)
            self.compress_and_encrypt_xml(ENVIROMENT_PROJ_FILE_PATH)
