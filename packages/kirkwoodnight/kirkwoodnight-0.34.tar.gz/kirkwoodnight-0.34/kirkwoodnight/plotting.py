...
def get_summary(path):
    ...
    time,Observable,alt,az = np.loadtxt(path,skiprows=10,unpack=True,dtype=str)
    return id,ra,dec,type_,constellation,magnitude,obs_frac,time,Observable,alt,az
...
def plot_it(files_path):
    ...
    for summ in summaries[:1]:
        ...
        id,ra,dec,type_,constellation,magnitude,obs_frac,time_series,Observable,alt,az = get_summary(path)
    ...
    for i,time in enumerate(time_series):
        ...
        # get data
        id_list = [];ra_list = [];dec_list = [];type_list = [];constellation_list = [];magnitude_list = [];obs_frac_list = []
        observable_list = [];alt_list = [];az_list = [];names = []
        for summ in summaries:
            path = os.path.join(files_path,summ)
            id,ra,dec,type_,constellation,magnitude,obs_frac,time_series,Observable,alt,az = get_summary(path)
            
            id_list.append(id);ra_list.append(ra);dec_list.append(dec);type_list.append(type_);constellation_list.append(constellation)
