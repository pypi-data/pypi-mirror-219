...
class Grid(Element[str], Generic[T]):
    ...
    def __init__(self, parent: State, label: str, rows: int, columns: int, cellClass: Type[T] = Cell) -> None:
    ...
    # override
    def setup(self, change_state: Callable[[str, str], None], interrupt_speech=False) -> bool:
    ...
    def populate(self) -> None:
        self.cells = [[self.cellClass() for c in range(self.columns)] for r in range(self.rows)]

    def add_row(self, index: int = 0) -> None:
        self.cells.insert(index, [self.cellClass() for c in range(self.columns)])
