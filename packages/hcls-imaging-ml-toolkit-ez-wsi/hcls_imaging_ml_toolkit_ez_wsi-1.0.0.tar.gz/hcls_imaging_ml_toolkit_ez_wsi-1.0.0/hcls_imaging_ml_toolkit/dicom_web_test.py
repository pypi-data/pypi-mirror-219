...
def _CreateMockInstanceMetadata() -> Dict[str, Any]:
  instance_metadata = {}
...
def FakeHttpResponse(
    status_code: int, body: Optional[str] = _BODY
) -> Tuple[httplib2.Response, Optional[str]]:
...
class DicomWebTest(parameterized.TestCase):
  ...
  @parameterized.parameters(
      dicom_web._TOO_MANY_REQUESTS_ERROR,
  ...
  def testGetAllMetaData(self):
    expected_url = (
        ...
        % (
            tags.STUDY_INSTANCE_UID.number,
            tags.SERIES_INSTANCE_UID.number,
            tags.SOP_INSTANCE_UID.number,
            _TEST_INSTANCES,
        )
    )
    ...
    all_meta_data = dicom_web.GetInstancesMetadata(
        mock_client, dicomweb_url, study_uid, tag_list, _TEST_INSTANCES
    )
  ...
  def testStowRsJsonError(self):
    ...
    with self.assertRaises(Exception):
      self._dwc.StowRsJson('', [{}], [bulk_data])  # pytype: disable=attribute-error

  @parameterized.parameters(199, 300, 403)
  def testStowRsHttpErrors(self, error_value):
    bulk_data = ['0x12', '0x13']
    expected_response = (
        f'StowRs error. Response Status: {error_value},\nURL: '
        'https://some_stow_url,\nContent: body.'
    )
    with mock.patch.object(
      ...
      with self.assertRaisesRegex(
          dicom_web.UnexpectedResponseError, expected_response
      ):
  ...
  def testQidoSuccess(self, *_):
    ...
    http_mock.request.side_effect = [
        FakeHttpResponse(
            http_client.OK, json.dumps([_MOCK_CT_INSTANCE_METADATA])
        ),
  ...
  def testPathToUrl(self):
    ...
    expected_url = dicom_path.DicomPathJoin(
        dicom_web.CLOUD_HEALTHCARE_API_URL, dicom_path_str
    )
    self.assertEqual(url, expected_url)

  def testPathStrToUrl(self):
    dicom_query_path_str = dicom_path.DicomPathJoin(
    ...
    expected_url = dicom_path.DicomPathJoin(
        dicom_web.CLOUD_HEALTHCARE_API_URL, dicom_query_path_str
    )
    self.assertEqual(url, expected_url)
