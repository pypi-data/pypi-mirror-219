...
class Feature(Generic[_TPrim, _TRich]):
    ...
    def converter(self) -> FeatureConverter:
        ...
        if self.typ.is_dataframe:
            column_converters = {col.root_fqn: col.converter for col in  underlying_typ.columns}
            ...
            def frame_encoder(x: _TRich) -> List[Dict[str, _TPrim]]:
                ...
                for row in x.to_features():
                    ...
                    for fqn, value in row.items():
                        converted_value = column_converters[fqn].from_rich_to_primitive(value)
            ...
            def frame_decoder(x: _TPrim):
                ...
                if len(x) == 0:
                    return DataFrame({k: [] for k in column_converters.keys()})

                results = []
                for row in x:
                    rich_features = {}
                    for fqn, value in row.items():
                        if fqn not in column_converters:
                            continue
                        # Strip namespace from FQN before passing in to rich type constructor
                        rich_features[fqn.split(".")[-1]] = column_converters[fqn].from_primitive_to_rich(value)
        ...
        if hasattr(underlying_typ, "__chalk_feature_set__") and underlying_typ.__chalk_feature_set__:
            raise TypeError(
                f"Feature '{self.fqn}' is annotated with feature set '{underlying_typ}' but no join was found. "
                f"Please ensure that join between keys is specified between '{self.namespace}' and '{underlying_typ}'."
            )
