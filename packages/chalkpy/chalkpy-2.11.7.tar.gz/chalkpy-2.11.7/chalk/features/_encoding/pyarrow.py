...
def rich_to_pyarrow(
    ...
    if (
        ...
        or getattr(python_type, "__chalk_feature_set__", False)
    ):
        ...
        for field_name, type_annotation in annotations.items():
            ...
            if getattr(python_type, "__chalk_feature_set__", False):
                field_name = f"{python_type.namespace}.{field_name}"
            fields.append(pa.field(field_name, underlying_dtype))
        return pa.struct(fields)
    from chalk import DataFrame  # avoid circular import

    if issubclass(python_type, DataFrame):
        column_fields = [(col.fqn, col.typ.parsed_annotation) for col in python_type.columns]
        pa_fields = [pa.field(fqn, rich_to_pyarrow(typ, name=fqn, in_struct=True)) for fqn, typ in column_fields]
