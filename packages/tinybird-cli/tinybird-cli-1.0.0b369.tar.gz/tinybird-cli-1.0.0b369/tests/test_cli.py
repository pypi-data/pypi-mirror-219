...
"""
    ...
    @tornado.testing.gen_test
    async def test_pipe_regression_test__tb__semver_filtered(self) -> None:
        await self._auth()
        ...
        await Users.add_release(self.workspace, 'commit', '1.0.0', self.workspace, ReleaseStatus.live, force=True)

        output = await self._tb(["pipe", "data", name])

        output = await self._tb(["pipe", "data", name, '--__tb__semver', '1.0.0'])
