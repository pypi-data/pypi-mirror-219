...
class TestProcessedUsageMetrics(BaseTest):
    ...
    async def get_last_stats(self, stat_name: str) -> List[Dict[str, Any]]:
        ...
        response = await self.fetch_async(
            path=f"/v0/billing/{self.WORKSPACE_ID}/stats?{urlencode(call_params)}",
            method='GET')
    ...
    async def test_no_billing_for_internal_datasources(self):
        ...
        for _ in range(3):
            params = {'token': self.admin_token, 'q': 'SELECT 1'}
            response = await self.fetch_async(
                path=f"/v0/sql?{urlencode(params)}",
                method='GET')
            self.assertEqual(response.code, 200, response.body)

        params = {'token': self.admin_token, 'q': 'SELECT count() FROM tinybird.pipe_stats_rt'}
        response = await self.fetch_async(
            path=f"/v0/sql?{urlencode(params)}",
            method='GET')
        self.assertEqual(response.code, 200, response.body)

        pipe_name = f"test_using_cte_to_not_bill_query_api_pipe_{uuid.uuid4().hex}"
        ...
        response = await self.fetch_async(
            path=f"/v0/pipes/{pipe_name}.json?{urlencode(params)}",
            method='GET')
        self.assertEqual(response.code, 200, response.body)
        ...
        self.assert_billing_usage_logs(self.base_workspace.database, '3', '0')

    @tornado.testing.gen_test
    async def test_using_cte_to_not_bill_query(self):

        datasource_name = f"test_using_cte_to_not_bill_query_api_ds_{uuid.uuid4().hex}"
        ...
        await self.tb_api_proxy_async.create_pipe_endpoint(
            ...
            pipe_name,
            f"""
WITH (SELECT count() FROM {datasource_name}) as rows
   SELECT count(), rows
...
""")
        ...
        processed_usage_logs = self.poll_usage_logs(self.base_workspace.database, 'tb-api-query')
        self.assertTrue(int(processed_usage_logs[0]['read_bytes']) > 3, processed_usage_logs)
