...
def _mock_ch_guarded_query_response():
...
class BaseTestJobs(unittest.IsolatedAsyncioTestCase):
    ...
    def initialize(self, table_source: str, table_target: str, view_node_id: str, partition_key_source: str, partition_key_target: str, extra_params: Optional[dict]):
        ...
        return table_source_ds, table_target_ds, view_sql
...
class TestJobsPopulatePartitions(BaseTestJobs):
    async def _test_by_partition_populate(self, partition_key_source='', partition_key_target=''):
        ...
        table_source_ds, table_target_ds, view_sql = self.initialize(
            table_source=table_source,
            table_target=table_target,
            view_node_id=view_node_id,
            partition_key_source=partition_key_source,
            partition_key_target=partition_key_target,
...
class TestPopulateQuerySettings(BaseTestJobs):
    @patch('tinybird.job.ch_guarded_query', return_value=_mock_ch_guarded_query_response())
    async def test_populate_job_use_correctly_default_settings(self, mock_ch_guarded_query):
        id = uuid.uuid4().hex
        table_source = f'test_populate_default_settings_source_{id}'
        table_target = f'test_populate_default_settings_target_{id}'
        view_node_id = f'test_populate_default_settings_mv_{id}'
        ...
        populate_settings = mock_ch_guarded_query.call_args[1]
        self.assertEqual(populate_settings.get('max_threads'), None)
        self.assertEqual(populate_settings.get('max_insert_threads'), 2)

        # 2. Test threads settings
        self.user.set_user_limit('populate_max_insert_threads', 16, 'populate')
        self.user.set_user_limit('populate_max_threads', 16, 'populate')
        ...
        populate_settings = mock_ch_guarded_query.call_args[1]
        self.assertEqual(populate_settings.get('max_threads'), 16)
        self.assertEqual(populate_settings.get('max_insert_threads'), 16)

        # 3. Test reset settings
        self.user.delete_limit_config('populate_max_insert_threads')
        self.user.delete_limit_config('populate_max_threads')
