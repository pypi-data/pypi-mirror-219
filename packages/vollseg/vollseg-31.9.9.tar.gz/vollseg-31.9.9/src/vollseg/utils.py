...
def VollOne(
    ...
    if len(image.shape) == 4 and "T" not in axes:
        ...
        if roi_model is None and noise_model_membrane is not None:
            ...
            membrane_seg, membrane_skeleton, membrane_denoised = membrane_res
            membrane_mask = np.zeros_like(membrane_denoised)
            thresh = threshold_otsu(membrane_denoised)
            membrane_mask[membrane_denoised > thresh] = 1
            membrane_mask = binary_fill_holes(membrane_mask)
