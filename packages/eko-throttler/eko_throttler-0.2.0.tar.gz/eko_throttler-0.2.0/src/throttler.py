...
class Throttler:
    ...
    def _get_loop(self: Throttler) -> AbstractEventLoop:
        loop: AbstractEventLoop = _get_running_loop()
    ...
    def slot_time(self: Throttler) -> timedelta:
        ...
        with self.__lock:
            if not self._expiry:
                ...
                soonest: datetime = self._expiry[0] # type: ignore[index]

            waiters: Deque[Future[Literal[True]]] = self.slot_waiters
