...
async def ajg():
    ...
    try:
        ...
        await ayra_bot.join_chat("@cidsupport")
        await ayra_bot.join_chat("@kazusupportgrp")
    except rpcerrorlist.ChannelPrivateError:
        print("Lu Di Ban Di @cidsupport, @KynanSupport, @kazusupportgrp Jadi Ga Bisa Pake Bot Ini ! Minta Unban Dulu @kenapanan atau @.")
        sys.exit(1)
...
async def autobot():
    ...
    if who.username:
        ...
        username = "cid_" + (str(who.id))[5:] + "_bot"
    bf = "@BotFather"
    ...
    if isdone.startswith("Sorry,"):
        ran = randint(1, 100)
        username = "cid_" + (str(who.id))[6:] + str(ran) + "_bot"
        await ayra_bot.send_message(bf, username)
        await asyncio.sleep(1)
        isdone = (await ayra_bot.get_messages(bf, limit=1))[0].text
...
async def autopilot():
    ...
    if not channel:
        ...
        try:
            r = await ayra_bot(
                CreateChannelRequest(
                    title="CID-Premium Logs",
                    about="Ini adalah grup logs dari CID-Premium\nJangan keluar dari grup logs ini\nPowered By @CidSupport",
                    megagroup=True,
                ),
            )
...
async def customize():
    ...
    try:
        ...
        await ayra_bot.send_message(
            "botfather",
            f"✨ Powerful CID-Premium Assistant  ✨\n✨ Master ~ {sir} ✨\n\n✨ Powered By ~ @CidSupport ✨",
        )
        await asyncio.sleep(2)
        ...
        try:
            async for x in ayra_bot.iter_messages(
                ...
                try:
                    ...
                    LOGS.info(f"CID-Premium - PLUGIN_CHANNEL - ERROR - {plugin}")
                    LOGS.exception(e)
                    os.remove(plugin)
        except Exception as er:
            LOGS.exception(er)
...
async def ready():
    ...
    if not udB.get_key("INIT_DEPLOY"):  # Detailed Message at Initial Deploy
        MSG = """ **Thanks for Deploying CID-Premium!**
• Here, are the Some Basic stuff from, where you can Know, about its Usage."""
        PHOTO = "./resources/extras/logo.jpg"
        BTTS = Button.inline("• Click to Start •", "initft_2")
        udB.set_key("INIT_DEPLOY", "Done")
    else:
        MSG = f"**CID-Premium has been deployed!**\n╼┅━━━━━━━━━━━┅╾\n**UserMode**: {inline_mention(ayra_bot.me)}\n**Assistant**: @{asst.me.username}\n╼┅━━━━━━━━━━━┅╾\n**Support**: @CidSupport\n╼┅━━━━━━━━━━━┅╾"
