...
def show_year_trends_with_fulltext(
    ...
    for idx, item in enumerate(list_g20_news_final):
        ...
        if year=="":
            continue
        if year in dict_year_stat:
            dict_year_stat[year]+=1
        else:
            dict_year_stat[year]=1

    dict_year_stat = OrderedDict(sorted(dict_year_stat.items(), key=lambda obj: obj[0], reverse=False))
    print("Year\tNews number")
    for year in dict_year_stat:
        if int(year)<minimum_year or len(year)!=4:
            continue
        print(f"{year}\t{dict_year_stat[year]}")
    print()
