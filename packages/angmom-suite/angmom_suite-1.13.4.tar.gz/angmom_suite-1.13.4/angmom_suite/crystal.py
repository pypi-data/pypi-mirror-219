...
class MagneticSusceptibility(Store):
    ...
    def evaluate(self, **ops):
        ...
        tensor_func = partial(powder_susceptibility_tensor if self.powder else
                              susceptibility_tensor,
                              hamiltonian=ops['hamiltonian'], spin=ops['spin'],
                              angm=ops['angm'], field=self.field,
        ...
        if self.differential and not self.powder:
...
def powder_susceptibility_tensor(temp, hamiltonian, spin, angm, field=None, **kwargs):

    raise NotImplementedError("Powder averaging still under construction!")

    def integrant(phi, theta, i, j):
        field_vec = field * np.array([np.sin(theta) * np.cos(phi), np.sin(theta) * np.sin(phi), np.cos(theta)])
        return np.sin(theta) * susceptibility_tensor(temp, hamiltonian, spin, angm, field=field_vec, **kwargs)[i, j]

    def integrate_func(i, j):
        return integrate.dblquad(integrant, 0., np.pi, lambda x: 0., lambda x: 2 * np.pi, (i, j))[0] / (4 * np.pi)

    return np.array([[integrate_func(i, j) for j in range(3)] for i in range(3)])
