...
class ReportGenerator:
    ...
    def generate_stacked_bar_chart(self):
        ...
        # In relative mode, always place 'Other Phyla' at the end
        df_final = pd.concat([top_filos, others]) if self.absolute else pd.concat([top_filos, others]).groupby('phylum').sum().reset_index()
        ...
        # In relative mode, we always want 'Other Phyla' to be last
        df_final.sort_values(by='Sample', ascending=False, inplace=True)
        if not self.absolute:
            df_final = df_final.set_index('phylum').loc[top_filos['phylum'].tolist() + ['Other Phyla']].reset_index()
