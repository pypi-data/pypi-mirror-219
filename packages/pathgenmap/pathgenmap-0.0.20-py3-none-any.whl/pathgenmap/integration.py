...
class Manager:
    def __init__(self):
        ...
        self.id_path = 0
        self.processed_combinations = set()  # Added this line
    ...
    def integrate_all(self) -> None:
        for pathway in self.pathways.pathways_list:
            if pathway.path_name != 'Organonitrogen compound biosynthetic process':
                continue
                            ...
                            for phylum_dict in self.abundance.phylo.phylum_list:
                                if phylum == phylum_dict['phylum_name']:
                                    for species in phylum_dict['species_list']:
                                        ...
                                        combination = (pathway.path_name, gene, phylum, species.species_name, species.num_reads)
                                        if combination not in self.processed_combinations:
                                            self.id_path += 1
                                            ...
                                            self.processed_combinations.add(combination)  # Add the processed combination to the set
