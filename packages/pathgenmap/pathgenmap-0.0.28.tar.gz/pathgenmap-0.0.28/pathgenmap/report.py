...
class ReportGenerator:
    ...
    def generate_stacked_bar_chart(self):
        ...
        df_final_pivot = df_final.pivot(columns='phylum', values='reads').fillna(0)

        # Generate the chart
        fig, ax = plt.subplots()
        df_final_pivot.loc[:, df_final['phylum']].plot.bar(stacked=True, ax=ax)
        ax.set_ylabel('Reads (%)' if not self.absolute else 'Reads')
