...
class SQLDatabase:
    ...
    def from_databricks(
        ...
        **kwargs: Any,
        ...
        host = host or os.environ.get("DATABRICKS_HOST", default_host)
        if host is None:
            raise ValueError(
                "host is None. Please pass in non-null value for host or set"
                " environment variable DATABRICKS_HOST."
            )

        default_api_token = context.apiToken if context else None
        api_token = api_token or os.environ.get("DATABRICKS_TOKEN", default_api_token)
        if api_token is None:
            raise ValueError(
                "api_token is None. Please pass in non-null value for api_token or set"
