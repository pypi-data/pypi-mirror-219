...
def load_module_from_name(
    ...
    if module is None:
        raise ImportError(f"module {dotted_name} doesn't exist")
    return module
...
def load_module_from_modpath(
    ...
    for part in parts:
        ...
        if not _file and len(modpath) != len(parts):
            raise ImportError(f"no module in {'.'.join(parts[len(modpath):])}")
        path = [dirname(_file)]
    return module
        ...
        if importer is not None:
            if importer.find_module(modpath[0]):
                if not importer.find_module("/".join(modpath)):
                    raise ImportError(
                        f"No module named {'.'.join(modpath[1:])} in {filepath}/{modpath}"
                    )
    ...
    while modpath:
        ...
        if modpath:
            if mtype != PKG_DIRECTORY:
                raise ImportError(f"No module {'.'.join(modpath)} in {'.'.join(imported)}")
