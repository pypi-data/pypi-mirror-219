...
from ethrpc_accounts._utils.legacy_transactions import (
    Transaction,
    vrs_from,
)
from ethrpc_accounts._utils.signing import (
...
from ethrpc_accounts._utils.typed_transactions import (
    TypedTransaction,
)
from ethrpc_accounts.datastructures import (
    SignedMessage,
    SignedTransaction,
)
from ethrpc_accounts.hdaccount import (
    ETHEREUM_DEFAULT_PATH,
    generate_mnemonic,
    key_from_seed,
    seed_from_mnemonic,
)
from ethrpc_accounts.messages import (
    SignableMessage,
...
class Account:
    ...
    def create(self, extra_entropy=""):
        ...
        :class:`~ethrpc_accounts.local.LocalAccount`.
        ...
        .. code-block:: python
            ...
            >>> from ethrpc_accounts import Account
    ...
    def encrypt(cls, private_key, password, kdf=None, iterations=None):
        ...
        :type private_key: hex str, bytes, int or :class:`ethrpc_keys.datatypes.PrivateKey`
    ...
    def create_with_mnemonic(
        ...
        Creates a new private key, and returns it as a
        :class:`~ethrpc_accounts.local.LocalAccount`, alongside the mnemonic that can
    ...
    def set_key_backend(self, backend):
        """
        Change the backend used by the underlying ethrpc-keys library.

        *(The default is fine for most users)*

        :param backend: any backend that works in
            `ethrpc_keys.KeyApi(backend)
            <https://github.com/ethereum/ethrpc-keys/#keyapibackendnone>`_
    ...
    def sign_message(
        ...
        :returns: Various details about the signature - most importantly the
            fields: v, r, and s
        :rtype: ~ethrpc_accounts.datastructures.SignedMessage
    ...
    def signHash(self, message_hash, private_key):
        ...
        .. WARNING:: *Never* sign a hash that you didn't generate,
            ...
            Instead, prefer :meth:`~ethrpc_accounts.account.Account.sign_message`,
            which cannot accidentally sign a transaction.

        .. CAUTION:: Deprecated for :meth:`~ethrpc_accounts.account.Account.sign_message`.
    ...
    def _parsePrivateKey(self, key):
        """
        Generate a :class:`ethrpc_keys.datatypes.PrivateKey` from the provided key.

        If the key is already of type :class:`ethrpc_keys.datatypes.PrivateKey`,
        return the key.

        :param key: the private key from which a :class:`ethrpc_keys.datatypes.PrivateKey`
                    will be generated
        :type key: hex str, bytes, int or :class:`ethrpc_keys.datatypes.PrivateKey`
        :returns: the provided key represented as a
                  :class:`ethrpc_keys.datatypes.PrivateKey`
        """
