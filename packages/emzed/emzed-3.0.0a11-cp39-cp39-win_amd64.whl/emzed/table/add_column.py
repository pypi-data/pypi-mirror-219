...
def add_column_with_values(model, name, values, type_, format_):
    ...
    conn.execute(f"CREATE TABLE _temp ({name} {db_type});")
    conn.executemany("INSERT INTO _temp VALUES (?);", ((v,) for v in values))

    conn.execute(
        f"""
        CREATE TABLE _temp2 AS
               SELECT * FROM {_access_name}
               JOIN _temp ON {_access_name}.rowid = _temp.rowid;
        """
    )
