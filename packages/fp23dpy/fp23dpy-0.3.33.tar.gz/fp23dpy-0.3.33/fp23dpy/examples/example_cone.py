...
def radius(Y, cone_angle=default_cone_angle, radius0=radius0):
    return -np.tan(cone_angle / 2) * Y + radius0  # estimate radius of cone at Y location
...
def get_projected_coordinate_grid(shape=shape, calibration=None):
    ...
    return simulation.get_rotsym_projection_map(
        radius, calibration, shape=shape, scale=calibration["scale"], mid_y=get_mid_y() # not the same as mid_y above
    )
